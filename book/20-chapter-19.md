...
The two orders are not the same operation, and the difference is visible from across the room. Other pairs are indifferent: left sock then right sock, or right sock then left sock, and you finish in the same state either way. Dressing contains both kinds of pair.

So take the operations themselves as the objects worth studying, rather than the states they act on. You can do one operation and then another, which composes them. You can scale an operation, or add two of them, in any setting where addition means something. A collection of operations that admits all three of those moves is an **algebra**, and the word carries no more than that. When every pair in an algebra composes the same way in both orders, the algebra is **commutative**. When some pair does not, the algebra is non-commutative. Adding up a shopping list is commutative. Getting dressed is not.
...
That expression is zero exactly when the two operations can be swapped freely, and its size when it is not zero says how much difference the swap makes. Socks and shoes have a large one. Left sock and right sock have zero.
...
A complete list of mutually exclusive alternatives is a set of projectors that never overlap and that add to the operation that does nothing at all. The three magnets gave two such lists on the same atom, one for up-down and one for sideways, and neither list is a refinement of the other. The list a question is asked inside of is its **context**, and in richer systems than a single atom the same question sits inside many different complete lists.

To turn questions into odds you need one more instrument, and it is arithmetic. Add up the diagonal entries of a square array of numbers. That sum is the **trace**, and it has the one property that makes it usable: rewriting the array in a different set of directions leaves the trace alone. The trace of a projector counts how many independent directions the question says yes to.
...
Look at what an observer finds when it looks at its own center. Questions with definite answers. Answers that survive being read. Answers that survive being copied to a neighbor, and that come back the same. Order that never matters, because everything in the center commutes with everything. Which is a complete description of the world of tables and chairs and pointer readings, and it arrives as the commuting part of a structure that is not commuting, rather than as a separate layer of reality sitting underneath.
