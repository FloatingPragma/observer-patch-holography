...
The stiffness is computed off the thirty seams. At long wavelengths it comes out as the square of the wavenumber. The first correction subtracts a twentieth of the fourth power times the square of the seam spacing, and across the whole range, down to wavelengths as short as a single seam, the stiffness sits between nineteen twentieths of the square and the square. At zero wavenumber it is exactly zero, so the frequency there is exactly zero. A disturbance of unlimited wavelength costs nothing whatever. That exact zero is why the long waves all travel at one speed. The speed is the ratio of frequency to wavenumber, which chapter nine fixed by counting one seam per event.

A wave is what mismatch does while it is being repaired. Its frequency and its speed are readings of the wiring. Nothing about either was posted in from outside.
