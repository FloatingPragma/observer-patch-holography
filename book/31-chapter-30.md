...
The prologue promised that by the last third of the book you would be able to read that line and distinguish the exact finite results from the declared physical bridge premises behind each symbol. Chapter one gave the standing list of what a framework begins by assuming: a spacetime, a collection of fields living on it, a symmetry group chosen to match what was observed, and a page of numbers measured in laboratories. The four terms above are that list turned into reconstruction problems. Chapter fifteen's rank-three carrier, joined to an independent real axis, defines an exact finite four-dimensional ambient Lorentzian precursor. Chapter seven's authenticated parent order supplies canonical source height, computed without execution-list position and strictly increasing along generated ancestry; one explicit positive global scale on source height enters only through an event placement in the precursor. Its dimension is the dimension of that target carrier, not an intrinsic dimension of the event poset, and scaled source height is not a physical clock. Its spacetime reading remains conditional on a faithful physical placement, manifoldlike refinement and count-to-volume rule. The fields are chapter twenty-seven's five places a pattern can sit. The group is one, three and eight because twelve dials come apart into a center of one and a remainder of eleven, and eleven splits as three plus eight and in no other way. Each quantitative statement belongs either to an exact finite result or to a named physical calibration and matching branch; the chapter does not turn the latter into source-derived facts.
