...
The prologue promised that by the last third of the book you would be able to read that line and distinguish the exact finite results from the declared physical bridge premises behind each symbol. Chapter one gave the standing list of what a framework begins by assuming: a spacetime, a collection of fields living on it, a symmetry group chosen to match what was observed, and a page of numbers measured in laboratories. The four terms above are that list turned into reconstruction problems. Chapter fifteen's rank-three carrier, joined to an independent real axis, defines an exact finite four-dimensional ambient Lorentzian precursor. Chapter seven's authenticated parent order supplies the exact longest-parent-chain source height, and every finite log has an auxiliary injective forward-causal placement in that target. Conversely, every finite strict partial order compiles into the abstract authenticated grammar; that generic compiler records the whole supplied order as parenthood rather than its Hasse links alone. These are finite existence and expressivity theorems, not physical selection. Target dimension is not intrinsic event-poset dimension, and scaled source height is not a physical clock. Physical spacetime requires a source-selected order embedding, compatible refinement, a count-to-volume rule, and independent dimension and manifoldlikeness tests; together, order and density uniformity are the causal-set faithful-embedding requirement. The fields are chapter twenty-seven's five places a pattern can sit. The group is one, three and eight because twelve dials come apart into a center of one and a remainder of eleven, and eleven splits as three plus eight and in no other way. Each quantitative statement belongs either to an exact finite result or to a named physical calibration and matching branch; the chapter does not turn the latter into source-derived facts.
