...
One factor is owed, and chapter twenty-one supplied it. Entropy is heat divided by temperature. The temperature an accelerated observer reads is its acceleration divided by two pi. Dividing by that temperature multiplies the heat by two pi, which takes the matter side to eight pi squared times the fourth power of the radius, divided by fifteen.
