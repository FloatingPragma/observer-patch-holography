...
The rank-three carrier comes out of a bell. That is the exact result in this chapter. Independently of event order, adjoining one real axis to this chapter's three positive carrier directions gives an ambient target of real dimension four, one positive axis, three negative axes and a two-sphere of null directions. Chapter seven's authenticated parent order computes a canonical whole-number source height: roots have height zero, every nonroot has one plus the greatest direct-parent height, and the value increases strictly along generated ancestry. Source height enters only when one explicit positive `timeScale` and a supplied spatial readback place an event in the ambient target. This is an exact finite Lorentzian precursor, not an intrinsic dimension theorem for the event poset. Calling its coordinates physical positions and times requires a source-selected placement, an operational clock, agreement in both directions between dependency and light-cone order, and one manifoldlike refinement whose event counts calibrate volume. The pages below follow the rank-three half of that construction.
...
Both constructions have a limit in them. Nothing in the world is at either limit.
