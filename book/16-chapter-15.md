...
The rank-three carrier comes out of a bell. That is the exact result in this chapter. Independently of event order, adjoining one real axis to this chapter's three positive carrier directions gives an ambient target of real dimension four, one positive axis, three negative axes and a two-sphere of null directions. Chapter seven's authenticated parent order computes the exact longest-parent-chain source height. Every finite log can be placed injectively and forward-causally in the target by an auxiliary event enumeration and a sufficiently large height scale. This is an exact finite Lorentzian precursor, not an intrinsic dimension theorem for the event poset. Calling its coordinates physical positions and times requires a source-selected placement, an operational clock, the edge-speed bound, equal-height spatial separation, spacelike increasing-height incomparable pairs, and one refinement whose event counts calibrate volume and whose independent dimension and manifoldlikeness tests select the four-dimensional continuum. The pages below follow the rank-three half of that construction.
