...
That is enough to settle every question about order that has an answer. Two repairs on seams that share an observer have a fact about which came first, because the second one read what the first one wrote. The numbers it worked from are the numbers the first one left behind. Two repairs on seams with no observer in common have no such fact. There is no register anywhere holding it, neither observer at either seam was in a position to write it down, and no clock could supply it afterwards, because supplying it would mean constructing the shared present chapter seven found nothing to build out of.
...
A repair step is a process. It preserves the reference. So the relative entropy of any description against the reference falls at every step and never climbs. That is the second law, in the arrangement, with no appeal to counting microstates and no claim about how anything started.
...
Every transition has one. Some are positive and some are negative, which is the bead in Canberra. Average the exponential of minus that quantity over every transition the step can make, and the answer is exactly one, for any repair that leaves the reference alone. That identity holds whether or not anything is balanced. The second law leans on it.
...
One more relation drops out of the same bookkeeping. Take two quantities you can measure about the arrangement, evaluate the first before a step and the second after, and average over the settled state. Swap which one you evaluate first. The two averages are equal. Lars Onsager published that symmetry in *Physical Review* on 15 February 1931, worked out its consequences for coupled processes such as the way a temperature difference across a metal junction drives an electric current and a current drives heat, and collected the Nobel Prize in Chemistry for it in 1968. Transport coefficients come in pairs and the pairs are equal. Here it is a two-line consequence of the same relation that gave the arrow.

## Faking a past

If a commit destroys what it resolved, a record of the past is the only remaining access to it. The question is whether a record can be forged.
...
Chapter six left one unit of disagreement sitting in a world of twelve. One flipped record, thirty seams, nineteen loops, and a leftover of exactly one that no repair removes. Repair in a different order and it sits somewhere else. Every move that put it where it sits was a commit, each one discarding the arrangement it came from. A defect that survives every repair the world can perform is a thing with a history. The history is the chain of writes that carried it, in an order that any observer holding two of those records can check.

Every strand in that web was laid by one commit reading what another commit wrote, and chapter nine counted how fast that can spread: one seam per event, no faster, so a commit can only depend on commits within a certain number of seams of it. The web has a shape, the shape has an edge, and the edge advances at a fixed number of seams per event. Turn that count into meters and seconds and it is a speed. Every laboratory that has ever measured that speed has come back with the same number, whatever the laboratory was doing at the time.