...
15. Why does the position carrier have three directions? On the declared operational-cost branch, only the slowest-fading block survives enough rounds to carry the candidate position readback, and its rank is three. Joined to an independent real axis, it gives an exact finite one-plus-three ambient Lorentzian carrier. Canonical source height, computed from authenticated parent edges and multiplied by one explicit positive global scale, enters only through event placement in that carrier. This does not derive intrinsic poset dimension or a physical clock; physical spacetime requires a faithful manifoldlike refinement and calibrated clocks and volumes.
