...
constraints the first two axioms supply, the realized compatible local state
family is the information projection onto that constraint set. The objective
uses an exact compatible reference, a finite observer cover generated by the
screen architecture, and strictly positive exact weights. The cover must
determine every feasible state family: two feasible families that agree on
the whole cover are the same family. "Random" here means least informative
