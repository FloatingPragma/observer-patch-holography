...
Three consequences arrive free. If a and b are equal to start with, the move does nothing, so the states where the law idles are exactly the consistent ones. Neither observer is corrected by the other: both move, by the same amount, in opposite directions. And Maskelyne's move, which was to overwrite the assistant's reading with the Astronomer Royal's, fails the second requirement, because it changes the total of the two numbers it touches, and needs somebody appointed before it can be carried out at all.
