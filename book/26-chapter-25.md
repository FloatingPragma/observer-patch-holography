...
Two more collections, and they are the ones physicists reach for. Take an object with two slots in it, each holding a complex number, and ask for every reversible change that leaves the total size alone. Then ask the same of an object with three slots. Both counts are short enough to do on your fingers.
