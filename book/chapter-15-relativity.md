...
normalization and an irrelevant constant. This is the geometric branch on the
...
Lorentz kinematics is not assumed. It is recovered on the extracted prime
geometric subnet once the scaling-limit cap pair is realized there, the cap
modular flow is geometric, and the same rigidity hypotheses identify that flow
with the conformal action.
...
generator is read as the effective local null-energy charge on the fixed-cutoff
null bridge, and the same bridge feeds the tensor reconstruction used by the
framework. That step also uses the inherited strip hypotheses, the
same-half-line local modular-Hamiltonian input, and the bounded-interval
projective branch.
...
This is the semiclassical Einstein equation on the declared scaling branch,
obtained by combining the thermodynamic argument with patch consistency in the
scaling regime.

### The Derivation Chain

The chain is straightforward once the branch conditions are in view. MaxEnt
selects the equilibrium state among overlap-consistent configurations.
Entanglement equilibrium gives the thermodynamic relation in each local rest
frame. Geometric modular flow turns modular energy into physical energy. The
...
A third route remains open here. Extra gravitational pull may come from
imperfect information recovery.
...
correlation remains out of reach. That leftover correlation can feed an extra
...
introducing new particle species, but it is not yet a finished derivation of
galaxy-scale dynamics.

### Why It's Dark

This sector is dark by construction. It comes from information structure, it
gravitates, and it does not couple electromagnetically. What is still missing
is a controlled nonrelativistic limit, a derived source-response law, and the
lensing, cluster, and cosmology checks that would turn the continuation into a
full phenomenology.
...
the continuation singles out a characteristic acceleration:
...
logic that shaped the horizon from the start. The current status is an IR
benchmark, not a complete dark-matter or MOND replacement theorem.
...
the Einstein relation on the recovered scaling branch. We have seen how both
