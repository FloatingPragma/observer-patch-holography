...
This is the chapter's Gödel-style analogy: the system contains a description of itself and is interpreted through that self-reference.
...
Returning to the broader observer-first OPH framework, the rest of this chapter addresses:
...
| `H` | `P -> alpha_U -> (t_U, t_tr) -> (eta_source, tau2_tree_exact, delta_n_tree_exact) -> sigma_H_exact -> M_H` | `125.1995304097179 GeV` | exact Higgs calibration theorem on the declared D10/D11 running, matching, and threshold surface, with a compare-only exact inverse Higgs/top slice on the same Jacobian surface |
...
The cosmology-facing theorem package is conditional. If the MaxEnt constraint family is SO(3)-invariant and the optimizer is unique, the reference state is isotropic and the stress tensor has perfect-fluid form. If that isotropy condition holds for all observers, the spatial slices are constant-curvature. Combined with the semiclassical Einstein branch and positive screen-capacity $\Lambda$, the metric takes FLRW form. Markov-control estimates bound patch-to-patch anisotropy and give the collar benchmark used in the CMB homogeneity bookkeeping. No theorem states that the bare OPH axioms force the realized cosmological branch to lie on that symmetric MaxEnt sector.

## 18.8 The Claim Boundary

The core picture has definite physical content. The framework recovers the Standard Model gauge group, three generations, three colors, massless photon, gluons, graviton, and the absence of gauge-mediated proton decay. It also carries a non-hadron package with clear boundaries: an exact \(W/Z\) pair on the electroweak theorem surface, an exact Higgs calibration theorem with a companion D11 top-side calibration output and a compare-only exact inverse validation slice, an exact same-family charged witness, an exact PDG 2025 running quark sextet together with exact forward Yukawas on the quark frame selected by \(P\), and a weighted-cycle neutrino theorem branch with \((m_1, m_2, m_3) = (0.017454720257976796, 0.019481987935919015, 0.05307522145074924)\,\mathrm{eV}\) together with the theorem-grade physical Majorana pair \((\alpha_{21}^{(\mathrm{Maj})},\alpha_{31}^{(\mathrm{Maj})})=(153.618518^\circ,257.003241^\circ)\) on its shared-basis transport branch. Charged leptons require one final absolute anchor. The exact quark theorem is restricted to the quark frame selected by \(P\). Hadron masses lie outside the proved package because they require a production backend plus nonperturbative strong-dynamics work. On the gravity side, the boundary sits at geometric cap-pair realization in the scaling limit and ordered cut-pair rigidity on that realized limit.
...
| Higgs/top branch | An exact Higgs calibration theorem on the declared D10/D11 running, matching, and threshold surface yields \(H = 125.1995304097179\) GeV and a companion D11 top-side output \(t = 172.388646\) GeV, with an exact inverse validation slice on the same Jacobian surface. | the Higgs theorem is the claim surface; the inverse slice is a validation surface |
