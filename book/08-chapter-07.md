...
The observer construction starts one step earlier and derives more of the finite precursor. It generates an authenticated informational order: the child certifies a read, the parent actually wrote that register, the child's writer mark names that parent, and the value read equals the value written. That order has a canonical **source height**: every root has height zero, and every nonroot has one plus the greatest height of its direct authenticated parents. Source height therefore increases strictly on every parent edge and along every generated ancestry chain. It is computed from provenance alone, not from execution-list position, but it is still an ordinal coordinate rather than a physical clock. Independently, a real axis and a positive source carrier of rank three define an ambient product target with real dimension four and one-plus-three Lorentz signs; the unit sphere of the rank-three carrier labels its future-null directions. This carrier exists before any event placement. Its dimension is not an intrinsic dimension assigned to the finite event poset.

A supplied spatial readback and one explicit positive global scale \(\tau\) place an event at \((\tau\,h,x)\) in that ambient target, where \(h\) is source height. If spatial changes satisfy the corresponding edge-speed bound, every authenticated dependency enters the future cone. If the converse holds as well, and no two events share a point, the placement is a faithful finite causal embedding, and every source interval is exactly the corresponding cone interval on the placed event image. The proof exposes these last conditions instead of assuming a four-dimensional chart. Neither \(\tau h\) nor the supplied spatial readback is thereby a physical clock or position, and no continuum limit follows. A manifoldlike refinement must preserve the source order and directions, calibrate event count to volume, recover topology and intrinsic dimension independently, and make the cone and physical signal order agree.

The tested histories make the boundary concrete. At five independently generated and replayed cutoffs from 24 to 384 events, each reconstructed source order agrees exactly with the restriction of the next. Their width stays two while their ordering fraction rises from about 0.884 to 0.992, so the extension is reliable provenance custody and becomes more chainlike. On the 24-event history, one prescribed placement uses positive `timeScale` times source height only for its temporal event-placement coordinate, record-port anchors for position and arithmetic consumed-record barycentres, all expressed in a single stipulated reference frame. It sends every precedence relation into the cone, but eight same-height pairs collide and eight incomparable pairs have zero spatial separation. Injectivity and the converse cone test fail. This rejects that single-frame ansatz. It neither supplies inter-carrier frame gluing nor rules out other source-selected placements.

A second audit asks the causal-set question directly. It checks exact intervals against synthetic four-dimensional Minkowski and curved de Sitter controls, along with deliberately nonmanifold controls. The larger current local-domain cut contains 2,304 events and 736 intervals large enough for a post-hoc exploratory comparison frozen after evaluation for exact replay. None lands in the comparison's four-dimensional ordering-fraction band. That is an exact negative result for this cutoff and this choice of recorded events. It is not a theorem that OPH cannot produce a spacetime: the small source control is too small, the test has no certified density-refinement family, the recurrent seam-repair transactions have not yet been promoted into the physical event carrier, and a held-out confirmation has not run.
