...
There is a physics program built around the same shape. Luca Bombelli, Joohan Lee, David Meyer and Rafael Sorkin called a locally finite partial order a **causal set** in 1987. Its order is meant to become the causal order of spacetime, while the number of elements in a region supplies its volume. Under suitable conditions the causal order of a continuum fixes its light cones and conformal geometry; volume supplies the missing scale. Standard causal-set approximations obtain discrete events by random Poisson sprinkling, which introduces no preferred lattice direction. Most partial orders do not resemble manifolds, so the hard part is proving that a given order has a faithful, four-dimensional spacetime approximation.

The observer construction starts one step earlier. It generates an authenticated informational order: the child certifies a read, the parent actually wrote that register, the child's writer mark names that parent, and the value read equals the value written. Calling that order physical causality requires another result. The commit log is not a Poisson sprinkling. A faithful causal embedding, four-dimensional manifoldlikeness, the count-to-volume rule, agreement with physical signal cones and uniqueness of the continuum approximation do not follow from a finite web of dependencies.

The available controls make that boundary concrete. The current source history misses the exploratory four-dimensional ordering-fraction band. A separate, deliberately imposed face-centred-cubic `O_h` fixture has exact nested provenance. Its carrier-ball counts match the cubic formula on the checked radii `0 <= r <= 20`, and its fine-carrier counts match the quartic formula on the checked values `1 <= n <= 10`; these are bounded matches, not formulas proved for all radii or depths. At depth 20 its ordering fraction sits near 0.10; the quoted dimension near four is just the Myrheim-Meyer inversion of that same number. Its apparent profile improvement reverses in an additional out-of-family depth-24 extrapolation, and its depth-20 interval profile is about 37 times farther from matched Minkowski controls than two independent controls are from each other. Held-out confirmation has not been run. The first result is a negative diagnostic of one event semantics, not a no-go theorem. The second is only a bounded imposed-architecture coarse-value control, not a finite causet similarity or a spacetime derivation.
