...
transport follows on the OPH branch when its geometry, electromagnetic action,
physical photon, electron mass, and recombination history are supplied. It
locates the repair-exchange and collar-stress interfaces where an OPH-specific
effect could enter. A physical CMB prediction requires finite source receipts,
transfer functions, and likelihood closure.
\end{abstract}
...
$\Lambda\ell_\star^2=3\pi/N_\star$ from capacity closure. $N_\star$ is the
SL-4 estimate read from measured $\Lambda$ with the scale bridge of the
strange-loop principle register; $\Lambda$ is an input to this assumption, not an
OPH output. Promotion of $N_\star$ beyond the SL-4 estimate runs through
generator G2 of the project consistency stack: constructing the readback
map $F$ and its contraction certificate. Provenance:
...
the finite tilt certificate is issue \#522. The radial theorem packet in issue
\#330 proves that one-shell inversion is nonunique and supplies physical
source-dilation and cross-covariance tomography as conditional uniqueness
routes. Finite source instantiation in issue \#579 is work in progress. This
note consumes \(A_s\) and \(n_s\) as gated inputs and derives no initial
spectrum.
\end{assumption}

\section{Theorem B1: conditional refinement Liouville transport}
...
(ii) a source-derived between-commit dynamics whose finite-resolution maps
preserve the corresponding phase-space measures, and (iii) receipt
\textsc{Boltz-R1}, which certifies convergence of its characteristics to the
null geodesic flow with vanishing dispersion and measure defect, the
...
\emph{Characteristics.} The Maxwell and overlap-geometry inputs identify the
local positive-energy mass shell and its null directions. Receipt
\textsc{Boltz-R1} supplies the missing dynamical statement: coarse-grained
record continuations converge to the branch metric's null-geodesic
characteristics, independently of the accepted repair schedule. Thus
...
\emph{Measure preservation.} Let $T_h$ denote the finite-resolution
between-commit map and $\mu_h$ its phase-space measure. Assumption (ii) gives
$(T_h)_\ast\mu_h=\mu_h$. The measure-defect and convergence clauses in
\textsc{Boltz-R1} pass this identity to the limiting characteristic flow,
which therefore preserves the Liouville measure. Accepted repair confluence
...
\emph{Receipt status.} Photon-sector convergence of the finite patch flow to
geodesic flow with vanishing dispersion is work in progress. The abstract Lean
layer proves the corresponding schedule-independence and refinement-naturality
statement in its setting. The photon receipt is named \textsc{Boltz-R1}, and
\S\ref{s:receipts} gives its simulator test. Theorem~\ref{t:liouville} is
conditional on this receipt.
\end{proof}
...
between-commit dynamics remain separate structures.
\end{remark}
...
vertex is work in progress. The polarization sector and the associated $16/15$
...
polarized Thomson matrix stated above.  The remaining steps are the textbook
computation (Ma--Bertschinger 1995; Dodelson, \emph{Modern Cosmology}); no
OPH-specific term survives except the two gated channels, which we have
displayed at their unique insertion points.
\end{proof}
...
measured TT curve is produced by this system and by no OPH-specific term.
\end{proposition}
...
\Big\},
\]
($\Pi=F_{\gamma2}+G_{\gamma0}+G_{\gamma2}$; every unlabelled
...
with vanishing measure defect under refinement. This receipt is independent
...
charged-family absolute-scale theorem is work in progress. &
declared import\\
\textsc{Boltz-X1} &
...
conditional radial mathematics \#330; finite source receipt \#579 is work in progress\\
Kernels &
...
It records, conditionally on the cited branch theorems, the \emph{scalar
angular form} of the transport and the locations where the two gated OPH
kernels would enter.  The polarization hierarchy, blackbody background,
absolute normalizations, and recombination history remain imported.  Thus
the physical-CMB prediction gate contains a source-level polarized
collision bridge, blackbody/thermalization provenance, \textsc{Boltz-R1},
and \textsc{Boltz-V1}, in addition to the pre-existing gates (\#330, \#522,
\#374, \#363). It does not certify initial conditions, normalizations,
recombination, or likelihoods, and nothing here reads Planck data. Under the
anti-fitting rule, the acoustic peak structure remains attributable to
standard plasma physics; OPH's falsifiable exposure stays concentrated in
the primordial inputs, exactly where issues \#330 and \#522 put it.
