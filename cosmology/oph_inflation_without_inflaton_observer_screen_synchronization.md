...
\end{gathered}}
```
This claim is not part of the recovered SM/GR core. It becomes predictive only when the source objects are fixed before observational comparison. Screen synchronization instantiates C3 of `../CONSISTENCY_STACK.md` — modular self-consistency, with internal time generated by the state–algebra pair rather than by an external clock.
