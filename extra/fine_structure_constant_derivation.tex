...
seen from within. Principle SL-2 of the strange-loop principle set (\texttt{../STRANGE\_LOOP\_PRINCIPLES.md})
...
substrate pixel readout, so CODATA \(\alpha\) estimates \(P\). The source-only
...
\texttt{../STRANGE\_LOOP\_PRINCIPLES.md} and are cited here rather than restated. Principle SL-2
...
the substrate pixel readout are one quantity, so measured \(\alpha\) estimates
\(P\). Neither principle is derived in this paper; each is a starting point of
deduction, and evidence bears on the pair through the closure-ledger residuals
CL-1 and CL-2 quoted throughout.

In the selection chain of the consistency stack
(\texttt{../CONSISTENCY\_STACK.md}), the detuning law is row C6: after C1--C5
...
Informally: \(\varphi\) is the fixed point of the simplest self-similar split, where the whole-to-large ratio equals the large-to-small ratio. OPH takes it as the zero-detuning balance point for the local pixel by principle SL-2 (\texttt{../STRANGE\_LOOP\_PRINCIPLES.md}); the choice is declared, not deduced from prior structure.
...
existing and unique on its interval (certificate:
\texttt{code/P\_derivation/runtime/p\_interval\_contraction\_certificate\_2026-07-14.json};
...
\(J_{24,Q}\), \(\omega_Q\), \(\Xi_Q\), or \(\rho_Q\). The endpoint target for
that blind completion is frozen and externally timestamped: the target file
and its OpenTimestamps proofs are committed together in
\texttt{falsification/frozen\_targets/}, so the freeze date is verifiable
...
Uniqueness lemma L1 of the consistency stack
(\texttt{../CONSISTENCY\_STACK.md}) states the interval form of assumptions
...
uniqueness of the fixed point in \(I\). The stage-2 interval contraction
certificate of the basin-then-contract protocol is recorded at
\texttt{code/P\_derivation/runtime/p\_interval\_contraction\_certificate\_2026-07-14.json}.
The global at-most-one statement on the full physical domain additionally
requires monotonicity of \(\ATh\) in \(P\), which remains an open analytic
item.
...
Bare-\(\alpha_U\) closure status & \(A_{\alpha_U}^{\mathrm{fp}}\), Eq.~\eqref{eq:barealphaUaddition} & Closure-status diagnostic at the SL-3 pixel; mixed provenance, not a derivation of \(\alpha\). \\
