...
\(3.0\times10^{-4}\) in relative units, and this loop residual is a row of
the closure ledger. The self-consistent gauge-width map has the certified
fixed point \(\alpha^{-1}=137.035660136946577\ldots\), \(2.5\times10^{-6}\)
below the measured \(137.035999177(21)\), about \(1.6\times10^{4}\)
measurement sigma, a second ledger row; the value
\(137.035959513609\ldots\) is a mixed-provenance diagnostic (certified source root
plus \(\alpha_U\) at the SL-3 pixel), not a fixed point of any single declared map.
...
Why this step is needed: the fine-structure constant is measured at low energy, but the source map begins at the cell scale. The transmutation formula explains how the electroweak scale is produced from the same source data. After independent direct public-record closure, the \textsc{COMMON-EW-LOAD-CARRIER} receipt identifies the same source load on the screen and electroweak lanes. It then gives the conditional bridge \(N_{\rm bridge}=\pi\exp[6\pi/(P\alpha_U)]\), certified by interval arithmetic at \(3.53\times10^{122}\). The bridge tests and identifies the capacity; it does not construct the public-record readback.
...
from the certified source root plus \(\alpha_U\) at the SL-3 pixel); it is not a fixed
point of any single declared map. The certified
...
source derivation from an OPH-QCD spectral backend is work in progress.
...
SL-3 pixel and is not a fixed point of a single declared map. The
