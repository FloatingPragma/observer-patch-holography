...
\]
The source/root audit gives \(\alphainv_{\mathrm{root}}=136.9948351646\ldots\) before the low-energy hadronic transport closure. Combining that source root with the unified gauge width evaluated at the CODATA-derived comparison pixel gives the mixed-provenance diagnostic
\[
...
\begin{quote}\small
\textbf{Source/root audit.} The executable source map emits the undressed source/root inverse coupling \(\alphainv_{\mathrm{root}}=136.9948351646\ldots\). Combining it with \(\alpha_U(\Ppub)\), evaluated at the CODATA-derived comparison pixel, gives the mixed-provenance no-hadron diagnostic \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\). It is not a source-only prediction.
...
diagnostic \(A_{\alpha_U}^{\mathrm{fp}}\). It is close to the laboratory endpoint because
...
gauge-width contribution is added. It is near one because \(\alpha_U(\Ppub)\)
...
\label{eq:epsdelta}
\end{equation}
The current OPH corpus does not derive the balanced-circulant condition
\(|b|/a=1/\sqrt2\), a phase-transport law fixing \(\delta=2/9\), or the
attachment of the ordered roots to physical charged-family lines.  For a
general Hermitian circulant the square-root-mass invariant is
\[
Q=\frac{1+2|b/a|^2}{3},
\]
so \(|b|/a=1/\sqrt2\) is exactly equivalent to imposing the empirical Koide
value \(Q=2/3\); it is not a consequence of circulant symmetry alone.  Nor
does the declared MaxEnt law automatically supply it: the natural trace base
weights the real singlet and charged doublet as \(1{:}2\).  Equal block power
requires a separately derived action gap \(S_c-S_0=\ln2\) and a bridge from
ensemble probabilities to deterministic Yukawa powers.  No current \(12/24\)
receipt supplies that attachment.
...
\(0.02284\%\).  On the current public-pixel probe
...
ppm comparison. Its executable receipt is
...
the measured endpoint through \(\Ppub\).
...
\label{eq:c24qfactor}
\end{equation}
The factor \(C_{24,Q}\) is compact comparison accounting for the finite same-scheme hadronic endpoint
...
Informally: the mixed comparison diagnostic is \(137.035959500817\ldots\), and the CODATA/NIST
measured value is \(137.035999177(21)\). Their difference is the size required of the missing
same-scheme endpoint contribution; the open source task is to compute that contribution from an
OPH-QCD spectral backend.
