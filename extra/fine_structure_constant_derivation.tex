...
\]
Solving the source branch gives the fine-structure fixed point and lands close to the Thomson-limit value. The remaining gap has a specific address: low-energy hadronic and endpoint transport in the same electromagnetic scheme. With that empirical endpoint correction stated separately, the public display is \(\alpha^{-1}(0)=137.035999177(21)\) and \(P\simeq1.6309682094\).
\end{abstract}
...
must be supplied to turn the source diagnostic into the public endpoint. That
...
closure, and comparison display.
...
\begin{quote}\small
\textbf{Pure source calculation.} The executable source map solves the OPH fixed point without importing the measured Thomson value. Its no-hadron output is \(\alphainv_{\mathrm{root}}=136.9948351646\ldots\). Adding the emitted public-branch unification width \(\alpha_U(P_\star)\) gives the source-side no-hadron prediction \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\), a number close to the laboratory Thomson endpoint and distinct from it.
...
The OPH source-plus-bare-\(\alpha_U\) calculation lands at
\[
...
\]
Putting back the high-energy unification-width contribution shifts the root value to
\[
A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots.
\]
...
&=
A_{\mathrm{calc}}(P_{\mathrm{root}})+\Dcalib^{\mathrm{fp}}
\nonumber\\
&=
137.035999177
...
\sigma_A=0.000000021.
\end{equation}
Here \(A_{\mathrm C}\) is the CODATA/NIST 2022 inverse fine-structure constant \cite{nist-value,nist-2022}. The source value \(A_{\mathrm{calc}}(P_{\mathrm{root}})=\alpha_{\mathrm{root}}^{-1}\) is the reproducible OPH fixed-point solve \cite{oph-code}. Equation~\eqref{eq:Dempfp} is a calibrated hadronic correction in inverse-alpha units.

It is tempting to identify the correction with the public-branch unification width
\(\alpha_U(P_\star)=0.041124336195630495\), but the exact arithmetic does not support that
identity. Adding only the bare 24-register-scale contribution gives
\begin{equation}
...
\end{equation}
The \(A_{\alpha_U}^{\mathrm{fp}}\) value is the source-side prediction without the missing
hadronic spectral transport. It falls short of the CODATA/NIST central endpoint by
\[
0.000039676182720047050005414212807\ldots
\]
in inverse-alpha units. Equivalently, the calibrated fixed-point residual is
\begin{equation}
\Dcalib^{\mathrm{fp}}
...
transport missing from the source-only proof; the 24-register argument does not emit it.

Informally: the source calculation lands short by a small number with a known physical meaning. The source plus bare-\(\alpha_U\) result is the OPH no-hadron prediction. Adding the hadronic correction calibrated from the measured value lands on the CODATA/NIST central value. It does not predict extra digits, because the hadronic part has not been derived mathematically inside OPH.
...
Calibrated hadronic correction & \(\Dcalib^{\mathrm{fp}}\), Eq.~\eqref{eq:Dempfp} & Empirical endpoint-closure correction, not a source-only prediction. \\
...
\]
Adding the emitted unification-width scale gives the source-side no-hadron prediction,
\[
A_{\alpha_U}^{\mathrm{fp}}=137.035959500817279952949994585787\ldots.
\]
The remaining gap is a named low-energy hadronic and same-scheme endpoint contribution. The public exact-value row is a calibrated endpoint comparison, not a source-only prediction. The pure source theorem requires the Ward-projected hadronic spectral measure, the same-scheme endpoint bridge, and the interval-certificate step.
