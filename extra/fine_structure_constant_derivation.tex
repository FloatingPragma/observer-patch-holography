...
\begin{abstract}
One OPH screen cell has two readings. From the outside, the cell has a dimensionless area \(P\). From the inside, it appears as the electromagnetic observation scale available to observers in the encoded world. The two readings meet at
\[
P=\varphi+\frac{\sqrt\pi}{\ATh(P)}.
\]
Solving this self-referential pixel equation with the OPH source map gives a fine-structure fixed point close to the Thomson-limit value. Informally, the calculation starts from the OPH screen-cell rules and uses zero fitted parameters. Its current output is the source-plus-unified-gauge-width value
\[
...
\[
\alpha^{-1}_{\mathrm{Rb}}=137.035999206(11),
\]
so the OPH output is low by
\[
0.000039705182720047050005414213\ldots,
\]
about \(2.89743\times10^{-7}\) relative in inverse-alpha units. The remaining gap has a specific address: low-energy hadronic and endpoint transport in the same electromagnetic scheme. The source map has not yet emitted the Ward-projected QCD spectral contribution needed to dress the no-hadron value into the laboratory Thomson endpoint.
\end{abstract}

\section*{What This Paper Contributes}

The fine-structure constant is usually treated as a measured input. This paper
turns it into a branch readout of one OPH screen cell. The known physics is the
Maxwell endpoint, charged-lepton running, and low-energy hadronic spectral
transport. OPH adds the pixel self-reference: the outside area coordinate
\(P\) must agree with the inside electromagnetic observation scale read by
observers on the same branch.

The source map gets close without importing the Thomson value. The remaining
gap has an explicit address: the same-scheme hadronic endpoint transport that
must be supplied to turn the source diagnostic into the measured endpoint. That
split is the contribution: a fixed-point
equation for the pixel branch, a visible address for the missing low-energy
...
\begin{quote}\small
\textbf{Pure source calculation.} The executable source map solves the OPH fixed point without importing the measured Thomson value. Its no-hadron output is the undressed source/root inverse coupling \(\alphainv_{\mathrm{root}}=136.9948351646\ldots\). Adding the emitted finite-screen unified gauge-width contribution \(\alpha_U(P_\star)\) gives the source-side no-hadron near-endpoint \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\), a number close to the laboratory Thomson endpoint and distinct from it.

\textbf{CODATA/NIST measured endpoint.} The measured comparison value is \(\alphainv(0)=137.035999177(21)\). It is the CODATA/NIST value used to size the remaining endpoint correction. The OPH source calculation reaches it when the low-energy hadronic spectral step is supplied in the same endpoint convention as the source map. The published electroweak number \(\Delta\alpha_{\mathrm{had}}^{(5)}(M_Z^2)\simeq0.0276\) is the measured running-alpha input behind that physics, but it is not itself the additive inverse-alpha correction used here.
\end{quote}
...
\longrightarrow
\text{fully observed Thomson endpoint},
\]
or numerically,
\[
136.9948351646\ldots
\longrightarrow
...
The contribution \(\alpha_U(P_\star)\) is the finite-screen unified gauge-width
contribution. It is the main high-scale gauge-sector width carried by the same
...
Adding this width to the root gives the source-side no-hadron near-endpoint
\(A_{\alpha_U}^{\mathrm{fp}}\). It is close to the laboratory endpoint because
it supplies almost all of the inverse-alpha gap from the naked source value, but
it is not itself the observed fine-structure constant.

The factor \(C_{24,Q}\) is the small endpoint dressing required after the
gauge-width contribution is added. It is near one because \(\alpha_U(P_\star)\)
supplies the large part of the correction. Its excess
\(\alpha_U(P_\star)(C_{24,Q}-1)\) represents the remaining same-scheme
low-energy hadronic endpoint transport. A zero-momentum laboratory photon does
...
measures the dressed Thomson endpoint, not the naked pixel and not the
source-plus-unified-gauge-width near-endpoint.

\paragraph*{Folklore version.}
The OPH source-plus-unified-gauge-width calculation lands at
\[
A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots.
\]
The 2020 rubidium recoil benchmark gives
\[
1/\alpha_{\mathrm{Rb}}=137.035999206(11),
\]
so the no-hadron OPH number is within one part in about \(3.4\) million of that laboratory
benchmark. The remaining sliver has a physical address. It is the low-energy quark/gluon
vacuum-polarization cloud and same-scheme endpoint transport that a zero-momentum photon sees
when it probes an electron. OPH fences that piece off instead of fitting it.

Strip that quark cloud away and the source problem becomes a piece of geometry: one observer cell
has to agree with itself. The stripped source value is
\[
\alpha_{\mathrm{root}}^{-1}=136.9948351646\ldots.
\]
Putting back the finite-screen unified gauge-width contribution shifts the root value to
\[
A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots.
\]
The remaining difference to the measured Thomson endpoint is the hadronic endpoint package.
One observer cell, asked to agree with itself, fixes the strength of electromagnetism up to that
explicit QCD endpoint boundary.
...
That permission does not turn the calculation into a fit. The source map has to define a
...
\(\Dcalib^{\mathrm{fp}}\) & Root-to-CODATA bookkeeping difference & The inverse-alpha amount between the naked root value and the CODATA/NIST central endpoint; it includes the emitted \(\alpha_U(P_\star)\) contribution and is not the post-\(\alpha_U\) hadronic gap. \\
\(\Dhadreq^{\mathrm{fp}}\) & Required hadronic endpoint correction after \(A_{\alpha_U}^{\mathrm{fp}}\) & The inverse-alpha amount by which the current OPH output misses the CODATA/NIST central endpoint. \\
\(\Rq(P)\) & \(\ATh(P)-[\AZ(P)+\Dcalc(P)]\) & The remaining same-scheme hadronic endpoint contribution. \\
\(A_{\alpha_U}^{\mathrm{fp}}\) & \(\alpha_{\mathrm{root}}^{-1}+\alpha_U(P_\star)\) & Source-side no-hadron prediction before the Ward-projected hadronic transport is supplied. \\
...
\end{equation}
This \(A_{\alpha_U}^{\mathrm{fp}}\) value is the numerical output of the OPH fine-structure calculation in this paper. It is the source-side prediction before the missing Ward-projected hadronic spectral transport is supplied.
...
\end{equation}
This is the amount by which the current OPH output misses the CODATA/NIST central value in inverse-alpha units. Its physical address is the low-energy QCD/hadronic vacuum-polarization and same-scheme endpoint transport of the Ward-projected electromagnetic current.
...
\nonumber\\
&=
\alpha_U(P_\star)+\Dhadreq^{\mathrm{fp}}.
\label{eq:rootcodatafp}
\end{align}
This larger difference is not itself the missing hadronic endpoint correction after the OPH output, because it also contains the emitted finite-screen unified gauge-width contribution.

Equivalently, the CODATA/NIST central value can be written as
\begin{equation}
A_{\mathrm C}
=
\alpha_{\mathrm{root}}^{-1}+\alpha_U(P_\star)\,C_{24,Q},
\qquad
C_{24,Q}=1.0009647859732326253849511140702475\ldots.
\label{eq:c24qfactor}
\end{equation}
The factor \(C_{24,Q}\) is a compact accounting of the finite same-scheme hadronic endpoint
transport missing from the source-only proof. Equivalently,
\(\Dhadreq^{\mathrm{fp}}=\alpha_U(P_\star)(C_{24,Q}-1)\). The 24-register argument emits
\(\alpha_U(P_\star)\), but it does not emit this hadronic correction factor.

Here \(A_{\mathrm C}\) is the CODATA/NIST 2022 inverse fine-structure constant \cite{nist-value,nist-2022}. The source value \(A_{\mathrm{calc}}(P_{\mathrm{root}})=\alpha_{\mathrm{root}}^{-1}\) and the source-plus-unified-gauge-width value \(A_{\alpha_U}^{\mathrm{fp}}\) are reproducible OPH fixed-point outputs \cite{oph-code}. Equation~\eqref{eq:Dempfp} is the measured-comparison residual in inverse-alpha units.

Informally: the OPH calculation currently outputs \(137.035959500817\ldots\). The CODATA/NIST measured value is \(137.035999177(21)\). The difference is a small number with a known physical meaning: the hadronic/QCD endpoint contribution in the same electromagnetic-current convention. OPH has not yet computed that hadronic piece from source data.

Why this step is needed: this marks the present limit of the calculation. More digits require a source-derived hadronic spectral calculation, not more decimal padding or relabeling the CODATA/NIST value as an OPH output.
...
Calculated endpoint & \(A_{\mathrm{calc}}=\AZ+\Dcalc\) & Raw no-hadron source value. \\
Bare-\(\alpha_U\) source prediction & \(A_{\alpha_U}^{\mathrm{fp}}\), Eq.~\eqref{eq:barealphaUaddition} & Source-side no-hadron prediction. \\
...
A_{\alpha_U}^{\mathrm{fp}}=137.035959500817279952949994585787\ldots.
\]
The remaining gap is a named low-energy hadronic and same-scheme endpoint contribution. The CODATA/NIST exact-value row is a measured endpoint comparison. The pure source theorem requires the Ward-projected hadronic spectral measure, the same-scheme endpoint bridge, and the interval-certificate step.
