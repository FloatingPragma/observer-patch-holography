...
\begin{abstract}
Observer Patch Holography (OPH) is the finite observer-overlap framework introduced in \emph{Observers Are All You Need} \cite{oph-synthesis}. In that framework, one OPH screen cell has two readings. From the outside, the cell has a dimensionless area \(P\). From the inside, it appears as the electromagnetic observation scale available to observers in the encoded world. The two readings meet at
\[
P=\varphi+\frac{\sqrt\pi}{\ATh(P)}.
\]
Solving this self-referential pixel equation with the OPH source map gives a fine-structure fixed point close to the measured low-energy value. Informally, the calculation starts from the OPH screen-cell rules and uses zero fitted parameters. Its current output is the source-plus-unified-gauge-width value
\[
...
The source map gets close from OPH source data alone. The remaining
...
\begin{quote}\small
\textbf{Pure source calculation.} The executable source map solves the OPH fixed point from screen-cell data alone. Its no-hadron output is the undressed source/root inverse coupling \(\alphainv_{\mathrm{root}}=136.9948351646\ldots\). Adding the emitted finite-screen unified gauge-width contribution \(\alpha_U(P_\star)\) gives the source-side no-hadron near-endpoint \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\), close to the laboratory value.

\textbf{CODATA/NIST measured endpoint.} For comparison, CODATA/NIST reports \(\alphainv(0)=137.035999177(21)\). The gap from \(A_{\alpha_U}^{\mathrm{fp}}\) identifies the remaining low-energy hadronic correction in the same electromagnetic convention as the source map.
\end{quote}
...
it supplies almost all of the inverse-alpha gap from the naked source value.
...
\(A_{\alpha_U}^{\mathrm{fp}}\) is the no-hadron value before that dressing.
...
transport still needed for source-only closure. Equivalently,
\(\Dhadreq^{\mathrm{fp}}=\alpha_U(P_\star)(C_{24,Q}-1)\). The 24-register argument emits
\(\alpha_U(P_\star)\); the correction factor is the remaining hadronic/QCD payload.
...
Informally: the OPH calculation currently outputs \(137.035959500817\ldots\). The CODATA/NIST measured value is \(137.035999177(21)\). The difference is a small number with a known physical meaning: the hadronic/QCD contribution in the same electromagnetic-current convention. The open task is to compute that hadronic piece from source data.

Why this step is needed: this marks the present limit of the calculation. Additional digits require a source-derived hadronic spectral calculation.
...
Informally: the endpoint number is the same number NIST/CODATA reports. The derivation exposes every OPH-side step and identifies where the low-energy hadronic calculation enters. The missing hadronic spectral calculation remains a named open step.
