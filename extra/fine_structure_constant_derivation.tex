...
\begin{abstract}
OPH starts from a simple demand: finite observer patches must agree on the observations they share. This paper follows one local screen cell through that demand. The outside reading is geometry: a dimensionless pixel area \(P=a_{\mathrm{cell}}/\ell_P^2\), measured relative to the golden-ratio balance point \(\varphi=(1+\sqrt5)/2\). The inside reading is electromagnetism: the Thomson-limit observation width emitted by the world encoded on the same screen. Boundary Gaussian normalization converts the outside displacement into \((P-\varphi)/\sqrt\pi\). The inside readout is \(\alpha_{\mathrm{in}}(P)=1/\ATh(P)\), where \(\ATh(P)=\alpha_{\mathrm{em}}^{-1}(0;P)\) is the inverse electromagnetic coupling at zero momentum. The cell is consistent when
\[
P=\varphi+\frac{\sqrt\pi}{\ATh(P)}.
\]
The executable source calculation gives
\[
\begin{aligned}
\alphainv_{\mathrm{cand}}&=\decnum{136.994835164621649457949994585787193262029}\\
P_{\mathrm{cand}}&=\decnum{1.63097209569432901817967892561191884270169}
\end{aligned}
\]
This number is expected to miss exactly the part left out of that source calculation: low-energy hadronic vacuum polarization and the finite endpoint terms that belong to the same electromagnetic-current scheme. These terms enter the inverse-alpha transport from the electroweak anchor to the Thomson limit. Calibrating that missing hadronic correction against the CODATA/NIST central value gives
\[
\Dcalib^{\mathrm{fp}}
=
\decnum{0.041164012378350542050005414212806737971}
\]
Adding it gives the CODATA/NIST central value
\[
\begin{aligned}
A_{\mathrm{calc}}^{\mathrm{fp}}
&=
\decnum{136.994835164621649457949994585787193262029}\\
\Dcalib^{\mathrm{fp}}
&=
\decnum{0.041164012378350542050005414212806737971}\\
A_{\mathrm{calc}}^{\mathrm{fp}}+\Dcalib^{\mathrm{fp}}
&=
137.035999177
\end{aligned}
\]
The correction above is not an independently measured constant with all those digits. Its uncertainty is inherited from the measured fine-structure constant. The public endpoint is
\[
\alphainv(0)=137.035999177(21),
\qquad
P_{\mathrm C}=1.630968209403959324879279847782648941\ldots .
\]
\end{abstract}
...
\(P\) & \(a_{\mathrm{cell}}/\ell_P^2\) & The dimensionless area of one screen cell in Planck-area units. \\
\(a_{\mathrm{cell}}\) & Physical screen-cell area & The area of the local pixel on the holographic screen. \\
\(\ell_P\) & Planck length & The length unit used to make \(P\) dimensionless. \\
...
\(E_P\) & Planck energy & The energy unit used by the source map. The numerical equations use Planck units unless GeV labels are attached. \\
...
\label{eq:Pdef}
\end{equation}
Here \(a_{\mathrm{cell}}\) is the screen-cell area and \(\ell_P^2\) is the Planck area.

Informally: \(P\) says how large the screen cell is when measured in Planck-area units. It is dimensionless, so it can be compared directly with pure numbers such as \(\varphi\) and \(\sqrt\pi\).

Why this step is needed: \(\alpha\) has no units. A unitless electromagnetic number has to be matched to a unitless geometric number. Dividing the cell area by the Planck area gives that number.
...
Pixel variable & \(P=a_{\mathrm{cell}}/\ell_P^2\) & Defined. \\
...
\[
\alphainv_{\mathrm{cand}}=136.994835164621649457949994585787193262029.
\]
