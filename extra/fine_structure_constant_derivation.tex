...
matching enters the map, and the convention's status is a counted selection
under the claim rules of the program scorecard.
...
\]
Combining that comparison-pixel gauge width with the historical printed root display of
Equation~\eqref{eq:Acalcfp} gives the mixed-provenance
...
\end{equation}
where \(A_{\mathrm{calc}}(P_{\mathrm{root}})\) carries the superseded printed tail of the
historical record; with the certified root \(\alphainv_{\mathrm{root}}=136.994835177413\ldots\)
the same sum reads \(137.035959513609\ldots\), a \(1.3\times10^{-8}\) shift inside the packet's
display tolerance,
or
\begin{equation}
