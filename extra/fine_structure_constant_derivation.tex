...
\]
Solving this self-referential pixel equation with the OPH source map gives a fine-structure fixed point close to the measured low-energy value. Informally, the calculation starts from the OPH screen-cell rules and uses zero fitted parameters. The source-plus-unified-gauge-width value is
\[
...
Informally: the OPH calculation outputs \(137.035959500817\ldots\). The CODATA/NIST measured value is \(137.035999177(21)\). The difference is a small number with a known physical meaning: the hadronic/QCD contribution in the same electromagnetic-current convention. The open task is to compute that hadronic piece from source data.
