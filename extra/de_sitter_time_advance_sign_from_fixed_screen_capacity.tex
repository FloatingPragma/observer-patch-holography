...
These statements feed a proposed conditional stance row listed pending the
owner's freeze. A valid protocol must name post-freeze survey posteriors,
freeze the scored range, statistic, thresholds, likelihood combination,
fallbacks, and direction-neutral systematics checks, and treat every
pre-freeze contour as seen data. Exclusion of \((-1,0)\) in any direction
...
electroweak-closure tangent. It requires three extra premises: the closure
capacity is the same \(N\), the closure holds at each epoch, the local clock
reads the same homogeneous cosmological \(\alpha\) and cosmic-time drift, and
physical evolution follows that one-dimensional solver-root tangent while the
other inputs and unit definitions stay fixed. At the present root the numerical
tangent is \(\mathrm d\ln\alpha/\mathrm d\ln N=-4.76\). Only under all three
premises does the optical-clock drift limit imply the local bound
\(\lvert1+w_0\rvert\lesssim7\times10^{-10}\)~\cite{filzinger2023}. The
