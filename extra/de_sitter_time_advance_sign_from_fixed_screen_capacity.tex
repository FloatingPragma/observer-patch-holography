...
\section{The fixed-capacity equation of state}

The capacity reading of the vacuum term supplies a conditional
model-discrimination test through the equation of state of dark energy,
and its algebraic law is machine-checked. Read the
dark-energy density through the capacity relation as
\(\rho_{\rm DE}(a)=\kappa/N(a)\), with \(\kappa\) the single positive
constant packaging \(c^4\,3\pi/(8\pi G\,l_P^2)\), and define the effective
equation of state through the FRW continuity relation,
\(w(a)=-1-\tfrac13\,\mathrm d\ln\rho_{\rm DE}/\mathrm d\ln a\), which is
the effective-fluid quantity projected by a \(w_0w_a\mathrm{CDM}\)
analysis within its chosen cosmological likelihood. Three exact
conditional statements follow, each proved in
\texttt{FixedCapacityWLaw} of the Lean corpus with the capacity value
cancelling throughout.

First, on the fixed-capacity branch, constant \(N\), the equation of state
is \(w(a)=-1\) identically, and the CPL representation is forced to
\((w_0,w_a)=(-1,0)\) exactly, with zero continuous freedom within that
branch. This is the immediate consequence of the assumed
\(\rho_{\rm DE}=\kappa/N\) identification and the continuity definition of
\(w\); it does not derive fixed capacity, the density identification, or a
closed dark sector. Second, the exact converse holds on the drift branch:
\(w(a)=-1+\tfrac13\,\mathrm d\ln N/\mathrm d\ln a\), so any measured
departure from \(-1\) is a capacity-drift readback only conditional on
those same identifications; the power law
\(N\propto a^{\varepsilon}\) gives the exact line
\(w=-1+\varepsilon/3\). Third, if the record capacity is monotone
nondecreasing, then \(w(a)\ge-1\) at every epoch: this conditional branch
forbids a phantom equation of state, and an established
\(w(a_0)<-1\) forces capacity loss at \(a_0\) through the strict readback.

A prospective test would require the dataset combination, redshift range,
statistic, thresholds, fallbacks, and direction-neutral systematics policy to
be fixed before exposure. The comparisons below are retrospective diagnostics
and carry no prospective score. Exclusion of \((-1,0)\) in any direction
conflicts with the fixed branch; a posterior-mean quadrant is not a valid
extra gate. Conversely, compatibility with \((-1,0)\) does not validate
fixed capacity or OPH because that point is also the standard
\(\Lambda\)CDM null. No comparison is frozen or armed by this paper.

For a seen-data diagnostic, a script checked the official SHA-256 values for
four DESI DR2 chains in each of four \(w_0w_a\)CDM combinations. It then
applied the exact CPL endpoint condition
\(w_0\ge-1\) and \(w_0+(2/3)w_a\ge-1\) on \(0\le z\le2\). The posterior-weight
fractions in that conditional monotone subset are \(0.00705\%\),
\(0.1125\%\), \(0.01206\%\), and \(0.02054\%\) for BAO+CMB and its Pantheon+,
Union3, and DESY5 combinations, respectively~\cite{desidr2}. Two
combinations have at least one chain with no raw sample in the rare subset.
The respective raw tail counts are 6, 126, 15, and 11. Union3 is represented
in all four chains but remains sparse.
These seen-data fractions depend on the collaboration's model, likelihood,
and priors; they are not model evidences, frequentist exclusions, direct
capacity measurements, or a prediction score.

Four separately authenticated official DESI DR2 BAO+CMB base-\(\Lambda\)CDM
chains give, under a direct samplewise nonlinear transform,
\[
 \Lambda\ell_P^2=(2.96770\mathbin{\pm}0.03978)\times10^{-122},
\]
with weighted 2.5--97.5 percent interval
\([2.88997,3.04530]\times10^{-122}\). This automatically retains the sampled
\(H_0\)--\(\Omega_\Lambda\) covariance. It is a target-informed retrospective
\(\Lambda\)CDM posterior display, not an OPH likelihood or theory
uncertainty~\cite{desidr2,ophrepo}.

A second conditional diagnostic composes the capacity law with the local
electroweak-closure tangent. It requires three extra physical premises: (B1)
the closure capacity is the same physical \(N\); (B2) the finite closure
relation is an epochwise physical law; and (B3) physical evolution selects the
committed root branch and co-variation convention while the local clock reads
the same homogeneous cosmological \(\alpha\) with the required time map. The
mathematical derivative is interval-certified independently of those physical
premises. An outward-rounded implicit-function calculation proves the selected
declared root branch continuously differentiable for
\(\lvert\Delta\ln\alpha\rvert\le10^{-5}\), with
\[
...
\]
The reciprocal is formed only after zero is excluded. Using the conservative
slope bound and \(H_0=67.4\,\mathrm{km\,s^{-1}\,Mpc^{-1}}\), all three
physical premises together let the optical-clock drift limit imply
\(\lvert1+w_0\rvert\le6.94\times10^{-10}\)~\cite{filzinger2023}. The static
closure solver does not select the physical cosmic branch, and its certified
local domain cannot be extrapolated over the order-one changes suggested by
some CPL histories. The diagnostic restricts but does not empty the monotone
branch.
...
Manifest authenticated by the replay:
\url{https://data.desi.lbl.gov/public/papers/y3/bao-cosmo-params/dr2_vac_dr2_bao-cosmo-params_v1.0.sha256sum}

\bibitem{ophrepo}
B.\ M\"uller et al.,
\emph{Observer-Patch Holography: formal proofs and executable receipts},
2026.
\url{https://github.com/FloatingPragma/observer-patch-holography/tree/main/code/cosmology/fixed_capacity_wlaw}

\bibitem{filzinger2023}
M.~Filzinger et al.,
\emph{Improved Limits on the Coupling of Ultralight Bosonic Dark Matter to
Photons from Optical Atomic Clock Comparisons},
Phys. Rev. Lett. \textbf{130}, 253001 (2023).
