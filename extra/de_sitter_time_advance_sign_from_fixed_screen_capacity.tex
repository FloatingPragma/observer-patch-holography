...
The capacity reading of the vacuum term is falsifiable through the
equation of state of dark energy, and the law is machine-checked. Read the
...
exactly the object a \(w_0w_a\mathrm{CDM}\) analysis reconstructs from the
expansion history. Three exact statements follow, each proved in
...
\((w_0,w_a)=(-1,0)\) exactly, with zero continuous freedom. Second, the
exact converse holds on the drift branch:
\(w(a)=-1+\tfrac13\,\mathrm d\ln N/\mathrm d\ln a\), so any measured
departure from \(-1\) is a direct readback of capacity drift; the power
law \(N\propto a^{\varepsilon}\) gives the exact line
\(w=-1+\varepsilon/3\). Third, if the record capacity is monotone
nondecreasing, then \(w(a)\ge-1\) at every epoch: the capacity reading
forbids a phantom equation of state on both branches, and an established
\(w(a_0)<-1\) forces capacity loss at \(a_0\) through the strict readback.

These statements feed the registered stance row of the frozen-prediction
ladder, which enters pending the owner's freeze: the comparison protocol
names post-freeze survey posteriors only, every contour published before
the freeze is seen data eligible for postdiction bookkeeping alone, and
every numeric kill cell is fixed at freeze time. No comparison is armed by
this paper.
