...
\begin{abstract}
This paper sketches an OPH dark-sector continuation. In OPH, gravity is the stress bookkeeping of stable agreement between finite observer patches. Imperfect agreement leaves a record-repair remainder on overlap collars. That remainder is electromagnetically dark and gravitates as an information-defect stress correction.

The main achievements are the galaxy-limit equation, the cluster and cosmology contracts, and the coefficient target. In old, settled, low-acceleration galaxies the correction can approximate the MOND acceleration law. In clusters and cosmology it must be treated as transported stress with its own equations. The full likelihood evaluation is work in progress.
\end{abstract}
