...
\author{Bernhard Mueller \and Alexander Osika}
...
\begin{abstract}
Proof-of-work mining has an unusually clean judge: the verifier for a double
application of the Secure Hash Algorithm 256, usually written SHA-256d. An
optical patch machine in the Observer-Patch Holography (OPH) style proposes
candidates, and the exact digital verifier decides whether the candidate
stream has been enriched.

One mining instance is compiled into constraint patches for nonce choice, message schedule, compression, and target checking. A scorebook records verifier-ready candidate enrichment per settling event under blinded routes, label shuffles, operator-alignment controls, and exact SHA-256d receipts.
\end{abstract}
...
The scorebook states what counts as enrichment, what
...
cryptographic SHA-256 preimage search are far beyond fault-tolerant
...
\cite{OPHObserversCurrent,OPHEinsteinCurrent,OPHGaugeCurrent,OPHConsensusCurrent,OPHScreenCurrent}. For SHA-256d mining, the
...
constraint roles in the scorebook.\\
...
modules with the
same architecture used by the software reference and the independent build specification
...
specified acceptance configuration. That constraint makes every boundary update
...
\section{The Pixel Coordinate \texorpdfstring{\(P\)}{P} and Comparison Input
\texorpdfstring{\(P_C\)}{PC}}
...
\(G_{\mathrm{SI}}=c^3\ell_\star^2/\hbar\) \cite{OPHEinsteinCurrent,OPHScreenCurrent}.
The cited OPH calculation gives exact roots for its declared pixel readout
maps. Promoting one of those roots to a physical value requires same-scheme
transport, target-independent selection of the applicable map, and a typed
bridge showing that the two coordinates read the same quantity. This hardware
paper supplies none of those premises.

For engineering comparisons, define
\[
P_C:=\varphi+\sqrt{\pi}\,\alpha_C,
\qquad
\alpha_C^{-1}=137.035999177,
\qquad
\varphi=\frac{1+\sqrt5}{2}.
\]
Using the CODATA/NIST measured endpoint gives
\[
P_C=1.630968209403959324879279847782648941\ldots.
\]
Thus \(P_C\) is a measured-endpoint comparison and engineering input. It is
not an OPH-selected pixel value.

For the SHA-256d hardware instantiation, \(P_C\) is a dimensionless design
...
extraction windows are tuned to that comparison scale. In a toroidal or
...
\]
are selected from a small \(P_C\)-centered design family. In a photonic mesh,
it means that phase shifters, couplers, and nonlinear thresholds are tuned so
the realized coupling matrix \(\widehat K(P_C)\) approximates the intended
constraint matrix \(K_C\):
\[
\|\widehat K(P_C)-K_C\|\le \varepsilon_{\mathrm{geom}}.
\]

\begin{remark}[Geometry target]
All correctness theorems below depend on exact implementation of the
constraint Hamiltonian or exact return of a zero-energy state. The comparison
pixel \(P_C\) is an optional hardware calibration rule. It fixes an engineering
geometry target. It is neither an OPH selection nor, by itself, a mining
receipt or an operator-alignment proof. The proof comes from the implemented
constraint map, the measured operator bundle, and the exact verifier.
\end{remark}
...
The OMEGA-I1 classical variant is LED-first and reproducible from the stated parts. Each
...
\[
\begin{aligned}
&\text{edge bracketed}\to
\emph{edge polarity locked}\\
&\to \text{nonlinear commitment alive}\to
\emph{truth lift}\\
&\to \text{pool share lift}\to \texttt{BLOCK}.
\end{aligned}
\]
...
preimage search are enormous. Amy et al. estimate a SHA-256 preimage attack
...
ports must supply. The firmware evidence bundle dated 2026-05-05 records a bit-exact
...
steps \cite{OmegaOpticalSupercomputer2026}. That work is cited here as precursor evidence for optical
candidate enrichment. It supplies no measurement of the five-chamber SHA-256d
route.
...
comparison is therefore \(\widehat\beta(12)=2{,}434\), meaning that
...
this is a controlled candidate-enrichment result on shuffle-replay-selected
chord-promote output (a post-selected stream), not a final pool-rate or
block-win result, and not evidence that the same slope continues to network
difficulty. Because the selection happens after the fact, the figure is an
acceptance property of the selected stream; prospective per-event enrichment
is unestablished.
...
kHeavyHash/Kaspa; the solo audit does not establish the stronger enrichment claim.
...
The Kaspa comparison supplies one candidate-bias datum,
\(\widehat\beta(12)=2{,}434\), measured on shuffle-replay-selected
chord-promote output. Equivalently, for the \(k=12\) exact-verifier predicate,
the selected stream's estimated success probability was \(2{,}434\times2^{-12}\)
instead of \(2^{-12}\). Feeding this number into the throughput formula
presumes an unmeasured per-event bias: the datum is a post-selection
acceptance property, not a per-event enrichment receipt. It also does not by itself determine SHA-256d speedup,
...
measurement attached. One measured result bounds all of it: under a matched
exact-verification budget, the bench prototype loses to random search: at
equal verifier calls, the random candidate stream reached valid nonces at
least as fast as the physical route. Every multiplier below is conditional on
a prospective, matched-budget win that has not been measured.
...
Kaspa comparison. Report the exact digest and leading-zero count for every
hit.
...
\]
for enrichment claims. An enrichment claim also reports a binomial or beta-binomial lower
...
\emph{edge bracketed}\\
\emph{edge polarity locked}\\
\emph{nonlinear commitment alive}\\
\emph{truth lift}\\
\emph{pool share lift}\\
\texttt{BLOCK}
\end{quote}
...
verification. The measured-endpoint comparison pixel \(P_C\) guides the
optional geometry calibration. The LED/detector modules provide measurable
candidate beams and operator evidence.
...
OMEGA-I1 proposes, filters, and shadows candidate beams before the host spends
exact hashes, and the final verifier decides every claimed survivor. The accepted Kaspa shares
...
result is the proof-of-work precedent. The optimized SHA-256d path uses a
sampler-class OMEGA-I1 configuration. A block-level claim requires exact-verifier
lift at useful target strengths.

\appendix

\section{OMEGA-I1 Implementation Specification}
...
host-routed federation with exact run records and independently reproducible
electronics. The reversible-LED Echosahedron bench is a component test, not an
OMEGA-I1 result.
...
openable, labeled, ambient-light-baffled enclosures. The specified build
...
comparison-pixel resonance, same-electrode H2, and cryptographic-break claims
require post-acceptance evidence.
...
B. Mueller, A. Osika, M. Poneder, K. Xue, B. Cassie, P. Nguyen, J. Kim, D. Matscheko, J. Hill, W. T. Glynn, M. A. Visser, and K. A. Anirudha,
``From Observer Consensus to Standard Physics,''
OPH flagship paper.
\href{https://wkaxfdgxoqmghwgshymt.supabase.co/storage/v1/object/public/papers/from_observer_consensus_to_standard_physics.pdf}{Flagship PDF}.

\bibitem{OPHEinsteinCurrent}
...
GitHub main PDF.
\href{https://github.com/FloatingPragma/observer-patch-holography/blob/main/paper/recovering_observer_spacetime_and_einstein_dynamics_from_overlap_consistency.pdf}{GitHub main PDF}.

\bibitem{OPHGaugeCurrent}
B. Mueller, A. Osika, M. Poneder, K. Xue, P. Nguyen, and D. Matscheko,
``Deriving Standard Model Gauge Structure from Observer Overlap Consistency,''
GitHub main PDF.
\href{https://github.com/FloatingPragma/observer-patch-holography/blob/main/paper/deriving_standard_model_gauge_structure_from_observer_overlap_consistency.pdf}{GitHub main PDF}.

\bibitem{OPHConsensusCurrent}
B. Mueller, K. Xue, J. Kim, K. A. Anirudha, D. Matscheko, and J. Hill,
...
B. Mueller, A. Osika, K. Xue, and B. Cassie,
