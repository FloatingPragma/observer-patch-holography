...
\author{Bernhard Mueller, Alexander Osika, and David Matscheko}
...
\begin{abstract}
The coherent-material branch of Observer-Patch Holography (OPH) supplies a canonical
...
\(\chi_\nu^{\rm can}=1-P/24\). At the CODATA-derived comparison pixel this
is \(0.9320429912748350\ldots\). Under the weaker
...
cannot support a closed device: no formula proportional only to
\(g^2A\chi_\nu\Delta S/(4\pi G)\) is a force theorem for that device class.
This paper supplies no dimensional source calibration, ambient repair field,
compact-phase completion, or laboratory source receipt; no laboratory force
prediction and no numerical lift prediction follow from \(\chi_\nu\).
\end{abstract}
...
action in the dark-sector paper~\cite{oph-dark}. The stated axioms and
theorems supply neither that action nor a dimensional source normalization; both enter
as premises, and every force statement below is conditional on them.

No laboratory force prediction follows from this paper. The formula
\(g^2A\chinu\Delta S/(4\pi G)\) is not a force theorem for a closed compact
device, and a static, closed, repair-neutral device cannot lift itself.
...
Dimensional coupling \(q_\star\) & undetermined; a named premise of every force statement \\
Device force & conditional on source charge, external field, and momentum closure \\
Closed compact neutral device & no force theorem; the formula \(g^2A\chinu\Delta S/(4\pi G)\) is excluded for this class \\
Laboratory force prediction and numerical lift & none \\
\bottomrule
\end{tabularx}
\end{center}
...
\label{eq:reserve}
\end{equation}
The symbolic result below holds for the supplied pixel coordinate. Its decimal
evaluation uses the measured-endpoint comparison input
\(P_C=1.630968209403959\ldots\), obtained from the CODATA/NIST fine-structure
endpoint. It is not a source-selected OPH value.
...
\label{eq:canonical}
\end{equation}
For the comparison input \(P=P_C=1.630968209403959\ldots\),
\begin{equation}
...
nonnegative integer occupancy, then at the same comparison input
\begin{equation}
...
\begin{remark}[Depth limit]
The iterated fine-grained survival family
\((1-(P/24)\,2^{-m})^{2^m}\) tends to \(e^{-P/24}\) as the refinement depth
\(m\) grows. That exponential is the depth limit, unreachable at any finite
regulator; the finite one-step survival is the presence value \(1-P/24\),
and the presence branch of the theorem uses it.
\end{remark}
...
The dark-sector completion takes integer scalar repair occupation and a
...
positive localized source gives force \(-Q_R\nabla\theta\). The action is a
posit: it comes with no globally single-valued compact-phase coupling, no
derivation from finite repair dynamics, and no dimensional constants. Each of
these three objects is a premise of the conditional consequences below.
...
nonzero balance signal does not establish repair charge. A test of
\(\chican\) requires the action, source calibration, and operational source
observable listed above.
...
excluded, the closed-compact-device formula
\(g^2A\chinu\Delta S/(4\pi G)\) is not a force theorem, and no laboratory
force prediction follows from this paper.
