...
pair carries a single remaining source object: the criticality law fixes the
whole family from the gauge sector, and the boundary-scale candidate is written
down and frozen before any three-loop computation, with the three-loop implied
scale registered as the discriminating test. Along the fit-free criticality
...
selection. Back-solving the declared calibration surface from the measured pair
gives the sharper \((125.20,\,172.35)\)~GeV fit; that surface validates the
formula stack, is never a prediction, and is kept in the particle branch and
not headlined here.
