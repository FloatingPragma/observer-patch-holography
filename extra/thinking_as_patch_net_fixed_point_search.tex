...
This is a definitional identification inside OPH, not a derived result. It replaces the production picture with a primitive self-read channel inside a bounded observer patch. The definition rides consistency-stack row C1 (\texttt{../CONSISTENCY\_STACK.md}): the same self-reading requirement that forces records and repair at the substrate level is what this identification names when it is satisfied inside one bounded patch.
