...
imports these nine modules alongside the wider OPH event-algebra library.

The checked development contributes nine pieces:
...
Table~\ref{tab:modules} lists the nine source modules. The comparison in
...
Table~\ref{tab:scale} describes the nine modules discussed here and the
...
declarations, and a clean build emits the generated output. The 141 audited
...
theorems. The supplied-state Robertson bound and the partition-relative
operational quotient use the same finite matrix API. Each formal statement in
the paper points to a declaration in the repository source.
...
partition-relative quotient, the supplied-state Robertson inequality, and the
state-level CHSH corollary. It contains no complete-positivity theorem,
completely positive trace-preserving channel bundle, Physlib
spectral-partition correspondence, source-selected quantum instrument, or
two-qubit sharpness witness for the CHSH bounds.
