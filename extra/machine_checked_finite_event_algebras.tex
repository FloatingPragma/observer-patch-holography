...
bound $2\sqrt2$ exactly, while every record-diagonal state read through
diagonal observables valued in $[-1,1]$ obeys the classical bound $2$. A
typed operational phase-instrument structure reduces the discharge of the
repository's open phase-tomography premise to the inhabitation of one
structure, with exact necessity receipts and an uninhabited missing-data
type. The artifact pins
...
imports these thirteen modules alongside the wider OPH event-algebra library.

The checked development contributes thirteen pieces:
...
\item a typed operational phase-instrument structure that reduces the
discharge of the repository's open phase-tomography premise to the
inhabitation of one structure, with conditional sufficiency, exact
diagonal-insufficiency necessity, and a partial witness recording which
fields committed objects inhabit; and
...
trace preserving; the thirteen-module audit here covers their event-algebra
...
delimitation. Section~\ref{sec:phase-instrument} describes the typed
operational phase instrument.
...
Table~\ref{tab:modules} lists the thirteen source modules. The comparison in
...
\texttt{Operational\allowbreak Phase\allowbreak Instrument} & typed
phase-instrument structure, conditional tomography sufficiency, exact
diagonal-insufficiency necessity, partial witness, and the discharge
reduction \\
\bottomrule
\end{tabular}
...
witness. Its carrier objects come from the wider library. The repository
tracks open physical premises in a numbered register; row PR-44 supplies a
...
instruments are constructed. The register keeps those items open (row
PR-52), and the receipt consumes the slot split of row PR-44 as a
supplied datum.
...
diagonality is therefore insufficient for the value $2\sqrt2$. Attainment
requires non-diagonal structure, of exactly the kind whose operational
status the register row PR-04 leaves open, and no theorem asserts that
the repository's committed phase lift supplies it.

\section{A typed operational phase instrument}
\label{sec:phase-instrument}

The module \texttt{OperationalPhaseInstrument.lean} types the operational
content that register row PR-04 owes, on the committed two-dimensional
record algebra. The structure \texttt{OperationalPhaseInstrument} has one
shared preparation, a finite context family consisting of the committed
web contexts plus one designated phase context, per-context binary
outcome effects forming exact POVMs with an explicit phase-sensitivity
clause, realized outcome counts with the diagonal counts pinned to the
committed run literals $(111,68)$, and one exact Born-consistency law
tying every context's realized frequency to the same preparation.

Sufficiency is conditional. Every inhabitant of the structure yields
fixed-trace state identification through the committed algebraic phase
completion (\texttt{operational\_instrument\_completes\_tomography}), and
the preparation is the unique state reproducing the realized frequencies
(\texttt{preparation\_identified\_by\_realized\_data}). Discharge of the
row therefore reduces to inhabiting the structure, and the reduction is
itself a theorem: the proposition \texttt{DischargesPR04} is equivalent
to \texttt{Nonempty MissingPhaseReceipts}
(\texttt{dischargesPR04\_iff\_missing\_receipts}), where
\texttt{MissingPhaseReceipts} collects exactly the missing data: a
preparation, rotated and phase outcome counts, and Born-consistency
receipts. The partial witness \texttt{partialWitness} records which
fields committed objects inhabit: the algebraic core, every context
effect matrix with its POVM certificates, and the diagonal counts; the
missing data enter as explicit hypotheses.

Necessity is exact. Two certified distinct states carry identical Born
...
\texttt{phase\_effect\_genuinely\_complex}). The phase context is
therefore necessary (\texttt{phase\_context\_necessary}).

The boundary is custody. Row PR-04 stays open: no declaration in the
repository inhabits \texttt{OperationalPhaseInstrument},
\texttt{MissingPhaseReceipts}, or \texttt{DischargesPR04}, since the
committed payload supplies no preparation declaration and no rotated or
phase outcome counts. An inhabitant certifies internal coherence of the
supplied data: one preparation, exact POVMs, and an exact Born match. The
provenance of the supplied counts, namely that they are producer receipts
of a committed simulator run rather than chosen numbers, is a custody
condition the register tracks outside the type system; the type checker
cannot enforce it, and the register's discharge discipline prohibits
synthetic counts. None are constructed here.
...
Table~\ref{tab:scale} describes the thirteen modules discussed here and the
...
\texttt{TsirelsonSaturation} & 829 & 39 \\
\texttt{OperationalPhaseInstrument} & 844 & 39 \\
\texttt{EventAlgebra} (root) & 130 & 0 \\
\midrule
Total & 5036 & 230 \\
\bottomrule
\end{tabular}
...
declarations, and a clean build emits the generated output. The 230 audited
...
corollary, and an exact saturation witness on declared data; record-diagonal
readouts capped at the classical bound \\
...
theorem produces the state or the events from a committed source run, and
the typed operational phase instrument has no inhabitant. The entire
...
record-diagonal delimitation, and the typed phase-instrument reduction; the
...
phase-instrument structure is uninhabited.
