...
\item a static conformance layer for that structure: under the register row
that declares the committed exact lift as a phase-sensitive effect, a
...
requires non-diagonal preparation: non-jointly-diagonal or contextual
readouts are outside the bound, and no slot-local separability theorem for
arbitrary settings is proved here. Those remain viable construction routes.
The result is a statement about the committed operation class and the
declared Bell-state witness, not about every conceivable architecture
extension. Like \texttt{Dynamics/ChoiCPTP.lean},
...
open is not encoded by this static type: PR-64 asks for completely positive
outcome maps, a trace-preserving summed channel, and effect/readback
compatibility, while PR-65 asks for a source-produced common preparation,
context selection, public outcomes, provenance, and custody. The window is a
...
declared phase-sensitive effect. It is not a state transition or an
instrument. The module \path{OperationalPhaseAttainment.lean} consumes that
decision and inhabits \texttt{IdealPhasePOVMCountModel} over the committed
...
least-denominator arithmetic separately. PR-64 and PR-65 are not discharged,
nor is the independent cross-context additivity premise PR-03.
...
operational phase instrument and source custody required by PR-64 and PR-65
are not constructed.
