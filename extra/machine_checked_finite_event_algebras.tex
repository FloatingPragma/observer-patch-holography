...
maximally entangled witness on a checked slot-locality interface attains the
    ...
    operational instrument or PR-04 discharge. The artifact pins
...
producer receipts, or PR-04 discharge;
...
\item a recorded-decision attainment layer for that structure: under the
register row that axiomatizes the committed exact lift as a declared
architecture operation, a deterministic exact-arithmetic producer receipt,
an independent verifier, and a composed Lean receipt inhabit the structure
with produced count literals satisfying exact Born consistency, the
deterministic-scaling law, and the integer receipt window
...
recorded-decision attainment of the phase-count layer.
...
recorded-decision inhabitant of the count-fit structure with produced count
literals, exact Born consistency, deterministic scaling in every context,
...
the genuinely complex Pauli-$Y$ direction of PR-04, and no theorem asserts
that the repository's committed phase lift is necessary or sufficient for
attainment.
...
reaches it from the class. Producing the attained saturation from source
data within the committed operations therefore requires an operation that
writes off-diagonal record coherence; the premise register keeps exactly
that production open. The delimitation constrains readouts outside the
jointly diagonal class only through the reachable states' diagonality, and
it is a statement about the committed operation class, not about every
conceivable architecture extension. Like \texttt{Dynamics/ChoiCPTP.lean},
...
algebraic fit statement, not a PR-04 discharge theorem.
...
certify custody or discharge PR-04. A genuine target needs
...
deterministic semantics). Operational additivity/composition remains a
separate PR-03 obligation.
...
The reduction states the PR-04 residue exactly: for a committed-core
...
committed diagonal literals and the committed effects. What the row keeps
open is count provenance, a custody condition the register holds outside the
type system, together with source production of the phase operation and the
common preparation. The window is a necessary condition on any producer-bound
phase receipt and carries no provenance information, since the synthetic
inhabitant meets it without a producer.

\subsection{Recorded-decision attainment of the phase-count layer}
\label{sec:phase-attainment}

Register row PR-04 of the premise register carries a recorded decision dated
2026-08-18 under the row's axiomatize disposition: the committed exact lift
$I/2 - (2\sqrt{3}/3)\,i\,(QP - PQ)$, equal to the Pauli $+Y$ projector, is a
declared architecture operation. The module
\path{OperationalPhaseAttainment.lean} consumes that decision and it
inhabits \texttt{IdealPhasePOVMCountModel} over the committed core with
produced count literals. A deterministic producer computes the complete outcome table
of the declared operation and the committed web contexts on the committed
record-diagonal run state $\mathrm{diag}(111/179, 68/179)$ in exact
arithmetic over $\mathbb{Q}(\sqrt{3}, i)$: each context count pair is the
exact Born weight of the context effect scaled to the least positive integer
multiple of the committed run mass $179$ clearing its denominator, giving
...
$358$ in the phase context. This is the explicitly exhaustive deterministic
semantics that the non-discharge boundary above names as an acceptable
validation route; no sampling, statistical estimate, or floating-point step
occurs, and rerunning the producer reproduces the committed receipt byte for
byte. An independent verifier sharing no arithmetic code rebuilds the table
...
from one antecedent bundle pinned to the decision's operation and
preparation, the conjunction: committed core, declared operation in the
phase slot, committed run state with vanishing off-diagonal as common
preparation, committed diagonal literals, exact Born consistency and the
deterministic-scaling law in every context, every count mass a positive
multiple of $179$, and phase counts $(179, 179)$ inside the window of
\texttt{phase\_counts\_receipt\_window}. The attainment is conditional
structural under the axiomatized row: the operation is declared, never
source-produced, the counts are produced, never measured, the row is
consumed, never discharged, and the synthetic-inhabitant boundary of the
preceding sections is unchanged. Source production of a phase operation
remains the open obligation of the row.
...
phase-operation instrument required by PR-04 is not constructed.
