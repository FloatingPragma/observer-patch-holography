...
classicality, and a sharpness witness lie outside the formalization. A
...
imports these eleven modules alongside the wider OPH event-algebra library.

The checked development contributes eleven pieces:
...
Three results are outside the current library. No theorem identifies an
...
nothing asserts that $2\sqrt2$ is attained. A companion module,
...
trace preserving; the eleven-module audit here covers their event-algebra
...
Table~\ref{tab:modules} lists the eleven source modules. The comparison in
...
Table~\ref{tab:scale} describes the eleven modules discussed here and the
...
declarations, and a clean build emits the generated output. The 152 audited
...
state-level CHSH corollary; the companion module adds the custom CPTP proofs.
