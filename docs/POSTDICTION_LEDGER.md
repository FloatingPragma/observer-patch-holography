...
- `thermodynamic_four_law_package`: five receipts stay open under issue #671: the global objective representation, the common source-derived reference for both optimizers, identification of the source collar matrix with the resampling kernel through the equal-fibre-row receipt, physical energy and clock calibration beyond the attained central-interface modular split, and refinement-uniform low-temperature control; the strict-descent normalizer carries no entropy inequality
