...
- Evidence: `code/born_context_phase_lift/README.md` (`statement`), `code/born_context_phase_lift/verify_source_phase_lift.py` (`conditional_consumer`), `claims/assumption_dictionary.md` (`statement`).
...
- Disposition note: Removal needs a source-selected time-indexed or Cauchy-region interface and a net-compatible evolution proving the required slice-to-region generation or isomorphism. A continuous flow or larger symmetry action is one possible route, not the definition of the time-slice property. The Z/2 slot-swap receipt on the equal-alphabet carrier is a finite internal symmetry only and does not discharge this row.
