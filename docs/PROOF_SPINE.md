...
- Release: `r2009`
- Projection SHA-256: `acd2976948502012ce33219b9110c7f91565111004dbc8bc93e7e5208201d267`
...
| A2 | The two closure equations defined: $P=\varphi+\sqrt\pi/A_T(P)$ and $\mathfrak F_{r,0}(D_\star)=\{D_\star\}$ with $N_\star=\log D_\star$ | formal equations stated; the physical $A_T$ transport is open, and the completed capacity-indexed A1--A3 class carries a locked non-identifiability verdict that supplies no cosmic value |
...
| A6 | Terminal-fiber readback | PROVEN set-valued construction $\mathfrak F_{r,\varepsilon}(D)=\{M_\varepsilon(q)\}$; a scalar map exists only on a complete nonempty fiber with singleton support. Stable closure is $\mathfrak F_{r,0}(D)=\{D\}$. The bounded base-agreement, positivity, and carrier-bound completion class does not select one such physical $D$, and the completed A1--A3 class entails no unique slack zero (GAP-A3 resolved negative) |
| A7 | Finite order theorem and uniqueness boundary | PROVEN implication: a total monotone deflationary scalar map reaches its greatest fixed point from the top. The all-rung identity, erasure, two-class, and spectator completions have different exact fixed sets inside the bounded completion class. The completed source class carries the locked non-identifiability verdict, so a capacity selector is an additional named source law (GAP-A3 resolved negative) |
| A8 | Extension, refinement, and sewing | PROVEN implications: no-new-confusability injections preserve independent codes; fixed-$D$ refinement eventually stabilizes; exact seam gluing is a fiber product. Unit asymptotic density is insufficient. The physical embeddings are open, and a sharp finite-size seam or transfer selector would be an additional named source law (GAP-A3 resolved negative) |
...
- **GAP-A3 (direct public-record producer and selector): RESOLVED NEGATIVE, 2026-07-30.**
  ...
  extension/refinement controls. The complete capacity-indexed A1--A3 packet
  lift transports terminal fibers, observer/interface atoms, global joint
  kernels, A2 meaning maps, A3 feasible-set projections, and regulator
  controls across the generation-register family. Under the source-closed
  continuation reading the slack vanishes at every rung, and under the
  widened reading the surviving completions carry inequivalent zero sets, so
  the completed declared class entails no unique slack zero and the direct
  verdict is locked non-identifiability. A capacity selector is an
  additional named source law, and the issue #589 horizon identification
  closed not evaluable on the same ground.
...
- **GAP-A5 (comparison surface; #589 closed not evaluable, #547 queued).** The nominal 6.6% central-value difference between the
  ...
  named capacity source law replacing the resolved-negative GAP-A3, the common electroweak load carrier from GAP-A2, and the
...
- **GAP-B9.** Gauge-route identity and the remaining physical attachments.
  ...
  incidence and readback do not reconstruct a matrix current. The declared
  ...
  physical matter-content selection rule.
