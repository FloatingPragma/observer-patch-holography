...
| B4 | Gauge structure and matter: the D7-D9 Tannaka/MAR route and the finite A5 screen-current route are separate conditional constructions | the A5 module, coefficient bracket, exterior branching, three one-Higgs invariant lines, anomaly cancellation, and weak multiplicity four are closed under their packet premises; the conditional matter-lift certificate (#314) proves the algebraic spin double cover, the projector-realized fifteen-state chiral module, kernel emission, and MAR nonemptiness under its declared branch premises, meeting the #314 acceptance criteria; source binding of the inherited response premises is tracked in #599. MAR selects $N_g=3$ only as the least value in its declared economy class; physical rank-45 family attachment is open. The source-bound commuting square identifying the two gauge routes is also open (GAP-B9) |
...
- **GAP-B9.** Physical screen-to-Standard-Model forcing and gauge-route
  ...
  (tracked in issue #599).
  The conditional matter-lift certificate (issue #314,
  `code/a5_closure/super_tannakian_matter_lift_certificate.py`) proves,
  conditional on that packet and the declared matter-lift contracts, the
  exact algebraic PORT-SPIN-LIFT target (a non-split SU(2) double cover,
  ...
  descent, and the declared MAR class nonempty. This meets the #314
  acceptance criteria as written (the issue types the exterior package
  conditional and the MAR class declared, with dependencies #565 and #566
  only). The physical PORT-SPIN-LIFT receipt inherits the #599 premise
  binding.
  The remaining producer gates are block determinant balance;
  physical $\mathbb Z_6$ deck/line descent (consuming the emitted
  kernel); source binding of the inherited response premises (#599); selection of the
