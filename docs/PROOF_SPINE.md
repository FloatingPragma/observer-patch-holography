...
| B4 | Gauge structure and matter: the D7-D9 Tannaka/MAR route and the finite A5 screen-current route are separate conditional constructions | the A5 module, coefficient bracket, exterior branching, three one-Higgs invariant lines, anomaly cancellation, and weak multiplicity four are closed under their packet premises; the conditional matter-lift certificate (#314) proves the algebraic spin double cover, the projector-realized fifteen-state chiral module, kernel emission, and MAR nonemptiness under its declared branch premises, with physical source binding open. MAR selects $N_g=3$ only as the least value in its declared economy class; physical rank-45 family attachment is open. The source-bound commuting square identifying the two gauge routes is also open (GAP-B9) |
...
- **GAP-B9.** Physical screen-to-Standard-Model forcing and gauge-route
  ...
  kernel); physical source binding of both conditional packets; selection of the
