...
| B4 | Gauge structure and matter: the conditional finite A5 screen-current chain and the independent D7-D9 Tannaka/MAR route | incidence proves the $1+3+3'+5$ decomposition, the unique central graph involution $J$, and $10J=A^3-4A^2-5A+10I$. A target-blind impulse/readback producer derives $R=-J$, and the current certificate constructs $\mathfrak{su}(3)+\mathfrak{su}(2)+\mathfrak u(1)$. Under the declared conjugate matter-projector pair, anomaly freedom derives determinant balance and exhaustive tensor-kernel descent computes the common $\mathbb Z_6$ kernel and maximal faithful matter image. The cover and its $\mathbb Z_2$, $\mathbb Z_3$, and $\mathbb Z_6$ quotients carry the same local tensors. No MAR clause enters this conditional finite implication. Physical matter typing, physical global-form selection, laboratory current identification, rank-45 family attachment, extra-light-sector exclusion, and the commuting square to the Tannaka group are open (GAP-B9) |
...
- **GAP-B9.** Gauge-route identity and the remaining physical attachments.
  ...
  form. This conditional chain consumes no MAR clause.
...
- **GAP-X1.** Discrete structural selections without a consistency-forcing
  ...
  $N_g=3$ is an economy-axiom minimum until the frozen face-phase attachment
