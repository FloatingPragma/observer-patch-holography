...
- Release: `r2003`
- Projection SHA-256: `bc232e484af294d4d6fe0aac729850901a22e94796a67ddde65e0e8591c9f2b3`
...
| A6 | Terminal-fiber readback | PROVEN set-valued construction $\mathfrak F_{r,\varepsilon}(D)=\{M_\varepsilon(q)\}$; a scalar map exists only on a complete nonempty fiber with singleton support. Stable closure is $\mathfrak F_{r,0}(D)=\{D\}$. The bounded base-agreement, positivity, and carrier-bound completion class does not select one such physical $D$; the complete A1--A3 class is undecided (GAP-A3) |
| A7 | Finite order theorem and uniqueness boundary | PROVEN implication: a total monotone deflationary scalar map reaches its greatest fixed point from the top. The all-rung identity, erasure, two-class, and spectator completions have different exact fixed sets inside the bounded completion class. The complete source class requires the missing packet lift (GAP-A3) |
...
- **GAP-A3 (direct public-record producer and selector): PARTIAL NEGATIVE.**
  ...
  extension/refinement controls. A target-clean all-rung generator supplies
  reversible identity, copy-collapse, two-class, and hidden-spectator
  completions. Exact code checks their finite controls, a separate simulator
  replays the sample graphs, and Lean proves fixed-set disagreement for the
  base-agreement, positivity, and carrier-bound structure. The complete
  capacity-indexed A1--A3 packet lift remains absent. It must carry terminal
  fibers, observer/interface atoms, global joint kernels, A2 meaning maps, A3
  feasible-set projections, and regulator controls before the full source
  class can receive a positive or negative verdict.
...
- **GAP-A5 (comparison issues #589/#547).** The nominal 6.6% central-value difference between the
  ...
  evaluable as a common-observable residual. Evaluation requires a positive
  replacement for GAP-A3, the common electroweak load carrier from GAP-A2, and the
