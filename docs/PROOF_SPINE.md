...
The compact paper records the typed theorem boundary, and the particle paper
...
- **GAP-B5.** Charged-lepton source completion. A 24-stage register fixes no
  ...
  Constructing that clock and attachment is open. The MCPR triple and A5/W5
...
- **GAP-B9.** Physical screen-to-Standard-Model forcing and gauge-route
  ...
  carrier manifest. The super-Tannakian matter lift (issue #314,
  `code/a5_closure/super_tannakian_matter_lift_certificate.py`) is closed
  on the same declared branch: it constructs PORT-SPIN-LIFT as an exact
  non-split SU(2) double cover (binary icosahedral, unique involution),
  realizes the fifteen-state chiral module by a source-derived equivariant
  projector on the auxiliary CAR/Fock space with all listed realized
  anomaly traces and three Yukawa invariant lines checked exactly, emits
  the order-six common action kernel as data for the global-form descent,
  and witnesses the declared MAR class nonempty — with the trace-balanced
  exterior matter contract, statistics contract, and category typing
  declared in its source packet. The remaining producer gates are block determinant balance;
  physical $\mathbb Z_6$ deck/line descent (now consuming the emitted
  kernel); physical selection of the
