...
- Release: `r1584`
- Projection SHA-256: `7c3390b4a013a82592098498ed39e1105c564857df64ac4cace562342f452a3e`
...
- Claim gates: 22 distinct open issues across 20 gated claims
...
| B2 | Kinematics: Conf⁺(S²) ≅ SO⁺(3,1), 3+1 events, H³ observer space | conditional theorem on the typed global-support branch. BW requires `FiniteCapBWCertificate` and independent complete `MGNS-1` on the same tower; physical MGNS construction is issue #574. A structural carrier-to-geodesic-support map is refinement-natural on the declared reference tower, while its physical federation source, nonvacuous higher-overlap coherence, H3/KMS capture, and source-derived common-domain tower are open or quarantined (GAP-B1, GAP-B13) |
...
| B4 | Gauge structure and matter: the conditional finite A5 screen-current chain and the independent D7-D9 Tannaka route | incidence proves the $1+3+3'+5$ decomposition, the unique central graph involution $J$, and $10J=A^3-4A^2-5A+10I$. A target-blind impulse/readback producer derives $R=-J$, and the current certificate constructs $\mathfrak{su}(3)+\mathfrak{su}(2)+\mathfrak u(1)$. Under the declared conjugate matter-projector pair, anomaly freedom derives determinant balance and exhaustive tensor-kernel descent computes the common $\mathbb Z_6$ kernel and maximal faithful matter image. The cover and its $\mathbb Z_2$, $\mathbb Z_3$, and $\mathbb Z_6$ quotients carry the same local tensors. No matter-content selection rule enters this conditional finite implication. Physical matter typing, physical global-form selection, laboratory current identification, rank-45 family attachment, extra-light-sector exclusion, and the commuting square to the Tannaka group are open (GAP-B9) |
