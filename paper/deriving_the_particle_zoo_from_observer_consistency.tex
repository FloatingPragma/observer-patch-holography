...
Neutrinos & \(\nu_e\) & rejected target-informed weighted-cycle candidate & absolute mass withheld from public prediction table; the frozen candidate gives \(\theta_{12}=34.2259^\circ\), \(\theta_{23}=49.7228^\circ\), \(\theta_{13}=8.68636^\circ\), \(\delta=305.581^\circ\), \(J=-0.02753\), and \(\Delta m_{21}^2/\Delta m_{32}^2=0.03072111\) & the upstream flavor kernel is a hand-written template; target-ranked selector development prevents historical-blindness status; the NuFIT 6.1 correlated profile rejects the point; Majorana and absolute-attachment values remain compare-only \\
Neutrinos & \(\nu_\mu\) & rejected target-informed weighted-cycle candidate & absolute mass withheld; same frozen scale-free comparison point as the electron-neutrino row & no source-only or public prediction row \\
Neutrinos & \(\nu_\tau\) & rejected target-informed weighted-cycle candidate & absolute mass withheld; same frozen scale-free comparison point as the electron-neutrino row & no source-only or public prediction row \\
...
Transporting the weighted-cycle Majorana matrix through the declared shared same-label basis gives
\[
...
\]
This cancellation checks the chosen representation; it does not independently identify the
physical charged-lepton basis or validate the PMNS point. Canonical Takagi congruence of the
transported symmetric matrix, with
\[
U_{\mathrm{PMNS}}^T M_{\mathrm{shared}} U_{\mathrm{PMNS}}=\operatorname{diag}(m_i)\in\mathbb R_{>0}
\]
and the electron-row gauge \(U_{e1}\in\mathbb R_{>0}\), gives the candidate Majorana pair
\[
...
sidecars remain diagnostic surfaces.
\[
...
enumeration of both orientations and all row and mass-column permutations finds no
normal-ordering-consistent convention that passes both NuFIT 6.1 correlated \(3\sigma\) gates.
