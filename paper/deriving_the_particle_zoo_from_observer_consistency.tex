...
completions with defect zero and with nonzero defect. The source chain emits the
...
contraction interval, a target-independent selector for one admitted map, a
typed same-quantity bridge for its input and output, and a prediction interval narrower than the declared
...
class \(f_P\) chosen by \(P\). Granting the candidate-only ordered three-point shape law supplied by
...
is excluded at its truncation order: the quartic truncation of the
relative-entropy expansion restricted to \(W_5\) at fixed band amplitude
\(r\) has two minimizing branch candidates on the compared strata within
the positivity domain \(r<1/|\min_i w_i|\) of the record. The first is the
degenerate vertex-axis prolate orbit, selected by the leading cubic term
alone and at every amplitude below the exact crossing \(r_c=2.776\). The
second is a simple-spectrum twofold-fixed orbit with centered eigenvalues
proportional to \((-(\varphi^4-1),-1,\varphi^4)\) and sorted-gap ratio
exactly the golden ratio \(\varphi\), selected for \(r_c<r<2\sqrt6\); the
observed centered log-mass gap ratio \(1.889\) differs from \(\varphi\)
by \(16.7\) percent in the closest orientation, so the entropy packet
selects neither branch as the charged shape. The golden branch is excluded
by that comparison only, and no source mechanism beyond the quartic
...
phase, or numerical ratios are supplied. The analysis proves a
