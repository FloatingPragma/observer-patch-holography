...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a forward calculation from one common scale. Close the outer/inner self-reference of the screen, extract the fine-structure constant, and compute the pixel ratio \(P\). Propagate that same \(P\) through the quantitative particle branch. The result is a compact package: massless carriers, the \(W\) and \(Z\) masses, the Thomson-limit fine-structure constant, a Higgs mass, running quark masses with explicit Yukawas, and a neutrino family in the observed hierarchy and mixing regime. Charged-lepton absolute masses and hadrons remain open.
\end{abstract}
...
pixel ratio \(P\) can be propagated through the whole spectrum. The working hypothesis is that one
fixes one common pixel ratio \(P\) on the synthesis paper's outer/inner closure branch and then uses that same \(P\)
...
The quantitative particle side adds one Phase-II closure variable together with one external
continuous input. The common pixel ratio \(P\), fixed on the synthesis paper's outer/inner closure
branch~\cite{ophsynthesis}, feeds the forward D10 electroweak map and its descendants. The
...
\subsection{Why a single \(P\) still matters}
...
The role of the Phase-II pixel closure is therefore to supply one common quantitative scale
...
electroweak quantitative-closure branch and \(D12\) names the Phase-III phenomenological-continuation
...
On the declared electroweak running/matching surface, the Phase-II pixel variable \(P\) fixes the
source basis
...
\]
the physical Thomson-limit inverse fine-structure constant on the Ward-projected lane.
\end{corollary}
