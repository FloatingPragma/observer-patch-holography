...
the charged and quark families have exact same-family witnesses, and the neutrino lane carries
an emitted weighted-cycle theorem pair together with diagnostic sidecars beneath it. Charged
leptons and flavor-labeled neutrinos therefore split for different reasons:
the charged lane fixes the centered hierarchy but not the absolute normalization, while the
weighted-cycle neutrino branch lands on the observed PMNS/hierarchy pattern and emits
\[
C_\nu=G^2Q S^{-1/2}=0.9994295999075177,
\qquad
P_\nu=I_\nu^{1/2}\widehat{\mathrm{ratio}}^{\,1/2}\mathrm{sum\_defect}^{-1}=6.699825740519345,
\]
so the paper-facing amplitude \(B_\nu=P_\nu C_\nu=6.696004159297337\) and the positive
normalization \(\lambda_\nu=(m_{\star,\mathrm{eV}}/q_{\mathrm{mean}}^{p_\nu})P_\nu C_\nu=1.7237014208357415\)
follow by exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization. Hadron masses are outside the current particle roadmap:
unlike the upstream elementary-particle lanes, the hadron branch requires large-scale classical
lattice-QCD computation together with production continuum, volume, chiral, and statistics
systematics, and that nonperturbative execution burden is not a realistic target within the
current project compute scope. This paper gives a concise
...
\]
The exact two-parameter positive-segment adapter, the bridge corridor, and the reduced-correction candidate audit remain on disk only as diagnostic surfaces and do not feed back into theorem state. Charged leptons and neutrinos are split for different reasons. Charged leptons wait on promotion of \(\widehat C_e^{\mathrm{cand}}\) and then on the post-promotion lift whose descended scalar is \(\mu_{\mathrm{phys}}(Y_e)\), with the physical identity-mode equalizer as the exact smaller forcing object beneath that scalar; the weighted-cycle neutrino branch carries the emitted bridge-rigid absolute family.
Taken together with the exact \(W/Z\) frozen repair pair, the closed Higgs/top forward seed together with its compare-only exact inverse sidecar, the exact charged/quark same-family witnesses, and the emitted weighted-cycle neutrino theorem pair, the exact non-hadron output lane is complete, on a mixed theorem/compare-only/same-family surface rather than one uniform theorem tier.
...
Read this section by claim tier. The same absolute residual can indicate either a theorem-grade emitted value, an exact sidecar, or a compare-only continuation benchmark. On this table, the structural zeros and the electroweak calibration pair \(m_W,m_Z\) are exact on their declared surfaces; the Higgs, charged, and quark lanes each carry a stronger exact sidecar than the theorem lane beneath them; and the weighted-cycle neutrino lane carries an emitted theorem pair \((C_\nu,B_\nu)\) above its scale-free branch, while the older exact adapter remains diagnostic-only.
...
Quark family & continuation + exact sidecar witness & shared excitation dictionary \(\rightarrow\) D12 mass-side continuation \(\rightarrow\) selected D12 sheet & exact same-family quark sextet on a closed ordered-three-point readout chain & same-family-only; the exact closure package above that sidecar is three-step: \(\Theta_{ud}^{\mathrm{mass}}=\text{\ophid{quark_d12_t1_value_law}}\), with \(\Delta_{ud}^{\mathrm{overlap}}=\tfrac16\log(c_d/c_u)=t_1/5\) on the emitted D12 mass ray and \ophid{intrinsic_scale_law_D12} as the derived wrapper; then \(\Theta_{ud}^{\mathrm{phys}}\), a sector-attached same-label left-handed lift to the physical CKM shell; then \(\Theta_{ud}^{\mathrm{abs}}\), a target-free absolute readout theorem for \((g_u,g_d)\) on that physical sheet \\
...
Neutrinos & continuation + emitted theorem pair + diagnostic sidecars & same-label scalar certificate \(\rightarrow\) weighted-cycle branch \(\rightarrow\) bridge rigidity \(\rightarrow\) absolute attachment & emitted absolute masses and central splittings on the weighted-cycle branch & theorem-grade on the declared weighted-cycle branch; the exact positive-segment adapter, bridge corridor, and correction audit remain diagnostic-only and do not feed back into theorem state \\
...
Neutrinos & \(\nu_e\) & weighted-cycle theorem branch & \(0.017454720257976796~\mathrm{eV}\); the weighted-cycle branch emits \(\theta_{12}=34.2259^\circ\), \(\theta_{23}=49.7228^\circ\), \(\theta_{13}=8.68636^\circ\), \(\delta=305.581^\circ\), \(J=-0.02753\), \(\Delta m_{21}^2/\Delta m_{32}^2=0.03072111\), and the theorem pair \(C_\nu=0.9994295999075177\), \(B_\nu=6.696004159297337\), \(\lambda_\nu=1.7237014208357415\) & closed on the declared weighted-cycle theorem branch; the positive-segment adapter, bridge corridor, and bridge-coordinate sidecars remain diagnostic-only and do not feed back into theorem state \\
Neutrinos & \(\nu_\mu\) & weighted-cycle theorem branch & \(0.019481987935919015~\mathrm{eV}\); same weighted-cycle theorem branch as the electron-neutrino row & same emitted theorem pair and same diagnostic-only retirement rule for the positive-segment adapter and bridge corridor \\
Neutrinos & \(\nu_\tau\) & weighted-cycle theorem branch & \(0.05307522145074924~\mathrm{eV}\); same weighted-cycle theorem branch as the electron-neutrino row & same emitted theorem pair and same diagnostic-only retirement rule for the positive-segment adapter and bridge corridor \\
...
weighted-cycle bridge-rigidity and absolute-attachment theorem pair above the scale-free branch. The second is a microphysical bridge that tries to carry edge
...
theorem are also closed, so the exact two-parameter positive-segment adapter, the bridge corridor, and the bridge-coordinate sidecars are diagnostic surfaces beneath the emitted theorem pair rather than proof-facing ingredients.
\[
...
scale-free mass normal form, and the bridge-rigidity and absolute-attachment theorems emit the
absolute eV-scale family on that branch.
...
The weighted-cycle route closes the PMNS and hierarchy steps, and the bridge-rigidity theorem
emits
\[
C_\nu=G^2Q S^{-1/2}=0.9994295999075177,
\qquad
...
continuations beneath the emitted theorem pair.
