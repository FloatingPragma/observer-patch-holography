...
lepton absolute masses and source-only hadrons are outside the emitted source-only theorem surface,
comparison rows are named as comparison rows, and measured endpoint data are isolated when a
transport bridge uses them.
...
\]
The selected-carrier chart and the freeze-once coherent repair surface are recorded in the claim
ledger below.
...
used later in the paper because flavor continuations have different claim tiers, whereas the
...
This claim-tier boundary governs the rest of the paper. Whenever a later chapter gives more
...
electroweak claim ledger records the residual map and the
...
claim surfaces: the \(W/Z\) compare-only validation sidecar, the exact Higgs/top theorem surface,
...
label rather than as a closed D10 Phase-II prediction theorem.
...
organized by one local screen scale rather than by a list of unrelated particle inputs. The claim
