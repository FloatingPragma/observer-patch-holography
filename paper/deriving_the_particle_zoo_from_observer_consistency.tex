...
The selected branch gives exact charge quantization, a conditional dimensionless
electroweak hierarchy, a declared-map commuting-square identity whose defined
defect is zero, and conditional Higgs and top coordinates.  The zero is a
packaging consequence of the declared comparison maps, not a source-derived
Higgs-naturalness result: the current source antecedents admit exact finite
completions with defect zero and with nonzero defect. A target-free but
declared and incomplete running/map packet evaluates to the electroweak chart
coordinates \((80.330,\,91.119)\)~GeV; they are not source-emitted. Separate strict
...
tests. Conditional on the current registered source signature, the displayed
centered-spread counterfamily prevents identification of six absolute quark
masses; the exact free two-modulus fiber is a restricted lower bound after a
candidate fixed-ray shape law is granted. Absolute lepton and hadron masses are empirical inputs rather
...
The declared matrix current and matter chain are conditional. Transportable sectors plus
...
emits \(v/E_\star\).  Separately, its declared comparison maps make the
defined commuting-square defect \(\epsilon_H^{\mathrm{decl}}=0\) identically.  Those maps are
not emitted by the source antecedents, so the identity is not a Higgs
naturalness result; a weak scale
...
are physical branch data. The framework supplies no particle-sector selector or numerical
...
candidate is on its declared quantitative surface. The dimensionless hierarchy implication is
conditional, and the companion zero-defect square is a declared-map identity rather than a
source-derived naturality result. The restricted quark source-spread non-identifiability theorem, the common-scale
...
\[
\phi_0=\frac{v_{\rm SM}}{\sqrt2}\binom{0}{1},\qquad v_{\rm SM}\ne0,
\]
one has
\[
...
geometry of the one-Higgs slot. It does not derive \(m_H\), the quartic, \(v_{\rm SM}\), or
Coleman--Weinberg dynamics; the
weak scale and Higgs/top quantitative surface are separate parts of the OPH quantitative branch.
The stored \(\epsilon_H^{\mathrm{decl}}=0\) row is only a declared-map commuting-square
identity and does not close Higgs naturalness.

The symmetry-breaking geometry of that slot carries a machine-checked
composition (\texttt{Lean/Screen/ElectroweakBreakingComposition.lean}). One
typed premise bundle extends the committed structure premises by five
declared finite constants: a quartic coupling \(\lambda>0\), a
model-shell radius \(r>0\), two coupling constants \(g,g'>0\), and one Yukawa
coefficient \(y\). On the doublet carrier whose complex dimension and
\(q=6Y\) charge are the committed table entries \((2,3)\), one theorem on
that bundle proves five exact statements. The global minimizer set of the
declared potential \(\lambda(|h|^2-r^2)^2\) is exactly the nonzero shell
\(|h|^2=r^2\). The stabilizer of the chosen minimum \((0,r)\) inside the
four-parameter electroweak direction space is exactly one unbroken line, the
\((g',g)\) mix of the third weak and hypercharge directions. The potential
along every ray from the minimum is an exact polynomial identity whose
quadratic coefficient is the second variation; that coefficient is positive
semidefinite, and its kernel equals the set of infinitesimal gauge images at
the minimum, with exactly three real parameters. The induced mass form
vanishes exactly on the unbroken line and takes the normalized values
\(g^2r^2/4\) and \((g^2+g'^2)r^2/4\) on the charged and neutral broken
directions, so their tree ratio is \(g^2/(g^2+g'^2)\) in the declared
constants. The epsilon-paired Yukawa line produces at the minimum a bilinear
with exact coefficient \(yr\), vanishing exactly when \(y=0\) or \(r=0\),
and either committed parity sector supplies a colorless doublet/singlet row
pair whose committed charges balance against the scalar row charge or its
conjugate. The five constants are declared bundle parameters: no committed
theorem selects \(\lambda\), \(r\), \(g'/g\), or \(y\), the committed
invariant-form receipts leave the relative coupling normalization free, and
the composition supplies no source action, physical vacuum, measured mass,
or family structure. The quantitative electroweak and Higgs/top surfaces are
the separate branches described in their own sections. If the finite minimum
is identified with the conventional vector displayed above, then
\(r=v_{\rm SM}/\sqrt2\). Translating the finite coefficients into physical
mass formulae additionally requires the scalar kinetic and generator
normalizations and a physical vacuum attachment, neither of which is supplied
here.

The neutral sector of that composition carries a machine-checked coupling
dictionary (\texttt{Lean/Screen/NeutralCurrentDictionary.lean}). A weak slot
is a row of the committed component table together with a weak component
index: its third isospin is \(\pm\tfrac12\) on the two components of a
doublet row and \(0\) on a singlet row, its hypercharge is the committed
\(q=6Y\) charge column, and its electric charge is the sum \(Q=T_3+Y\) in
the committed integer units. The neutral coupling of a slot along a
direction is the diagonal action \(g a_3 T_3 + g' b\,Y\), and on every
neutral direction the committed infinitesimal action is exactly diagonal
with the two slot couplings of the scalar row, so the normalization is the
committed one, with no freedom left. The unbroken direction of the breaking
composition, normalized by the common direction length
\(g_Z=\sqrt{g^2+g'^2}\), couples every slot of either committed parity
sector at exactly \(e\,Q\) with \(e=gg'/g_Z\), and the neutral broken
direction, normalized the same way, couples it at exactly
\(g_Z\bigl(T_3-Q\sin^2\theta\bigr)\), where
\(\sin^2\theta=g'^2/(g^2+g'^2)\) is a defined abbreviation in the declared
constants and never a measured value. The Z direction is exactly the
orthogonal complement of the unbroken line in the neutral plane, two
committed rows with different electric charges have provably different
photon couplings, and in either parity sector the zero-charge component of
the colorless doublet row has photon coupling exactly zero and Z coupling
exactly \(\pm(g^2+g'^2)/2\). The dictionary is exact tree-level algebra on
the committed grammar: it supplies no physical current operator, source action, mixing angle, or
decay amplitude, and the chirality and source-action attachments are not supplied.

The charged sector of the same composition carries the companion
machine-checked dictionary
(\texttt{Lean/Screen/ChargedCurrentDictionary.lean}). The two charged
directions are orthogonal to the photon and Z directions and to each other
under the declared pairing, span exactly the orthogonal complement of the
neutral plane, and both carry the broken mass value \(g^2v^2/4\). Their two
complex combinations are, in exact closed form, the strict shift operators
that move the lower weak slot of a doublet row to the upper one and back
with coefficient \(g\); normalizing by the derived direction length
\(\sqrt{2}\) of the charged pair gives the coupling \(g/\sqrt{2}\), a
constant derived from the committed pairing rather than inserted, pinned to
the committed infinitesimal action on the scalar row, and any drifted
coefficient breaks the alignment identity. At positive \(g\) a nonzero
charged coupling exists exactly on the committed partner pairs, the two
weak components of a doublet row, and committed singlets are annihilated;
every nonzero coupling raises the committed electric charge by exactly one
unit of the \(q=6Y\) normalization, from the committed charge columns
alone. The dictionaries compose: the commutator of the photon-direction
action with the raising and lowering shifts is exactly \(\pm i\,g_Z e\)
times the shift, so the charged directions carry exactly one declared
electromagnetic unit per committed photon normalization; the commutator of
the two charged actions lands on the photon and Z actions with exact
coefficients \(-gg'/(g^2+g'^2)\) and \(-g^2/(g^2+g'^2)\); and the
raising/lowering commutator is \(2gi\) times the third-weak action. These
joins are exact operator-commutator identities of the committed action on
the scalar-row carrier; no abstract bracket on the direction space is
committed. The dictionary supplies no physical current operator, propagator, decay
amplitude, or flavor content, and the same chirality and source-action
attachments are not supplied.

A kinematic internal-clock packet is machine-checked on the same declared
Hermitian Lorentz module that carries the mass-shell receipts
(\texttt{Lean/Geometry/InternalClockRestFrequency.lean}). For one declared
positive mass parameter and one declared plane-wave phase form, the rest
phase along a frame worldline advances at exactly the mass parameter in
every oriented chart, the declared internal clock obeys the exact rotation
law \(i\,\partial_\tau z = m\,z\), and its period set is exactly the
\(2\pi/m\) integer lattice, the frequency-rigidity identity for the declared
clock. The spatial phase gradient reads off the momentum coordinate on the
de Broglie orientation of the phase, and the temporal rate reads off the
energy coordinate on the opposite invariant orientation, tied by the
committed shell identity, and the zero-mass branch degenerates exactly: the clock is constant, every
real number is a period, and no oriented chart brings a future null vector
to rest. The oscillation is a declared plane-wave form on a declared module:
no localized particle state, self-starting oscillation, source-produced
state, laboratory frequency unit, or Compton value is derived, and the
clock-calibration and physical-spacetime attachments are not supplied.

The declared plane-wave functions have pointwise amplitude-modulus invariance under their declared free evolution
(\path{Lean/Geometry/FreeEvolutionPersistence.lean}). The free evolution
is the translation flow along a declared frame worldline, acting on wave
functions by precomposition, and it is an exact one-parameter group. Under
this flow a plane-wave function changes by a unit-modulus global phase only,
so its mass-shell membership, its momentum and energy readouts, and its
unit modulus are exact constants of the flow, and two functions with distinct
positive mass or momentum labels stay distinct at every parameter value.
Along the wave's own frame the global phase is exactly the internal clock,
and this identity is a clause of the composed machine-checked receipt. On
the zero-mass branch the massive parameterization degenerates to the
constant function and the frame label is not recoverable, while a future null
vector carries the shell, global-phase-covariance, and pointwise-modulus
clauses of the packet on the zero-mass shell. This is pointwise algebra for a
declared kinematic wave under a declared flow, separate from the
record-repair stability results: no physical particle, decay rate,
localized wave packet, Hilbert-space normalization, survival probability,
or laboratory persistence time is claimed, and the
clock calibration, spacetime attachment, and source production premises
are not supplied.

The gyromagnetic ratio of the spin sector has an exact algebraic origin
(\path{Lean/EventAlgebra/PauliMinimalCoupling.lean}). Over any complex
algebra, for a declared triple \(X\) with commutators
\([X_i,X_j] = i\,q\,\epsilon_{ijk}B_k\), the square of the Pauli
contraction is exactly
\[
(\sigma\!\cdot\!X)^2 = X\!\cdot\!X - q\,(\sigma\!\cdot\!B),
\qquad X\!\cdot\!X:=\sum_i X_iX_i.
\]
This ordered algebraic sum is not an adjoint norm and carries no positivity
claim. With a separately supplied Hermitian kinetic-momentum realization, the spin
sector inherits the full coupling \(q\) where the orbital convention
assigns \(q/(2m)\) per unit angular momentum: gyromagnetic ratio two. The
commutation data, the coupling, and the carrier are declared. A commuting
triple has identically vanishing spin term, the Pauli matrices themselves
inhabit the hypothesis class with \(q = 2\) and a nonzero spin term, and
the coefficient is pinned on that inhabitant. No gauge potential, Dirac
equation, Spin structure, laboratory calibration, or spacetime attachment
is constructed, and no anomalous-moment content is claimed.
...
observer-patch federation and spherical support, observer agreement, and
...
source-class verdict does not follow. A positive direct result requires completion of
...
from counting the declared settings against the independently selected basins. The same \(P\) must drive all
...
the stronger source-class verdict does not follow. A positive result requires a
...
one-loop anchor; it is not evidence by itself that a source chain closes. A source-only endpoint
needs the OPH hadronic spectral measure, a target-independent choice among the admitted maps, and
a typed construction identifying the two closure readings as one quantity. The empirical endpoint
is a comparison calculation and supplies none of those objects.
...
Affine event-record stitching & conditional certificate theorem & physically
faithful source-causal continuum with affine charts and observer clock atlas\newline
\(\rightarrow\) affine event support for each descended token and clock slice\newline
\(\rightarrow\) separately typed conditioned-frame ball where required\newline
\(\rightarrow\) positive event-location gap\newline \(\rightarrow\) overlap descent\newline
\(\rightarrow\) real transverse interface-crossing germs\newline
\(\rightarrow\) common-chart sector/gauge transport\newline
\(\rightarrow\) ID-independent one-to-one assignment gap\newline
\(\rightarrow\) coarse/fine contraction check & nonbranching event-record paths,
rays, or isolated records when the location and stitch certificates pass;
otherwise \(\mathrm{AMBIGUOUS}\), \(\mathrm{REJECTED}\), or
\(\mathrm{INTERACTION}\) \(\mathrm{REQUIRED}\) & This conditional theorem
concerns observer-visible records after the source-causal continuum certificate
has supplied physical event charts. It does not derive particle species,
masses, gauge charges, scattering amplitudes, or geodesic motion. Repeated
implementation IDs, nearest-neighbor fits, frame coordinates used as event
positions, and file-boundary coincidences are inadmissible evidence. \\
\bottomrule
\end{tabular}
\end{table}
...
same record classifies the hierarchy formula as conditional and the stored
\(\epsilon_H^{\mathrm{decl}}=0\) formula as a declared-map packaging identity.
The source antecedents do not select those maps and admit nonzero-defect
completions; the strict source-root, source-derived comparison-map, and
physical-scale certificates are separate premises. Where a
...
output. Source-only closure requires Ward-projected same-scheme transport,
target-independent map selection, and the typed same-quantity bridge. The separate
...
benchmark. Charged leptons carry an underived bridge: the corpus does not derive
the determinant trace-lift attachment \(3\mu(r)=S_M(r)\) from the electroweak
descendants of \(P\) to physical charged data, the bridge artifact is a current-corpus gap
certificate, and a future \(P\)-driven charged-mass law stays admissible. The auxiliary
...
extraction codomains; the direct-top response kernel carries a fixed-ray point statement, the reference central pair 0.7 sigma off the one-scalar ray on the declared linear surface. Neutrino absolute masses are
...
event projectors, and a declared L\"uders instrument on that representation.
The effect table does not select that instrument. The
...
completion class. No universal all-rung membership in the complete A1--A3
capacity-source contract or executable-to-Lean bridge is supplied. Direct
...
premises. The quotient is exact for the declared representation, but the
construction does not select it as the physical global gauge group. Separate intrinsic-CP
...
separate conditional compact-group route. No physical source producer,
intertwiner, or laboratory attachment binds the finite current or identifies
it with the Tannaka current.
\item \textbf{Conditional topic notes.} The integrated branch appendices carry useful material
on supersymmetry, the finite-quotient baryogenesis source theorem without a
derived record generator, proton stability, generation structure, and Yukawa hierarchy,
...
No physical family interpretation or extra-sector completeness theorem is
supplied. This logical
...
preserves the same diagonal \(\mathbb Z_6\) weight kernel. Partition averaging
defines a surjective adaptor into the supplied partition's commutative public
projector-span algebra and factors through the block readout. Every adapted
value commutes with every element of the partition commutant. The supplied central weights define a sixth-root
character action on the component-labelled finite product of the ten mapped
projector ranges. This action is not ambient-projector conjugation and does
not bind the projector ranks to exterior multiplicities. The kernel of this central-parameter action
has exactly six elements and agrees with the tensor-character kernel. A
separate supplied chiral anomaly-free mask is forced to one of the conjugate
exterior-degree parity sectors; its support is nonzero, invariant under the
same action, and detects the same kernel. No central parameter acts as a
universal minus sign on that support. This is a nonconflation control and does
not construct or identify physical fermion parity. The partition, maps,
weights, and mask are premises. The theorem derives no source-selected
physical matter action, parity representative, global form, or laboratory
charge.
...
No theorem identifies them as one physical gauge-current object, and no
physical source binding of the first route is supplied. The rank-three response
band is therefore only a candidate family
...
contraction. It supplies no physical position, scale, or detector map. If the seam symbol is physical
...
head-on, collinear residual. No general independent-lepton or full anisotropic
optimization is supplied.

One nontrivial universal subcase can nevertheless be closed exactly at the
same leading kinematic order. Assume the hard photon, electron, and positron
all carry the common negative coefficient \(\delta=-d=-a^2/20\), while the
soft photon is Lorentz invariant at leading order. Writing
\(u=x(1-x)\in(0,1/4]\), the required soft energy is
\[
 \epsilon_{\rm req}(k,u)=\frac{m_e^2}{4ku}+\frac{3dk^3}{4}u .
\]
The share identity, positive-domain bound, exact square-factor AM--GM
envelope, equal-share endpoint regime, transition condition
\(3dk^4\le16m_e^2\), and the absence of a solution below the envelope are
machine-checked in \texttt{SeamCurrentPhotonLeptonThreshold}~\cite{ophrepo}.
The same file proves that at and above the transition the balanced equality witness is
realized by an open physical share, closing the constrained-minimum step. Above
\(k_t=2\sqrt{m_e}(3d)^{-1/4}\), the interior minimizer
\(u_*=m_e/(\sqrt{3d}\,k^2)\) gives
\(\epsilon_{\rm env}=(\sqrt3/2)m_e k\sqrt d\). A deterministic companion
calculator verifies the analytic and bracketed roots and keeps the
Lorentz-invariant-lepton photon-only comparator separate. For
\(a=\ell_P\), representative CMB energies \(6.34\times10^{-4}\) and
\(3.0\times10^{-3}\,\mathrm{eV}\) have upper leading roots
\(7.82\times10^{19}\) and \(3.70\times10^{20}\,\mathrm{eV}\), respectively.
These are conditional kinematic windows, not an opacity or propagated-flux
prediction. No published photon-only air-shower exclusion transfers to this
common-coefficient branch without the interaction, background, source,
cascade, shower, and detector layers.
...
The running/chart coordinates carry no OPH theory covariance, and no physical
readout contract is supplied. No comparison to physical resonance
...
renormalization scheme, and originating electroweak kernel throughout. Assume
also that \(t_Q(q^2;P)>0\) on the readout domain and that it has a finite,
strictly positive Thomson limit
\[
t_Q(0;P):=\lim_{q^2\to0}t_Q(q^2;P)>0.
\]
...
g_Q^2=\frac{t_Q}{2\pi},
\]
so the Ward-projected \(Q\)-channel determines a unique positive transport time \(t_Q(q^2;P)\)
on the assumed readout domain. The separately assumed finite positive limit makes the Thomson
ratio defined. Because
...
\begin{remark}[Promotion contract for the declared pixel--Ward pipeline]
Promotion of this particular pixel--Ward construction to a source-only
fine-structure row requires that no endpoint target feed the same-source Ward
kernel.  Its provenance packet must contain a source-derived Ward-current
spectral measure and source kernel, the electromagnetic normalization, a
same-scheme finite remainder, a no-target-leak dependency graph, the full
pixel-map contraction, a target-independent selector for one admitted map,
and a typed same-quantity bridge between its input and output.  The equality
\[
\lim_{q^2\to0}\alpha_{\mathrm{em}}^{-1}(q^2;P)
=\alpha_{\mathrm{Th}}^{-1}(P)
\]
is definitional in this construction and supplies no independent validation.
Empirical decision readiness additionally requires a prospectively frozen
uncertainty model and decision rule.  A predictive interval narrower than a
comparison experiment can improve competitive power, but it is not a
condition for source-only provenance.  Without the listed provenance packet,
the displayed endpoint retains its empirical hadron-closure classification.
This contract does not rule out another analytic or computational source-only
construction with a different, fully typed provenance chain.
\end{remark}

For this pipeline, the same-source backend contract treats the scalar row as
a two-current marginal of a larger source-derived hadronic spectral backend.
Using that backend for both the Thomson endpoint and the HVP branch requires
the source law to record the QCD quotient ensemble, source parameter map, Ward
current normalization, systematics accounting, and no-target-leak DAG.  Full
hadronic precision claims require higher-point and transition spectral
exports from that backend instead of reusing the scalar fine-structure
residual.
...
The selected branch carries a conditional dimensionless electroweak
hierarchy relation. It does not certify Higgs naturalness. The large
...
The current packet supplies no OPH result about Higgs naturalness.
On the declared electroweak/criticality surface, the
...
coordinate split, but declaring that split unobservable does not prove that
all relevant deformations are absent or fixed.  The current comparison maps
are chosen so that their square commutes and their declared defect is zero.
Exact countermodels on the same source antecedents also have nonzero defect.
The live route is to derive the comparison maps from the source and then
evaluate both residuals independently.
...
one physical zero. No universal all-rung membership of its countermodels in the
complete A1--A3 capacity-source contract or executable-to-Lean bridge is
supplied. Direct \(N\) is not evaluable on that incomplete antecedent. An
...
distinct objects. The positive readback scale is a unit convention.
...
load-carrier map are physical branch premises. The exact exterior
...
\begin{theorem}[Declared-map RG/Higgs commuting-square identity]\label{thm:rg-higgs-naturality}
...
\]
Consequently, the defect of these declared maps is
\[
\varepsilon_H^{\mathrm{decl}}
=
\max\{\varepsilon^n_{sH},\varepsilon^h_{sH}\}
=0,
\qquad
\varepsilon_H^{\mathrm{decl}}\in[0,0].
\]
\end{theorem}
...
\]
Both controlled squares preserve this tuple by their declared definitions, so
their product defect is zero.  This proof establishes no source selection of
the maps and no statement about physical relevant deformations.
\end{proof}

The declared-map identity does not itself use the measured weak scale. Measured \(W/Z\),
...
mathematical EW bridge-map fixed point, conditional operational-readback implications, and one declared
RG/Higgs square.  Exact finite completions sharing the current source
antecedents attain defects zero and one, so the antecedents do not determine a
naturality defect.  A source-derived definition of all comparison maps followed
by independent evaluation of both residuals is the constructive continuation.
The present package does not supply that continuation, the additional capacity-selecting source law,
the universe-level carrier attachment, or the common screen/electroweak
load-carrier identification. No strict
source-root certificate is supplied, and the result does not independently attach
...
verdict does not follow. Direct \(N\) is not evaluable on the incomplete capacity
...
correlated 55-electron matrix. The unsupplied clock parents are the atomic-scheme
...
\(c_r(P_\star,N_\star)\) are required inputs not supplied by that mechanism.
...
not satisfy this source theorem because it supplies no beta provenance,
threshold origins, matching interval composition, or truncation enclosure.
...
and source-only pixel branches differ, and no physical scale,
renormalization-group transport, complex pole, or target-independence proof is
supplied.
...
The narrower current-carrier slice with \(\delta n=0\) and the fixed
fiber law has one exact central-value obstruction conditional on treating
the displayed finite-decimal inputs as the exact rationals they serialize.
The charged mass is
strictly increasing for \(\tau_2>-1\), so matching the displayed central
\(W\) value fixes the unique coordinate
\(\tau_2=-0.0005918464744070\ldots\). Substitution in the exact nonlinear
neutral formula gives \(M_Z=91.1682226626\ldots\,\mathrm{GeV}\), a residual
of \(-0.0196773374\ldots\,\mathrm{GeV}\) from the displayed central \(Z\)
value; an exact rational interval evaluation of the squared residual excludes
zero. Thus that closed one-scalar slice cannot hit both displayed central
values exactly. This is not an uncertainty-region exclusion. The opposite
directions requested by the two first-order germs are
only a local diagnostic and are not a global no-go. Fits to uncertainty
regions, an additional neutral scalar, a source-derived change of the fiber
law, and a different carrier or selector remain open
(\path{code/particles/runs/calibration/d10_ew_tau2_current_carrier_obstruction.json}).
...
On the target-free declared-map branch the two branch points give the tree/chart
coordinates
\[
\begin{aligned}
...
\((79.53284,89.71232)~\mathrm{GeV}\). It supplies no definition of \(v\),
tadpole treatment, field-content or threshold matching, scale choice, or
higher-order corrections. The outputs establish
...
separately declared, and no derivation from the complete three-axiom schema is
supplied. The reconciliation placement is exact within a
...
no derivation from the three axioms is supplied. The finite criticality carrier
...
OPH flavor observable is theorem-level. Instead, it builds one explicit sufficient
proof-obligation chain that the present construction would have to discharge; other
carriers, direct source actions, dynamics, and observables are not excluded. This chain
has the following main architecture: start from a refinement-indexed family transport kernel, derive a centered
...
determinant trace-lift attachment \(3\mu(r)=S_M(r)\) on the charged determinant channel. The charged determinant channel carries a current-corpus gap certificate: the uncentered trace lift is not emitted, the attachment \(3\mu(r)=S_M(r)\) is the named live route, and the only no-go on this lane is the common-shift theorem of the charged absolute-scale section. The neutrino branch carries a target-informed weighted-cycle
candidate above a template family kernel, with compare-only bridge and absolute-attachment diagnostics. The quark
branch carries an exact restricted source-spread obstruction after the candidate ordered shape law
is granted: its ordered profile shapes then leave a free \((\mathbb R_{>0})^2\) fiber.
Selected-class descent proves representative independence but does
...
Conditional on the current registered source signature containing no typed datum that separates or
excludes the displayed centered-spread orbit, the quark branch carries a non-identifiability
argument on a declared generic fixed-frame quark package. Separately, granting the candidate-only
ordered three-point shape law supplied by
the template-descended branch generator, the target-free source packet fixes the ordered up- and
down-sector profile rays and leaves their endpoint spans free as two independent positive moduli.
That shape law is unproven and is rejected on the common-scale physical surface below, so the
generic interface carries six unemitted scalars and the two-modulus statement is a restricted
lower bound inside the granted subfamily. Selected-class
...
are mass textures rather than physical dimensionless Yukawa matrices. Neither result classifies
...
source-only physical spread datum. The target-free source equations do not emit that datum.
Granting the candidate-only ordered shape law, the source packet fixes two ordered profile rays
and leaves their positive endpoint spans independent; without that grant the shape law, the
sector means, and the spans are all unemitted. The compatible fiber inside the granted subfamily
is \((\mathbb R_{>0})^2\), so the six numeric rows are not source-only predictions.
...
rows are withheld because, under the stated registry-completeness premise, the current registered
source signature does not identify the spread pair. A physical
...
\(1.497823\). Those values are not quark-mass evidence. The public receipt
binds the four principal run inputs by hash; their upstream arrays are not
mirrored in that compact receipt archive \cite{ophparticlesimdata}.
...
\paragraph{Conditional non-definability from the current registered signature.}
...
color count, and generation count. The members are nevertheless physically inequivalent because
the declared physical-equivalence relation preserves Yukawa invariants. Axiom~3 also does not
remove the family through any registered typed output: its gauge-invariant local
constraint values and associated multipliers are not numerically specified and no registered map
from \(P\) to quark Yukawa multipliers is emitted. The load-bearing registry-audit premise is that
the current registered A1--A3 signature and declared structural package contain no other typed
constraint, dynamics, action, carrier, or output map that separates or excludes this orbit. Under
that premise, the current antecedents plus fixed \(P\) admit a free
...
degenerate boundary rather than the observed spectrum. Therefore, conditional on the registry-audit
premise, the current registered signature does not define a unique quark mass spectrum. Any
completion retaining this counterfamily must add or derive data
that separate or exclude this explicit counterfamily down to a unique physical-equivalence class.
A source functional nonconstant on the family is one possible route, but so are a derived constraint,
a source-selected dynamics or action, a stronger carrier or representation, or an independently
justified new physical premise with the required separating power.
...
counterfamily proves as a necessary condition is narrower and decisive: any
successful completion retaining this fixed counterfamily must separate or
exclude its independent \((\lambda_u,\lambda_d)\) centered-spread rescaling
orbit. In particular, a source functional acting on this same family must be
nonconstant on that orbit.
...
additional sign-sector value \(6^{(1)}\); no physical regular-heat-to-family attachment is supplied.
...
proposed matrices would make the simulator test circular. The archived
machine-readable report binds the four source-state arrays by hash but does
not include those arrays, so it provides report custody rather than an
independent raw-state replay \cite{ophparticlesimdata}.
...
conclusion of both. The spread fiber survives that certified structure set, the two receipts
and the twelve frozen orbit-selector candidates with input hashes pinned at emission, with its
coordinates realized as the free scalar coefficients along those lines. A machine-checked
transport certificate records the survival together with the exclusion of the twelve candidates,
and flips fail-closed if any scanned datum moves under the rescaling; the scan is a key-name
heuristic over the receipt leaves and covers that set only. A cut of this fiber requires a
physical binding of the response representation, the attachment of the screen action to three
physical families, a selector admitted under the frozen single-comparison discipline, or the
conditional Higgs/top criticality coordinate granted together with a source-side scheme
conversion.
...
are not source-only results, and no third-generation normalization or
threshold packet is supplied.
...
two of these 31 real axes. The same exact table lacks the two
eigenvector-overlap readings \(|V_{us}|=\cos\theta_{\mathrm{axis}}\) and
\(|V_{us}|^{2}=\sin^{2}\theta_{\mathrm{axis}}/2\) as well. The coordinate
construction of the axes is certified by the exact receipt; the Lean
companion certifies the finite comparison on the emitted table. The no-go
does not exclude spinorial representations, higher-order symmetry breaking,
additional dynamical corrections, or overlap models beyond those two
readings. The \(31.7175^\circ=\arctan(1/\varphi)\) entry provides an
...
\sigma_{\mathrm{ext}},
\]
and the charged absolute scale \(g_e\) is not determined. The scalars
...
\]
Hence every readout that factors through the centered readback and these four
invariants determines only the quotient class
\[
E_e^{\mathrm{centered}}\in \mathbb R^3/\langle(1,1,1)\rangle,
\]
and leaves the absolute scalar \(g_e=e^c\) free. The same-label \(q_e\) readback and the
determinant character \(S_M\) are chain outputs outside this list; the theorem makes no
...
therefore gives a no-go for a \(P\)-driven charged mass read from centered data alone: the theorem fixes
the centered shape, while the absolute normalization sits in the determinant-line landing,
an underived bridge on the corpus.
...
is not derived here. Koide balance fixes a shape condition and is
...
Z_c=P_c\otimes q_+.
\]
The blocks \(Z_0,Z_c\) both have rank two. Under the declared L\"uders
instrument, Born--L\"uders conditioning of the unique MaxEnt
...
The required continuous family connection and its source selector are not
constructed. Under the stated connection and attachment hypotheses, \(\delta=2/9\). The phase of one
...
\(|b|/a=e^{-\chi_\rho}/\sqrt2\); no source-derived bare-to-endpoint repair bridge
is supplied.
...
kernel, and singularity transport are not supplied.
...
with \(|\mathrm{d}P/\mathrm{d}\kappa|\approx6\times10^{-5}\), and the absolute masses are not
derived from the source branch.
...
The leading cubic entropy truncation can be classified globally, rather than
by the former finite-seed search. A \(W_5\) port vector consists of six values
...
That boundary is a viable route, not a no-go. On the closed probability
...
quadrupole-to-log-mass attachment is not supplied. Thus the strict full-support
quartic minima are conditionally excluded as the observed shape, while the
closed boundary, positive regularizations approaching it, higher entropy
orders, the full entropy, and other source-derived potentials remain live
...
No charged-basis construction is supplied, and the three normalized shape
singular values of the template differ only at about
...
Any solar mover must therefore consume data beyond this same-label
\(S_3\)-isotropic eta-class. Realized flavor-side same-label gap/defect
readback is one route; a neutrino-side extension with new non-isotropic source
data, spontaneous breaking, another carrier or representation, or additional
dynamics is not excluded.
...
prediction because no charged basis is constructed and the basis used is template-derived.
...
The fixed-cutoff measurement package is also explicit about probabilities and
updates under its supplied density-state valuation and declared L\"uders
...
\]
In the supplement, the same conditional statement appears as the declared
L\"uders update on the record algebra. In
the flagship paper \emph{From Observer Consensus to Standard Physics}~\cite{ophsynthesis}, this fixed-cutoff
Born/L\"uders package and its Bell/CHSH extension are imported as theorem-bearing
under their displayed representation and instrument premises, not
...
Within this declared instrument model, measurements assign particles
conditioned observer-accessible states on the central record algebra. A
...
physics. After a nonzero-weight event is observed and the declared L\"uders
instrument is applied, an immediate reread with the same projector has
probability \(1\). An intervening repair preserves that conclusion only when
its output state is explicitly supported in the same event projector. So the
conditional measurement interface is well defined and operationally auditable.
...
physical event charts only after two-way cone/order faithfulness,
...
Given a valid affine event atlas satisfying the spacetime paper's
source-causal continuum certificate, a common
...
separately declared algebra-state, L\"uders instrument, and two-wing
representation, the measurement theorem gives Born probabilities, the selected
L\"uders conditioning, immediate repeated-read stability (and persistence
under an explicit support-preserving intervening operation), and the stated
Bell/CHSH bound. It does not prove global observer continuation or
...
conditional attachment. No derivation from OPH repair data is supplied. On
...
The physical baryon abundance is not derived. Its required source
...
from \(P\) to physical charged data is an underived bridge, a corpus gap certificate
whose named live route is the trace-lift attachment \(3\mu(r)=S_M(r)\), and the neutrino
...
representation. The construction does not select the physical global quotient.
The transportable-sector/Tannaka chain is a separate conditional
classification. No physical source binding, laboratory current identification,
...
cross-domain Spin transport, scalar multiplicity/dynamics, or continuum QFT is
supplied. The
strengthened
...
The common-load premise gives a conditional dimensionless hierarchy.  The
separately declared comparison maps give only the packaging identity
\(\epsilon_H^{\mathrm{decl}}=0\); no physical naturality value is emitted. The
weak-boson and Higgs numbers are chart coordinates.
...
physical reference transition, and SI clock binding are not supplied. The
...
Neither is constructed for those stronger claims.
...
the conditional finite tracial-GNS packet. No physical chiral-mass attachment,
phase, or numerical ratios are supplied. Conditional on the current registered
source signature, the analysis proves a quark counterfamily; after granting a
candidate fixed-ray shape law, it gives an exact restricted two-modulus fiber.
It also rejects the tested
...
are degenerate, the twofold fixed locus retains two parameters after
scaling, the Klein four-group fixed locus retains one, and every other
subgroup of \(A_5\) contains a threefold or fivefold rotation. Absolute
...
backend. Empirical hadronic transport is an empirical input.
...
2026.
\url{https://github.com/FloatingPragma/observer-patch-holography/tree/main/Lean/Screen}
...
2026.
\url{https://github.com/FloatingPragma/observer-patch-holography/tree/main/evidence/particle_simulation_receipts}.
