...
structural zeros are exact, the \(W/Z\) pair has an exact frozen-repair sidecar, the Higgs has an
exact inverse slice, the charged and quark families have exact same-family witnesses, and the
neutrino lane has an exact two-parameter compare-only adapter on the explicit positive selector
segment. Charged leptons and flavor-labeled neutrinos are nevertheless open for different reasons:
...
current cloud quantum devices. This paper gives a concise
...
and several quark rows are emitted, while charged leptons, the still-open exact physical quark branch, flavor-labeled neutrinos, and
...
because the corpus proves neither exact vanishing nor uniform quadratic smallness after the central split. The second arrow concerns the common-shift quotient surface, which is closed negatively, so centered data alone do not emit an absolute charged scale. The irreducible post-promotion object is the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), from which the uncentered lift, determinant-line section, and affine anchor follow canonically,
\[
...
The weighted-cycle / Majorana-holonomy branch closes the PMNS/hierarchy shape and leaves no further discrete branch object on that lane. The normalized same-label overlap-defect weight section closes beneath that shape, so the remaining freedom is the positive rescaling orbit of the absolute spectrum together with the paper-facing attachment amplitude
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
above the closed normalizer. Because the proxy
\[
P_\nu:=I_\nu^{1/2}\,\widehat{\mathrm{ratio}}^{\,1/2}\,\mathrm{sum\_defect}^{-1}
\]
is internal to the emitted stack, the exact missing object is the reduced correction invariant
\[
C_\nu:=\frac{B_\nu}{P_\nu}.
\]
...
\]
but that family does not emit \(C_\nu\) or \(B_\nu\) by itself. The compare-only value is \(C_\nu^\ast=0.9995473560\). A direct correction audit gives the target-containing reduced window \(C_\nu\in[0.9994296,1.0015784]\), which induces the narrower primary corridor \(B_\nu\in[6.69600,6.71040]\) with midpoint \(6.70320\); the earlier three-route bridge corridor \([6.69315,6.71259]\) remains as a wider comparison envelope, and the shortlist-consensus window \([6.70484,6.71259]\) remains diagnostic-only because it misses the compare-only target. Charged leptons and neutrinos are open for different reasons. Charged leptons wait on promotion of \(\widehat C_e^{\mathrm{cand}}\) and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\). Neutrinos wait on the reduced bridge-correction invariant that would emit one positive normalization after the shape closure.
The explicit positive selector segment also supports a stronger compare-only continuation adapter: solving \(\tau_\nu\) on \(D_\tau=(1-\tau_\nu)\chi+\tau_\nu(1+\gamma_{1/2})\) against the representative central ratio and then solving \(\lambda_\nu\) against \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\) yields \(\tau_\nu=0.49717\), \(\lambda_\nu=1.72396\), \((m_1,m_2,m_3)=(0.01745663,\,0.01948420,\,0.05308139)\,\mathrm{eV}\), and exact representative central splittings \(\Delta m_{21}^2=7.49\times10^{-5}\,\mathrm{eV}^2\), \(\Delta m_{31}^2=2.5129\times10^{-3}\,\mathrm{eV}^2\), \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\). This sidecar is compare-only because the weighted-cycle theorem lane waits on \(C_\nu\).
Taken together with the exact \(W/Z\) frozen repair pair, the exact Higgs/top inverse slice, and the exact charged/quark same-family witnesses, the exact non-hadron output lane is complete, even though the theorem-grade charged, quark, and neutrino lanes remain open at their sharper objects.
...
Quarks have continuation-level rows on an emitted D12 branch. Charged leptons sit on a narrower continuation surface: the same-carrier centered readback is exact, the common-shift quotient surface is closed as a no-go for absolute scale, and the theorem-grade frontier is the two-layer chain
\[
\widehat C_e^{\mathrm{cand}}
\longrightarrow
\widehat C_e
...
\mu_{\mathrm{phys}}(Y_e),
\]
with the first arrow identified with the package \ophid{oph_generation_bundle_branch_generator_splitting} and reduced in practice to the missing descended-commutator clause \ophid{compression_descendant_commutator_vanishes_or_is_uniformly_quadratic_small_after_central_split}.
If that promotion closes, the uncentered lift, determinant-line section, and affine anchor follow canonically from \(\mu_{\mathrm{phys}}(Y_e)\); until then no public charged-lepton mass theorem is claimed. A target-anchored same-family exact witness reproduces the charged reference triple exactly on the same ordered eigenvalue family, but its scope is same-family-only and it does not promote the theorem lane. Neutrinos are open for a different reason: the weighted-cycle / Majorana-holonomy lane closes the PMNS/hierarchy shape and misses the reduced correction invariant that would emit one positive normalization. Hadrons remain behind the explicit nonperturbative frontier.
...
Read this section by claim tier. The same absolute residual can indicate either a closed emitted value or a compare-only continuation benchmark. On this table, the closed emitted-output set is the electroweak calibration pair \(m_W\) and \(m_Z\). Charged-lepton near-matches are compare-only continuation diagnostics, and the weighted-cycle neutrino branch is one reduced bridge-correction invariant \(C_\nu\) short of promoted absolute masses, with the paper-facing amplitude \(B_\nu\) induced only after exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization above the emitted proxy.
...
\toprule
Sector & Status & Derivation stage & Exact output(s) & Caveat / remaining step \\
\midrule
...
Higgs/top stage & secondary quantitative + exact sidecars & D10 gauge core followed by D11 / family exact witnesses & exact sidecars \(m_H=125.1995304097179~\mathrm{GeV}\), \(m_t=172.3523553288311~\mathrm{GeV}\) & Higgs exactness is a compare-only D11 inverse slice; top exactness is a same-family witness, not a theorem-grade forward closure \\
Quark family & continuation + exact sidecar witness & shared excitation dictionary \(\rightarrow\) D12 mass-side continuation \(\rightarrow\) selected D12 sheet & exact same-family quark sextet & same-family-only; the local selector closes to \(\sigma_{\mathrm{ref}}\), so the next theorem-side object is \ophid{intrinsic_scale_law_D12} on \ophid{D12_ud_mass_ray} \\
Charged leptons & continuation + exact sidecar witness & shared excitation dictionary \(\rightarrow\) ordered charged carrier \(\rightarrow\) exact centered readback & exact same-family charged triple & same-family-only; theorem-grade waiting set is promotion of \(\widehat C_e^{\mathrm{cand}}\) and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\) \\
...
Higgs exact sidecar & exact Higgs inverse slice & axioms \(\rightarrow D10\) gauge core \(\rightarrow D11\) Jacobian \(\rightarrow\) exact inverse slice & compare-only inverse slice, not the public forward theorem row \\
Charged exact witness & exact same-family charged triple & axioms \(\rightarrow\) shared excitation dictionary \(\rightarrow\) ordered charged carrier \(\rightarrow\) exact centered readback \(\rightarrow\) same-family exact witness & same-family-only; theorem lane waits on \(\widehat C_e^{\mathrm{cand}}\) and then \(\mu_{\mathrm{phys}}(Y_e)\) \\
Quark exact witness & exact same-family quark sextet & axioms \(\rightarrow\) shared excitation dictionary \(\rightarrow\) D12 mass-side continuation \(\rightarrow\) selected D12 sheet \(\rightarrow\) same-family exact witness & same-family-only; the selected D12 sheet is the wrong CKM branch, and the remaining theorem-side object is \ophid{intrinsic_scale_law_D12} on \ophid{D12_ud_mass_ray} \\
...
The detailed inventory below still records the public theorem/continuation rows sector by sector. Whenever an exact sidecar or same-family witness is stronger than the public theorem row, the caveat remains the one stated in the exact-hit table above.
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger two-scalar candidate \(0.00215992~\mathrm{GeV}\) & the selected D12 sheet is a strict no-go for the physical CKM shell; the local selector closes to \(\sigma_{\mathrm{ref}}\), so the next exact object is the intrinsic scale law on the emitted D12 mass ray \\
Quark family & down quark & overlap continuation & reported value \(0.00512144~\mathrm{GeV}\); stronger two-scalar candidate \(0.00470005~\mathrm{GeV}\) & same wrong-branch D12 frontier; same-sheet CKM/CP transport is closed, the local selector is fixed to \(\sigma_{\mathrm{ref}}\), and the next quark object is the intrinsic scale law on the emitted D12 ray \\
Quark family & strange quark & overlap continuation & reported value \(0.0916111~\mathrm{GeV}\); stronger two-scalar candidate \(0.0935045~\mathrm{GeV}\) & same wrong-branch D12 frontier; the local selector is fixed to \(\sigma_{\mathrm{ref}}\), so the next quark object is the intrinsic scale law on the emitted D12 ray \\
...
Quark family & bottom quark & overlap continuation & reported value \(3.820118~\mathrm{GeV}\); stronger two-scalar candidate \(4.182754~\mathrm{GeV}\) & same wrong-branch D12 frontier; the local selector is fixed to \(\sigma_{\mathrm{ref}}\), so the remaining theorem-side quark burden is the intrinsic scale law on the emitted D12 ray \\
Charged leptons & electron & continuation gap & \(n/a\); continuation pair \((\eta,\sigma)=(-6.72959,\,8.15406)\) gives near-exact centered-log shape closure, but the absolute scale remains quotient-only; compare-only target \(g_e^\star=0.0457789\), equivalently \(\Delta_e^{\mathrm{abs},\star}=3.00398633\) & promote the latent candidate \(\widehat C_e^{\mathrm{cand}}\) by closing branch-generator splitting, then emit the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\) with \(\widetilde C_e(Y_e)=\widehat C_e(Y_e)+\mu_{\mathrm{phys}}(Y_e)\,\mathbf 1\), \(A_{\mathrm{ch}}(Y_e)=\mu_{\mathrm{phys}}(Y_e)\), and finally read out \(g_e\) and \(\Delta_e^{\mathrm{abs}}\) \\
Charged leptons & muon & continuation gap & \(n/a\); same centered-shape closure and common-shift no-go as the electron row & same \(\widehat C_e^{\mathrm{cand}} \rightarrow\) promotion \(\rightarrow \mu \rightarrow A_{\mathrm{ch}} \rightarrow g_e\) closure chain \\
...
For the BW/geometric side, the smallest internal extension route is a two-step chain:
...
eventual positive lower spectral bound on each fixed local collar model. On the current branch the
single live missing clause inside that datum is precisely that eventual spectral floor, which feeds
the faithful modular-defect term.
...
Yukawa/excitation dictionary is explicit as the common proof-facing base. The first remaining open front above that base is the lane-specific readout
...
The strongest theorem-facing quark object is the intrinsic scale law on the emitted mass ray. On the emitted same-label left-handed solver surface the selector is fixed, and the exact-hit sidecar on top of that theorem boundary is the same-family witness \((u,d,s,c,b,t)=(0.00216,0.00470,0.0935,1.273,4.183,172.3523553288311)\,\mathrm{GeV}\). Its caveat is precise: it does not move the selected D12 sheet. The exact primitive split is as follows.
...
\]
So the compact same-family continuation does not emit a distinguished scalar \(t_1\). It
...
+ \theta_{13}^{(K)}\bigl(e^{i\phi_K}E_{13}-e^{-i\phi_K}E_{31}\bigr).
\]
The quark frontier is split in two:
\begin{enumerate}[leftmargin=2em]
\item the intrinsic scale law on the emitted D12 mass ray. The local same-label left-handed orbit closes to the singleton \(\sigma_{\mathrm{ref}}\), so the selector does not carry the theorem-side burden on the emitted solver surface. The mass side is equally sharp in the negative direction: the same-family D12 branch has an explicit underdetermination theorem saying that it emits only the one-parameter ray \(D12_{ud}^{\mathrm{mass}}\), not a theorem-grade modulus on that ray; and
...
The smallest local finite basis orbit is diagnostic-only. Replacing the ordered same-label
...
The quark lane has exactly two open theorem-facing burdens, and only one of them lives on the emitted same-family D12 ray:
\begin{enumerate}[leftmargin=2em]
\item the builder-facing pure-\(B\) payload pair beneath the reported quark values remains open;
\item on the emitted same-label left-handed solver surface, the selector question is settled negatively: the orbit closes to \(\sigma_{\mathrm{ref}}\), and \(\sigma_{\mathrm{ref}}\) is the wrong D12 CKM sheet. The remaining same-family theorem object is the intrinsic ray law \ophid{intrinsic_scale_law_D12} on \(D12_{ud}^{\mathrm{mass}}=\mathcal R_{D12}^{ud}\) (paper id \ophid{D12_ud_mass_ray}).
\end{enumerate}

The correct headline for quarks is narrower: quarks are the only matter family with reported numerical rows, and the row-emission branch sits above visible constructive objects, while the selector problem on the emitted solver surface is closed in the negative sense. A target-anchored same-family exact witness reproduces the six quark reference masses exactly on the same ordered three-point family, but it remains same-family-only and does not collapse the wrong-branch D12 boundary. Any compare-only orbit improvement or chirality-basis diagnostic that is not sector-attached and left-handed same-label remains outside the theorem ledger.
...
this step, and on the current isotropic branch the same-label phase-cocycle theorem, the
...
C_\nu:=\frac{B_\nu}{I_\nu^{1/2}\,\widehat{\mathrm{ratio}}^{\,1/2}\,\mathrm{sum\_defect}^{-1}}
\]
above the emitted proxy and closed normalizer. The current attached stack also factors exactly through
\(q_e = q_{\mathrm{mean}} qbar_e\), so a \(qbar_e\)-only collapse law for the bridge factor does
not follow from the present corpus. The best closed constructive local object beneath that bridge is the
defect-weighted same-label edge family \(q_e=\sqrt{g_e d_e}\) together with the induced
\(\mu_e\) family, but that family still sits below \(C_\nu\) and the induced paper-facing amplitude \(B_\nu\) rather than replacing them.
