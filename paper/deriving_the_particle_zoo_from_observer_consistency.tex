...
\begin{abstract}
Observer-Patch Holography (OPH) asks how much of the particle spectrum follows from one shared local input rather than from a long fitted parameter list. Building on the OPH recovery of relativity and Standard Model structure, this paper develops the particle branch from the dimensionless pixel constant \(P \equiv a_{\mathrm{cell}}/\ell_P^2\). It fixes the structural massless carriers, closes a target-free \(W/Z\) lane, reads the low-energy fine-structure constant as the Thomson endpoint of the Ward-projected electromagnetic transport family with \(\alpha^{-1}(0)=137.035999177\), carries a quantitative Higgs/top branch, closes the exact PDG 2025 API running-quark sextet together with explicit exact forward Yukawas on the selected public quark frame class \(f_P\), and gives a theorem-grade weighted-cycle neutrino family in the observed hierarchy and mixing regime together with a theorem-grade physical Majorana phase pair on the shared-basis weighted-cycle transport branch. The exact non-hadron surface includes exact structural zeros, an exact frozen \(W/Z\) pair, exact same-family charged witnesses, the conditional charged determinant-line lift, the downstream algebraic charged-mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\), the selected-class exact quark theorem together with its supporting exact surfaces, and exact neutrino-side readout objects. The open charged source theorem is the landing from the D10 calibration descendants of \(P\) to theorem-grade physical charged data or the charged determinant line. A global classification of all quark frame classes, flavor-labeled neutrinos, and hadrons remain explicit boundary objects.
\end{abstract}
...
branch, to the emitted ray \(\mathcal R_{D12}^{ud}\). The top quark also remains visible on the
...
rather than the auxiliary direct-top entry. The Higgs/top branch also carries its own public top
row \(172.388646~\mathrm{GeV}\) on the D11 forward seed. The table above records the quark-family
running-mass theorem surface on \(f_P\).
