...
The public numeric particle prediction surface contains the structural carrier
zeros and the conditional Higgs candidate on the declared D10/D11 surface.
Target-anchored charged/quark witnesses and compare-only absolute neutrino
attachments are audit or comparison rows, not public predictions. Hadron masses
and absolute charged-lepton masses are not derived here.
\end{abstract}
...
pipeline claims public numeric closure only for the structural massless carriers and the Higgs
candidate on its declared quantitative surface. The hierarchy/naturality bridge is closed on its
...
stress test. Adding the emitted finite-screen unified gauge-width contribution
\(\alpha_U(P_\star)\) gives the source-side no-hadron near-endpoint,
\(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\), below the Thomson endpoint.
This sum, not the root-only audit value, is the source-side fine-structure
prediction displayed in the repository README.
...
\midrule
\(P\)-closure and fine structure & \(\alpha_{\mathrm{root}}^{-1}=136.9948351646\ldots\); \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\); endpoint \(\alpha^{-1}(0)=137.035999177(21)\) & source-side no-hadron near-endpoint plus empirical endpoint closure \\
...
Neutrinos & continuation + emitted bridge invariant + auxiliary checks & same-label scalar certificate \(\rightarrow\) weighted-cycle branch \(\rightarrow\) bridge rigidity \(\rightarrow\) absolute attachment \(\rightarrow\) shared-basis transport readout & scale-free weighted-cycle branch, central splitting and PMNS/Majorana comparison rows; compare-only absolute mass attachments withheld from public prediction tables & theorem-level only for the declared scale-free branch and emitted bridge invariant.\newline The exact positive-segment adapter, bridge corridor, correction check, and absolute attachments are diagnostics and do not feed back into theorem state \\
...
Neutrino theorem branch & scale-free branch plus comparison rows; compare-only absolute mass attachments withheld & axioms \(\rightarrow\) same-label scalar certificate \(\rightarrow\) weighted-cycle branch \(\rightarrow\) bridge-rigidity theorem \(\rightarrow\) absolute-attachment candidate \(\rightarrow\) shared-basis transport readout & theorem-level on the declared scale-free weighted-cycle branch and emitted bridge invariant; exact positive-segment adapter, bridge-coordinate sidecars, and absolute attachments are diagnostics \\
\bottomrule
\end{tabular}
\end{table}
...
\]
inverse-alpha units. Adding the finite-screen unified gauge-width contribution
\(\alpha_U(P_\star)\) to the undressed source/root inverse coupling gives the
source-side no-hadron near-endpoint
\[
...
\]
The sum \(\alpha_{\mathrm{root}}^{-1}+\alpha_U(P_\star)\), not \(\alpha_U(P_\star)\) by
itself, is the source-side no-hadron near-endpoint. It lands close to the Thomson endpoint
...
Neutrinos & \(\nu_e\) & weighted-cycle theorem branch with compare-only absolute attachment & absolute mass withheld from public prediction table; the weighted-cycle branch emits \(\theta_{12}=34.2259^\circ\), \(\theta_{23}=49.7228^\circ\), \(\theta_{13}=8.68636^\circ\), \(\delta=305.581^\circ\), \(J=-0.02753\), \(\Delta m_{21}^2/\Delta m_{32}^2=0.03072111\), \(\alpha_{21}^{(\mathrm{Maj})}=153.618518^\circ\), \(\alpha_{31}^{(\mathrm{Maj})}=257.003241^\circ\), the bridge invariant \(C_\nu=0.9994295999075177\), the emitted proxy \(P_\nu=6.699825740519345\), the paper-facing amplitude \(B_\nu=6.696004159297337\), and \(\lambda_\nu=1.7237014208357415\) & closed on the declared scale-free weighted-cycle branch; the absolute attachment, positive-segment adapter, bridge corridor, and bridge-coordinate sidecars are diagnostic-only and do not feed back into theorem state \\
Neutrinos & \(\nu_\mu\) & weighted-cycle theorem branch with compare-only absolute attachment & absolute mass withheld from public prediction table; same weighted-cycle theorem branch as the electron-neutrino row & same bridge-invariant / induced-amplitude package and same diagnostic-only retirement rule for the absolute attachment, positive-segment adapter, and bridge corridor \\
Neutrinos & \(\nu_\tau\) & weighted-cycle theorem branch with compare-only absolute attachment & absolute mass withheld from public prediction table; same weighted-cycle theorem branch as the electron-neutrino row & same bridge-invariant / induced-amplitude package and same diagnostic-only retirement rule for the absolute attachment, positive-segment adapter, and bridge corridor \\
...
The strongest source-side candidate currently on disk emits
\[
\sigma_u^{\mathrm{edge}}
=
...
Ward-projected Thomson lane records a source-side no-hadron value, the source-plus-unified-gauge-width
