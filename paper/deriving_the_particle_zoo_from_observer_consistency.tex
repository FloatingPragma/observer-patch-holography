...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a status-tiered forward calculation from one local screen scale. The completion pipeline compresses the candidate \(P\)-closure branch into five executable layers: unification scale, heat-kernel pixel closure, declared running/matching, electroweak mixing, and structured Thomson transport. It then propagates that branch into the non-hadron particle rows. The public surface emits theorem-grade structural massless carriers, a Higgs/top row on its declared electroweak calibration surface, selected-class running quark rows, and a weighted-cycle neutrino absolute-mass branch. It also records compare-only \(W/Z\) and charged-lepton witnesses, a candidate \(P\) root outside the prediction tier, and no hadron predictions. The endpoint package isolates the Ward-projected Thomson residual; the source-only residual map and RG/matching interval certificate remain work in progress. The available corpus gives no-go boundaries for the charged determinant landing, quark global-classification lane, and direct-top auxiliary conversion. Hadrons require a separate backend and carry no emitted mass predictions in this paper.
\end{abstract}
...
pipeline claims closure only for the structural massless carriers, the Higgs/top row on the
declared D10/D11 surface, the selected-class quark theorem, and the weighted-cycle neutrino
...
promoted from candidate metadata to a certified prediction root. The pipeline records the candidate
value
\[
...
\]
and keeps it outside promoted particle predictions while the Ward-projected Thomson residual map and
RG/matching interval gates are work in progress. The
...
\]
through this outer formula. The five-layer OPH trunk emits the candidate
\[
P=1.63097210492078846050203640439,
\qquad
\alpha_{\mathrm{cand}}^{-1}=136.994020662724205139718642793.
\]
The difference is not hidden as numerical noise. It is the visible effect of using the
...
endpoint contracts, but the trunk may feed only audit and comparison surfaces.
...
final bundle, the published \(W/Z\) numbers are not promoted as theorem-grade predictions.
...
\]
The reason is mechanical: the same branch depends on the uncertified \(P\) root, the
Ward-projected Thomson residual map, and the RG/matching interval certificate. The selected-carrier
...
benchmark. The completion pipeline emits an executable closure matrix with explicit claim tiers:
the structural massless bosons, the Higgs/top declared surface,
the selected-class quarks, and the weighted-cycle neutrino absolute-attachment branch are the
closed or scoped non-hadron chains; the charged-lepton lane is scoped as a corpus-limited no-go; and
the \(P\)-closure root plus electroweak \(W/Z\) chain remain nonclosed. Hadrons are closed out-of-scope for this pipeline because the
...
\toprule
Chain & Status & Promotable & Open gates & Output \\
\midrule
\(P\)-closure root & candidate root, not certified & no & \#32, \#235, \#224 & \(P=1.63097210492078846050203640439\), \(\alpha^{-1}_{\mathrm{cand}}=136.994020662724205139718642793\) \\
Structural massless carriers & closed structural zero & yes & none & \(m_\gamma=m_g=m_{\mathrm{grav}}=0\) \\
Electroweak \(W/Z\) & compare-only frozen adapter, not a prediction theorem & no & \#32, \#235, \#224 & \(80.377~\mathrm{GeV}\), \(91.18797809193725~\mathrm{GeV}\) \\
...
Charged leptons & current-family target-anchored witness; corpus-limited end-to-end no-go & no & none; \#201 closed no-go & \(m_e,m_\mu,m_\tau\) witness values only \\
...
Electroweak bosons & compare-only adapter & D10 electroweak branch plus frozen validation adapter & \(M_W=80.377~\mathrm{GeV}\), \(M_Z=91.18797809193725~\mathrm{GeV}\) & not a prediction theorem. The candidate \(P\) root, Ward-projected Thomson residual map, and RG/matching interval certificate remain work in progress \\
...
Hadrons & closed out-of-scope / computationally blocked & Ward-projected spectral-measure contract for an OPH hadron backend & no paper-derived hadron masses & no local, Chrome-worker, or surrogate backend is promotable. Issues \#153/\#157 are closed as out-of-scope, not as derivations; a promotable row requires a working OPH hadron backend, such as GLORB/Echosahedron, with the Ward-projected hadronic spectral measure and systematics \\
\bottomrule
\end{tabular}
\end{table}
...
\[
0.041163974743011720681199819215801821
\]
inverse-alpha units. At the CODATA-mapped pixel value
\[
P=1.630968209403959324879279847782648941\ldots
\]
the conditional endpoint package requires
\[
\Delta_{\mathrm{source}}(P)=0.041465835978928681137459869870016982\ldots.
\]
That scalar must come from a source-only hadronic spectral transport and scheme-remainder map,
not from an inserted comparison endpoint.
...
theorem branch. Hadrons have no emitted prediction because the required
...
Electroweak D10 branch & \(W\) boson & compare-only adapter & \(80.377~\mathrm{GeV}\) on the frozen \(W/Z\) validation surface & not a prediction theorem while the candidate \(P\) root, Thomson residual map, and RG/matching interval certificate are work in progress \\
...
reconstruction. On the central branch the matching gluing obstruction is the gluing-side
...
On the D10 branch, a promoted forward prediction must respect exactly that ordering: first certify
...
public prediction rows once the \(P\), Thomson, and RG/matching gates are closed.
...
The final bundle keeps it behind the open \(P\), Thomson, and RG/matching gates, so the
...
The candidate trunk itself emits
\[
\alpha_{\mathrm{cand}}^{-1}=136.994020662724205139718642793,
\]
...
it is explicitly outside the local prediction scope. The artifact is not a hadron
mass row but a Ward-projected spectral-measure contract for a backend. Closing the lane
...
semantics that are not present in the repository environment. This paper therefore treats
...
budget, production hadron masses are not planned deliverables of the particle pipeline, and
...
the realized Standard Model branch and the exact massless carrier sectors. The completion
...
descendants of \(P\) to physical charged data is closed as a corpus-limited no-go. Hadrons require a working OPH hadron backend,
so issues \#153/\#157 are closed out-of-scope rather than treated as paper predictions.
...
boundaries on the unfinished sectors. Where a row disagrees with measurement, or agrees
only on a target-anchored or selected surface, the paper treats that as diagnostic information:
endpoint and matching gaps for \(P\) and \(\alpha(0)\), compare-only status for \(W/Z\), corpus-limited no-go status for the charged determinant
landing, corpus-limited no-go status for direct-top codomain response, visible PMNS comparison tension for
