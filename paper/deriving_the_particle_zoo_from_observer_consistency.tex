...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a status-tiered forward calculation from one local screen scale. The current completion pipeline compresses the candidate \(P\)-closure branch into five executable layers--unification scale, heat-kernel pixel closure, declared running/matching, electroweak mixing, and structured Thomson transport--and then propagates that branch into the non-hadron particle rows. The resulting public surface is deliberately conservative. It emits theorem-grade structural massless carriers, a Higgs/top row on the declared D10/D11 surface, selected-class running quark rows, and a weighted-cycle neutrino absolute-mass branch. It also exposes compare-only \(W/Z\) and charged-lepton witnesses, a candidate but non-live \(P\) root, and no hadron predictions. The remaining open explanations are the Ward-projected Thomson endpoint and RG/matching interval certificate. The charged determinant landing, quark global-classification lane, and direct-top auxiliary codomain are closed as current-corpus no-go boundaries, not promoted derivations. The hadron backend tasks, issue \#153 and issue \#157, are closed as out-of-scope/computationally blocked, not solved.
\end{abstract}
...
inputs, it tries to propagate one dimensionless pixel ratio \(P\) through the spectrum. The current
pipeline does not claim that every such chain is closed. The structural massless carriers, the
Higgs/top row on the declared D10/D11 surface, the selected-class quark theorem, and the
weighted-cycle neutrino absolute-attachment branch are the positive non-hadron closures. The
\(P\)-closure root, the electroweak \(W/Z\) rows, charged leptons, and hadrons remain nonclosed or
out of scope in the precise senses recorded below. This paper develops the derivation chain from
the axioms and candidate \(P\) trunk to the reported particle outputs, with the closure boundary
stated by sector.
...
\item compact gauge reconstruction from the refinement-stable transportable edge-sector category once its coherent bosonic refinement ladder, symmetry, and finite-fiber premises are supplied;
...
explicit coherent refinement ladder, symmetry, and finite-fiber premises are supplied, organize into a compact gauge
...
The quantitative particle side adds one local closure variable together with one external
continuous input. The common pixel ratio \(P\), fixed on the synthesis paper's outer/inner closure
branch~\cite{ophsynthesis}, feeds the forward electroweak map and its descendants once it is
promoted from candidate metadata to a live prediction root. The current code path records the
candidate value
\[
P=1.63097210492078846050203640439,
\qquad
\alpha_{\mathrm{cand}}^{-1}=136.994020662724205139718642793,
\]
but marks it as non-promotable for live particle predictions until the Ward-projected Thomson
endpoint and RG/matching interval gates close. The
separate screen-capacity input \(N_{\mathrm{scr}}\) belongs to the cosmological-capacity branch and
...
ledger. Section~\ref{sec:particle-premise-ledger}
...
variable for the entire downstream spectrum. In the synthesis paper, OPH finds the fine-structure
constant by asking for the nonzero detuning of a holographic screen cell such that the cell's
outer geometric displacement from perfect self-similar equilibrium equals the electromagnetic
observation scale emitted by the universe living on that same screen.
The outer side of the closure is
\[
P=\varphi+\alpha_{\mathrm{in}}(P)\sqrt{\pi}.
\]
For comparison, the 2022 CODATA/NIST central value
\(\alpha^{-1}(0)=137.035999177\) gives the outer-readout value
\[
P=1.630968209403959\ldots
\]
through this outer formula. The current five-layer OPH trunk instead emits the candidate
\[
P=1.63097210492078846050203640439,
\qquad
\alpha_{\mathrm{cand}}^{-1}=136.994020662724205139718642793.
\]
The difference is not hidden as numerical noise. It is the visible effect of using the present
structured Thomson continuation before the zero-momentum Ward endpoint, hadronic spectral
measure, scheme/matching certificate, and interval fixed-point certificate are theorem-grade. The
repository records the candidate trunk, a numerical fixed-point witness, and the constructive
endpoint contracts, but the trunk may feed only audit and comparison surfaces for now.
...
Once a fixed point is certified, the particle question becomes whether all downstream readouts
can be written as
\[
X_j = G_j(P)
\]
...
\]
emitted from one declared input \(P\) on the printed running and matching package. The construction
...
The electroweak branch is the place where the particle derivation must show whether one common
input \(P\) emits the electroweak rows or merely hides many fitted inputs behind notation. In the
current final bundle, the published \(W/Z\) numbers are not promoted as theorem-grade predictions.
They are the frozen-target comparison adapter values
\[
M_W=80.377~\mathrm{GeV},
\qquad
M_Z=91.18797809193725~\mathrm{GeV}.
\]
The reason is mechanical: the same branch still depends on the non-live \(P\) root, the open
Ward-projected Thomson endpoint, and the open RG/matching interval certificate. The selected-carrier
chart and the freeze-once coherent repair surface remain useful validation surfaces, but the
closure matrix classifies the electroweak \(W/Z\) chain as compare-only until those gates close.

\subsection{What the electroweak branch fixes}

On the declared electroweak running and matching surface, the pixel variable \(P\) fixes the
source basis
...
whereas the electroweak quantitative-closure lane does not.
...
The particle derivation carries three different kinds of rows: rows that are
...
Read this section by claim tier. The same absolute residual can indicate a theorem-grade emitted
value, a selected-class exact row, a target-anchored witness, or a compare-only continuation
benchmark. The current completion pipeline emits an executable closure matrix. It says that not
all derivation chains are closed: the structural massless bosons, the Higgs/top declared surface,
the selected-class quarks, and the weighted-cycle neutrino absolute-attachment branch are the
closed or scoped non-hadron chains; the charged-lepton lane is scoped as a current-corpus no-go; and
the \(P\)-closure root plus electroweak \(W/Z\) chain remain nonclosed. Hadrons are closed out-of-scope for the current pipeline because the
required OPH hadron backend is computationally unavailable in the local environment.

\begin{table}[h]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.15\textwidth}@{}}
\toprule
Chain & Current status & Promotable & Open gates & Current output \\
\midrule
\(P\)-closure root & candidate root, not live & no & \#32, \#223, \#224 & \(P=1.63097210492078846050203640439\), \(\alpha^{-1}_{\mathrm{cand}}=136.994020662724205139718642793\) \\
Structural massless carriers & closed structural zero & yes & none & \(m_\gamma=m_g=m_{\mathrm{grav}}=0\) \\
Electroweak \(W/Z\) & compare-only frozen adapter, not a prediction theorem & no & \#32, \#223, \#224 & \(80.377~\mathrm{GeV}\), \(91.18797809193725~\mathrm{GeV}\) \\
Higgs/top declared surface & closed on declared D10/D11 surface; direct-top auxiliary codomain no-go & yes, within that surface & none & \(m_H=125.1995304097179~\mathrm{GeV}\), \(m_t=172.35235532883115~\mathrm{GeV}\) \\
Charged leptons & current-family target-anchored witness; current-corpus end-to-end no-go & no & none; \#201 closed no-go & \(m_e,m_\mu,m_\tau\) witness values only \\
Selected-class quarks & selected public class closed; global classification no-go & yes, selected-class only & none; \#199/\#207/\#212 closed no-go & exact running sextet on \(f_P\) \\
Neutrino absolute attachment & weighted-cycle absolute branch closed; comparison tension visible & yes, declared branch & none & \(0.017454720257976796,0.019481987935919015,0.05307522145074924~\mathrm{eV}\) \\
Hadrons & closed out-of-scope / computationally blocked & no & none; \#153/\#157 closed out-of-scope & no hadron predictions emitted \\
\bottomrule
\end{tabular}
\end{table}
...
Hadrons & closed out-of-scope / computationally blocked & Ward-projected spectral-measure contract for a future OPH hadron backend & no paper-derived hadron masses & no local, Chrome-worker, or surrogate backend is promotable. Issues \#153/\#157 are closed as out-of-scope, not as derivations; reopening requires a working OPH hadron backend, such as GLORB/Echosahedron, to emit the Ward-projected hadronic spectral measure and systematics \\
\bottomrule
\end{tabular}
\end{table}

Two points are worth stating explicitly. First, the theorem rows and the exact-hit rows are not
the same object. The theorem table carries selected-class quark closure and a current-corpus charged
...
zeros; the \(W\) boson and \(Z\) boson are currently compare-only frozen-adapter values; and the
...
\]
which matches the official PDG 2025 API running-quark target surface exactly, with the top coordinate taken from the PDG cross-section mass entry. The auxiliary direct-top row is a compare-only codomain; issue \#207 is closed as a current-corpus no-go. On the declared common-refinement transport-frame carrier, the restricted exact chain goes further and emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\).
and
\[
(m_1,m_2,m_3)=
...
Exact numerical agreement is not by itself a proof of a blind prediction. The current provenance
...
row does not match, or where it matches only on a constrained surface, the present explanations are
as follows. The \(P\)-closure and \(\alpha(0)\) displacement is expected until the Ward-projected
Thomson endpoint, hadronic spectral measure, and RG/matching interval certificate are populated.
The electroweak \(W/Z\) row is therefore compare-only. Charged leptons are closed as a current-corpus
no-go until a determinant trace-lift attachment from the D10 descendants of \(P\) to physical charged data is emitted. The auxiliary
...
extraction codomains; issue \#207 is closed as a current-corpus no-go until the direct-top response kernel is emitted. Neutrino absolute masses are
not directly measured, and PMNS-angle residuals are kept as visible comparison tension rather than
absorbed into the theorem branch. Hadrons have no emitted prediction because the required
Ward-projected hadronic spectral measure must come from a working OPH hadron backend.
...
Electroweak D10 branch & \(W\) boson & compare-only adapter & \(80.377~\mathrm{GeV}\) on the frozen \(W/Z\) validation surface & not a live prediction theorem until the candidate \(P\) root, Thomson endpoint, and RG/matching interval certificate close \\
...
Hadrons & proton & closed out-of-scope / computationally blocked & no emitted prediction & issues \#153/\#157 are closed out-of-scope, not solved; reopening requires a working OPH hadron backend, such as GLORB/Echosahedron, plus a Ward-projected spectral-measure export and production systematics; surrogate rows are non-promoting \\
Hadrons & neutron & closed out-of-scope / computationally blocked & no emitted prediction & same OPH-backend gate, with isospin-resolved hadron systematics deferred \\
Hadrons & neutral pion proxy & closed out-of-scope / computationally blocked & no emitted prediction & same OPH-backend gate; local stable-channel surrogate output is not a paper prediction \\
Hadrons & \(\rho(770)^0\) proxy & closed out-of-scope / computationally blocked & no emitted prediction & same OPH-backend gate, plus finite-volume resonance extraction after the backend exists \\
\end{longtable}
\normalsize
...
scope, the theorem-local equilibrium input used in the Jacobson step, and the coherent
transportable bosonic refinement-ladder clauses required by compact-gauge
reconstruction. On the central branch the matching gluing obstruction is already the gluing-side
form of that supplied transportability rather than a second theorem-side input. Any local-Gibbs
or collar-mixing language on that branch is therefore only fixed-cutoff recoverability/support
control; it is not what decides whether the
...
Higgs/top critical stage can be used as a closed source-only split quantitative theorem on the declared D10/D11 running, matching, and threshold surface. The quark,
...
group from the transportable refinement-directed edge-sector category under the compact paper's
explicit coherent bosonic refinement ladder, \(3+1\)D symmetry, and finite-fiber premises and only then applies MAR to
...
categorical premises. The second stage uses MAR and anomaly cancellation to fix the realized
...
On the D10 branch, a future forward prediction must respect exactly that ordering: first certify
the shared pixel input \(P\) on the declared D10 running/matching surface, thereby fixing the source basis
\[
...
and only then emit the electroweak transport family forward from that source basis. The current
completion pipeline has not promoted that full chain. Its \(W/Z\) rows are the frozen validation
adapter values, and their exact numerical agreement is recorded as comparison status rather than
as a closed D10 Phase-II prediction theorem.
...
The compare-only \(W/Z\) adapter rows in the current final bundle are
\[
m_W = 80.377~\mathrm{GeV},
\qquad
m_Z = 91.18797809193725~\mathrm{GeV}.
\]
These numbers sit on the frozen validation surface, not on a live target-free prediction theorem.
...
freeze-once coherent repair surface, and the target-free source-only theorem surface that would
supply public prediction rows only after the \(P\), Thomson, and RG/matching gates close.
...
The current final bundle keeps it behind the open \(P\), Thomson, and RG/matching gates, so the
...
\]
on the same source-locked family and without any separate comparison endpoint feeding that lane.
\end{corollary}

Using the source-locked D10 anchor \(a_0=128.30576920234813\), the displayed
Ward-transport comparison family has
\[
\delta_\alpha(0;m_Z^2;P)=0.06804238054863787,
\qquad
\frac{t_Q(0;P)}{t_Q(m_Z^2;P)}=0.936292433907271,
\]
and therefore the comparison endpoint
\[
\alpha_{\mathrm{Th,comp}}^{-1}=137.035999177.
\]
The repository's \texttt{code/P\_derivation} witness path records the fixed-point candidate without
a built-in external inverse-\(\alpha\) value. The displayed CODATA/NIST value enters only when
supplied explicitly as compare-only metadata, not as an input to the closure solve.
The current candidate trunk itself emits
\[
\alpha_{\mathrm{cand}}^{-1}=136.994020662724205139718642793,
\]
so the difference from the Thomson comparison endpoint remains a visible endpoint-and-matching
gap rather than a hidden fit.

\begin{remark}
The CODATA/NIST comparison is against the Thomson-limit inverse fine-structure constant. The
source anchor \(a_0=128.30576920234813\) is the electroweak-scale running-family value at
\(m_Z^2\), not the low-energy endpoint. The 2022 CODATA/NIST value used for comparison is
...
It is described by a source-only split law together with a Jacobian readout map on the declared D10/D11 running, matching, and threshold surface. The live D10 tuple
\[
...
direct-top PDG row is closed as a current-corpus codomain no-go; the auxiliary row remains compare-only.
...
source-only theorem surface that would supply promoted prediction rows, the selected-carrier chart
beneath it, and the freeze-once coherent repair surface used for the current compare-only \(W/Z\)
...
determinant character with the physical charged determinant line. The missing source theorem would be the
determinant trace-lift attachment \(3\mu(r)=S_M(r)\) on the charged determinant channel; issue \#201 is closed as a current-corpus no-go until such an uncentered trace lift is emitted. The neutrino lane closes on the weighted-cycle
...
auxiliary direct-top row is a compare-only extraction codomain; issue \#207 is closed as a current-corpus no-go. The Higgs/top branch also carries the same exact
...
\((1,1,1)\) direction leaves them unchanged. This is the exact reason the current corpus closes the
...
The hadron derivation is the most operationally demanding part of the whole particle program, and
it is explicitly outside the current local prediction scope. The current artifact is not a hadron
mass row but a Ward-projected spectral-measure contract for a future backend. Closing the lane
requires a working OPH hadron backend, such as GLORB/Echosahedron, that can emit real
Ward-projected hadronic spectral data with manifest provenance and production systematics. Local
surrogates, ordinary Chrome/Oracle workers, and bookkeeping scripts cannot promote this sector.
Issues \#153 and \#157 are therefore closed as out-of-scope/computationally blocked, not as solved
hadron derivations. Accordingly, the hadron rows are not derived outputs of this paper. The derivation nevertheless
keeps the mathematical execution bridge, deterministic runtime receipt, writeback/evaluation path,
and surrogate validation machinery as non-promoting scaffolding for the future backend.
...
semantics that are not present in the current repository environment. This paper therefore treats
GLORB/Echosahedron-class OPH hardware, or an equivalent working OPH hadron backend, as the
promotion gate. Until such a backend emits the Ward-projected spectral measure and its uncertainty
budget, production hadron masses are not planned deliverables of the current particle pipeline, and
the closed issue status of \#153/\#157 should be read only as a scope boundary.
...
excitation map, and CKM closure remain open, while the charged source landing from \(P\) to physical charged data is a current-corpus no-go and the neutrino
...
the realized Standard Model branch and the exact massless carrier sectors. The current completion
pipeline then closes or scopes four non-hadron chains: structural massless carriers, the Higgs/top
row on the declared D10/D11 surface, selected-class quarks on \(f_P\), and neutrino absolute
attachment on the weighted-cycle branch. It also refuses to promote four chains: the candidate
\(P\)-closure root, the electroweak \(W/Z\) row, the charged-lepton absolute-mass landing, and
hadrons. The \(W/Z\) row is a frozen compare-only adapter until the Ward-projected Thomson endpoint
and RG/matching certificate close. In the charged lane the theorem surface carries an exact
...
descendants of \(P\) to physical charged data is closed as a current-corpus no-go. Hadrons require a working OPH hadron backend,
so issues \#153/\#157 are closed out-of-scope rather than treated as current paper predictions.
...
nonperturbative hadron branch. The same accounting also sharpens the local unification picture:
...
boundaries on the unfinished sectors. Where a row currently disagrees with measurement, or agrees
only on a target-anchored or selected surface, the paper treats that as diagnostic information:
endpoint and matching gaps for \(P\) and \(\alpha(0)\), compare-only status for \(W/Z\), current-corpus no-go status for the charged determinant
landing, current-corpus no-go status for direct-top codomain response, visible PMNS comparison tension for
neutrinos, and missing OPH backend execution for hadrons.
