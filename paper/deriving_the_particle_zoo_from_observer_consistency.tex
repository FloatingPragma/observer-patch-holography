...
electromagnetic observation scale. Matching those readings defines a closure
equation. The source-audit witness, the empirical hadron-closure solution, and
the CODATA-conditioned comparison pixel remain distinct evaluations of it.
...
quantization, physical-pole, and phase receipts.  No strict source-only
\(W/Z/H\) complex-pole row is promoted.  The bosonic numerical surface reads
its target-free headline from the strict source-audit \(W/Z\) branch
\((80.330,91.119)\)~GeV and the double-criticality Higgs/top family
\((125.77,172.63)\)~GeV at a frozen boundary-scale candidate, with
\(m_H=125.72\)~GeV on the fit-free curve at the measured top; the
selected-carrier chart, the comparison-only value-law adapter, and the
target-anchored declared-surface fit stay as audit objects.
The quark reciprocal-ray candidate fails on dimensionless Yukawas evaluated in
one effective theory, scheme, and scale: at \(M_Z\),
\(\rho_u\rho_d=0.835323\), rather than one, and the best endpoint-granted
held-out error is \(21.56\%\). The generic physical interface has six scalar
coordinates. The archived \(0.2946\%\) and \(0.29436\%\) rows fit a
mixed-convention table and are retrospective diagnostics, not mass
postdictions. A finite charged-carrier model proves its stipulated record
schema nonempty, but hard-codes its response and does not select a physical
charged sector. A conditional nature/pole theorem transports that response
only after the physical Yukawa identity and Dyson singularity readout are
assumed; its zero-self-energy kernel is a free witness. First-principles hadron masses and source-only absolute
charged-lepton masses remain outside the emitted theorem surface; their
empirical closure rows are labeled separately.
\end{abstract}
...
\midrule
\(P\)-closure and fine structure & \(\alpha_{\mathrm{root}}^{-1}=136.9948351646\ldots\); \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\); empirical endpoint \(136.2636589431\in[136.1583832834,136.3690975240]\); measured \(137.035999177(21)\) & four distinct rows: source/root witness, mixed diagnostic, empirical hadron closure with anchor gap \([0.6485541111,0.8547920666]\), and measurement \\
...
That freeze-once law provides an exact coherent inverse-calibration surface. On that
target-frozen surface the reference \(W/Z\) pair is hit exactly, but only as compare-only
validation. It is excluded from every prediction ledger.
...
\end{aligned}
\]
with the public Thomson endpoint excluded upstream. These two named branches
...
selector.  The current repository evaluates the displayed formulas but does
not emit that certificate.
This gives
\[
...
it is a target-anchored fit and not a certified Higgs or top complex pole; the
...
source clock packet is not currently emitted.
\end{proposition}
...
that agree on every currently named object and differ in one unfixed parent
produce different unique gaps, so no unique \(\varepsilon_{\mathrm{Cs}}\) is a
theorem of the current source, and further evaluation of the existing
...
This is now an emission boundary rather than a missing generic implication.
...
(\texttt{wzh\_residual\_elimination\_boundary.json}).  Numerical \(W/Z\)
...
class) that the \(W/Z\) value law already needs; one certificate closes the
...
selection spread between the two displayed laws is below current experimental resolution in every
...
By the freedom theorem and the non-definability theorem above, the current corpus selects neither
...
\end{aligned}
\]
The ray coordinates are now closed symbolically:
\[
b_u^{\mathrm{ray}}
...
signs and orientation, and exclusion of competing assignments are hypotheses, not current OPH
theorems.  Thus this lemma closes the denominator arithmetic conditional on the physical channel
...
The numerical table above is not its corollary: the current evaluator's coordinates are
...
and exponentiation laws, the downstream map is deterministic.  Using the current D10
...
hypotheses contain the physical content still owed: the typed register construction, exhaustive
...
receipts are false.  The conclusion is scoped to the current natural direct carrier.  A future OPH
theorem may derive an additional complex lift, but choosing phases or paths because they reconstruct
the already proposed matrices would not be an independent simulator test.
...
\item the phenomenological charged-family phase choice \(\delta = 2/9\).
\end{enumerate}
These are continuation inputs, not consequences of the OPH axioms.  Indeed, for the general
Hermitian circulant \(C=aI+b e^{i\delta}R+\overline b e^{-i\delta}R^2\), the square-root-mass
Koide invariant is
\[
Q=\frac{1+2|b/a|^2}{3},
\]
so \(|b|/a=1/\sqrt2\) is exactly equivalent to imposing \(Q=2/3\).  Circulant symmetry does
not derive that balance.  More sharply, writing the singlet and charged Hilbert--Schmidt powers as
\(E_0=3a^2\) and \(E_c=6|b|^2\), the OPH relative-MaxEnt law gives
\[
\frac{E_c}{E_0}=\frac{m_c}{m_0}e^{-(S_c-S_0)}.
\]
The natural trace base has \(m_c/m_0=2\), so Koide balance needs both a source law
\(S_c-S_0=\ln2\) and a bridge identifying MaxEnt block probabilities with deterministic Yukawa
powers.  The current \(12/24\) register supplies no such charged-block attachment.
Likewise, the count identity
\((N_c+1)/(2N_cN_g)=2/9\) is not by itself a phase-transport or holonomy theorem.  The current
corpus supplies neither a charged-family connection that fixes this phase nor a source theorem
that attaches the ordered roots below to the physical charged-family lines.  Hypercharge acts on
the three generation copies as a common scalar phase, not as the cyclic family shift \(R\), so the
D9 \(\U(1)_Y\) connection does not fill that gap.
...
same formula on the current public-pixel probe
...
prediction because the balanced carrier and \(2/9\) phase were prior empirical art, the electron
...
\texttt{code/particles/runs/leptons/charged\_stage5\_frozen\_candidate.json}; any future change
...
NI6 is already the desired operator attachment squared, and the submitted RP4
premise already sets the Dyson singularity readout equal to the face response.
...
coordinate on the completed theory.  Three sharper boundaries replace the
earlier gap description.  First, the public-block amplitude \(1/\sqrt2\)
corresponds to the tracial weight \(t=1/2\) on
\(\mathbb C\oplus M_2(\mathbb C)\); traciality and \(C_3\)-invariance admit
the full one-parameter family, and the Hilbert-space normalized trace gives
\(t=1/3\) with amplitude \(1/2\).  Second, the arithmetic identity
\((N_c+1)/(2N_cN_g)=2/9\) does not select the oriented phase; a holonomy
reading requires link variables, a loop, and a nonlocal readout map.  Third,
