...
Hadrons & backend-dependent strong-binding descent & OPH hadron backend \\
\bottomrule
\end{tabular}
\end{table}
...
scheme-remainder map. The source transport excludes an inserted comparison
endpoint. The electroweak status ledger records the residual map and the
...
\]
and the fixed-point display value
\[
...
a built-in reference inverse-\(\alpha\) value. The public endpoint value is excluded as a closure-solve
input.
...
\begin{remark}
The public experimental reference is the Thomson-limit inverse fine-structure constant. The
source anchor \(a_0(P)=128.308268045165213892552005990181778935450\ldots\) is the electroweak-scale running-family value at
\(m_Z^2\), not the low-energy endpoint. The 2022 CODATA/NIST reference value is
...
The selected-class quark theorem closes the running sextet and explicit forward Yukawas on the
public quark frame class \(f_P\). That is not yet a theorem-grade derivation of the physical
strong-CP quantity. The current corpus does not derive the bare QCD angle
\(\theta_{\mathrm{QCD}}\), does not derive the physical anomaly-invariant combination usually
denoted \(\bar\theta\), and therefore does not prove \(\bar\theta=0\).

This boundary is narrower than the open flavor-classification and CKM fronts. The quark branch
already emits exact selected-class running rows and explicit forward Yukawas. What is still
missing is the phase-side bridge: a theorem-grade descent from the exact quark/Yukawa data to the
determinant-line phase contribution together with a theorem fixing the topological-angle
contribution on the realized branch, so that the physical invariant \(\bar\theta\) itself is
emitted.

So the public status should be read literally. Strong CP remains open on the present corpus, and
claims that OPH already forces \(\theta_{\mathrm{QCD}}=0\) or \(\bar\theta=0\) would go beyond
the emitted theorem chain.
...
pair, charged-lepton witnesses, direct-top auxiliary codomain, neutrino comparison tension, and
hadron backend.
