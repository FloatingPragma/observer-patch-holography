...
closed or scoped non-hadron chains; the \(P\)-closure root, electroweak \(W/Z\) chain, charged
leptons, and hadrons remain nonclosed. Hadrons are hardware-gated and outside the current local
particle scope.
...
Hadrons & hardware-gated out of scope & no & \#153, \#157 & no hadron predictions emitted \\
\bottomrule
\end{tabular}
\end{table}
...
Hadrons & hardware-gated out of scope & Ward-projected spectral-measure contract for a future OPH hadron backend & no paper-derived hadron masses & no local, Chrome-worker, or surrogate backend is promotable. Closing this lane requires a working OPH hadron backend, such as GLORB/Echosahedron, to emit the Ward-projected hadronic spectral measure and systematics \\
\bottomrule
\end{tabular}
\end{table}
...
Hadrons & proton & hardware-gated out of scope & no emitted prediction & requires a working OPH hadron backend, such as GLORB/Echosahedron, plus a Ward-projected spectral-measure export and production systematics; surrogate rows are non-promoting \\
Hadrons & neutron & hardware-gated out of scope & no emitted prediction & same OPH-backend gate, with isospin-resolved hadron systematics deferred \\
Hadrons & neutral pion proxy & hardware-gated out of scope & no emitted prediction & same OPH-backend gate; local stable-channel surrogate output is not a paper prediction \\
Hadrons & \(\rho(770)^0\) proxy & hardware-gated out of scope & no emitted prediction & same OPH-backend gate, plus finite-volume resonance extraction after the backend exists \\
\end{longtable}
\normalsize
...
Accordingly, the hadron rows are not derived outputs of this paper. The derivation nevertheless
...
budget, production hadron masses are not planned deliverables of the current particle pipeline.
...
and are not current paper predictions.
