...
\begin{abstract}
Observer-Patch Holography asks how much of the particle spectrum can be organized by one shared local input instead of a long fitted parameter list. Building on the OPH recovery of relativity and Standard Model structure, this paper derives the massless carriers, the \(W\) and \(Z\) masses, the Thomson-limit fine-structure constant, a precise Higgs mass, a precise running quark spectrum with explicit Yukawas, and a neutrino family in the observed hierarchy and mixing regime. Charged-lepton absolute masses remain open, and hadrons remain outside the closed package. The quantitative branch is built from one dimensionless pixel input \(P \equiv a_{\mathrm{cell}}/\ell_P^2 = 1.63094\). A self-similar balance theorem identifies the golden ratio \(\varphi=(1+\sqrt5)/2 \approx 1.61803\) as the exact symmetric equilibrium point of the total/bulk/edge hierarchy. A universe with durable records, structure, and dynamics cannot sit exactly at that point. The realized value of \(P\) is therefore read as a small equilibrium-breaking detuning away from it. Quantitatively, \(P\) sits only about \(0.8\%\) above \(\varphi\). Its exact numerical value comes from the electroweak calibration surface. No first-principles derivation of the exact number is claimed. The paper keeps those calibration statements separate from the structural core.
\end{abstract}
...
The same calibration lane has one exact golden-ratio benchmark. If one writes
\[
...
\]
Equivalently the order parameter \(A_\varphi(x)=x-1-\frac1x\) vanishes there. The exact numerical
value of \(P\) still comes from the D10 calibration surface. The point of the equilibrium theorem
is that the proximity to \(\varphi\) has a structural reason. Exact balance is too symmetric to
support durable records, structure, and dynamics, so the calibration solve fixes a small
