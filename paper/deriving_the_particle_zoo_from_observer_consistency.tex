...
as obstruction or comparison surfaces, while target-anchored quark mass textures and compare-only
...
Higgs/top relation & \(m_H=125.1995304097179~\mathrm{GeV}\), with a companion top coordinate on the same split surface; conditional envelope \(m_H\in[125.183,125.232]\), \(m_t\in[172.278,172.352]\), \(m_W\in[80.3692,80.3774]\), \(m_Z\in[91.1880,91.1983]~\mathrm{GeV}\) over the repair-selection and pixel freedoms & conditional declared split surface; repair coefficients \(c=\sqrt{N_c}/2\), \(d=N_c/2\) confirmed on \(m_W,m_Z\) with the channel identification open; top coordinate is not a separate public top-mass prediction row \\
Charged leptons & target-anchored witness values withheld & empirically anchored same-family audit witness \\
Selected-frame quarks & target-anchored mixed-convention mass coordinates withheld; dimensionful mass textures retained for audit & two-modulus source obstruction plus selected-fiber representative independence \\
...
Quark family & source-spread non-identifiability theorem + target-anchored audit surfaces & ordered profile rays \(\rightarrow\) free \((\mathbb R_{>0})^2\) spread fiber\newline \(\rightarrow\) selected-fiber representative independence\newline \(\rightarrow\) conditional affine mass readout & no public numeric quark row; target-anchored mixed-convention coordinates and dimensionful mass textures retained only as audit data & The source equations do not select either positive spread modulus.\newline Light \(u,d,s\) comparison coordinates use \(\overline{\mathrm{MS}}\) at \(2\,\mathrm{GeV}\); \(c,b\) use \(\overline{\mathrm{MS}}\) at self-scale; the top pole extraction coordinate is separate.\newline The GeV-valued matrices are not physical dimensionless Yukawa matrices.\newline The auxiliary direct-top row, the global classification of public quark frames, and strong CP are separate corpus boundaries \\
...
Quark source-spread boundary & numeric quark rows withheld; target-anchored mixed-convention mass coordinates and dimensionful mass textures retained only for audit & source shape laws \(\rightarrow\) free \((\mathbb R_{>0})^2\) spread fiber \(\rightarrow\) selected-fiber representative independence \(\rightarrow\) conditional affine mass readout & theorem-grade non-identifiability on the present source corpus: neither positive spread modulus is selected. The light, heavy, and top comparison rows do not form one running-mass sextet, and the GeV-valued matrices are not physical dimensionless Yukawa matrices. Global public-frame classification, the auxiliary direct-top row, and strong CP remain separate boundaries \\
...
The promoted D10 tuple sits inside a one-parameter freedom that the emitted corpus does not, by
itself, close. Beneath the target-free repair the coherent runtime-forward law is
\[
\tau_{2,\mathrm{tree}}^{\mathrm{exact}}=-c\,\eta_{\mathrm{source}}^2,
\qquad
\delta n_{\mathrm{tree}}^{\mathrm{exact}}=d\,(1-\beta_{EW})\,\eta_{\mathrm{source}}^2,
\]
and for arbitrary real \((c,d)\) this defines a consistent D10 repair. The charged leg is the
...
The compact-gauge reconstruction fixes both the color count and the normalization that closes this
freedom. Doplicher--Roberts/Tannaka reconstruction returns the color triplet sector with statistical
dimension \(d(\rho_3)=N_c=3\), and its conjugate intertwiners satisfy \(R^*R=d(\rho_3)=N_c\), so a
single intertwiner isometry carries norm \(\sqrt{N_c}\) while a closed color loop carries the full
...
large-\(N_c\) scaling is the decay-constant scaling \(\sqrt{N_c}\) of one intertwiner isometry. The
neutral leg is the unbroken hypercharge screening channel, a vacuum-polarization loop that closes
the color line and carries \(N_c\). This assigns
\[
c=\frac{\sqrt{N_c}}{2}=\frac{\sqrt3}{2},
\qquad
d=\frac{N_c}{2}=\frac32,
\]
with the shared factor \(1/2\) the common half-weight current normalization that cancels in every
relative statement.

The assignment is parameter-free once \(N_c=3\) is carried from the same reconstruction, so it is a
falsifiable prediction on the cleanest tree observables. Profiling the charged coefficient against
the tree \(W\) mass, which uses neither the D11 Jacobian core nor the running scheme, selects
\(c=0.8670\) against the predicted \(\sqrt3/2=0.8660\), agreement to about one part in a thousand.
...
\(N_c/2\) and \(N_c\) and the running-tree companion value \(1/(4\beta_{EW})=0.464\) all lie
outside. A loop-symmetric color law that carries \(N_c\) on both legs is therefore excluded on the
tree masses, and the running-tree companion is excluded on the same rows even though the softer
Higgs and top observables tolerate it.

The status is one sharply stated object short of a source-only theorem. The color amplitude/loop
assignment is a parameter-free prediction that the two cleanest observables confirm and every
competitor fails, together with a large-\(N_c\) mechanism for the \(\sqrt{N_c}\) versus \(N_c\)
split. The confirmation uses the measured \(W\) and \(Z\) masses, and the reconstruction fixes
\(N_c\) and the intertwiner norms without emitting the explicit collar-transport operators of the
two channels. Promoting the physical channel identification to a proof requires those operators and
a check that the charged channel factors through a single intertwiner while the neutral channel
closes a loop. Until they are emitted, the assignment is a data-confirmed parameter-free prediction
rather than a promoted source-only row.

\subsection{Conditional electroweak mass envelope}\label{ew-conditional-envelope}

With the repair coefficients read as above and the pixel carried at its endpoint value together
with the empirical-closure interval, the electroweak masses form a conditional envelope over the
repair-selection freedom and the pixel freedom. The envelope is compare-only; nothing in it is
promoted while the channel identification and the fine-structure anchor bridge stay open.
\[
\begin{aligned}
...
selection spread between the two repair laws is below current experimental resolution in every
observable, at roughly eight \(\mathrm{MeV}\) in \(m_W\), thirty \(\mathrm{keV}\) in \(m_Z\), sixteen
\(\mathrm{MeV}\) in \(m_H\), and thirty \(\mathrm{MeV}\) in \(m_t\), so the conditional envelope is
robust against which repair law is eventually promoted.

\subsection{Residual attribution across the electroweak rows}\label{ew-residual-attribution}

The four electroweak rows sit at \(0.31\), \(0.70\), \(0.36\), and \(2.63\) standard deviations
...
The \(m_Z\) row is the sharpest case, and its \(2.63\)-standard-deviation excess admits a single
mechanical explanation. The excess is a pixel-channel effect. The empirical-closure pixel inherits
...
into the running couplings and re-evaluates the closed \(W/Z\) forward map. That branch shifts
...
equivalently \(-0.05~\mathrm{MeV}\) in \(m_Z\). The entire \(m_Z\) excess is the frozen-anchor
...
The consequence for the electroweak surface is that a closed fine-structure anchor bridge collapses
the endpoint pixel interval onto the calibration pixel, removes the \(m_Z\) excess, and leaves the
repair-selection width as the sole electroweak boundary. The anchor bridge is the object developed
...
cannot emit the ordered ratio constant, the mean-law coordinate, or the spans. Second, at fixed
shared scale \(g_{\mathrm{ch}}\) the whole quark sector factors through exactly the invariant
triple
\[
...
masses closed-form in the triple, and with the explicit left inverse
...
closes at machine precision. By the freedom theorem and the non-definability theorem above, no
component of the triple is determined by the current corpus. The open kernel program is therefore
exactly the source derivation of one point of \((\mathbb R_{>0})^3\) with clean ancestry, and the
executable acceptance harness scores any candidate point against fail-closed gates
...
not a set of public numeric rows.
...
on the declared D10/D11 surface, and the two-modulus quark source-spread obstruction on \(f_P\). The neutrino lane records an
