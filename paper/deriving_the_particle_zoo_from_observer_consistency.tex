...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a status-tiered forward calculation from one local screen scale. The current completion pipeline compresses the candidate \(P\)-closure branch into five executable layers--unification scale, heat-kernel pixel closure, declared running/matching, electroweak mixing, and structured Thomson transport--and then propagates that branch into the non-hadron particle rows. The resulting public surface is deliberately conservative. It emits theorem-grade structural massless carriers, a Higgs/top row on the declared D10/D11 surface, selected-class running quark rows, and a weighted-cycle neutrino absolute-mass branch. It also exposes compare-only \(W/Z\) and charged-lepton witnesses, a candidate but non-live \(P\) root, and no hadron predictions. The open explanations are part of the result: the Ward-projected Thomson endpoint, RG/matching interval certificate, charged determinant landing, and direct-top codomain bridge remain unclosed. The hadron backend tasks, issue \#153 and issue \#157, are closed as out-of-scope/computationally blocked, not solved.
\end{abstract}
...
closed or scoped non-hadron chains; the \(P\)-closure root, electroweak \(W/Z\) chain, and charged
leptons remain nonclosed. Hadrons are closed out-of-scope for the current pipeline because the
...
Higgs/top declared surface & closed on declared D10/D11 surface & yes, within that surface & \#207 for direct-top codomain bridge & \(m_H=125.1995304097179~\mathrm{GeV}\), \(m_t=172.35235532883115~\mathrm{GeV}\) \\
Charged leptons & current-family target-anchored witness only & no & \#201 & \(m_e,m_\mu,m_\tau\) witness values only \\
Selected-class quarks & selected public class closed; global classification open & yes, selected-class only & \#199, \#207, \#212 & exact running sextet on \(f_P\) \\
...
Hadrons & closed out-of-scope / computationally blocked & no & none; \#153/\#157 closed not planned & no hadron predictions emitted \\
\bottomrule
\end{tabular}
\end{table}
...
\]
which matches the official PDG 2025 API running-quark target surface exactly, with the top coordinate taken from the PDG cross-section mass entry. The bridge to the auxiliary direct-top row is open and is tracked in \href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}. On the declared common-refinement transport-frame carrier, the restricted exact chain goes further and emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\).
and
\[
(m_1,m_2,m_3)=
...
The electroweak \(W/Z\) row is therefore compare-only. Charged leptons need the determinant
trace-lift attachment from the D10 descendants of \(P\) to physical charged data. The auxiliary
...
extraction codomains until the direct-top response kernel is emitted. Neutrino absolute masses are
...
Top quark & Higgs/top critical stage & quantitative theorem on declared surface; direct-top bridge open & \(172.35235532883115~\mathrm{GeV}\) \\
\bottomrule
\end{tabular}
\end{table}
...
determinant character with the physical charged determinant line. The charged source theorem is the
determinant trace-lift attachment \(3\mu(r)=S_M(r)\) on the charged determinant channel. The neutrino lane closes on the weighted-cycle
...
bridge to the auxiliary direct-top row is open and is tracked in
...
The work in progress component is the map from that intrinsic base to the named low-energy fermion
...
excitation map, CKM closure, and the charged source landing from \(P\) to physical charged data are open, while the neutrino
...
descendants of \(P\) to physical charged data is open. Hadrons require a working OPH hadron backend,
...
endpoint and matching gaps for \(P\) and \(\alpha(0)\), compare-only status for \(W/Z\), determinant
landing for charged leptons, codomain response for direct top, visible PMNS comparison tension for
