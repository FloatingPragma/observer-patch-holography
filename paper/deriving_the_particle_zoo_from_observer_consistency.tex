...
\]
and keeps it outside live particle predictions while the Ward-projected Thomson
residual map and the open RG/matching/threshold/scheme packet are work in
progress. The
...
measure, and the RG/matching/threshold/scheme packet become theorem-grade. On the current runtime
surface that packet means scheme lock, threshold map, beta provenance, and interval composition. The
...
\]
The reason is mechanical: the same branch depends on the uncertified \(P\)
root, the Ward-projected Thomson residual map, and the open
RG/matching/threshold/scheme packet. The selected-carrier
...
whereas the electroweak quantitative-closure lane already has an explicit declared-surface contract and explicit open promotion gates.
...
Electroweak bosons & compare-only adapter & D10 electroweak branch plus frozen validation adapter & \(M_W=80.377~\mathrm{GeV}\), \(M_Z=91.18797809193725~\mathrm{GeV}\) & not a live prediction theorem. The candidate \(P\) root, Ward-projected Thomson residual map, and the open RG/matching/threshold/scheme packet remain open \\
...
not from an inserted comparison endpoint. The electroweak promotion gate stays
open until that source-only residual map and the open
RG/matching/threshold/scheme packet are closed.
...
Electroweak D10 branch & \(W\) boson & compare-only adapter & \(80.377~\mathrm{GeV}\) on the frozen \(W/Z\) validation surface & not a live prediction theorem while the candidate \(P\) root, Thomson residual map, and the open RG/matching/threshold/scheme packet remain open \\
...
The final bundle keeps it behind the open \(P\), Thomson, and RG/matching/threshold/scheme gates, so the
...
and the RG/matching/threshold/scheme certificate packet close. In the charged lane the theorem surface carries an exact
