...
\author{B.\ M\"uller \and Alexander Osika \and Kai Xue \and Mario Poneder \and Maarten Antonie Visser}
...
\begin{abstract}
This paper asks how much of the particle table can be read from one OPH screen cell. The cell has an outside area and an inside electromagnetic observation scale. Matching those two readings gives the pixel fixed point
\[
P_\star=\varphi+\frac{\sqrt{\pi}}{A_T(P_\star)}.
\]
The main achievements are the charge-quantized particle skeleton, exact structural zero masses for photon/gluon/graviton carriers, the source-side fine-structure calculation, the Maxwell-normalized electroweak transport surface, the Higgs/top split theorem on its declared quantitative branch, selected-class quark rows, and a weighted-cycle neutrino branch. Hadrons and charged-lepton absolute masses are work in progress. Compare-only validation rows are marked as comparison rows.
\end{abstract}
...
\end{aligned}
\]
with the detailed endpoint table in Section~\ref{sec:particle-input-ledger}. The separate
...
The large cosmic horizon ratio and the electroweak hierarchy belong to different OPH branches.
...
A sharper continuation would identify the D10 transmutation exponent with a joint local/global
closure involving the cosmic capacity branch. The proposed relation is
\[
...
the global screen-capacity fixed point. It also separates the two hierarchy scales: the full
\(24\)-round contraction controls the cosmic length ratio, while the local weak hierarchy samples
the same contraction through a \(4P_\star\) electroweak projection.
...
meaning \(G_N(1)=\rho_\star\) exactly. On the declared resonance branch the cycle decomposes
into \(24\) homogeneous ticks, \(G_N=g_N^{24}\) with \(g_N\) the positive root. Then
\[
G_N(\rho)
=
...
\(|g_\star'|=(N_{\mathrm{CRC}}/\pi)^{-1/(2m)}\); the declared \(m=24\) reproduces the stated
exponent.
\end{lemma}
...
scale-covariant extension of \(N=\pi(r/\ell)^2\), gives a declared identification. The corpus
...
Lemma~\ref{lem:global-repair-tick} closes the global tick side on the declared \(24\)-round
branch. The proof derives the closure transport at the interface level of \(F\) under the declared
area-law counting model of \(\operatorname{Cap}_{\mathrm{read}}\). It takes the round
count as declared branch architecture.
...
\max\{a+b/r,\ d+rc\}<1
\]
in the weighted sup metric. Without that bound, the coupled branch is residual freedom rather
than a uniqueness theorem.
\end{proof}
...
Higgs/top, \(G\), Planck-area, and \(\Lambda\) data are excluded inputs. The full local/global
resonance relation remains a continuation target until the finite readback-resolution
verification, the representation-to-spectrum derivation of the \(24\)-round count, and the
electroweak tick-projection lemma are supplied. The promoted hierarchy row does not use it as an
input.
...
list of unrelated particle inputs. The status table records scope boundaries for the weak validation
