...
\begin{abstract}
Observer-Patch Holography (OPH) asks how much of the particle spectrum follows from one shared local input rather than from a long fitted parameter list. Building on the OPH recovery of relativity and Standard Model structure, this paper develops the particle branch from the dimensionless pixel constant \(P \equiv a_{\mathrm{cell}}/\ell_P^2\). It fixes the structural massless carriers, closes a target-free \(W/Z\) lane, reads the low-energy fine-structure constant as the Thomson endpoint of the Ward-projected electromagnetic transport family with \(\alpha^{-1}(0)=137.035999177\), carries an exact source-only Higgs/top split theorem on the declared D10/D11 running, matching, and threshold surface, closes the exact PDG 2025 API running-quark sextet together with explicit exact forward Yukawas on the selected public quark frame class \(f_P\), and gives a theorem-grade weighted-cycle neutrino family in the observed hierarchy and mixing regime together with a theorem-grade physical Majorana phase pair on the shared-basis weighted-cycle transport branch. The exact non-hadron surface includes exact structural zeros, an exact frozen \(W/Z\) pair, the exact D11 Higgs/top split theorem together with its compare-only inverse Higgs/top slice, exact same-family charged witnesses, the conditional charged determinant-line lift, the downstream algebraic charged-mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\), the selected-class exact quark theorem together with its supporting exact surfaces, and exact neutrino-side readout objects. The open charged source theorem is the landing from the D10 calibration descendants of \(P\) to theorem-grade physical charged data or the charged determinant line. A global classification of all quark frame classes, flavor-labeled neutrinos, and hadrons remain explicit boundary objects.
\end{abstract}
...
branch, to the emitted ray \(\mathcal R_{D12}^{ud}\). The D11 split theorem also realizes the same exact
top value on its declared calibration surface. On the mixing side the historical same-label left-handed orbit closes
...
Taken together with the exact \(W/Z\) frozen repair pair, the exact D11 Higgs/top source-only split theorem together with its compare-only exact inverse sidecar, the exact charged/quark same-family witnesses, and the emitted weighted-cycle neutrino bridge-rigid absolute family, the exact non-hadron output lane is complete, on a mixed theorem/compare-only/same-family surface rather than one uniform theorem tier.
...
Read this section by claim tier. The same absolute residual can indicate either a theorem-grade emitted value, an exact sidecar, or a compare-only continuation benchmark. On this table, the structural zeros are exact on their declared surfaces; the electroweak lane carries forward-emitted public rows plus an exact frozen sidecar; the D11 Higgs/top lane is theorem-grade on its declared split surface and keeps only the inverse Jacobian hit as compare-only; the charged and quark lanes each carry stronger exact sidecars than the theorem lane beneath them; and the weighted-cycle neutrino lane carries the bridge-rigidity invariant \(C_\nu\) above the emitted proxy \(P_\nu\), with \(B_\nu\) retained only as the induced paper-facing amplitude, while the older exact adapter remains diagnostic-only.
...
\]
which matches the official PDG 2025 API running-quark target surface exactly, with the top coordinate taken from the PDG cross-section mass entry. The bridge to the auxiliary direct-top row remains open and is tracked in \href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}. On the declared current-family common-refinement transport-frame carrier, the restricted exact chain goes further and emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\).
and
\[
(m_1,m_2,m_3)=
...
Higgs/top critical stage & Higgs boson & calibration theorem & \(125.1995304097179~\mathrm{GeV}\) on the declared D10/D11 running, matching, and threshold surface & exact source-only Higgs/top split theorem; compare-only inverse Higgs/top slice remains on the same Jacobian surface \\
Quark family & top quark & selected-class theorem & \(172.35235532883115~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & exact running-mass theorem row on \(f_P\); the D11 lane carries the same exact top value on its declared split calibration surface, while the repo-wide exact public top row is emitted by the selected-class quark theorem \\
...
Higgs/top critical stage can be used as a closed source-only split calibration theorem on the declared D10/D11 running, matching, and threshold surface. The quark,
...
\]
remains on disk only as the fixed-ray companion branch with \(\pi_y=\pi_\lambda=\sigma_{D11,\mathrm{HT}}\).
...
Top quark & Higgs/top critical stage & calibration theorem; direct-top bridge open & \(172.3523553288312~\mathrm{GeV}\) \\
\bottomrule
\end{tabular}
\end{table}
...
bridge to the auxiliary direct-top row remains open and is tracked in
\href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}. The Higgs/top branch also carries the same exact
