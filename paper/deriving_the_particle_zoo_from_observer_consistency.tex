...
\author{B.\ M\"uller \and Kai Xue \and Mario Poneder}
...
\begin{abstract}
Observer-Patch Holography (OPH) asks how much of the particle spectrum follows from one shared local input rather than from a long fitted parameter list. Building on the OPH recovery of relativity and Standard Model structure, this paper develops the particle branch from the dimensionless pixel constant \(P \equiv a_{\mathrm{cell}}/\ell_P^2\). It fixes the structural massless carriers, closes a target-free \(W/Z\) lane, reads the low-energy fine-structure constant as the Thomson endpoint of the Ward-projected electromagnetic transport family with \(\alpha^{-1}(0)=137.035999177\), carries an exact Higgs calibration theorem on the declared D10/D11 running, matching, and threshold surface together with a companion D11 top-side calibration output, closes the exact PDG 2025 API running-quark sextet together with explicit exact forward Yukawas on the selected public quark frame class \(f_P\), and gives a theorem-grade weighted-cycle neutrino family in the observed hierarchy and mixing regime together with a theorem-grade physical Majorana phase pair on the shared-basis weighted-cycle transport branch. The exact non-hadron surface includes exact structural zeros, an exact frozen \(W/Z\) pair, the exact Higgs calibration theorem together with its companion D11 top-side output and compare-only inverse Higgs/top slice, exact same-family charged witnesses, the conditional charged determinant-line lift, the downstream algebraic charged-mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\), the selected-class exact quark theorem together with its supporting exact surfaces, and exact neutrino-side readout objects. The open charged source theorem is the landing from the D10 calibration descendants of \(P\) to theorem-grade physical charged data or the charged determinant line. A global classification of all quark frame classes, flavor-labeled neutrinos, and hadrons remain explicit boundary objects.
\end{abstract}
...
the selected-class exact quark theorem, and the weighted-cycle neutrino theorem branch with its shared-basis physical Majorana pair are emitted, while charged leptons, flavor-labeled neutrinos, and
...
Quarks are the first matter family with a selected-class exact public theorem. The quark derivation
takes the shared flavor data, performs the mean split and sector descent, builds forward Yukawa
surfaces, and closes the exact PDG 2025 API running-quark sextet together with explicit exact forward Yukawas on the selected public quark
frame class \(f_P\). The mass-side route compresses transported same-label data to
\((\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})\) and, on the compact same-family
branch, to the emitted ray \(\mathcal R_{D12}^{ud}\). A companion D11 top-side output also remains visible on the
Higgs/top critical stage. On the mixing side the historical same-label left-handed orbit closes
negatively at the singleton \(\sigma_{\mathrm{ref}}\), but the public theorem is stronger: the
selected public quark frame class chosen by \(P\) carries the exact physical sigma datum, the
affine mean law emits \((g_u,g_d)\) algebraically, the ordered three-point readout emits the exact
running sextet, and the exact forward construction emits explicit exact forward Yukawas
\(Y_u\) and \(Y_d\). The quark lane therefore splits into four linked surfaces: the exact
same-family witness on the selected D12 sheet, a restricted common-refinement transport-frame exact
chain that carries the same sextet together with explicit forward Yukawas \(Y_u\) and \(Y_d\) on
its declared carrier, the target-free mass bridge \ophid{quark_d12_t1_value_law} on the emitted
D12 mass ray, and the selected-class public theorem on \(f_P\). This is selected-class closure
...
\frac{(3-x_2)\sigma+\eta}{6}.
\]
So the centered charged hierarchy closes once a charged source pair is emitted. The public mass rows sit on a different surface.

The centered same-carrier readback surface is common-shift invariant, so centered data do not determine an absolute charged scale. The charged theorem boundary therefore splits into a closed conditional lift, a closed downstream readout, and one open source theorem. On theorem-grade physical charged data \(Y_e\) and theorem-grade centered charged operator \(\widehat C_e\), the determinant line fixes the physical affine scalar
\[
\mu_{\mathrm{phys}}(Y_e)=\tfrac13\log\det Y_e,
\]
and therefore the canonical uncentered lift
\[
...
\]
Once theorem-grade \(A_{\mathrm{ch}}(P)\) exists on theorem-grade physical charged data, the downstream absolute readout is algebraic:
\[
g_e(P)=e^{A_{\mathrm{ch}}(P)},
\qquad
\Delta_e^{\mathrm{abs}}(P)=\log g_{\mathrm{ch}}^{\mathrm{shared}}-A_{\mathrm{ch}}(P),
\qquad
m_i(P)=\exp\!\bigl(A_{\mathrm{ch}}(P)+\ell_{i,\mathrm{centered}}(P)\bigr).
\]
So no extra theorem sits between a theorem-grade affine anchor and the charged mass triple. The open source-facing theorem is the landing from the D10 calibration descendant package \(B_{D10}(P)\) to theorem-grade physical charged data \(Y_e(P)\), or equivalently to the charged determinant-line section \(s_{\det}(P)\). That landing would force
\[
A_{\mathrm{ch}}(P)=\tfrac13\log\det Y_e(P)=\tfrac13 s_{\det}(P).
\]
Without that landing, no public charged masses, \(g_e\), or \(\Delta_e^{\mathrm{abs}}\) are emitted from \(P\). The public claim is exact same-carrier centered readback together with a closed common-shift no-go, the conditional determinant-line lift, and the downstream algebraic readout theorem. Any balanced-circulant / \(Q=2/3\) / \(\delta=2/9\) branch is compare-only continuation and not an emitted charged-lepton mass theorem.
...
\end{aligned}
\]
The exact two-parameter positive-segment adapter, the bridge corridor, and the reduced-correction candidate audit remain on disk only as diagnostic surfaces and do not feed back into theorem state. Charged leptons and neutrinos are split for different reasons. Charged leptons carry exact same-carrier readback and a closed common-shift no-go, together with a conditional determinant-line lift on theorem-grade physical charged data and an algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\); the open source theorem is the landing from the D10 calibration descendants of \(P\) to theorem-grade physical charged data or the charged determinant line. The weighted-cycle neutrino branch carries the emitted bridge-rigid absolute family together with the theorem-grade physical Majorana pair on its shared-basis transport branch.
Taken together with the exact \(W/Z\) frozen repair pair, the exact Higgs calibration theorem together with its companion D11 top-side calibration output and compare-only exact inverse sidecar, the exact charged/quark same-family witnesses, and the emitted weighted-cycle neutrino bridge-rigid absolute family, the exact non-hadron output lane is complete, on a mixed theorem/compare-only/same-family surface rather than one uniform theorem tier.
\paragraph{Claim-tier boundary.}
The Stage-5 summary is this: quarks close on the selected public quark frame class \(f_P\); charged leptons have an exact same-carrier readback theorem, a conditional determinant-line lift on theorem-grade physical charged data, and an algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\), with one open source theorem from \(P\) to physical charged data; neutrinos carry an emitted weighted-cycle bridge-rigid absolute family above the scale-free branch together with the theorem-grade physical Majorana pair on the shared-basis transport branch. None of those later-family statements belongs on the recovered-core surface fixed earlier by the gauge and relativity chain.
\subsection{Structural results and open closures}\label{step5-structural-results-and-open-closures}

The move from overlap structure to compact gauge reconstruction and the realized Standard Model branch belongs to the structural core. The move from the realized branch to structural carriers and to the electroweak/Higgs-top bosonic outputs is reflected in the reported numerical rows. The move from the shared flavor dictionary to full family closure is split by sector.

Quarks close on the selected public quark frame class \(f_P\). The lower theorem-emitted quark package on the declared ledger consists of the D12 mass ray \(\mathcal R_{D12}^{ud}\), the negative same-label left-handed selector closure \(\sigma_{ud}=\sigma_{\mathrm{ref}}\), and the restricted-scope affine mean package with \(g_{\mathrm{ch}}=0.9231656602589082\) on \texttt{shared\_budget\_only} and \((g_u,g_d)=(0.7797392875757557,0.12172551081512113)\) on \texttt{current\_family\_only}. Separate target-free theorems internalize the mass bridge \(\Delta_{ud}^{\mathrm{overlap}}=\tfrac16\log(c_d/c_u)\), equivalently \(\Theta_{ud}^{\mathrm{mass}}=\text{\ophid{quark_d12_t1_value_law}}\), on the emitted D12 ray. The selected-class public theorem is the direct descent statement on \(f_P\): the exact physical sigma datum is target-free public on that selected class, the affine mean law emits \((g_u,g_d)\) algebraically, the ordered three-point quadratic readout emits the exact sextet, and the exact forward construction emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\). A separate D12 internal backread sidecar also fixes the mass-side scalar package numerically on a continuation-only surface, with \(\Delta_{ud}^{\mathrm{overlap}}=0.13431740757663183\), \(t_1=0.6715870378831591\), \(\eta_Q^{\mathrm{centered}}=-0.018210067243314802\), and \(\kappa_Q=-0.012436796997836279\); that sidecar does not replace the selected-class public theorem and does not classify all quark frame classes. Charged leptons sit on a split surface: the same-carrier centered readback is exact, the common-shift quotient surface is closed as a no-go for absolute scale, the determinant line fixes the canonical physical affine scalar and uncentered lift on theorem-grade physical charged data, and once theorem-grade \(A_{\mathrm{ch}}(P)\) exists the charged masses are algebraic descendants. The open source-facing theorem is the landing from the D10 calibration descendant package \(B_{D10}(P)\) to theorem-grade physical charged data \(Y_e(P)\) or the charged determinant-line section \(s_{\det}(P)\). A target-anchored same-family exact witness reproduces the charged reference triple exactly on the same ordered eigenvalue family, and within that same-family scope the ordered three-point quadratic readout is closed through \ophid{oph_lepton_current_family_quadratic_readout_theorem}. On that same fixed target-anchored witness, the scoped affine coordinate also closes through \ophid{oph_lepton_current_family_affine_anchor_theorem}, with \(A_{\mathrm{ch}}^{(\mathrm{sf})}=\tfrac13\log\det(Y_e^{(\mathrm{sf})})\) and \(g_e^{(\mathrm{sf})}=e^{A_{\mathrm{ch}}^{(\mathrm{sf})}}\); this remains same-family-only and does not promote the theorem lane. Neutrinos close on a different theorem surface: the weighted-cycle / Majorana-holonomy lane fixes the PMNS/hierarchy shape, and the bridge-rigidity theorem emits
\[
...
free-input sector. The low-energy fine-structure constant is the Thomson endpoint
\[
...
target-free source-only transport theorem from the source basis fixed by the forward D10 solve. The
derivation also contains two
auxiliary validation surfaces: the exact selected-carrier chart and the freeze-once coherent repair
...
source-only repair theorem emits the mass-side pair \((W,Z)\) together with the source-locked
running-family anchor \((a_0,s_0,v)\) from that source basis fixed by the forward solve. The
...
Higgs/top stage & exact Higgs calibration theorem on the declared D10/D11 running, matching, and threshold surface + companion D11 top-side row + compare-only exact sidecar & D10 gauge core followed by the D10 repair chart, the exact Higgs lambda-side seed, and the D11 Jacobian readout & exact Higgs row \(m_H=125.1995304097179~\mathrm{GeV}\), companion D11 top-side output \(m_t^{D11}=172.388646~\mathrm{GeV}\); exact inverse-sidecar pair on the D11 Jacobian & the Higgs theorem is closed on the declared surface; the inverse pair is compare-only and does not replace the theorem surface \\
...
\]
which matches the official PDG 2025 API running-quark target surface exactly, with the top coordinate taken from the PDG cross-section mass entry rather than the auxiliary direct-top entry. On the declared current-family common-refinement transport-frame carrier, the restricted exact chain goes further and emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\).
and
\[
(m_1,m_2,m_3)=
...
Higgs/top critical stage & Higgs boson & calibration theorem & \(125.1995304097179~\mathrm{GeV}\) on the declared D10/D11 running, matching, and threshold surface & exact Higgs calibration theorem; compare-only inverse Higgs/top slice remains on the same Jacobian surface \\
Quark family & top quark & selected-class theorem & \(172.35235532883115~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & exact running-mass theorem row on \(f_P\); the D11 lane carries a companion top-side output on its declared calibration surface, but the repo-wide exact public top row is emitted by the selected-class quark theorem \\
Quark family & up quark & selected-class theorem & \(0.00216~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & exact running-mass theorem row on \(f_P\); supporting exact same-family and restricted transport-frame surfaces realize the same value; no global classification of all quark frame classes is claimed \\
...
Hadrons & proton & execution-contract-frozen & not a paper-derived proton row; surrogate finest-ensemble \(N_{\mathrm{iso}}\) candidate \(0.938960210578~\mathrm{GeV}\) and production geometry summary with 3 ensembles / 6 cfg & outside the declared particle roadmap: replace the surrogate bridge with one production backend export bundle plus production systematics, requiring large-scale nonperturbative execution beyond the current project compute scope \\
Hadrons & neutron & execution-contract-frozen & not a paper-derived neutron row & outside the declared particle roadmap: production backend export bundle plus isospin-resolved hadron systematics, requiring large-scale nonperturbative execution beyond the current project compute scope \\
Hadrons & neutral pion proxy & execution-contract-frozen & not a paper-derived pion row; surrogate finest-ensemble \(\pi_{\mathrm{iso}}\) candidate \(0.135039383836~\mathrm{GeV}\) & outside the declared particle roadmap: replace the surrogate bridge with one production backend export bundle plus production systematics, requiring large-scale nonperturbative execution beyond the current project compute scope \\
Hadrons & \(\rho(770)^0\) proxy & execution-contract-frozen & not a paper-derived resonance row & outside the declared particle roadmap: production backend bundle plus finite-volume resonance extraction, requiring large-scale nonperturbative execution beyond the current project compute scope \\
\end{longtable}
\normalsize
...
Higgs/top critical stage can be used as a closed one-scalar calibration theorem on the declared D10/D11 running, matching, and threshold surface. The quark,
...
the Higgs/top critical stage for the Higgs boson together with a companion D11 top-side output.
...
and only then emit the electroweak transport family forward from that source basis. The
...
selected carrier. The exact selected-carrier chart remains explicit on disk and emits
the local pair
\[
...
\]
So the frozen-target electroweak validation law is not another chart search or selector shell. It is one
...
The active public electroweak surface is the target-free source-only transport law.
...
\begin{theorem}[Ward-projected \texorpdfstring{\(\U(1)_Q\)}{U(1)_Q} transport law and Thomson-limit fine-structure constant]\label{thm:ward_u1q_transport}
Let the D10 source-only running family be locked by the forward pixel solve on the declared
running/matching surface, with source basis
\[
Q_{\mathrm{src}}(P)=
\bigl(\alpha_{2,m_Z}(P),\alpha_{Y,m_Z}(P),\eta_{\mathrm{source}}(P),v(P)\bigr).
\]
...
\]
Hence the physical low-energy fine-structure constant is the Thomson endpoint of that same
...
\]
Consequently, \(\alpha\) is the Thomson endpoint of the D10 electromagnetic transport family
rather than a mass-chart byproduct.
\end{theorem}
...
kernel for all scalar readouts, so no mixed-family or \(Z\)-only surrogate remains promotable. The
Thomson formula is the \(q^2\to0\) limit of the same identity.
\end{proof}

\begin{corollary}[Exact release criterion on the Ward-projected D10 lane]
The Ward-projected D10 lane emits the physical fine-structure constant exactly when the populated
Ward-projected transport kernel satisfies
\[
\lim_{q^2\to0}\alpha_{\mathrm{em}}^{-1}(q^2;P)=137.035999177,
\]
the 2022 CODATA/NIST inverse fine-structure constant.
\end{corollary}

Using the source-locked D10 anchor \(a_0=128.30576920234813\), the Thomson endpoint
requires
\[
\delta_\alpha(0;m_Z^2;P)=0.06804238054863787,
\qquad
\frac{t_Q(0;P)}{t_Q(m_Z^2;P)}=0.936292433907271,
\]
and therefore emits
\[
\alpha_{\mathrm{Th}}^{-1}(P)=137.035999177.
\]

\begin{remark}
The 2022 CODATA/NIST comparison is against the Thomson-limit inverse fine-structure constant. The
source anchor \(a_0=128.30576920234813\) is the electroweak-scale running-family value at
\(m_Z^2\), not the low-energy endpoint.
\end{remark}
...
couplings. These objects locate the public repair law inside the calibration branch without
changing the emitted public surface.

\subsection{Higgs/top critical stage}

The Higgs/top critical stage is a calibration-theorem stage built on top of the electroweak gauge core.
It is described by a compact shared seed together with a Jacobian readout map on the declared D10/D11 running, matching, and threshold surface. The reported
...
\qquad
m_t^{D11} = 172.388646~\mathrm{GeV}.
\]
On this branch the paper-facing closure contract splits into one exact Higgs calibration theorem and one companion D11 top-side calibration row. Explicitly,
\[
\sigma_{D11,H}^{\mathrm{exact}}
=
\frac{\eta_{\mathrm{source}}-\frac43\tau_{2,\mathrm{tree}}^{\mathrm{exact}}-c_H^{\mathrm{exactifier}}\delta n_{\mathrm{tree}}^{\mathrm{exact}}}{\sqrt{\pi}},
\qquad
\delta\lambda(\mu_t)
=-\frac{16}{9}\sigma_{D11,H}^{\mathrm{exact}}\lambda^{\mathrm{core}}(\mu_t),
\]
which lands the Higgs row exactly on the declared codomain. The companion D11 top-side row continues to use the one-scalar seed
\[
\sigma_{D11,\mathrm{HT}}=\frac{\alpha_U\cos(2\theta_{W0})}{\sqrt{\pi}},
\qquad
\frac{\delta y_t(\mu_t)}{y_t^{\mathrm{core}}(\mu_t)}
=-\frac{9}{16}\frac{\delta\lambda(\mu_t)}{\lambda^{\mathrm{core}}(\mu_t)}
=\sigma_{D11,\mathrm{HT}}.
\]
The Higgs theorem is closed on that declared surface, and the companion top row remains a substantive D11 calibration output on the same Jacobian surface.
...
the companion D11 calibration output on this surface, while the exact public top row on the repo-wide non-hadron mass surface is carried separately by the selected-class quark theorem.
...
substantive emitted outputs. The exact inverse slice remains a compare-only validation surface, and the theorem is confined to the declared D10/D11 calibration surface rather than promoted into the recovered core or into the selected-class exact quark theorem.
...
Higgs boson & Higgs/top critical stage & calibration theorem & \(125.1995304097179~\mathrm{GeV}\) \\
Top quark & Higgs/top critical stage & calibration theorem & \(172.388646~\mathrm{GeV}\) \\
\bottomrule
\end{tabular}
\end{table}

The electroweak chapter therefore presents three mathematically distinct surfaces: the reported
target-free source-only theorem rows, the selected-carrier chart beneath them, and the freeze-once
...
exact centered readback, a closed common-shift no-go, a conditional determinant-line lift on
theorem-grade physical charged data, and an algebraic mass readout from theorem-grade
\(A_{\mathrm{ch}}(P)\); the open source theorem is the landing from the D10 calibration
descendants of \(P\) to theorem-grade physical charged data or the charged determinant line. The neutrino lane closes on the weighted-cycle
...
\texttt{current\_family\_only} and the restricted current-family common-refinement
...
running-quark target surface, with the top coordinate taken from the PDG cross-section entry
rather than the auxiliary direct-top entry. The Higgs/top branch also carries its own companion
top-side D11 calibration output \(172.388646~\mathrm{GeV}\) on the forward seed. The table above
...
\]
on \ophid{current_family_only}, and the restricted current-family common-refinement
...
The charged-lepton derivation differs from the quark derivation in a precise way. It does not emit public charged masses from \(P\). It does emit an exact same-family witness, a closed common-shift no-go, a conditional determinant-line lift on theorem-grade physical charged data, and a downstream algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\). The open theorem is the source landing from the D10 calibration descendants of \(P\) to theorem-grade physical charged data or the charged determinant line. The electron, muon, and tau rows are therefore reported as \(n/a\) on the public theorem lane.
...
shape/scale surface. Those centered objects are common-shift invariant. The determinant line fixes the physical affine scalar once theorem-grade physical charged data are present, and the mass readout is then algebraic. The unresolved step is the source landing from the D10 calibration descendants of \(P\) to theorem-grade physical charged data or the charged determinant line.
...
\((1,1,1)\) direction leaves them unchanged. This is the exact reason the \(P\)-driven charged theorem is open:
the theorem surface fixes the centered shape, while the determinant-line landing fixes the absolute normalization.
...
production statistical and systematic control. The declared particle roadmap does not include hadron-mass closure, because that nonperturbative execution burden is not a realistic target within the current project compute scope. Accordingly, the hadron rows are not
...
\(\rho\)-scattering and isospin-breaking program. For that reason, the declared particle roadmap
...
excitation map, CKM closure, and the charged source landing from \(P\) to physical charged data remain open, while the neutrino
...
not a uniformly closed derivation of the full particle zoo. The structural theorem surface fixes
...
stage, closes the selected-class quark theorem on the selected public physical quark frame class \(f_P\), and
closes the neutrino lane on its declared weighted-cycle theorem branch. Charged leptons,
flavor-labeled neutrinos, and hadrons have derivation chains beyond that surface. In the charged
lane the theorem surface carries an exact same-family witness, a conditional determinant-line lift,
and an algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\), while the source landing
from the D10 calibration descendants of \(P\) to physical charged data is open.

That asymmetry belongs on the page because it is the mathematical state described in
this paper. The paper puts the finished structural results, the emitted quantitative rows, and the
named theorem boundaries onto one common page. The visible gates sit above the shared
excitation dictionary, not inside it: theorem-level charged-lepton closure, global classification
of public quark frame classes beyond the selected-class theorem on \(f_P\), extension of the
weighted-cycle neutrino theorem branch to a fuller flavor-labeled neutrino surface, and the
nonperturbative hadron branch. The same accounting also sharpens the local unification story:
the bosonic \(P\)-driven trunk is explicit, the stated local extension surface identifies the shared
edge entropy with the D10 nonabelian sum on the lifted product presentation of the realized
quotient branch, the same surface fixes that scalar to \(P/4\), and the gravity release retains one
strict classical-regime clause above the local \(G\) readout. This paper should be read as a technical derivation document
with exact structural outputs, exact selected-class quark closure, and explicit theorem
boundaries on the unfinished sectors.
