...
\(\epsilon_H=0\). The same paper keeps weaker rows visible instead of promoting them. Charged
lepton absolute masses and source-only hadrons remain work in progress, comparison rows are named
as comparison rows, and measured endpoint data are isolated when a transport bridge uses them.
...
source-only diagnostic trunk records
\[
\begin{aligned}
...
QCD/hadronic electromagnetic payload. The endpoint row is therefore a public fixed-point row with
empirical hadron closure records, while the QCD-free hierarchy witness is the cleaner source-only
...
of the OPH fixed-point equation. The source-only audit row and empirical hadron closure rows are
separate status-table records.
...
useful later in the paper because some flavor continuations are work in progress, whereas the
...
Electroweak \(W/Z\) & \(\begin{gathered}80.377\\91.18797809193725\end{gathered}~\mathrm{GeV}\) & weak-sector validation pair \\
...
Higgs/top surface & \(\begin{gathered}m_H=125.1995304097179\\m_t=172.35235532883115\end{gathered}~\mathrm{GeV}\) & declared D10/D11 split surface \\
...
detailed about a work in progress family, it should be read through this table. The table records the
...
Hierarchy/naturality bridge & exact bridge residual and zero Higgs naturality defect & axioms \(\rightarrow P\)-fixed source branch \(\rightarrow\) \(N_{\mathrm{CRC}}^{\mathrm{EW}}\) bridge fixed point \(\rightarrow\) screen sieve and 24-slot repair register \(\rightarrow\) source-to-Higgs normal-form square & selected-branch theorem for the hierarchy and naturality rows; it does not promote the \(W/Z\), Thomson endpoint, charged-lepton, hadron, Strong CP, or SI \(G\) rows \\
...
\]
which matches the official PDG 2025 API running-quark target surface exactly, with the top coordinate taken from the PDG cross-section mass entry. The auxiliary direct-top row is a compare-only codomain with a corpus-limited no-go boundary. On the declared common-refinement transport-frame carrier, the restricted exact chain goes further and emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\).
and
\[
(m_1,m_2,m_3)=
\begin{gathered}
...
closure on the public frame class rather than a global classification of all quark frames. The
same ledger classifies the hierarchy/naturality bridge as a selected-branch source-side theorem
...
electroweak status table records the residual map and the
...
status surfaces: the \(W/Z\) compare-only validation sidecar, the exact Higgs/top theorem surface,
...
Electroweak D10 branch & \(W\) boson & compare-only adapter & \(80.377~\mathrm{GeV}\) on the frozen \(W/Z\) validation surface & compare-only validation beneath the declared D10/P/fine-structure arithmetic bridge; public validation records are source spectral measure payload, same-scheme remainder, and interval certificate \\
Electroweak D10 branch & \(Z\) boson & compare-only adapter & \(91.18797809193725~\mathrm{GeV}\) on the frozen \(W/Z\) validation surface & same compare-only validation caveat as the \(W\) row \\
...
Quark family & strange quark & selected-class theorem & \(0.0935~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & same selected-class theorem surface; the lower selector \(\sigma_{\mathrm{ref}}\) is a negative sheet statement beneath that theorem \\
...
and local repair instruments. A spherical screen remains a support-visible regulator chart, not a
...
linear-vs-log coordinate discipline. The emitted value law for \(g_e\) is work in progress.
...
available dictionary and comparison surfaces. Where that dictionary is work in progress, the paper states
...
The detector scalar is built from record-algebra data, not from simulator object IDs. Extraction is
...
an interaction marker, the record is routed to the interaction solver rather than stitched by the
...
trivializations, and relabeling of simulator record IDs.
\end{theorem}
...
dependence. The only natural finite output is an ambiguity status.
\end{proof}
...
\subsection{Three generations, flavor closure, and work in progress}
...
excitation map, and CKM closure are work in progress, while the charged source landing from \(P\) to physical charged data is a corpus-limited no-go and the neutrino
...
derivation has a serious hadron pipeline, with source-only hadron masses gated by a production
backend export. The route toward ordinary matter is architecturally present, and the full
observed matter spectrum is work in progress.
...
status table onto one common surface. The bosonic \(P\)-driven trunk is explicit. The local
...
organized by one local screen scale rather than by a list of unrelated particle inputs. The status
table records scope boundaries for the weak validation pair, charged-lepton witnesses,
direct-top auxiliary codomain, strong-CP phase status,
