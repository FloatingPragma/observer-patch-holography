...
With the Standard Model structure supplied by the compact paper, the same setup
carries charge quantization, massless photon/gluon/graviton carriers, a
fine-structure diagnostic, an electroweak bridge, the hierarchy/naturality
result, the Higgs/top split on its stated quantitative branch, selected quark
running masses, and a weighted-cycle neutrino branch. Hadron masses and
absolute charged-lepton masses are not derived here; their rows remain
benchmark checks.
\end{abstract}
...
stress test. Adding the emitted \(\alpha_U(P_\star)\) width gives the source-side no-hadron
inverse-alpha prediction, \(137.0359595008\ldots\), below the Thomson endpoint.
...
\midrule
\(P\)-closure and fine structure & \(\alpha_{\mathrm{root}}^{-1}=136.9948351646\ldots\); \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\); endpoint \(\alpha^{-1}(0)=137.035999177(21)\) & source-side no-hadron prediction plus empirical endpoint closure \\
...
\]
inverse-alpha units. Adding only the bare public-branch \(\alpha_U\) contribution gives the
source-side no-hadron prediction
\[
...
\]
The bare \(\alpha_U\) or 24-register scale contribution lands close to the Thomson endpoint
and does not reach it. OPH source-only closure requires the Ward-projected hadronic spectral
...
Ward-projected Thomson lane records a source-side no-hadron value, the source-plus-bare-\(\alpha_U\)
prediction, and the empirical hadron-completed endpoint as separate rows on the declared
