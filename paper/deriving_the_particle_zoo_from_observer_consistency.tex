...
declared numerical pixel map; its physical endpoint relation remains unconstructed. This paper carries the Standard
...
fixed point, and the sector statuses below sit inside a standing falsification program with
frozen, externally timestamped targets. The same
...
The declared numerical map fixes a unique root on its certified interval. The map remains
...
fixed point of the declared numerical map; CL-6, closed).
...
described in the synthesis paper. Whether any parameter compression is realized is
...
\(N_{\mathrm{EW}}\simeq3.532\times10^{122}\) is about \(6.6\) percent above the
Planck-\(\Lambda\) central capacity \(N_\Lambda\simeq3.313\times10^{122}\). The
...
Converged precision-100 reruns supersede these printed tails beyond digits 10 and 9; the
certified forward pixel is \(P_{\mathrm{fwd}}=1.630972095858897\ldots\) and the certified
root is \(\alpha_{\mathrm{root}}^{-1}=136.994835177413\ldots\) (CL-6, closed).
This root is an incomplete declared-map output, not a physical fine-structure prediction.
...
\midrule
\(P\)-closure and fine structure & \(\alpha_{\mathrm{root}}^{-1}=136.9948351646\ldots\); \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\); empirical endpoint \(136.2636589431\in[136.1583832834,136.3690975240]\); measured \(137.035999177(21)\). Converged reruns supersede the printed witness tail beyond digit 9 (certified \(136.994835177413\ldots\); CL-6) and record the certified numerical gauge-width fixed point \(137.035660136946577\ldots\) (CL-2); the \(137.0359595008\ldots\) packet stays on record as a display packet only & four distinct rows: incomplete declared-map output with no derived physical endpoint relation, mixed diagnostic, empirical hadron closure with anchor gap \([0.6485541111,0.8547920666]\), and measurement \\
...
Electroweak bosons & running/chart audit plus conditional quotient-transport theorem and comparison adapter & source-audit zero-selector law; selected carrier \(\rightarrow\) two-coordinate chart; QT1--QT5 \(\Rightarrow\) conditional value law & no nonzero mass in the prediction ledger; source-audit coordinates \((80.330,\,91.119)\)~GeV and archived value-law coordinates \((80.3770000154,\,91.1879780779)\)~GeV remain chart outputs & the chart coordinates are not commensurate with PDG mass-dependent-width Breit--Wigner masses or complex-pole masses. QT1--QT5, source-law selection, matching, scale, threshold, tadpole, and pole receipts remain open \\
Electroweak hierarchy/naturality & selected-branch dimensionless theorem & local \(P\to\alpha_U\to v/E_\star\) hierarchy lane\newline \(\rightarrow\) 12-port screen sieve\newline \(\rightarrow\) 24-slot oriented repair register\newline \(\rightarrow\) mathematical capacity bridge and RG/Higgs square & \(N_{\mathrm{EW}}=3.5323546226929906511\ldots\times10^{122}\), \(\mathcal B_{\mathrm{EW}}=0\), \(\epsilon_H=0\) & Supports the dimensionless hierarchy and naturality identities on the named branch.\newline Equality with cosmic capacity is conditional on CP-1 to CP-3 and an unconstructed \(F\); the Planck-\(\Lambda\) central value is lower by about \(6.6\) percent. An independent physical \(E_\star\), the public Thomson endpoint, theorem-level \(W/Z\), and the other listed mass rows remain separate surfaces. \\
...
Hierarchy/naturality bridge & exact dimensionless bridge residual and zero Higgs naturality defect & axioms \(\rightarrow P\)-fixed source branch \(\rightarrow\) mathematical \(N_{\mathrm{EW}}\) bridge map \(\rightarrow\) screen sieve and 24-slot repair register \(\rightarrow\) source-to-Higgs settled-form square & selected-branch theorem for \(v/E_\star\) and the naturality identities; equality with the cosmic capacity is conditional on CP-1 to CP-3 and an unconstructed \(F\); it does not supply a physical \(E_\star\) or promote the mass rows \\
...
\(P_{\mathrm{fwd}}=1.630972095858897\ldots\) (CL-6, closed), and the certified
self-consistent gauge-width fixed point is \(\alpha^{-1}=137.035660136946577\ldots\), with
CL-2 residual \(2.5\times10^{-6}\) relative
...
covariance, and the physical readout contract remains open. No physical pull or
...
value is \(\alpha_{\mathrm{root}}^{-1}=136.994835177413\ldots\) (CL-6, closed).
...
is \(P_{\mathrm{fwd}}=1.630972095858897\ldots\). These two named branches
...
declared-map root and a conditional global screen-capacity coordinate. The screen branch supplies twelve irreducible curvature
...
global repair tick, hence \(12=48/4\). The notation
...
source-root certificate remains open, and the result does not independently attach
...
selected branch, so the mathematical local/global hierarchy-resonance package closes on the
selected branch. This finite map is not a construction of the physical cosmic readback map \(F\).
...
assigned across the noncommensurate chart and mass conventions. That slice is not a test of the complete archived
...
threshold matching, scale choice, and higher-order corrections remain open. The outputs establish
...
\subsection{Legacy electroweak chart envelope}\label{ew-conditional-envelope}
...
\end{aligned}
\]
The Higgs and top entries remain retrospective comparisons on their declared mass scheme. The
...
On this legacy comparison envelope, the Higgs and top residuals remain retrospective mass-scheme
diagnostics. The former \(W/Z\) standard-deviation assignments are withdrawn because the chart
coordinates, PDG mass-dependent-width parameters, and complex poles are different conventions.
...
The \(W/Z\) row contains running/chart coordinates and a separate frozen compare-only adapter.
...
exhaustive. G1 is the Ward-projected hadronic transport for CL-1 and CL-2. Its broad V1
...
G3 carries the repaired forward electroweak chain. The 96-entry one-loop sweep excludes entries
within its frozen menu only. Higher-order, matching, threshold, tadpole, scale, and field-content
prescriptions remain open. G4 supplies solver hygiene for the declared map. Twelve
...
The evidence ledger contains zero discriminating frozen-prospective hits. Electroweak checks that
freeze analysis choices against already known targets are audits, not prospective predictions.
Other positive rows have theorem, conditional, retrospective, or non-discriminating status; this
accounting does not classify every positive row as retrodictive.
