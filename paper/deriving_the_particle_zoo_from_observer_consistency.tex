...
\]
The main achievements are the source-side fine-structure calculation, the electroweak transport surface, the Higgs/top split theorem on its declared quantitative branch, selected-class quark rows, and a weighted-cycle neutrino branch. Hadrons and charged-lepton absolute masses are work in progress. Compare-only validation rows are marked as comparison rows.
\end{abstract}
...
\]
Thus the Planck-to-weak ratio is generated by the ordinary exponential transmutation factor
controlled by the unified diffusion coupling. It is not a twenty-four-round use of the global
capacity number.
...
coordinate split, not the observer-facing physical variable. A scalar relevant deformation absent
...
surface and the \(\mathcal R_U\) precision policy rather than becoming a recovered-core theorem.
...
The emitted particle rows continue to use the \(\mathcal R_U\) certificate above. A
theorem-grade promotion of the resonance relation requires a global repair-tick lemma, an
electroweak tick-projection lemma, a joint fixed-point and stability theorem for \((P,N)\), and
an RG/coarse-graining naturality statement showing that radiative corrections change scheme
coordinates rather than the selected normal-form readout. The relation is therefore a
continuation target connecting \(\mathcal R_U\) and \(\mathcal R_N\).
