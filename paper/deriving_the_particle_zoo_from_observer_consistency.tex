...
\(W/Z/H\) complex-pole row is promoted.  The bosonic numerical surface instead
contains a selected-carrier chart, a comparison-only \(W/Z\) adapter, and a
conditional Higgs coordinate on its declared D10/D11 branch.
Target-anchored charged/quark witnesses, including the archived \(0.2946\%\) retrospective
template diagnostic and a later \(0.29436\%\) RSCC module-ledger candidate, and compare-only
absolute neutrino attachments remain audit or comparison rows.  RSCC removes five legacy decimals
from the leaf evaluator but remains target-informed, inherits the Stage-5-dependent pixel witness,
and supplies neither a derived physical channel functor nor a common-scale RG packet.  Neither
quark diagnostic is a mass postdiction. Hadron masses and absolute
...
selected branch. The quark source-spread non-identifiability theorem and weighted-cycle neutrino lane remain visible
as obstruction or comparison surfaces, while target-anchored quark mass textures, the
runtime-target-separated but template-ancestry \(S_3/D12\) formula-discovery diagnostic, and compare-only
...
frozen validation adapter displays
\[
...
\(\Delta\alpha_{\mathrm{had}}^{(5)}(m_Z)=0.026759\pm0.000752\), consistent within one payload
standard deviation with the data-driven compilations. Inserting it into the endpoint map with the
frozen source anchor and lepton transport packet gives an inverse fine-structure endpoint
\(\alpha^{-1}=136.264\) on the interval \([136.158,136.369]\), on the empirical-closure row class.
...
\([0.649,0.855]\) inverse-alpha units. The anchor \(a_0(P)=\alpha_{\mathrm{em}}^{-1}(m_Z^2;P)\) is the
...
\(0.631\), at the lower edge of the certified gap. The gap is the nonperturbative hadronic and
higher-order running that a one-loop scheme cannot contain, so the anchor is not a source-chain
failure. A source-only bridge that closes this gap reduces to the OPH hadronic spectral measure,
...
\midrule
\(P\)-closure and fine structure & \(\alpha_{\mathrm{root}}^{-1}=136.9948351646\ldots\); \(A_{\alpha_U}^{\mathrm{fp}}=137.0359595008\ldots\); empirical-closure endpoint \(\alpha^{-1}=136.264\in[136.158,136.369]\); measured \(\alpha^{-1}(0)=137.035999177(21)\) & source/root audit, empirical-closure endpoint with certified same-scheme anchor gap \([0.649,0.855]\), and measured endpoint \\
...
Electroweak \(W/Z\) & selected carrier \((80.3862916924,91.1829044467)\,\mathrm{GeV}\); frozen adapter \((80.377,91.1879780919)\,\mathrm{GeV}\) & exact mass chart and comparison-only adapter; no promoted pole pair \\
...
Higgs/top relation & \(m_H=125.1995304097179~\mathrm{GeV}\), with a companion top coordinate on the same split surface; legacy conditional envelope \(m_H\in[125.183,125.232]\), \(m_t\in[172.278,172.352]\), \(m_W\in[80.3692,80.3774]\), \(m_Z\in[91.1880,91.1983]~\mathrm{GeV}\) & conditional D11 coordinate on the declared surface; it inherits the source-root, scale, D10 certificate, RG/scheme, rigidity, provenance, and complex-pole gates \\
Charged leptons & target-anchored witness values withheld & empirically anchored same-family audit witness \\
Selected-frame quarks & public numeric rows withheld; target-anchored mass textures, a \(0.2946\%\) retrospective template diagnostic, and a \(0.29436\%\) RSCC module-ledger candidate retained for audit & two-modulus source obstruction plus selected-fiber representative independence; RSCC removes direct legacy-decimal inputs but leaves all source-root, carrier, channel, scale, and RG receipts open \\
...
Electroweak bosons & exact chart plus conditional quotient-transport theorem and comparison adapter & selected carrier \(\rightarrow\) two-coordinate mass chart; QT1--QT5 \(\Rightarrow\) unique value law & carrier \((80.3862916924,91.1829044467)\,\mathrm{GeV}\); archived value-law candidate \((80.3770000154,91.1879780779)\,\mathrm{GeV}\); frozen adapter \((80.377,91.1879780919)\,\mathrm{GeV}\) & QT1--QT5 are not derived from the finite carrier; no complex-pole receipt is emitted \\
...
Higgs/top stage & conditional downstream split theorem on the declared running, matching, and threshold surface + benchmark exact sidecar & gauge core followed by the candidate repair chart, the shared scalar \(\rho_{HT}=\log(1+\tau_{2,\mathrm{tree}}^{\mathrm{exact}})\), the split selectors, and the Jacobian readout & conditional coordinate \(m_H=125.1995304097179~\mathrm{GeV}\); companion top coordinate on the same surface; exact inverse-sidecar pair on the Jacobian & Strict promotion inherits the source-root, physical-scale, QT1--QT5, RG/scheme, rigidity, provenance, and complex-pole gates.\newline The companion top coordinate is not a separate public top-mass prediction row. \\
Quark family & source-spread non-identifiability theorem + target-informed audit surfaces & ordered profile rays \(\rightarrow\) free \((\mathbb R_{>0})^2\) spread fiber\newline \(\rightarrow\) selected-fiber representative independence\newline \(\rightarrow\) conditional affine mass readout & no public numeric quark row; target-anchored mass textures, the \(S_3/D12\) template diagnostic, and the RSCC module-ledger candidate retained only as audit data & The source equations do not select either positive spread modulus.\newline RSCC replaces five direct template decimals by postdeclared module/rank/sign choices but retains Stage-5 \(P\) ancestry, inherited affine laws, mixed-branch scale use, and no RG packet.\newline Its simpler no-\(w^2\) ablation fits the same table better.\newline Light, heavy, and top coordinates use different charts.\newline The auxiliary direct-top row, global public-frame classification, and strong CP are separate boundaries \\
...
Electroweak chart and sidecar & selected-carrier chart, archived value-law candidate, and frozen \(W/Z\) adapter & source tuple \(\rightarrow\) exact two-coordinate chart; QT1--QT5 \(\Rightarrow\) candidate value law; inverse adapter separate & QT1--QT5 remain a finite-carrier certificate obligation; the adapter is comparison-only and no pole pair is promoted \\
...
Higgs/top exact sidecar & exact Higgs/top inverse pair & gauge core \(\rightarrow\) declared Jacobian \(\rightarrow\) exact inverse slice & benchmark inverse slice; the forward numeric row is a conditional D11 coordinate and inherits every open full-chain gate. The top coordinate on that surface is not promoted as a separate public top-mass prediction row \\
...
Quark source-spread boundary & numeric prediction rows withheld; target-anchored mass textures, the \(0.2946\%\) template diagnostic, and the \(0.29436\%\) RSCC candidate retained only for audit & source shape laws \(\rightarrow\) free \((\mathbb R_{>0})^2\) spread fiber \(\rightarrow\) selected-fiber representative independence \(\rightarrow\) conditional affine mass readout & theorem-grade non-identifiability on the present source corpus. RSCC is exact arithmetic on a postulated ledger, not a source-derived carrier or common-scale physical Yukawa packet. Global public-frame classification, the auxiliary direct-top row, and strong CP remain separate boundaries \\
...
Higgs/top critical stage & Higgs boson & conditional declared-surface coordinate; no pole promotion & \(125.1995304097179~\mathrm{GeV}\) on the declared running, matching, and threshold surface & close the independent source root and physical scale, QT1--QT5, RG/scheme and threshold transport, target-independent D11 rigidity, complex-pole, uncertainty, and no-target dependency-DAG gates \\
...
Quark family & up quark & source spread non-identifiable & withheld from public prediction table & target audit uses \(\overline{\mathrm{MS}}\) at \(2\,\mathrm{GeV}\); the present source corpus leaves its sector spread free \\
Quark family & down quark & source spread non-identifiable & withheld from public prediction table & same light-quark comparison chart and two-modulus source obstruction; the target-free mass bridge closes only on an emitted mass ray \\
Quark family & strange quark & source spread non-identifiable & withheld from public prediction table & target audit uses \(\overline{\mathrm{MS}}\) at \(2\,\mathrm{GeV}\); the lower selector does not fix the positive sector spread \\
Quark family & charm quark & source spread non-identifiable & withheld from public prediction table & target audit uses \(\overline{\mathrm{MS}}\) at its self-scale; stored GeV-valued matrices are mass textures, not physical Yukawas \\
Quark family & bottom quark & source spread non-identifiable & withheld from public prediction table & target audit uses \(\overline{\mathrm{MS}}\) at its self-scale; common-scale RG transport and Higgs normalization remain absent \\
...
value-law candidate, the conditional QT1--QT5 implication, and the
freeze-once repair surface.  None is a physical pole theorem.
...
couplings. These objects locate the archived repair formula inside the
...
\]
This is a coordinate on the declared D10/D11 Jacobian surface, not a certified
Higgs or top complex pole. The same surface emits a companion top coordinate.
...
Higgs boson & Higgs/top critical stage & conditional declared-surface coordinate; no pole promotion & \(125.1995304097179~\mathrm{GeV}\) \\
...
\paragraph{Source-spread non-identifiability theorem.}
...
\begin{tabular}{@{}lrrr@{}}
\toprule
Coordinate & ansatz value & mixed-chart reference (GeV) & relative residual \\
\midrule
\(u\) & \(0.002160238738\) & \(0.002160\) & \(+0.0111\%\) \\
\(d\) & \(0.004713845069\) & \(0.004700\) & \(+0.2946\%\) \\
\(s\) & \(0.09312611565\) & \(0.092900\) & \(+0.2434\%\) \\
\(c\) & \(1.273035054\) & \(1.272900\) & \(+0.0106\%\) \\
\(b\) & \(4.192020571\) & \(4.186000\) & \(+0.1438\%\) \\
\(t\) & \(172.1711958\) & \(172.100000\) & \(+0.0414\%\) \\
\bottomrule
\end{tabular}
\caption{Retrospective formula-discovery diagnostic.  The ansatz emits dimensionless coordinates;
the GeV comparison shown here inserts an unproved one-GeV unit.  The rows also mix light-quark
\(\overline{\mathrm{MS}}\) values at \(2\,\mathrm{GeV}\), heavy-quark
\(\overline{\mathrm{MS}}\) self-scale values, and a separate top extraction.}
\label{tab:quark-s3-d12-template-postdiction}
\end{table}
...
\paragraph{RSCC as a formula-level completion candidate.}
...
RSCC therefore contributes an explicit, falsifiable candidate for parts of receipts F2--F5, but
...
\(\mathbb Z_6\) determinant character, a quotient-measure and source-law
rigidity theorem selecting the displayed repair kernel, refinement naturality,
...
\texttt{charged\_face\_incidence\_conditional\_theorem.json}; the compare-only
audits are \texttt{charged\_icosahedral\_completion\_v1\_review.json} and
\texttt{charged\_face\_incidence\_conditional\_theorem\_review.json}.  This candidate is
