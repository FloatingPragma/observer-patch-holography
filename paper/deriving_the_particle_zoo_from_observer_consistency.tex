...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a forward calculation from one selected scale. The synthesis paper fixes the local pixel ratio \(P\) by describing the same screen cell from two sides: as an exterior screen cell and as the electromagnetic observation scale available inside the encoded universe. In this reading, the fine-structure constant is the small detuning that lets a screen cell sit close to self-similar equilibrium while still carrying records, light, and observation. With \(P\) fixed, the particle chain emits a compact package: massless carriers, the \(W\) and \(Z\) masses, the electromagnetic coupling, a Higgs mass, running quark masses with explicit Yukawas, and a neutrino family in the observed hierarchy and mixing regime. The same framework targets the full particle spectrum. Direct zero-momentum electromagnetic closure, charged-lepton absolute masses, and hadrons are work in progress.
\end{abstract}
...
The particle branch studied here is a one-parameter forward reconstruction problem. Instead of
treating masses, mixings, and Yukawa parameters as independent inputs, it propagates one
dimensionless pixel ratio \(P\) through the whole spectrum. The structural massless carriers, the
\(W\) boson and \(Z\) boson pair, the Higgs boson and top quark pair, the selected-class exact quark
theorem, and the weighted-cycle neutrino theorem branch with its shared-basis physical Majorana
pair are emitted from that common scale. Charged leptons, flavor-labeled neutrinos, and hadrons are
work in progress. This paper develops the derivation chain from the axioms and \(P\) to the
reported particle outputs, with the closure boundary stated by sector.
...
declared OPH fixed-point map. The repository records both a numerical fixed-point witness and
a local numerical contraction certificate for the implemented map.
...
The electroweak branch is the place where the particle derivation
shows whether one common input \(P\) emits the electroweak rows or merely hides many fitted
inputs behind fancy notation. In this branch, the public \(W/Z\) rows are emitted by a
...
Hadrons & proton & execution-contract-frozen & not a paper-derived proton row; surrogate finest-ensemble \(N_{\mathrm{iso}}\) candidate \(0.938960210578~\mathrm{GeV}\) and production geometry summary with 3 ensembles / 6 cfg & outside the declared particle roadmap: replace the surrogate bridge with one production backend export bundle plus production systematics, requiring large-scale nonperturbative execution beyond the available project compute scope \\
Hadrons & neutron & execution-contract-frozen & not a paper-derived neutron row & outside the declared particle roadmap: production backend export bundle plus isospin-resolved hadron systematics, requiring large-scale nonperturbative execution beyond the available project compute scope \\
Hadrons & neutral pion proxy & execution-contract-frozen & not a paper-derived pion row; surrogate finest-ensemble \(\pi_{\mathrm{iso}}\) candidate \(0.135039383836~\mathrm{GeV}\) & outside the declared particle roadmap: replace the surrogate bridge with one production backend export bundle plus production systematics, requiring large-scale nonperturbative execution beyond the available project compute scope \\
Hadrons & \(\rho(770)^0\) proxy & execution-contract-frozen & not a paper-derived resonance row & outside the declared particle roadmap: production backend bundle plus finite-volume resonance extraction, requiring large-scale nonperturbative execution beyond the available project compute scope \\
\end{longtable}
\normalsize
...
scope, the theorem-local equilibrium input used in the Jacobson step, the bosonic and fiber-functor clauses
...
selected carrier. The exact selected-carrier chart is explicit on disk and emits
the local pair
\[
...
production statistical and systematic control. The declared particle roadmap does not include hadron-mass closure, because that nonperturbative execution burden is not a realistic target within the available project compute scope. Accordingly, the hadron rows are not
