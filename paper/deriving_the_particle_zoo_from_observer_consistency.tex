...
normalization, while the repaired neutrino continuation branch lands on the observed PMNS/hierarchy
pattern but still leaves one overall positive normalization scalar open. Hadrons remain execution-bound. The purpose of the paper is therefore not to claim full closure, but to give a concise
...
The strongest current continuation bridge remains the balanced Hermitian circulant / \(Q=2/3\) / \(\delta=2/9\) branch, which gives near-exact centered-log closure at residual norm
\[
...
\paragraph{Neutrinos.}
The repaired weighted-cycle / Majorana-holonomy branch now closes the PMNS/hierarchy shape, but one positive mass-normalization scalar remains open. So neutrinos and charged leptons are open for different reasons: charged leptons still miss promotion of \(\widehat C_e^{\mathrm{cand}}\) plus one affine-covariant absolute anchor \(A_{\mathrm{ch}}\), while neutrinos miss one positive normalization scalar after the shape closure is already in hand.

\paragraph{Status boundary.}
The honest Stage-5 headline is therefore this: quarks have continuation-level rows; charged leptons have an exact same-carrier readback theorem together with an exact absolute-scale obstruction; neutrinos have a repaired continuation with one scalar still open. None of those later-family statements should be promoted to the recovered-core surface fixed earlier by the gauge and relativity chain.
...
Neutrinos & continuation & repaired weighted-cycle PMNS/hierarchy closure & no promoted masses & one positive neutrino mass-normalization scalar remains open; absolute masses and absolute \(\Delta m^2\) are compare-only until it is emitted \\
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger two-scalar candidate \(0.00215992~\mathrm{GeV}\) & the current D12 sheet is a strict no-go for the physical CKM shell; emit one discrete relative sheet selector \(\sigma_{ud}\), then derive the mass-side scale law on the selected branch \\
...
Quark family & strange quark & overlap continuation & reported value \(0.0916111~\mathrm{GeV}\); stronger two-scalar candidate \(0.0935045~\mathrm{GeV}\) & same wrong-branch D12 frontier; the next honest quark object is the discrete selector \(\sigma_{ud}\) rather than a continuous mass scalar \\
...
Neutrinos & \(\nu_e\) & repaired continuation & \(n/a\); repaired weighted-cycle branch emits \(\theta_{12}=33.9756^\circ\), \(\theta_{23}=49.7785^\circ\), \(\theta_{13}=8.64594^\circ\), \(\delta=307.818^\circ\), \(J=-0.02652\), and \(\Delta m_{21}^2/\Delta m_{32}^2=0.0316024\); with compare-only anchor \(\Delta m_{32}^2=2.433\times10^{-3}\,\mathrm{eV}^2\), it gives \(m_1=0.01730\), \(m_2=0.01940\), \(m_3=0.05300~\mathrm{eV}\) & emit the one positive neutrino mass-normalization scalar so the absolute masses and absolute \(\Delta m^2\) become theorem-grade rather than anchor-based \\
Neutrinos & \(\nu_\mu\) & repaired continuation & \(n/a\); same repaired weighted-cycle branch as the electron-neutrino row & same one-scalar normalization frontier \\
Neutrinos & \(\nu_\tau\) & repaired continuation & \(n/a\); same repaired weighted-cycle branch as the electron-neutrino row & same one-scalar normalization frontier \\
...
\begin{enumerate}[leftmargin=2em]
\item one discrete relative-sheet selector \(\sigma_{ud}\) that leaves the current D12 no-go class and changes the CKM moduli at all; and
...
The flavor rows are still \(n/a\), but the open boundary is now much smaller than a generic
``repair the branch'' instruction. The old isotropic intrinsic branch is ruled out exactly, and the
repaired weighted-cycle continuation branch already lands in the observed PMNS/hierarchy regime.
What remains open is one overall positive normalization scalar for the absolute neutrino scale.

The repaired weighted-cycle branch emits
\[
\theta_{12}=33.97561492020427^\circ,\qquad
\theta_{23}=49.778494281953684^\circ,\qquad
\theta_{13}=8.645937826302099^\circ,
\]
\[
\delta_{\mathrm{PMNS}}=307.81750422696206^\circ,\qquad
J=-0.026522158828835723,
\]
and the dimensionless hierarchy invariant
\[
\frac{\Delta m_{21}^2}{\Delta m_{32}^2}=0.031602426224679334.
\]
With the compare-only atmospheric anchor
\[
\Delta m_{32}^2 = 2.433\times10^{-3}\ \mathrm{eV}^2,
\]
the same repaired branch gives
\[
(m_1,m_2,m_3)=(0.01730382396,\ 0.01939873776,\ 0.05300293413)\ \mathrm{eV},
\]
\[
\Delta m_{21}^2=7.688870300\times10^{-5}\ \mathrm{eV}^2,\qquad
\Delta m_{31}^2=2.509888703\times10^{-3}\ \mathrm{eV}^2.
\]
...
The honest remaining task is no longer ``emit PMNS from the shared basis,'' and it is no longer
``repair the PMNS branch.'' Those steps are already complete on the repaired weighted-cycle route.
The real remaining burden is to emit the one positive normalization scalar that turns the repaired
dimensionless hierarchy into theorem-grade absolute masses and absolute splittings. Until that
scalar is emitted, the weighted-cycle branch gives a physical PMNS/hierarchy pattern and a
compare-only absolute spectrum once one atmospheric anchor is supplied.
...
gauge-field generation and Dirac solves, not the final analysis dump. So the honest deployment
target is a multi-node HPC cluster or cloud HPC allocation rather than a laptop or ordinary
workstation. As an operational estimate, a backend adapter and first real stable-channel pilot dump
