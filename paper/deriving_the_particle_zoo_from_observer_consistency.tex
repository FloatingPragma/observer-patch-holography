...
and graviton are structural massless carriers; the electroweak calibration stage yields the
\(W\) boson and \(Z\) boson; the Higgs/top critical stage yields the Higgs boson and top quark;
and the quark branch emits continuation-level rows for the up quark, down quark, strange quark,
charm quark, and bottom quark. Charged leptons, flavor-labeled neutrinos, and hadrons remain
open. The purpose of the paper is therefore not to claim full closure, but to give a concise
...
massless carriers, the \(W\) boson/\(Z\) boson pair, the Higgs boson/top quark pair,
and several quark rows are emitted, while charged leptons, flavor-labeled neutrinos, and
hadrons remain open. This paper develops that derivation chain from the axioms and \(P\) to the
...
Charged leptons use the same deeper family backbone, but the derivation does not promote
their masses. The chain reaches a forward charged shape/scale map, yet the support-
extension scalars that would close the mass values remain open.

\paragraph{Neutrinos.}
Neutrinos branch off through a different response route. The neutrino derivation has a scale
anchor, family-response tensor, Majorana holonomy machinery, a pullback metric, and a forward
splitting bundle. But flavor-labeled masses remain blocked by same-label closure and
PMNS closure.

\paragraph{Hadrons.}
Hadrons come last because they require more than the elementary-family branches. They descend
from the electroweak calibration branch and the quark outputs into the QCD scale bridge and then
into the nonperturbative
stable-channel readout pipeline. That is why the hadron derivation is the most simulation-
heavy and least publicly promoted part of the particle-spectrum story. On the current branch,
hadrons are execution-bound rather than theorem-blocked: promotable hadron rows require real
production computation and production systematics, so they are not counted as derived outputs of
this paper.
...
notation. If different calibration observables demand different implied pixel constants, then the
electroweak stage is not yet an exact single-\(P\) closure surface. In that case the honest task is
to identify which transport, matching, or readout object is still missing.

\subsection{What electroweak calibration implies}

The calibration analysis makes the criterion explicit: if the present implementation makes
\(m_W\), \(m_Z\), \(\alpha_{\mathrm{em}}\), \(\sin^2\theta_W\), and \(v\) imply different pixel
constants, then the non-closure is not explained merely by asking for more decimal places of
\(P\). The implied-\(P\) comparison records exactly that
diagnosis. The \(W\), \(Z\), and \(v\) observables imply nearby values close to the default
\(P=1.63094\), while the derived \(\sin^2\theta_W\) surface pushes the implied \(P\) far enough
away that exact single-\(P\) closure fails. In other words, the bottleneck is a transport/matching problem on the electroweak stage, not simply a lack of extra digits
in the declared pixel constant.

That diagnosis is scientifically useful. It distinguishes ``the underlying one-constant idea is
wrong'' from ``the electroweak construction has not yet reached its exact common closure
surface.'' The paper therefore describes two layers at once. First, one common pixel
constant \(P\) is the intended driver of the spectrum. Second, the remaining non-closure identifies the transport or
matching object that still has to be derived.
...
Electroweak bosons & calibration & electroweak calibration stage & \(W\) boson, \(Z\) boson & target-free electroweak repair value law from \(P\) alone \\
...
Quark family & continuation & forward Yukawa derivation & up, down, strange, charm, bottom quarks & two-scalar overlap law, same-label physical invariants, Yukawa dictionary, CKM/CP closure \\
Charged leptons & continuation & ordered-log and support-extension derivation & no promoted masses & \(\eta\) source readback, \(\sigma\) support-extension scalar, absolute scale \(g_e\), excitation dictionary \\
Neutrinos & continuation & intrinsic Majorana eta-chain & no promoted masses & same-label scalar certificate and shared charged-lepton basis \\
...
Electroweak calibration & \(W\) boson & calibration & \(80.377~\mathrm{GeV}\) on the frozen authoritative calibration surface & derive the target-free repair value law from \(P\) alone \\
Electroweak calibration & \(Z\) boson & calibration & \(91.18797809193725~\mathrm{GeV}\) on the frozen authoritative calibration surface & same target-free electroweak repair frontier as the \(W\) row \\
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger two-scalar candidate \(0.00215992~\mathrm{GeV}\) & derive the value laws for \(\Delta_{ud}^{\mathrm{overlap}}\), \(\eta_Q^{\mathrm{centered}}\), and the CKM generator \(K_{\mathrm{CKM}}\) \\
Quark family & down quark & overlap continuation & reported value \(0.00512144~\mathrm{GeV}\); stronger two-scalar candidate \(0.00470005~\mathrm{GeV}\) & same overlap-continuation value laws plus \(K_{\mathrm{CKM}}\) frontier \\
Quark family & strange quark & overlap continuation & reported value \(0.0916111~\mathrm{GeV}\); stronger two-scalar candidate \(0.0935045~\mathrm{GeV}\) & same overlap-continuation value laws plus \(K_{\mathrm{CKM}}\) frontier \\
Quark family & charm quark & overlap continuation & reported value \(1.256692~\mathrm{GeV}\); stronger two-scalar candidate \(1.272939~\mathrm{GeV}\) & same overlap-continuation value laws plus \(K_{\mathrm{CKM}}\) frontier \\
Quark family & bottom quark & overlap continuation & reported value \(3.820118~\mathrm{GeV}\); stronger two-scalar candidate \(4.182754~\mathrm{GeV}\) & same overlap-continuation value laws plus \(K_{\mathrm{CKM}}\) frontier \\
Charged leptons & electron & continuation gap & \(n/a\); continuation pair \((\eta,\sigma)=(-6.72959,\,8.15406)\) with near-exact centered-log shape closure and typed absolute-scale shell \(\mu_e^{\mathrm{abs}} \mapsto g_e=e^{\mu_e^{\mathrm{abs}}}\) & emit the charged sector-response functor \(\widehat C_e\), then the common-refinement transport equalizer, then the induced \(\eta,\sigma,g_e\) writeback \\
Charged leptons & muon & continuation gap & \(n/a\); same continuation bridge as the electron row & same \(\widehat C_e \rightarrow\) equalizer \(\rightarrow (\eta,\sigma,g_e)\) closure chain \\
Charged leptons & tau lepton & continuation gap & \(n/a\); same continuation bridge as the electron row & same \(\widehat C_e \rightarrow\) equalizer \(\rightarrow (\eta,\sigma,g_e)\) closure chain \\
Neutrinos & \(\nu_e\) & continuation gap & \(n/a\); intrinsic eigenmasses \((2.393,\,2.4048,\,2.5896)\times 10^{-12}~\mathrm{GeV}\) are exact once the same-label scalar certificate is fixed & emit the live same-label scalar certificate and the shared charged-lepton left basis \\
Neutrinos & \(\nu_\mu\) & continuation gap & \(n/a\); same intrinsic eta-demo branch as above & same eta-emission plus charged-basis frontier \\
Neutrinos & \(\nu_\tau\) & continuation gap & \(n/a\); same intrinsic eta-demo branch as above & same eta-emission plus charged-basis frontier \\
...
then a broader repair question beyond the present carrier if the selected point does not
coincide with the PDG pair. In practical terms, the construction starts from
...
and then asks whether exact PDG \(W/Z\) can honestly be forced on the same carrier.
...
\]
These numbers are emitted on the freeze-once coherent electroweak repair surface, not on the
unmodified selected carrier. The exact current-carrier chart remains explicit on disk and emits
the local pair
\[
...
\]
so the mathematics keeps the two layers separate: the closed current-carrier chart, and the
closed frozen-target repair surface above it.
...
\]
The only stricter remaining step in the electroweak calibration stage is the target-free
statement: emit the same nonzero repair directly from \(P\) alone with no frozen authoritative
\(W/Z\) input.

The target-free status is sharper than a generic ``missing selector'' description. Let
\[
\beta_{\mathrm{EW}}
=
\frac{\alpha_{2,m_Z}-\alpha_{Y,m_Z}}{\alpha_{2,m_Z}+\alpha_{Y,m_Z}}.
\]
At source-only level, the present corpus still underdetermines the repair coefficients: for
arbitrary real \(c,d\), the family
\[
\tau_2^{\mathrm{exact}}=-c\,\eta_{\mathrm{source}}^2,
\qquad
\delta n^{\mathrm{exact}}=d(1-\beta_{\mathrm{EW}})\eta_{\mathrm{source}}^2
\]
defines a coherent quadratic target-free repair ansatz. So the open target-free problem is
genuine.

The smallest honest conditional route is the color-balanced quadratic repair descent principle.
Assuming
\[
\mathrm{ColorBalancedQuadraticRepairDescent}_{EW},
\]
the repair law closes uniquely to
\[
\tau_2^{\mathrm{exact}}
=
-\frac{\sqrt{N_c}}{2}\eta_{\mathrm{source}}^2,
\qquad
\delta n^{\mathrm{exact}}
=
\frac{N_c}{2}(1-\beta_{\mathrm{EW}})\eta_{\mathrm{source}}^2.
\]
For \(N_c=3\), this conditional specialization gives
\[
\tau_2^{\mathrm{exact}}=-4.2477649513449626\times 10^{-4},
\qquad
\delta n^{\mathrm{exact}}=3.3952000966304088\times 10^{-4},
\]
and a coherent quintet
\[
M_W=80.36921677537332~\mathrm{GeV},
\qquad
M_Z=91.18800674424540~\mathrm{GeV}.
\]
This is the smallest current conditional theorem beneath the target-free frontier, but it is still
conditional.

There is also a stronger source-only candidate law on the current corpus,
\[
\lambda_{\mathrm{EW}}=\frac{\eta_{\mathrm{source}}^2}{4\beta_{\mathrm{EW}}},
\]
\[
\tau_2^{\mathrm{exact}}
=
-\lambda_{\mathrm{EW}}
\left(1+\frac23\eta_{\mathrm{source}}+\Bigl(1-\frac{\beta_{\mathrm{EW}}}{6}\Bigr)\eta_{\mathrm{source}}^2\right),
\]
\[
\delta n^{\mathrm{exact}}
=
\lambda_{\mathrm{EW}}
\left(1+\frac43\eta_{\mathrm{source}}+\Bigl(2-\frac{\beta_{\mathrm{EW}}}{6}\Bigr)\eta_{\mathrm{source}}^2\right),
\]
which emits
\[
...
\]
Its residuals relative to the frozen local authoritative surface are only
\[
\Delta M_W\approx 1.54\times 10^{-8}~\mathrm{GeV},
\qquad
\Delta M_Z\approx -1.40\times 10^{-8}~\mathrm{GeV},
\]
but this remains a new source-only candidate law, not yet an accepted paper theorem.
...
\midrule
\(W\) boson & electroweak calibration & calibration & \(80.377~\mathrm{GeV}\) \\
\(Z\) boson & electroweak calibration & calibration & \(91.18797809193725~\mathrm{GeV}\) \\
...
The electroweak chapter therefore has to keep both layers visible at once:
the reported numerical rows and the remaining exactness frontiers beneath them. That is the only
way to make the bosonic sector technically honest.
...
-\frac{1-x_2^2}{27}\,t_1.
\]
So the compact same-family continuation is controlled by a single scalar \(t_1\), even though the
broader overlap-continuation frontier still treats \(\Delta_{ud}^{\mathrm{overlap}}\) and
\(\eta_Q^{\mathrm{centered}}\) as separate constructive objects.
...
The mixing side is also reduced more sharply than before. A gauge-fixed same-label left-transport
generator carries the physical invariant shell
\[
(\theta_{12},\theta_{23},\theta_{13},\phi_{\mathrm{CP}})
=
(0.2256228751541542,\ 0.04380491981435288,\ 0.003473054182841341,\ -2.6946364809227097),
\]
with relative branch rigidity
\[
2.3461092057035416\times10^{-4}.
\]
Equivalently,
\[
...
+ \theta_{12}(E_{12}-E_{21})
+ \theta_{23}(E_{23}-E_{32})
+ \theta_{13}\bigl(e^{i\phi_{\mathrm{CP}}}E_{13}-e^{-i\phi_{\mathrm{CP}}}E_{31}\bigr).
\]
These invariants are compare-derived diagnostics, not emitted OPH laws. This is why the honest
quark frontier is split in two:
\begin{enumerate}[leftmargin=2em]
\item a continuation-level selector/value law for \(\Delta_{ud}^{\mathrm{overlap}}\); and
\item a generation-bundle / same-label left-transport generator that upgrades the mass branch to full CKM/CP closure.
\end{enumerate}
...
must resist overstating what this means. The existence of those rows does not mean CKM structure,
phase closure, or theorem-level odd-source completion has been achieved.
...
\item the broader honest overlap-continuation frontier consists of the two mass-side value laws \(\Delta_{ud}^{\mathrm{overlap}}\) and \(\eta_Q^{\mathrm{centered}}\) together with the matrix-valued CKM/CP transport generator \(K_{\mathrm{CKM}}\).
\end{enumerate}
...
the charged sector-response / declaration functor that emits the unique charged operator
\(\widehat C_e\), and then the common-refinement transport equalizer that upgrades the absolute
scale from shared-budget control to theorem-grade transport.
...
\sigma_{\mathrm{ext}},
\]
and the charged absolute scale \(g_e\) remains unresolved. But these are no longer the deepest
theorem objects. Once \(\widehat C_e\) exists, \(\eta_{\mathrm{ext}}\) and
\(\sigma_{\mathrm{ext}}\) become charged spectral invariants rather than separate primitive goals,
and the absolute-scale burden is pushed to the common-refinement transport equalizer.
...
the continuation branch, but the theorem-grade derivation still lacks the charged operator
\(\widehat C_e\), the common-refinement transport equalizer, and therefore the emitted
\(\eta_{\mathrm{ext}}\), \(\sigma_{\mathrm{ext}}\), and \(g_e\) they would induce.
...
The flavor rows are still \(n/a\) because the proof-facing same-label pullback package and
the shared charged-lepton left basis remain open. But once the centered same-label eta-class is
given, the intrinsic neutrino mass-eigenstate branch is exact.

The current neutrino-only forward branch is also understood much more sharply than before. On the
live same-label neutrino-only branch, the centered eta-class is exactly \(S_3\)-isotropic, so the
same-label data are edge-constant and the solar \(1\!-\!2\) split cannot open there by itself. The
first honest solar mover therefore has to come from realized flavor-side same-label gap/defect
...
\begin{enumerate}
\item the live same-label scalar certificate, preferably the full
\((g_e,\omega_e)\) or \((g_e,d_e)\) package and at minimum the centered eta-class itself. At
proof-facing level this is a five-dimensional continuous family modulo one common scale; at
builder-facing level it compresses to the centered eta-class, i.e. two continuous degrees of
freedom; and
\item the shared charged-lepton left basis needed to form physical PMNS mixing. At artifact level
this is a nine-parameter unitary object before phase quotients.
\end{enumerate}
