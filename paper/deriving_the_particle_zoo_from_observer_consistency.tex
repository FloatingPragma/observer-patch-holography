...
\author{B.\ M\"uller \and Alexander Osika \and Mario Poneder \and Kai Xue \and Maarten Antonie Visser}
...
\begin{abstract}
One OPH screen cell carries an outside area and an inside electromagnetic observation scale. Matching those two readings gives the pixel fixed point
\[
P_\star=\varphi+\frac{\sqrt{\pi}}{A_T(P_\star)}.
\]
The equation is solved as a self-reference: the candidate pixel emits the electromagnetic readout that corrects the same candidate pixel. Measurements may locate the branch interval; the value counted by OPH is the unique closure endpoint of the declared map. With the structural Standard Model branch supplied by the compact paper, this one cell gives the charge-quantized particle skeleton, structural zero masses for photon/gluon/graviton carriers, the source-side fine-structure diagnostic, the Maxwell-normalized electroweak transport surface, the capacity-electroweak bridge, the selected-branch hierarchy/naturality result with \(\epsilon_H=0\), the Higgs/top split on its declared quantitative branch, selected quark running masses, and a weighted-cycle neutrino branch. First-principles hadron masses and charged-lepton absolute masses are outside the theorem stated here; benchmark checks are marked as checks.
\end{abstract}

\section*{What This Paper Contributes}

The structural particle content is inherited from the compact SM/GR paper: compact gauge
reconstruction, the realized Standard Model quotient, exact hypercharge, three colors, three
generations, and the massless carrier zeros. This paper asks what that structure does after the
...
pipeline claims closure only for the structural massless carriers, the Higgs/top relation on its
declared quantitative surface, the selected-frame quark theorem, and the weighted-cycle neutrino
...
\item compact gauge reconstruction from the theorem-produced transportability criterion, fixed-cutoff bosonic sector category, refinement/fiber ladder, and realized MAR-admissible witness data;
...
SM/GR derivation in Ref.~\cite{ophcompact} supplies the structural chain used here: compact gauge reconstruction as a classification step, MAR selection of the realized
...
electroweak map and its descendants. The public endpoint branch displays
\[
...
\alpha_{\mathrm{cand}}^{-1}
&=\decnum{136.994835164621649457949994585787193262029}.
\end{aligned}
\]
The approximately \(0.041\) inverse-alpha gap to the public Thomson endpoint is the declared
QCD/hadronic electromagnetic input. The endpoint row is therefore a public fixed-point result with
empirical hadron closure records, while the QCD-free hierarchy witness is the cleaner first-principles
stress test. The detailed endpoint table appears in Section~\ref{sec:particle-input-ledger}.
The separate
...
\midrule
\(P\)-closure and fine structure & \(\alpha^{-1}(0)=137.035999177(21)\), \(P\simeq1.6309682094\) & local fixed-point scale \\
...
Higgs/top relation & \(\begin{gathered}m_H=125.1995304097179\\m_t=172.35235532883115\end{gathered}~\mathrm{GeV}\) & declared split surface \\
Charged leptons & \(m_e,m_\mu,m_\tau\) witness values & empirically anchored same-family witness \\
Selected-frame quarks & exact running sextet on \(f_P\) & selected public quark frame \\
Neutrino absolute attachment & \(\begin{gathered}0.017454720257976796\\0.019481987935919015\\0.05307522145074924\end{gathered}~\mathrm{eV}\) & weighted-cycle absolute branch \\
...
H3 record worldlines & certified cross-boundary record-token continuation or \(\mathrm{AMBIGUOUS}\) & conditional observer-record theorem; not a species, mass, or coupling input \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Structural carriers & structural & gauge/gravity structural theorems & exact \(m_\gamma=m_g=m_{\mathrm{grav}}=0\) & theorem-level structural exactness; no particle-side caveat \\
...
Higgs/top stage & exact first-principles split theorem on the declared running, matching, and threshold surface + benchmark exact sidecar & gauge core followed by the target-free repair chart, the shared scalar \(\rho_{HT}=\log(1+\tau_{2,\mathrm{tree}}^{\mathrm{exact}})\), the first-principles split selectors, and the Jacobian readout & exact pair \(m_H=125.1995304097179~\mathrm{GeV}\), \(m_t=172.3523553288312~\mathrm{GeV}\); exact inverse-sidecar pair on the Jacobian & Higgs row is closed on the declared surface.\newline The inverse pair is a benchmark check.\newline The auxiliary direct-top PDG row is a distinct benchmark codomain with a corpus-limited no-go boundary. \\
Quark family & selected-frame theorem + exact supporting surfaces & shared excitation dictionary \(\rightarrow\) selected public quark frame \(f_P\)\newline \(\rightarrow\) public running-mass descent\newline \(\rightarrow\) affine mean law\newline \(\rightarrow\) ordered three-point readout\newline \(\rightarrow\) exact forward construction & exact running quark sextet on the selected public quark frame \(f_P\), together with explicit exact forward Yukawas \(Y_u,Y_d\) on that selected frame & selected-frame closure only.\newline The exact sextet matches the official PDG 2025 API running-quark surface, with the top coordinate taken from the PDG cross-section entry.\newline The auxiliary direct-top PDG row is a benchmark check with a corpus-limited no-go boundary.\newline The stronger global classification of public quark frames is not emitted.\newline This paper does not derive \(\theta_{\mathrm{QCD}}\), does not emit physical \(\bar\theta\), and does not prove that the physical strong-CP phase vanishes.\newline Supporting exact surfaces use same-family and restricted common-refinement transport-frame checks, both realizing the same sextet and explicit exact forward Yukawas.\newline The separate target-free mass bridge closes on the emitted mass ray.\newline No global classification of all quark frame classes is claimed \\
Charged leptons & continuation + exact sidecar witness & shared excitation dictionary \(\rightarrow\) ordered charged carrier \(\rightarrow\) exact centered readback \(\rightarrow\) determinant-line lift on theorem-level physical charged data & exact same-carrier centered readback together with the exact same-family charged triple on an empirically anchored witness & public charged masses are not emitted from \(P\).\newline The theorem lane does not emit a theorem-level sector-isolated charged determinant exponent vector.\newline It does not attach a source-side determinant character to the physical charged determinant line.\newline The determinant-line lift and algebraic mass readout apply only on theorem-level physical charged data \\
Neutrinos & continuation + emitted bridge invariant + auxiliary checks & same-label scalar certificate \(\rightarrow\) weighted-cycle branch \(\rightarrow\) bridge rigidity \(\rightarrow\) absolute attachment \(\rightarrow\) shared-basis transport readout & emitted absolute masses, central splittings, and the physical Majorana pair \((\alpha_{21}^{(\mathrm{Maj})},\alpha_{31}^{(\mathrm{Maj})})=(153.618518^\circ,257.003241^\circ)\) on the weighted-cycle branch, with bridge invariant \(C_\nu\) above emitted proxy \(P_\nu\) and paper-facing amplitude \(B_\nu\) & theorem-level on the declared weighted-cycle branch; the exact positive-segment adapter, bridge corridor, and correction check are diagnostics and do not feed back into theorem state \\
Hadrons & strong-binding construction absent; empirical closure policy emitted & Ward-projected spectral-measure contract for an OPH hadron construction, plus empirical \(e^+e^-\to\mathrm{hadrons}\) source registry and payload schema & no first-principles hadron masses; empirical hadron closure rows are separate from first-principles OPH rows & First-principles hadron prediction requires a working OPH hadron construction, such as GLORB/Echosahedron, with the Ward-projected hadronic spectral measure and systematics. The empirical closure surface uses separate \(e^+e^-\to\mathrm{hadrons}\) input and cannot upgrade the first-principles theorem \\
H3 record-worldline stitching & conditional certificate theorem & declared \(H^3_R\) chart atlas and observer clock atlas\newline \(\rightarrow\) chart-natural proto-record extraction\newline \(\rightarrow\) overlap descent\newline \(\rightarrow\) real transverse interface-crossing germs\newline \(\rightarrow\) common-chart sector/gauge transport\newline \(\rightarrow\) ID-independent one-to-one assignment gap\newline \(\rightarrow\) coarse/fine contraction check & nonbranching localized record-token paths, rays, or isolated records when the certificate passes; otherwise \(\mathrm{AMBIGUOUS}\), \(\mathrm{REJECTED}\), or \(\mathrm{INTERACTION\ REQUIRED}\) & This is a continuation theorem for observer-visible records in \(H^3\), not a derivation of particle species, masses, gauge charges, or scattering amplitudes. Repeated implementation IDs, nearest-neighbor fits, and file-boundary coincidences are not admissible evidence. \\
\bottomrule
\end{tabular}
\end{table}
...
zeros; the \(W\) boson and \(Z\) boson are benchmark frozen-adapter values; and the
...
\midrule
Structural carriers & \(m_\gamma=m_g=m_{\mathrm{grav}}=0\) & axioms \(\rightarrow\) realized electromagnetic/color/dynamical-metric carrier skeleton \(\rightarrow\) symmetry-protected zeros & theorem-level structural exactness \\
...
Higgs/top exact sidecar & exact Higgs/top inverse pair & axioms \(\rightarrow\) gauge core \(\rightarrow\) Jacobian \(\rightarrow\) exact inverse slice & benchmark inverse slice; the public Higgs/top pair comes from the first-principles split theorem on the declared surface, while the repo-wide exact public top row is also carried by the selected-frame quark theorem \\
Charged exact witness & exact same-family charged triple & axioms \(\rightarrow\) shared excitation dictionary \(\rightarrow\) ordered charged carrier \(\rightarrow\) exact centered readback \(\rightarrow\) closed quadratic readout theorem \(\rightarrow\) same-family exact witness & same-family only; theorem lane carries exact centered readback plus the closed common-shift no-go. The available derivation has a no-go boundary: a theorem-level \(\widehat C_e\) lift emitting the physical scalar \(\mu_{\mathrm{phys}}(Y_e)\) is not part of the emitted theorem surface \\
Quark selected-frame theorem & exact running quark sextet plus explicit exact forward Yukawas on \(f_P\) & axioms \(\rightarrow\) shared excitation dictionary \(\rightarrow\) selected public quark frame \(f_P\) \(\rightarrow\) public running-mass descent \(\rightarrow\) affine mean law \(\rightarrow\) ordered three-point readout \(\rightarrow\) exact forward construction & selected-frame closure on the public physical quark frame chosen by \(P\); the sextet matches the official PDG 2025 API running-quark surface exactly, with the top coordinate taken from the PDG cross-section entry; the auxiliary direct-top PDG row is a benchmark check with a corpus-limited no-go boundary; the stronger global classification of public quark frame classes is not emitted; supporting same-family and restricted common-refinement transport-frame checks realize the same sextet and exact forward Yukawas; no global classification of all quark frame classes is claimed \\
Neutrino theorem branch & emitted absolute mass triple, representative central splittings, and physical Majorana pair & axioms \(\rightarrow\) same-label scalar certificate \(\rightarrow\) weighted-cycle branch \(\rightarrow\) bridge-rigidity theorem \(\rightarrow\) absolute-attachment theorem \(\rightarrow\) shared-basis transport readout & theorem-level on the declared weighted-cycle branch; the exact positive-segment adapter and bridge-coordinate sidecars are diagnostics \\
\bottomrule
\end{tabular}
\end{table}
...
(u,d,s,c,b,t)=
\begin{gathered}
(0.00216,\ 0.00470,\ 0.0935,\ 1.273,\ 4.183,\\
172.3523553288311)
\end{gathered}\,\mathrm{GeV},
\]
which matches the official PDG 2025 API running-quark target surface exactly, with the top coordinate taken from the PDG cross-section mass entry. The auxiliary direct-top row is a benchmark codomain with a corpus-limited no-go boundary. On the declared common-refinement transport-frame carrier, the restricted exact chain emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\).
and
\[
(m_1,m_2,m_3)=
\begin{gathered}
...
triple as an empirically anchored current-family witness, and the quark sextet as selected-frame exact
closure on the public frame class instead of a global classification of all quark frames. The
...
\(e^+e^-\to\mathrm{hadrons}\) input record carries the data-driven hadron
path. The first-principles transport row excludes an inserted comparison endpoint. The
...
\]
from the selected scale certificate. On that declared extension
...
Higgs/top-surface value. The paper keeps \(c\) as the structural Lorentz output and keeps the
...
Quark family & top quark & selected-frame theorem & \(172.35235532883115~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & exact running-mass theorem row on \(f_P\); the split lane carries the same exact top value on its declared quantitative surface, while the repo-wide exact public top row is emitted by the selected-frame quark theorem \\
Quark family & up quark & selected-frame theorem & \(0.00216~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & exact running-mass theorem row on \(f_P\); supporting exact same-family and restricted transport-frame surfaces realize the same value; global classification of all public quark frame classes is closed as a corpus-limited no-go \\
Quark family & down quark & selected-frame theorem & \(0.00470~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & same selected-frame theorem surface; target-free mass bridge on the emitted mass ray closes separately \\
Quark family & strange quark & selected-frame theorem & \(0.0935~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & same selected-frame theorem surface; the lower selector is a negative sheet statement beneath that theorem \\
Quark family & charm quark & selected-frame theorem & \(1.273~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & same selected-frame theorem surface; explicit exact forward Yukawas \(Y_u,Y_d\) are emitted on \(f_P\) \\
Quark family & bottom quark & selected-frame theorem & \(4.183~\mathrm{GeV}\) on the selected public quark frame class \(f_P\) & same selected-frame theorem surface; the exact sextet matches the official PDG 2025 API running-quark target surface \\
...
Hadrons & proton & strong-binding construction absent; empirical closure policy emitted & no first-principles emitted prediction & first-principles prediction requires a working OPH hadron construction, such as GLORB/Echosahedron, plus a Ward-projected spectral-measure export and production systematics; empirical closure rows use separate \(e^+e^-\to\mathrm{hadrons}\) input \\
...
explicit coherent refinement ladder, symmetry, and finite-fiber conditions'' into the realized Standard Model branch. This
...
and local repair instruments. A spherical screen remains an observer-visible regulator chart, not a
...
scaling theorem for the Lorentz/null-modular/Einstein branch. Transportability, the fixed-cutoff bosonic sector category, the refinement/fiber
ladder, and the realized MAR-admissible compact-gauge witness data are theorem-produced in the compact paper. On the central branch the matching gluing
...
ordered cut-pair rigidity, and KMS/BW normalization. The unregularized full-algebra common-floor
route is deliberately not claimed, because off-support directions may collapse without affecting
observer-facing matrix elements.
...
massless carrier zeros are not floating independently of the declared input and theorem checklist. They live on a
...
compact-gauge branch. OPH reconstructs a compact gauge
group from the zero-obstruction edge-sector category produced by the compact paper's
transportability, fixed-cutoff category, and refinement/fiber theorems. The overlap/transport obstruction calculus is the classification stage, not the selection stage. The compact-gauge witness theorem supplies nonempty realized MAR-admissible branch data, and MAR then selects the
...
why some rows are exact structural zeros instead of fitted or accidentally small numbers.
...
First, the photon is massless on the realized electromagnetic branch. On the same branch, the
abelian curvature and current obey the Maxwell equations after canonical electromagnetic
normalization. Second, the gluons are
...
carry their no-go boundary, the neutrino lane closes on a separate weighted-cycle theorem branch, and hadron rows are backend-gated. The bosonic numerical
...
\begin{corollary}[Ward-projected Maxwell normalization]\label{cor:ward_maxwell_normalization}
On the same Ward-projected \(\U(1)_Q\) lane, the electromagnetic field strength and current obey
\[
...
\begin{proof}
The compact reconstruction supplies the ordinary photon branch as the unbroken abelian
\(\U(1)_Q\) factor with charge generator \(Q=T_3+Y\). On an abelian factor the compact-gauge
...
substantive emitted outputs. The exact inverse slice is a compare-only validation surface, and the theorem is confined to the declared D10/D11 quantitative surface instead of promoted into the recovered core or into the selected-class exact quark theorem.
...
determinant trace-lift attachment \(3\mu(r)=S_M(r)\) on the charged determinant channel. The charged determinant channel has a corpus-limited no-go boundary: the uncentered trace lift is not emitted. The neutrino lane closes on the weighted-cycle
...
the exact forward construction. The same-label left-handed selector surface closes to
...
auxiliary direct-top row is a compare-only extraction codomain with a corpus-limited no-go boundary. The Higgs/top branch also carries the same exact
...
\(B_\nu\) and \(\lambda_\nu\), and that branch emits the absolute eV-scale family. The legacy one- and two-parameter adapters
...
The same weighted-cycle absolute-attachment branch implies
\[
\sum_i m_{\nu_i}
=
0.09001192964464505~\mathrm{eV}.
\]
Under standard relic-neutrino inheritance, and absent an explicitly declared extra OPH relativistic
coherence channel, the cosmological-neutrino-background branch uses
\[
N_{\mathrm{eff}}^{\mathrm{OPH}}=3.044,
\]
with the usual broad, low-amplitude normal-ordering free-streaming imprint. This is a
cosmological pressure surface for the weighted-cycle branch, not an input to its theorem proof.
The emitted mass sum sits below the Planck+BAO \(0.12~\mathrm{eV}\) bound~\cite{planck18} but is
exposed to strict DESI DR2 \(\Lambda\)CDM-style bounds near \(0.0642~\mathrm{eV}\) under their
model assumptions~\cite{desidr2nu}. A robust cosmological upper bound below the emitted value,
in a model class that also respects oscillation lower limits and the OPH background assumptions,
would directly pressure the weighted-cycle absolute-mass branch.
...
sector. The source-backend boundary has an empirical closure policy. The empirical closure row class uses a separate
...
grade, and the neutrino derivation does so only on its weighted-cycle theorem branch. It does not
close on a fully flavor-labeled surface, even though it uses the same downstream family machinery.
...
\[
\mathcal A_H=(R_H,\{U_i,\chi_i,J_i,\vartheta_i\},\{G_{ji},F_{ji}\},E,\nabla).
\]
Here \(\chi_i\) are \(H^3_{R_H}\) charts, \(J_i\) are chart domains for record support, and
...
threshold, together with its support, local clock interval, sector payload, and uncertainty collar.
...
Given a valid \(H^3\) record atlas and common clock atlas, chart-natural proto-record extraction,
overlap descent with positive separation margins, real transverse interface-crossing germs,
sector and gauge transport continuity, a frozen ID-independent one-to-one assignment with
\(\Delta_{\mathrm{cert}}>0\), no interaction marker in the free-propagation slab, and a passing
coarse/fine contraction gate, the emitted stitch graph is a unique natural nonbranching
\(H^3\) record-worldline graph. If any positive separation or assignment gap is absent, the
natural output is \(\mathrm{AMBIGUOUS}\); if an interaction marker is present, the output is
\(\mathrm{INTERACTION\ REQUIRED}\); if atlas, interface, or transport gates fail, the stitch is
rejected.
\end{theorem}

\begin{proof}
The atlas and extraction gates make local proto-records presentation-independent. Descent turns
overlap-compatible local components into descended records before any temporal decision is made.
...
lane is closed only on its declared weighted-cycle theorem branch instead of on a fuller
...
backend export. The paper emits the elementary and weighted-cycle rows named above, while the full
...
\emph{Recovering Relativity and Standard Model Structure from Observer Overlap Consistency}~\cite{ophcompact}. Charged-lepton source landing, fuller flavor-labeled neutrino, and source-only hadron mass theorems sit beyond the closed structural core; failure in those continuation branches would not undo the recovered-core gauge and gravity chain or erase the reported bosonic and quark rows.
...
carrier sectors. The completion pipeline closes or scopes five non-hadron chains: structural
massless carriers, the hierarchy/naturality bridge on the selected \(P/N\) branch, the Higgs/top row
on the declared D10/D11 surface, selected-class quarks on \(f_P\), and neutrino absolute attachment
on the weighted-cycle branch. The ledger also marks four separate row classes: the candidate
...
Ward-projected Thomson lane supplies the fine-structure endpoint on the declared \(P\)-closure
surface. In the charged lane the theorem surface carries an exact
