...
charm quark, and bottom quark. Charged leptons and flavor-labeled neutrinos are
open for different reasons: the charged lane fixes the centered hierarchy but not the absolute
normalization, while the weighted-cycle neutrino branch lands on the observed PMNS/hierarchy
pattern and leaves one overall positive normalization scalar open. Hadrons are execution-bound:
...
branch. The public paper ledger has no separate D11 row. The compact paper's
...
This section should be read by claim tier rather than by raw numerical proximity. The same absolute residual can mean either a closed emitted value or a compare-only continuation benchmark. The closed emitted-output set on this table is the electroweak calibration pair \(m_W\) and \(m_Z\). Charged-lepton near-matches remain compare-only continuation diagnostics, and the weighted-cycle neutrino branch remains one positive normalization scalar short of promoted absolute masses.
...
\]
These anchored absolute numbers are compare-only because the theorem branch lacks the
...
important honesty condition in the paper. At the deepest flavor surface used here, family
...
neutrino derivations do not do so at theorem grade, even though they use the same downstream family
machinery.
