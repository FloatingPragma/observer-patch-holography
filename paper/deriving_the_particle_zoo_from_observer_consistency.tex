...
\]
The equation is solved as a self-reference: the candidate pixel emits the electromagnetic readout
that corrects the same candidate pixel. Measurements may locate the branch interval; the value
counted by OPH is the unique closure endpoint of the declared map. The main achievements are the
charge-quantized particle skeleton, exact structural zero masses for photon/gluon/graviton
carriers, the source-side fine-structure diagnostic, the Maxwell-normalized electroweak transport
surface, the selected-branch electroweak hierarchy/naturality solution with \(\epsilon_H=0\), the
capacity-electroweak bridge, the Higgs/top split theorem on its declared quantitative branch,
selected-class quark rows, and a weighted-cycle neutrino branch. Hadrons and charged-lepton
absolute masses are work in progress. Compare-only validation rows are marked as comparison rows,
and the geometric \(12\)-port origin of the bridge exponent is the icosahedral screen-sieve theorem.
\end{abstract}
...
\(m_{\rm rep}=2(8+3+1)=24\), so the local/global hierarchy-resonance package
closes on the selected branch. The promoted hierarchy row does not use the
