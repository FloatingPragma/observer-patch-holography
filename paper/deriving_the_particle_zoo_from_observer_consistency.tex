...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a forward calculation from one selected scale. The synthesis paper fixes the local pixel ratio \(P\) by describing the same screen cell from two sides: as one hardware pixel of the simulating screen and as the electromagnetic observation scale available inside the simulated universe. With \(P\) fixed, the particle chain emits a compact package: massless carriers, the \(W\) and \(Z\) masses, the electromagnetic coupling, a Higgs mass, running quark masses with explicit Yukawas, and a neutrino family in the observed hierarchy and mixing regime. The same framework targets the full particle spectrum. Direct zero-momentum electromagnetic closure, charged-lepton absolute masses, and hadrons remain work in progress.
\end{abstract}
