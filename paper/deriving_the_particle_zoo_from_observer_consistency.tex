...
carrier, the conditional MAR economy selection \(N_g=3\), and the connection
...
matter packet. When supplied, a full-rank compact physical current with inner
\(A_5\) action independently classifies the same Lie type. A source-derived intertwiner,
determinant and Spin data, center/deck descent, and refinement naturality are
required before those routes describe one physical current object.
...
finite icosahedral route starts once a source-selected local physical current
with proper \(A_5\) action is supplied and reaches the same Lie type by compact-current
classification. Agreement at Lie type is a cross-check. Equality of the
physical current objects is open, and neither route turns a local
...
N_c=3.
\]
The same compact paper also gives a separate finite icosahedral current-classification route to the
Standard Model Lie type. That route assumes a full-rank compact physical current with inner \(A_5\)
action. Identification with the Tannaka-reconstructed current remains conditional on the stated
source-intertwiner, determinant/Spin, center/deck, and refinement receipts.
...
transmutation coefficient remains conditional on an intertwiner between the
...
finite \(A_5\) current chain meet at the Standard Model Lie type under their
respective premises. Their physical current identity, the source selection of
...
\(\epsilon_H=0\). The weak-boson and Higgs numbers remain chart coordinates.
The conditional QFT-Q3 implication from a complete renormalized packet to
strict charged and neutral complex-pole coefficients is specified. The present
OPH source has not produced that packet: its source-selected action, complete
Yukawas, FJ coordinate~\cite{dudenasLoschner2020,dittmaierRzehak2022},
