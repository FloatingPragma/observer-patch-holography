...
Electroweak bosons & compare-only adapter & D10 electroweak branch plus frozen validation adapter & \(M_W=80.377~\mathrm{GeV}\), \(M_Z=91.18797809193725~\mathrm{GeV}\) & not a live prediction theorem. The candidate \(P\) root awaits the source spectral measure payload, same-scheme remainder, and interval certificate \\
...
Electroweak D10 branch & \(W\) boson & compare-only adapter & \(80.377~\mathrm{GeV}\) on the frozen \(W/Z\) validation surface & not a live prediction theorem while the candidate \(P\) root awaits the source spectral measure payload, same-scheme remainder, and interval certificate \\
...
nontrivial.
For the BW/geometric side, the target is the extracted prime geometric subnet. The compact paper
now packages that bridge on the local GNS support quotient of the transported cap marginals:
support-visible modular transport controls the carried collar errors on each fixed local collar
model, local weak-\(*\) extraction plus GNS gluing emit the scaling-limit geometric cap pair, and
on round caps the surviving cut data determine the cap-preserving hyperbolic subgroup up to
normalization. The old common-floor collapse remains relevant only against a stronger raw
regulator-level unregularized modular-Hamiltonian statement on directions that disappear in the
support quotient; it is no longer the theorem-facing BW obstruction for the extracted cap pair.
...
The final bundle keeps it behind the source spectral measure payload, same-scheme remainder, and interval certificate, so the
...
The \(W/Z\) row is a frozen compare-only adapter; theorem promotion requires the source spectral
measure payload, same-scheme remainder, and interval certificate on the Ward-projected Thomson
lane. In the charged lane the theorem surface carries an exact
