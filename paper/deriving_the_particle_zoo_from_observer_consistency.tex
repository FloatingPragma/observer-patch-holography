...
declared scalar-sector constants: a quartic coupling \(\lambda>0\), a
breaking scale \(v>0\), two coupling constants \(g,g'>0\), and one Yukawa
...
declared potential \(\lambda(|h|^2-v^2)^2\) is exactly the nonzero shell
\(|h|^2=v^2\). The stabilizer of the chosen minimum \((0,v)\) inside the
...
\(g^2v^2/4\) and \((g^2+g'^2)v^2/4\) on the charged and neutral broken
...
with exact coefficient \(yv\), vanishing exactly when \(y=0\) or \(v=0\),
...
theorem selects \(\lambda\), \(v\), \(g'/g\), or \(y\), the committed
...
the separate branches described in their own sections.
...
The declared plane-wave states persist under their declared free evolution
...
this flow a plane-wave state changes by a unit-modulus global phase only,
so its mass-shell membership, its momentum and energy readouts, and its
unit modulus are exact constants of the flow, and two states with distinct
positive mass or momentum labels stay distinct at every parameter value.
Along the state's own frame the global phase is exactly the internal clock,
...
constant state and the frame label is not recoverable, while a future null
vector carries the shell, global-phase-covariance, and norm-persistence
clauses of the packet on the zero-mass shell. This is persistence of a
declared kinematic state under a declared flow, separate from the
record-repair stability results: no physical particle, decay rate,
localized wave packet, or laboratory persistence time is claimed, and the
...
\((\sigma\!\cdot\!X)^2 = |X|^2 - q\,(\sigma\!\cdot\!B)\), so the spin
