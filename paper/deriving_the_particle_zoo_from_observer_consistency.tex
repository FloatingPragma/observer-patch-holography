...
\begin{abstract}
Observer-Patch Holography turns the particle problem into a forward calculation from one selected scale. The synthesis paper fixes the local pixel ratio \(P\) by describing the same screen cell from two sides: as an exterior screen cell and as the electromagnetic observation scale available inside the encoded universe. With \(P\) fixed, the particle chain emits a compact package: massless carriers, the \(W\) and \(Z\) masses, the electromagnetic coupling, a Higgs mass, running quark masses with explicit Yukawas, and a neutrino family in the observed hierarchy and mixing regime. The same framework targets the full particle spectrum. Direct zero-momentum electromagnetic closure, charged-lepton absolute masses, and hadrons remain work in progress.
\end{abstract}

\section{Introduction}

Observer-Patch Holography asks a simple follow-up question after the gauge structure is fixed. Can one local screen constant organize the particle spectrum, or do the masses and mixings remain a bag of unrelated inputs?

The structural OPH papers start from observer consistency on a finite screen. Physical data are organized in local patch algebras, nearby observers must agree on overlaps, realized states obey a local maximum-entropy and refinement principle, generalized entropy is recoverable, and the realized low-energy branch is selected by a minimal admissibility criterion. From that basis one can derive, conditionally but explicitly, the Lorentzian and Einstein branches together with the realized Standard Model gauge structure
\[
...
The particle branch studied here is a reverse-engineering problem in one variable. Instead of treating masses, mixings, and Yukawa parameters as independent inputs, it asks how far one common dimensionless pixel ratio \(P\) can be propagated through the whole spectrum. The working hypothesis is simple: fix one common \(P\) on the outer and inner closure branch of the synthesis paper, then use that same \(P\) through the bosonic, quark, charged-lepton, neutrino, and hadronic lanes. The structural massless carriers, the \(W\) boson and \(Z\) boson pair, the Higgs boson and top quark pair, the selected-class exact quark theorem, and the weighted-cycle neutrino theorem branch with its shared-basis physical Majorana pair are emitted, while charged leptons, flavor-labeled neutrinos, and hadrons remain work in progress. This paper develops that derivation chain from the axioms and \(P\) to the reported particle outputs, while keeping the open closure boundary explicit.
...
branch~\cite{ophsynthesis}, feeds the forward electroweak map and its descendants. The
...
exact sidecars, and continuation lanes are therefore downstream branches of one common OPH premise
...
one mixing angle per sector, but to fix one universal pixel ratio \(P\) from the outer/inner
closure relation described in the synthesis paper and then demand that the same \(P\) drive all
...
variable for the entire downstream spectrum. In the synthesis paper, \(P\) is fixed by matching
the outer pixel detuning to the inner electromagnetic observation scale emitted by the same cell.
The outer side of the closure is
\[
P=\varphi+\alpha_{\mathrm{in}}(P)\sqrt{\pi}.
\]
For comparison, the 2022 CODATA/NIST central value
\(\alpha^{-1}(0)=137.035999177\) gives
\[
P=1.630968209403959\ldots
\]
through this outer formula. This is compare-only metadata until the same number is emitted by the
declared OPH fixed-point map.
A separate pending hardware note reports an optical-cavity check of the same fixed-point geometry;
this is treated as corroborating engineering evidence.
Once that fixed point is selected, the particle question becomes whether all downstream readouts
can be written as
\[
X_j = G_j(P)
\]
...
This is why the electroweak branch is not a side detail. It is the place where the particle derivation
...
target-free source-only transport theorem from the source basis fixed by the forward solve. The
...
surface. On the quantitative-closure tier, the electroweak mass-side lane has no open blocker.
...
come from electroweak quantitative closure; and the Higgs boson sits on the Higgs/top critical stage. Those
...
Electroweak D10 branch & \(W\) boson & quantitative closure & \(80.37700001539531~\mathrm{GeV}\) from the closed target-free D10 repair theorem on the source basis fixed by the forward solve & closed D10 electroweak row; freeze-once validation sidecar yields the exact \(W/Z\) pair \\
Electroweak D10 branch & \(Z\) boson & quantitative closure & \(91.18797807794321~\mathrm{GeV}\) from the same closed target-free D10 repair theorem & same closed D10 electroweak theorem; exact \(W/Z\) sidecar sits on the frozen validation surface \\
Higgs/top critical stage & Higgs boson & quantitative theorem & \(125.1995304097179~\mathrm{GeV}\) on the declared D10/D11 running, matching, and threshold surface & exact source-only Higgs/top split theorem; compare-only inverse Higgs/top slice remains on the same Jacobian surface \\
...
On the D10 quantitative-closure branch, forward prediction means exactly that ordering: first fix the shared pixel input \(P\) on the
...
closure; they are forward outputs of the closed D10 Phase-II quantitative-closure theorem.
...
Using the source-locked D10 anchor \(a_0=128.30576920234813\), the currently displayed
...
\]
The repository's \texttt{code/P\_derivation} witness path records this as a numerical fixed-point
candidate and keeps the CODATA/NIST value as compare-only metadata rather than as an input to the
closure solve.
...
substantive emitted outputs. The exact inverse slice remains a compare-only validation surface, and the theorem is confined to the declared D10/D11 quantitative surface rather than promoted into the recovered core or into the selected-class exact quark theorem.
...
\midrule
\(W\) boson & D10 electroweak branch & quantitative closure & \(80.37700001539531~\mathrm{GeV}\) \\
\(Z\) boson & D10 electroweak branch & quantitative closure & \(91.18797807794321~\mathrm{GeV}\) \\
Higgs boson & Higgs/top critical stage & quantitative theorem & \(125.1995304097179~\mathrm{GeV}\) \\
Top quark & Higgs/top critical stage & quantitative theorem; direct-top bridge open & \(172.3523553288312~\mathrm{GeV}\) \\
\bottomrule
\end{tabular}
\end{table}
...
exact centered readback, a closed common-shift no-go, the live same-label \(q_e\) readback, a
...
The charged-lepton derivation differs from the quark derivation in a precise way. It does not emit public charged masses from \(P\). It does emit an exact same-family witness, a closed common-shift no-go, the live same-label \(q_e\) readback, a conditional determinant-line lift on theorem-grade physical charged data, and a downstream algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\). For any fixed formal source exponent vector \(M_\bullet^{\mathrm{ch}}\), the same-label readback defines a source-side determinant character
\[
S_M=\sum_e M_e^{\mathrm{ch}}\log q_e.
\]
...
The hadron derivation is the most operationally demanding part of the whole particle program. The
...
available dictionary and comparison surfaces. Where that dictionary is still open, the paper states
...
then emits quantitative bosonic rows through electroweak quantitative closure and the Higgs/top critical
...
from the D10 quantitative descendants of \(P\) to physical charged data is open.
