...
\item keep the octahedral \(\mathbb Z_2/S_3\) simulator at appendix-level calibration status;
...
spherical cellulation or a digital finite-group simulator, as a calculational chart. Their claim
...
does not supply the Clay-admissible construction.
Conventional free-field or lattice-gauge vacuum baselines used for simulator calibration are
reference ensembles only. They are not OPH-native vacuum promotion unless the quotient-ensemble
selector and transfer/reflection-positive gate are supplied on the compact theorem surface.
...
evidence bundles exist. Before those bundles exist, it is an architectural target and a design
motivation. Source-only hadron theorem status requires the backend evidence listed above.
...
\begin{quote}
The simulator validates the finite interface logic. The primary physical picture is the
