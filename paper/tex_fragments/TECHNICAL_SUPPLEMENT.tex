...
\subsection{Geometric Modular Flow}\label{22-geometric-modular-flow}
...
\]
where \(\lambda_\ast>0\) is the lower spectral bound used to compare modular Hamiltonians. Every later ``exact'' collar formula is to be read either at literal exact Markovity or along a controlled collar family with
\[
\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta)\to0
\qquad(\delta\downarrow0),
\]
while the finite-stage errors \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(\eta_\delta^{\mathrm M}\) are carried explicitly.
...
Assume the OPH axioms, the regulator premises \(R0\) and \(R1\), the local MaxEnt/refinement-stability package, the collar-control hypotheses used in the supplement, and the same branch boundary as \textbf{T1}+\textbf{T3} in the main text. Assume in addition the branch condition \textbf{T1} that the realized refinement-stable scaling branch for caps and their conformal images lies in the Bisognano--Wichmann geometric modular class. Then:
...
\item for every realized scaling-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\) on that branch,
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(2\pi t)};
\]
equivalently \(s=2\pi t\);
\item if the scaling-limit cap algebra happens to remain type I, item \textup{(ii)} may be represented as \(K_C=2\pi B_C\); in the generic continuum case the limit algebra has left the regulator class and the geometric modular action is generally outer rather than inner.
\end{enumerate}
\end{theorem}

\begin{proof}
The fixed-cutoff collar theorems localize the modular defect to the shrinking collar and control the replacement of the physical collar state by an exact-Markov reference with the carried errors \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(\eta_\delta^{\mathrm M}\). By \textbf{T3} the theorem is about the scaling-limit cap pair rather than the finite regulator matrices. By \textbf{T1}, on the selected branch the scaling-limit modular automorphism group is the standard cap-preserving conformal flow \(\alpha_{\lambda_C(s)}\) up to normalization:
\[
\sigma_t^{\omega_\infty^C}=\alpha_{\lambda_C(\kappa_C t)}.
\]
The modular KMS condition together with the null-blow-up normalization \(v\mapsto e^{-s}v\) fixes \(\kappa_C=2\pi\). If the limit algebra remains type I, one may represent the automorphism by a modular Hamiltonian and write \(K_C=2\pi B_C\); otherwise the automorphism identity is the full statement.
\end{proof}

\begin{remark}[BW boundary and the remaining scaffold]
The theorem proves geometric modular flow only on the \textbf{T1}-selected scaling branch. It does \emph{not} prove that MaxEnt alone selects that branch, nor that the continuum state is a vacuum or canonical cap state. When the manuscript informally uses the phrase ``BW/canonical cap phase,'' it means only this selected scaling-limit branch, and in the generic non-type-I case its geometric modular action is outer.
...
\begin{corollary}[Lorentz kinematics on the BW branch]
...
\]
This common dictionary underlies the quark mass-side continuation, the charged same-carrier readback theorem, and the intrinsic neutrino eta-chain. The older \(6^{-n}\) suppression rule is at most an extra uniform-center-label collapse of this richer dictionary and is not a recovered-core theorem.
...
\mathcal R_{D12}^{ud}.
\]
That ray is the exact same-family mass object emitted by the corpus. Any numerical scale choice on it remains compare-only until an intrinsic ray law is derived.

The mixing-side transport
\[
V_{\mathrm{CKM}}^{\mathrm{fwd}}=U_u^\dagger U_d
\]
is also closed on the same selected D12 sheet, but that does not produce a physical branch change. On the emitted same-label left-handed solver surface the selector problem is closed in the negative sense: the local orbit collapses to the singleton \(\sigma_{\mathrm{ref}}\) (\ophid{sigma_ref}). Since \(\sigma_{\mathrm{ref}}\) is the wrong D12 CKM sheet, no theorem-grade branch-changing route to the physical CKM shell is emitted by the corpus. The exact quark object is the intrinsic ray law \ophid{intrinsic_scale_law_D12} on \(D12_{ud}^{\mathrm{mass}}\) (paper id \ophid{D12_ud_mass_ray}). Separately, the builder-facing pure-\(B\) payload pair beneath the reported quark rows remains open.
 A target-anchored same-family exact witness reproduces the six quark reference masses exactly on the same ordered three-point family, but it remains same-family-only and does not remove the wrong-branch CKM boundary or emit the intrinsic ray law.

\subsection{Charged and neutrino continuation boundary}

The charged lane is exact only up to centered same-carrier readback: once a charged source pair exists, the centered charged logs are fixed, but the theorem does not emit absolute masses. The exact sidecar on that surface is the same-family witness \((m_e,m_\mu,m_\tau)=(0.00051099895,0.1056583755,1.7769324651340912)\,\mathrm{GeV}\), and its caveat is exactly that it remains same-family-only. The theorem-grade waiting set is promotion of \(\widehat C_e^{\mathrm{cand}}\) and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), from which the uncentered lift, determinant-line section, and affine anchor follow canonically.

The neutrino lane closes the PMNS/hierarchy shape and the normalized same-label overlap-defect weight section. The remaining exact object is the reduced bridge-correction invariant \(C_\nu\); the paper-facing amplitude \(B_\nu\) is reconstructed only above that smaller invariant. A compare-only two-parameter continuation adapter on the explicit positive selector segment is on disk and hits the representative central splittings exactly, with \((m_1,m_2,m_3)=(0.01745663295,0.01948419960,0.05308139066)\,\mathrm{eV}\), but it remains non-promotable while \(C_\nu\) stays open. So the charged and neutrino lanes remain open for different reasons: charged leptons miss an emitter upgrade and one descended physical affine scalar, while neutrinos miss one reduced positive bridge scalar after the shape closure.
...
The waiting set is explicitly two-layered:
\[
...
The corpus proves neither exact vanishing nor uniform quadratic smallness of that descended commutator after the central split, so \(\widehat C_e^{\mathrm{cand}}\) is not promoted to \(\widehat C_e\).

The second arrow is not a fresh fitted scalar. Once a theorem-grade \(\widehat C_e\) and refinement-stable physical \(Y_e\) are available, the irreducible post-promotion object is the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\). The uncentered lift, determinant-line section, and affine anchor then follow canonically:
\[
...
\subsection{Compare-only continuation boundary}\label{134-the-koide-phase-from-oph}

Balanced Hermitian-circulant, \(Q=2/3\), and \(\delta=2/9\) relations may be used as compare-only continuation surfaces for centered-log audits. They do not remove either layer of the theorem-grade obstruction above. In particular, they do not promote \(\widehat C_e^{\mathrm{cand}}\), and they do not supply the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\). The public claim of this section is therefore only the exact centered readback law, the closed common-shift no-go, and the explicit two-layer remaining frontier.
