...
The collar technology determines the controlled approach to an exact Markov normal form, but by itself it does not force a specific continuum modular geometry. The theorem-scope must therefore be stated as an OPH branch statement. The sharp claim is: if the realized refinement-stable scaling branch lies in the Bisognano--Wichmann geometric modular class for caps, then that branch supplies the standard cap-preserving conformal dilation and the theorem fixes only its \(2\pi\) normalization together with the carried collar remainder.
...
Assume the OPH axioms, the derived fixed-cutoff regulator/collar package proved earlier in this supplement, and the controlled scaling-limit scope clause \textbf{T3}. Assume in addition the branch condition \textbf{T1} that the realized refinement-stable scaling branch for caps and their conformal images lies in the Bisognano--Wichmann geometric modular class. Then:
...
K_C=2\pi B_C.
\]
In the generic continuum case, the automorphism identity in item \textup{(ii)} is the full theorem statement and the geometric modular action is generally outer rather than inner.
\end{enumerate}
...
\]
The best closed constructive local object beneath that reduced scalar is still the defect-weighted same-label edge family
\[
q_e=\sqrt{g_e d_e},
\qquad
\eta_e=\log q_e-\frac13\sum_f \log q_f,
