...
\textbf{Internal small-ball bridge.} In the local Lorentzian d=4 scaling regime, the small-ball kernel is supplied by the OPH gravity chain itself. On the BW branch, a sufficiently small cap is the tangent causal diamond D_\ell, whose preserving conformal Killing field is
\[
\xi_{D_\ell}
=
\frac{1}{2\ell}
...
Together with Proposition~8.2 and Theorem~8.3, this makes the intended theorematic split explicit: D5 determines the local Einstein branch only modulo \(\Lambda g_{ab}\), and D6 fixes the remaining constant by the global screen-capacity input.
