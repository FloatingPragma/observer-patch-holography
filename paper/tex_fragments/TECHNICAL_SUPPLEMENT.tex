...
\textbf{What remains external.} This internalization claim stops at the fixed-cutoff local-Gibbs/Lieb--Robinson/refinement package. Quantitative exponential mixing or exact-Markov replacement, when invoked later, is separate state information tied to the recoverability side of Axiom 4 rather than to Axiom 3 alone. The genuinely external technical premises are therefore exactly the canonical list \(T1\)--\(T6\): the controlled scaling-limit scope clause, fixed-cap stationarity, and the explicit gauge-side package used in ordinary compact reconstruction. Within that package, \(T1\) keeps the loop-coherence branch criterion explicit on the central compact-group route, while the genuinely noncentral fixed-cutoff branch is handled separately by higher-gauge data rather than by the ordinary DR theorem. The geometric cap-pair extraction and ordered cut-pair rigidity inputs remain theorem-local UV objects rather than separate global labels. No separate regulator premise for quasi-local propagation or refinement stability survives once Axiom 3 is unpacked in this way.
...
\textbf{D8} & product structure up to finite quotient on the realized branch & compact Lie representation classification; Schur's lemma & the same ordinary or central-defect bosonic branch as D7, together with MAR, a connected admissible class, one connected abelian factor, and faithful action on the minimal coupled carrier & Phase I realized-branch theorem \\
