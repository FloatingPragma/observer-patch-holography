...
\begin{corollary}[Lorentz kinematics on the extracted geometric subnet]\label{cor:lorentz}
...
The fixed-cutoff null bridge reaches the derived half-sided-inclusion step and the exact half-line generator/charge identification. What is established here is: transferred cut-center data on null strips, exact-or-controlled strip additivity on one inherited strip model, endpoint-Lipschitz control of the renormalized half-line family, and the geometric blow-up argument that turns the null half-line net into a half-sided modular pair, so that Borchers--Wiesbrock yields an explicit positive null-translation generator on its Stone domain~\cite{wiesbrock93}. What remains downstream is only bounded-interval transport through the separate projective branch and the later tensor upgrade modulo the null-invisible metric term. This theorem-local package should therefore be read with the following split.
...
\textbf{Consequence.} This theorem-local package proves the fixed-cutoff strip bridge through the weak tail generator \(q(a,\Omega)\), derives half-sided modular inclusion on the null half-line blow-up net, and therefore identifies an explicit positive null-translation generator \(P_\Omega\) together with its Stone domain and half-line modular-Hamiltonian relation. The effective null-stress charge on that same half-line family is identified with \(P_\Omega\) in the next subsection. The remaining downstream burdens are the separate bounded-interval projective branch and the tensor upgrade modulo the null-invisible metric term.
\subsection{2.4 Stress Tensor Reconstruction}\label{24-stress-tensor-reconstruction}

\textbf{Generator/charge identification.} The null bridge identifies the positive OPH half-line generator with the effective null-stress charge itself. On the OPH side,
\[
...
\langle\psi,\widetilde K_a(\Omega)\phi\rangle
\]
on \(D(K_0(\Omega))\cap D(P_\Omega)\). In the effective spacetime description of that same renormalized half-line family, the local null-stress charge is defined by the identical endpoint derivative,
\[
Q_{kk}(\Omega)
:=
...
\]
as a quadratic-form identity on the common domain. The only imported continuum input is the standard local null modular-Hamiltonian formula for the same half-line family; there is no longer a separate null-stress identification premise. The remaining tensor-level issue is the standard null-invisible metric ambiguity described below. Accordingly, any later appeal in the supplement to a ``null-stress identification'' should be read only as this local modular-Hamiltonian input for the same half-line family, not as an extra operator-level assumption.

\textbf{Lemma 2.3 (Null Data Ambiguity):} If a symmetric tensor \(X_{ab}\) satisfies
\[
X_{ab}k^a k^b = 0
\]
for all null \(k\), then
...
\emph{Proof:} Decompose
\[
X_{ab}=Y_{ab}+\phi g_{ab}
\]
with \(Y\) traceless. For null \(k=(1,\hat n)\),
\[
X_{kk}=Y_{kk}+\phi g_{kk}=Y_{kk}
\]
since \(g_{kk}=0\). Expanding,
\[
Y_{kk}=Y_{00}+2\hat n^iY_{0i}+\hat n^i\hat n^jY_{ij}.
\]
If this vanishes for all \(\hat n\in S^2\), each angular moment (scalar, vector, traceless tensor) vanishes independently, so \(Y_{ab}=0\). Hence \(X_{ab}=\phi g_{ab}\). \(\square\)
...
\textbf{Reconstruction Formulas:} From
\[
f(\hat n):=T_{kk}(\hat n),
\]
one obtains
\[
T_{0i}=\frac{3}{2}\langle \hat n_i f\rangle_{S^2},
\]
and
\[
T_{ij}-\frac{\delta_{ij}}{3}T^{(\mathrm{spatial})}_{kk}
=
\frac{15}{2}
\left\langle
\left(
\hat n_i\hat n_j-\frac{\delta_{ij}}{3}
\right)f
\right\rangle_{S^2}.
\]

Thus the directional modular/null charges determine the effective stress tensor up to the unique null-invisible term \(\phi g_{ab}\). That metric ambiguity is the only residual tensor-level freedom carried into the Einstein step; the half-line generator/charge identification itself is exact.
...
\textbf{Internal small-ball bridge.} In the local Lorentzian d=4 scaling regime, the small-ball kernel is supplied by the OPH gravity chain itself. On the extracted prime geometric subnet, a sufficiently small cap is the tangent causal diamond D_\ell, whose preserving conformal Killing field is
\[
\xi_{D_\ell}
=
\frac{1}{2\ell}
...
\]
The bounded-interval formula from the null modular bridge gives the same affine-covariant quadratic weight on each null generator of the diamond, so no separate EFT small-ball first law is imported. The bulk first-law term therefore obeys
\[
...
\]
If \(\delta\langle T_{00}\rangle\) is approximately constant across the ball and
\[
...
\textbf{Theorem 2.5 (Conditional Jacobson-type rest-frame relation).} In the same local Lorentzian scaling regime, under generalized-entropy stationarity, the half-line generator/charge identification of the null bridge together with the bounded-interval transport input used in the local Lorentzian regime, the internal small-ball bridge above, and admissible first variations about a maximally symmetric reference state, one obtains
\[
\delta\!\Bigl[(G_{ab}+\Lambda g_{ab})u^a u^b\Bigr]
=
8\pi G\,u^a u^b\,\delta\langle T_{ab}\rangle,
\]
where \(u^a\) is the future-directed unit tangent of the local diamond rest frame. In the adapted frame \(u^a=(1,0,0,0)\), this is
\[
...
\def\labelenumi{\arabic{enumi}.}
\item
Use the edge-center split to write
\[
...
\]
\item
Insert the internal small-ball bridge:
\[
\delta S_{\mathrm{bulk}}(C)
=
...
\]
\item
Use the standard fixed-volume area-variation identity for a d=4 geodesic ball:
\[
...
\]
\item
Impose \delta S_{\mathrm{gen}}(C)=0, divide by \ell^4, and let \ell\to0. The derivative remainders vanish, leaving the displayed rest-frame relation.
\end{enumerate}

\textbf{Internal tensor upgrade.} Define
\[
Y_{ab}:=G_{ab}+\Lambda g_{ab}-8\pi G\,\langle T_{ab}\rangle.
\]
Overlap consistency plus the Lorentz branch imply that overlapping observer frames through the same point realize every local timelike unit vector u. The rest-frame relation therefore gives
\[
...
\]
for all \(|\vec v|<1\). The polynomial vanishes on an open ball, so its coefficients vanish:
\[
\delta Y_{00}=0,\qquad
\delta Y_{0i}=0,\qquad
\delta Y_{ij}=0.
...
Along any connected scaling branch of reference states, \(Y_{ab}\) is therefore constant. By Lemma 2.3, the only purely local ambiguity left by null reconstruction is a metric term. Fixing that term on one maximally symmetric reference state gives
\[
G_{ab}+\Lambda g_{ab}=8\pi G\,\langle T_{ab}\rangle.
\]
So the scalar-to-tensor upgrade is internal to OPH once the geometric branch, the null bridge, and the overlap-complete timelike family are in place.
...
\textbf{Interpretation:} When the collar conditional mutual information is small, the "interior partner" is approximately recoverable from outside + radiation. It is encoded rather than an independent tensor factor.
...
\textbf{Scope.} The structural claim of this section is limited to the edge-center obstruction to naive inside/outside factorization, the small-CMI recoverability statement for interior encoding, the Hawking/KMS normalization, and the discrete area spectrum together with \(\Delta E = k_B T_H \ln(d'/d)\) for an actual sector transition. Integer-transition combs, QNM selectors, linewidth estimates, PBH/LIGO templates, and any Page-curve or island closure require additional premises and are not proved in this supplement.
...
\]
This is the full structural OPH input used for proton-spin bookkeeping.
...
\]
This is not a theorem. The assignment of one power of \(\varepsilon\) per zero mode is not derived, the overall rate prefactor is unknown, and the comparison with observed \(\eta_B \approx 6.1 \times 10^{-10}\) omits transport, washout, and freeze-out. It should therefore be read only as a heuristic continuation benchmark.
...
This section keeps one imported structural corollary and the D12 flavor claim-tier boundary on one surface. The structural corollary is the realized D9 counting chain \(N_g=3\). The D12 advance is narrower: the corpus puts the quark mass-side continuation, the charged same-carrier readback theorem, and the intrinsic neutrino eta-chain on one common excitation dictionary. The current boundary consists of named continuation closures above that dictionary. They are not independent Yukawa parameters, and they do not belong to the recovered-core theorem surface.
...
\mathcal R_{D12}^{ud}.
\]
That ray is the exact same-family mass object emitted by the corpus. The mass-side theorem boundary is the scalar law
\[
...
\]
So the D12 mass-side package is functorial:
\[
...
\]
with \((\sigma_u,\sigma_d)=(5.5905,3.3049)\). The builder-facing pure-\(B\) payload pair is the smaller constructive object beneath this theorem, and \ophid{intrinsic_scale_law_D12} is the derived wrapper above \(\Theta_{ud}^{\mathrm{mass}}\).

The physical-sheet theorem boundary is the lift
\[
\Theta_{ud}^{\mathrm{phys}}
\]
to the same-label left-handed carrier
\[
...
\right\}/\!\sim,
\]
with diagonal rephasing equivalence
\[
(U_{u,L},U_{d,L},V)\sim
(U_{u,L}D_u,\ U_{d,L}D_d,\ D_u^\dagger V D_d).
\]
The emitted same-label left-handed solver surface closes to the singleton \(\sigma_{\mathrm{ref}}\) (\ophid{sigma_ref}), and \(\sigma_{\mathrm{ref}}\) is the wrong D12 CKM sheet. Same-sheet rephasing preserves CKM moduli, so the missing physical object is a sector-attached section of the common-refinement frame-overlap quotient into \(\Sigma_{ud}^{\mathrm{phys}}\), not a second local selector search.

The absolute-readout theorem boundary is
\[
\Theta_{ud}^{\mathrm{abs}},
\]
the target-free promotion of the affine sector-mean split to the physical sheet. On the current-family selected sheet, its formula package is
\[
...
The exact closure package is therefore three-step:
\[
\Theta_{ud}^{\mathrm{mass}}
\Longrightarrow
\bigl(\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}},\kappa_Q,\tau_u,\tau_d\bigr),
\]
\[
\Theta_{ud}^{\mathrm{phys}}
\Longrightarrow
...
\textbf{Maximal theorem-emitted quark package theorem.} Let
\[
P:=p_a+\text{(Axioms 1--5)}+\text{Assumption 6}
+\text{the emitted OPH nodes D1--D10}
+\text{the current public D12 quark objects}.
\]
Then the present corpus theorem-emits exactly the following quark-side package: the D12 mass ray \(D12_{ud}^{\mathrm{mass}}=\mathcal R_{D12}^{ud}\) with unresolved modulus \(\lambda=\mathrm{ray\_modulus}=t_1\); the negative selector closure \(\sigma_{ud}=\sigma_{\mathrm{ref}}\) on the emitted same-label left-handed local solver surface; and the restricted-scope affine mean package with \(g_{\mathrm{ch}}=0.9231656602589082\) on \texttt{shared\_budget\_only} and \(g_u=0.7797392875757557\), \(g_d=0.12172551081512113\) on \texttt{current\_family\_only}, using \(\sigma_u=5.5905\), \(\sigma_d=3.3049\), \(\sigma_{\mathrm{seed}}^{ud}=4.4477\), \(\eta_{ud}=1.1428\), \(A_{ud}=0.2467442927388686\), and \(B_{ud}=0.8125616987157023\). No stronger physical closure follows from that theorem-emitted package: \(P\) emits neither \(\Theta_{ud}^{\mathrm{mass}}\), nor \(\Theta_{ud}^{\mathrm{phys}}\), nor \(\Theta_{ud}^{\mathrm{abs}}\). Therefore the exact same-family sextet and the D12 internal backread scalar package are sidecar representatives on their declared surfaces, not \(P\)-emitted physical-sheet theorem outputs.

\textbf{Minimal-extension closure theorem.} The exact minimal extension triple above that theorem-emitted package is
\[
H_{\mathrm{mass}}
\;:\;
...
\]
With these three objects adjoined, the quark lane closes formally. First,
\[
t_1=\frac56\,\ell_{ud}
\]
emits \(\Theta_{ud}^{\mathrm{mass}}\) and therefore fixes \(\Delta_{ud}^{\mathrm{overlap}}\), \(\eta_Q^{\mathrm{centered}}\), \(\kappa_Q\), and the odd package. Second,
\[
\Sigma_{ud}^{\mathrm{phys}}
:=
s_{ud}^{\mathrm{phys}}([F_0^\dagger F_1])
\]
emits \(\Theta_{ud}^{\mathrm{phys}}\). Third,
\[
\log g_u
=
...
A_q^{\mathrm{phys}}-(A_{ud}\sigma_{\mathrm{seed}}^{ud}+B_{ud}\eta_{ud})
\]
emit \((g_u,g_d)\), hence \(\Theta_{ud}^{\mathrm{abs}}\), and the ordered three-point quadratic readout then emits the physical sextet. So
\[
...
\]
and within that same-family scope the selected-sheet exact readout closes through \ophid{oph_quark_current_family_selected_sheet_exact_closure}, built on \ophid{oph_quark_current_family_quadratic_readout_theorem}. The exact closure package above that sidecar has three missing theorem objects: \ophid{quark_d12_t1_value_law}; a sector-attached same-label left-handed lift to the physical CKM shell; and a target-free absolute sector readout theorem emitting \((g_u,g_d)\) on that physical sheet. The same-family exact witness does not remove the wrong-branch CKM boundary, does not emit the one-scalar law, and does not emit target-free physical-sector scales.
A separate D12 internal backread sidecar is also on disk. On that continuation-only surface, feeding the emitted reference-free forward light-quark pair through the coefficient-ratio identification \(c_u/c_d=y_u/y_d=m_u/m_d\) and the D12 normalization \(\Delta_{ud}^{\mathrm{overlap}}=\tfrac16\log(c_d/c_u)\) fixes \(\Delta_{ud}^{\mathrm{overlap}}=0.13431740757663183\), \(t_1=0.6715870378831591\), \(\eta_Q^{\mathrm{centered}}=-0.018210067243314802\), and \(\kappa_Q=-0.012436796997836279\). That sidecar closes the D12 mass-side scalar package numerically on its own continuation surface, but it does not replace the public theorem frontier and does not remove the wrong-sheet CKM boundary.
...
The second layer is a descent contract rather than a second fitted invariant: if theorem-grade \(\widehat C_e\) and an admissible refinement-stable uncentered lift exist, the physical identity-mode equalizer forces the descended scalar \(\mu_{\mathrm{phys}}(Y_e)\).
...
\]
On the weighted-cycle bridge-rigid branch, the physical reduced bridge invariant is
\[
C_\nu=G^2\,Q\,S^{-1/2}=0.9994295999075177,
\]
with
\[
G:=\mathrm{sum\_gap},\qquad
Q:=\mathrm{prod\_qbar},\qquad
...
\]
so every PMNS observable and every dimensionless hierarchy ratio propagate to one emitted absolute family. Numerically,
\[
B_\nu=6.696004159297337,
\qquad
\lambda_\nu=1.7237014208357415.
\]
The emitted absolute family is
\[
...
\]
but that family does not emit \(C_\nu\) or the induced \(B_\nu\) by itself. The exact two-parameter positive-segment adapter, the bridge corridor, and the reduced-correction candidate audit remain on disk only as diagnostic surfaces beneath the theorem pair \((C_\nu,B_\nu)\). The direct correction audit continues to record the target-containing reduced window \(C_\nu\in[0.9994296,1.0015784]\), which induces the sharpened paper-facing corridor \(B_\nu\in[6.69600,6.71040]\) with midpoint \(6.70320\); the earlier three-route bridge corridor \([6.69315,6.71259]\) remains as a wider comparison envelope, and the shortlist-consensus window \([6.70484,6.71259]\) remains diagnostic-only because it misses the compare-only target. So the charged and neutrino lanes split cleanly for different reasons: charged leptons miss an emitter upgrade and one descended physical affine scalar, while the weighted-cycle neutrino branch carries an emitted theorem-grade absolute family and retains only diagnostic sidecars beneath that theorem lane.
...
\textbf{D3} & geometric modular flow and Lorentz branch & Bisognano--Wichmann modular geometry; conformal-group classification & T2 together with the theorem-local geometric cap-pair extraction and ordered cut-pair rigidity hypotheses of Theorem~\ref{thm:bw} & Phase I conditional scaling-limit theorem \\
