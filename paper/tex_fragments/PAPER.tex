...
\textbf{Refinement-stable MaxEnt branch.} Choose any family of coarse-graining channels \(\Phi_{\ell\to L}\) between UV scale \(\ell\) and IR scale \(L\) that is compatible with the third OPH axiom, including its refinement-closure clause. Then on the realized branch one may write
...
\begin{quote}
\textbf{Lemma 2.6 (MaxEnt with local constraints implies local Gibbs form).} Under the local MaxEnt branch of Axiom 3, with its homogeneous global-sum constraints \(\langle\sum_x O_a(x)\rangle = C_a\), and the finite-dimensional regulator realization above, the MaxEnt state has the Gibbs form
...
observer-supporting refinement branch satisfying the five OPH axioms, the derived fixed-cutoff
regulator/collar/consensus package, the finite cap-normal BW certificate above, and independently
...
\subsubsection{Finite Standard Model gauge packet and the diagnostic complexity order}\label{62-selecting-the-sm-factors-derived-from-mar}
...
\right),
\]
then any refinement channel \(\Phi_{\ell\to L}\) compatible with Axiom 3, including its refinement-closure clause, acts on the realized branch by an induced finite-dimensional map
\[
...
\subsubsection{The conditional generation window from CKM CP capability and weak-sector completability}\label{64-generation-number-from-cp-violation-and-refinement-stability-derived-from-mar}
