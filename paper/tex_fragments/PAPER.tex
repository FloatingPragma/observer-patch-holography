...
content is not selected by any axiom: generation count, scalar multiplicity, and
...
inside that window and the exclusion of extra light sectors are open. Physical matter typing,
...
give \(3\le N_g\le5\); the displayed \(N_g=3\) value and the absence of extra light sectors
enter through the declaration and are open as physical statements: the
generation count is a physical attachment inside the window, and source completeness for extra
light sectors is a separate open theorem. The one-Higgs scalar is an assumption in this
...
complex rank-45 attachment, Spin/locality, residue, excluded-band, and
...
6.9 provides the conditional generation window; the displayed completion is a declaration. Equality with the independently
...
\tightlist
\item
  The clause set selects a \textbf{single integer} inside the admissible window \(\{3,4,5\}\); clause (v) (CKM CP capability) is a declared input that sets the lower edge, counted in the closure ledger.
\item
...
A separately declared three-family completion displays \(N_g=3\) inside the conditional window;
the physical generation count and source completeness for extra light sectors are open.
...
conditional window; the displayed \(N_g=3\) is a declaration inside it. The
