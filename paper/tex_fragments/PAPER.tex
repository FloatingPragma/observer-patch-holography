...
\textbf{Key conditionality.} The Lorentz/null-modular/Einstein package is a scaling-limit statement rather than a literal fixed-cutoff claim. Section 5 now removes any separate propagation, endpoint-control, or half-sided modular-inclusion premise from the fixed-cutoff null bridge and closes that bridge through the derived positive null-translation stage; the remaining explicit burdens are the density upgrade and the relativistic null-stress identification. The gauge group reconstruction yields \emph{a} compact group from a refinement-stable sector colimit; the explicit structural-economy selector MAR (Minimal Admissible Realization) then selects the SM gauge group \(SU(3) \times SU(2) \times U(1)/\mathbb{Z}_6\), \(N_c = 3\), and \(N_g = 3\) once the admissible class includes one connected abelian charge factor (see §6.2 and Part II of this manuscript).
...
\textbf{A4}, \textbf{F}, \textbf{MX} & Recoverability and the theorem-local collar/null-strip hypotheses used under Canonical 4; use: Sections 2 and 5; status: theorem-local shorthand for explicit local hypotheses, not a separate axiom package. \\
...
\textbf{D3} & Theorems 4.2--4.3: geometric modular flow and Lorentz branch & Canonical 1--4 & Bisognano--Wichmann modular geometry; conformal-group classification & controlled refinement limit together with the OPH Bisognano--Wichmann geometric-branch hypothesis used in Theorem 4.2 & conditional scaling-limit theorem \\
...
\textbf{D11} & Section 6.22 Higgs/top critical-surface branch & D10 & supplement-backed RG transport & critical-surface condition \(\lambda=\beta_\lambda=0\) at \(M_U\) together with the UV synchronization scale \(\mu_{\mathrm{sync}}\) used in the synchronized transport layer & supplement-backed quantitative branch \\
...
\item
\textbf{N3 (weak continuity/finite variation).} The bounded-interval and half-line endpoint control already follow internally from the local MaxEnt branch and now define the weak tail generator at fixed cutoff. What CMFP-3 adds is the continuum null-generator setting in which that weak-tail data can later be identified with the geometric null modular Hamiltonian of the relativistic phase.
\end{itemize}
...
\textbf{Proof.} On the inherited split of Proposition 5.2a, the exact-Markov case is the same HJPW block calculation as in the spatial collar theorem: the four logarithms cancel blockwise on the canonical model, and any residual bookkeeping term is central. The controlled replacement is the same fixed-model compactness argument used for ordinary collars, now relabeled \(A,B,D\mapsto I_-,J,I_+\); the operator-norm bound on \(\mathfrak D_J\) is the corresponding relabeling of the modular-transport estimate. QED.
...
\textbf{Proof.} The derived endpoint-control estimate already applies to the renormalized family after removal of the central endpoint term. For bounded intervals, the symmetric-difference length is bounded by \(|a'-a|+|b'-b|\); for half-lines it is exactly \(|a'-a|\). Therefore the matrix-element functions are Lipschitz in the endpoints. A Lipschitz function on a compact interval is absolutely continuous, so \(f_{\psi,\phi}(a)\) has an \(L^\infty_{\mathrm{loc}}\) derivative and obeys the fundamental theorem of calculus. Defining \(q\) as the rescaled negative derivative gives the displayed integral relation. QED.

\textbf{Downstream c.12--c.13 boundary.} The fixed-cutoff bridge established above is the end of the present subsection's proved content.
...
\]
with \(K_\partial\) central and boundary-supported, and knowledge of \(T_{kk}\) for all null directions reconstructs a symmetric tensor \(T_{ab}\) up to a metric term. The present subsection does not prove this theorem; it records the exact downstream burden beyond the derived half-sided-inclusion bridge.

\textbf{Gap-closure status.} The null bridge is therefore conditional rather than automatic, but its boundary is now explicit:
\begin{itemize}
\tightlist
\item
...
\end{itemize}
This is the disciplined route from null-strip factorization to the later Jacobson branch without pretending that c.12 has already completed the remaining density-upgrade and null-stress steps.
...
\tightlist
\item
  Fixed-cutoff null modular bridge (§5.2) ⟹ exact-or-controlled strip additivity and a weak tail generator
\item
  Derived half-sided modular pair + downstream density upgrade ⟹ half-line modular densities and {[}K, P{]} = i 2π P
\item
...
\begin{quote}
\textbf{Proposition 5.10a (Conditional internal QNEC).} Under the fixed-cutoff null bridge of §5.2 together with the downstream density-upgrade and resulting null-stress-identification premises, the second null variation of von Neumann entropy satisfies
...
Within the current supplement-backed implementation, the compact appendix supplies only the low-scale SM piece of the one-loop \(\overline{\mathrm{MS}}\) transport for the coupled \((y_t,\lambda,g_Y,g_2,g_3)\) system. The synchronized D11 branch actually used for the displayed numbers runs with SM coefficients from \(m_Z\) up to a UV synchronization scale \(\mu_{\mathrm{sync}}\approx 6.8\times 10^{11}\,\mathrm{GeV}\), then with the same MSSM-like coefficients as D10 from \(\mu_{\mathrm{sync}}\) to \(M_U\), and only afterward applies order-one-GeV low-scale matching corrections. The literal all-SM appendix flow by itself lands much lower, near
\[
m_t^{\mathrm{pole}}\approx 164.2\ \mathrm{GeV},
\qquad
m_H\approx 115.2\ \mathrm{GeV}.
\]
The synchronized core before those final matching shifts instead yields
\[
...
\]
Here ``supplement-backed implementation'' is literal: the compact appendix records only the low-scale SM part of the flow, while the synchronized D11 outputs above also use the UV synchronization scale, the MSSM-like UV coefficients inherited from D10, and the shared matching, low-scale extraction, and pole-mass conventions documented in the spectrum supplement. The appendix equations alone are not meant as a standalone reproduction recipe for these displayed numbers.
...
\textbf{D11} & Higgs/top critical-surface branch & supplement-backed RG transport and pole-mass conversion & critical-surface condition \(\lambda=\beta_\lambda=0\) at \(M_U\), plus the UV synchronization scale \(\mu_{\mathrm{sync}}\) used in the D11 transport; no new continuous fit once D10 is fixed & Phase II supplement-backed quantitative branch \\
...
\tightlist
\item
  \textbf{Dynamics}: The GR chain requires modular covariance plus the null-surface modular bridge (N1-N3). Section 5.2 now closes the bridge through the derived positive null-translation stage on the OPH geometric scaling branch, so the remaining explicit burdens are modular covariance, the downstream density upgrade, and the relativistic null-stress identification. Verifying those items in explicit UV regulators remains a target for future work.
\item
