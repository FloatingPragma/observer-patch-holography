...
\]
Because changing a bounded interval endpoint only adds or removes an \(O(|\Delta v|)\) collar of local terms, the same local branch also gives bounded-interval endpoint-Lipschitz matrix elements,
\[
...
\textbf{Scaling limit with geometric modular action.} In the refinement limit, the net \(\mathcal{A}(P)\) with cyclic and separating \(\Omega\) satisfies the \textbf{geometric modular action} property for caps and their conformal images: the modular group of a cap algebra acts as the unique conformal transformation preserving that cap. This is the continuum branch encoded by \(T1\) together with \(T3\).

This is precisely the Bisognano--Wichmann / geometric modular action package known to hold in conformal AQFT; see Brunetti, Guido, and Longo~\cite{bgl93}.

\textbf{Proposition 2.6 (Geometric modular action implies modular covariance and G).} On the \(T1\)/\(T3\) geometric-modular scaling branch:
...
\textbf{Alternative derivation via net regularity.} The same modular-covariance property can also be derived directly from a standard AQFT regularity condition, without invoking the full geometric-modular scaling package.
...
\textbf{Null-surface modular structure.} On the same \(T1\)/\(T3\) branch, the null-surface modular machinery narrows as follows:
...
MaxEnt then selects the unique invariant state compatible with those conserved charges. On the \(T1\)/\(T3\) scaling branch, this is exactly the vacuum or canonical state whose modular group is geometric.

\textbf{QNEC internalization.} The Quantum Null Energy Condition has rigorous QFT proofs in broad settings~\cite{bousso16}. On the \(T1\)/\(T3\) branch, the Recoverable Generalized Entropy axiom can be supported internally by:
...
The \(T1\)/\(T3\) geometric-modular scaling branch is therefore a phase statement: the refinement-stable MaxEnt fixed point must lie in the geometric modular action class. That is a concrete condition on the UV completion rather than an abstract axiom. The fixed-cutoff regulator bookkeeping, quasi-local propagation, and refinement stability belong to the OPH branch itself rather than to that scaling-branch premise.
...
\textbf{Stage 1 (\S{}6.1):} The mathematical reconstruction machinery. Given edge-center completion (Theorem 2.3), we get fixed-cutoff sector categories and a refinement-stable colimit category. If that colimit satisfies standard categorical properties (rigid, symmetric, \(C^*\)), Tannaka-Krein reconstruction yields \emph{some} compact gauge group \(G\). This stage has explicit hypotheses and is unconditional once the refinement-stable category is specified.

\textbf{Stage 2 (\S{}6.2 onward):} Axiom 5 (MAR, Minimal Admissible Realization) narrows from "some \(G\)" to the Standard Model gauge group once the admissible class includes one connected abelian charge factor. Product structure, minimal sector content, and the minimality steps for \(N_c\) and \(N_g\) are all consequences of MAR applied to the admissible class. The SM derivation uses the five OPH axioms together with the explicit transportability premise, written \([z]=0\) on the central branch or \(q_\Sigma=0\) on the higher-gauge branch, and the gauge-reconstruction hypotheses used in Theorem 6.1. See Section 6.2 below and the dedicated gauge-derivation surface for the complete proof.
...
The refinement-stability used later in the gauge branch is contained in the third OPH axiom. The UV states lie in one finite-dimensional MaxEnt family built from the same local constraint list at every cutoff. The selected stable branch of this family does not add a separate axiom beyond the MaxEnt branch itself.
...
\item
  \textbf{Loop gluing beyond central defect}: At fixed cutoff the genuinely noncentral branch is now closed by the crossed-module higher-gauge EC and transport package; what remains open is the refinement-limit transportable-sector lift together with quantitative matching to EFT anomalies.
\end{itemize}
...
\begin{itemize}
\item \textbf{Flavor and Yukawa data}: Yukawa matrices, flavor hierarchies, charged-lepton/Koide fits, quark-texture exponents, CKM/PMNS closure, and related downstream flavor fits. These require extra ans\"atze such as a uniform \(\mathbb Z_6\) center-label ensemble, circulant generation-space structure, discrete exponent assignments, or further external numerical machinery. The light-quark split remains a D12 continuation frontier rather than a recovered-core selector.
...
\item \textbf{Calibration sector}: the D10 numerical implementation depends on supplement-backed running, matching, and threshold conventions. The displayed carrier closes its own exact \(W/Z\) chart, but exact PDG \(W/Z\) still requires a repair branch beyond that carrier.
