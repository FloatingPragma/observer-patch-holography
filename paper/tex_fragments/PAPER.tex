...
The Lorentz/null-modular/Einstein chain is a scaling-limit statement on the declared recovered-core branch with an explicit D3/D4/D5 burden split: D3 carries \textbf{T2} together with the theorem-local realized transported geometric cap-local system, its derived carried-collar schedule above the transported fixed-local-collar Markov/faithfulness datum, and the later ordered cut-pair rigidity step; on that branch the scaling-limit observer algebra may leave the finite type-I regulator class, so the theorem is automorphism-level there and \(K_C=2\pi B_C\) is only the special type-I limit form; D4 carries theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed to inherit the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses together with the same-half-line local modular-Hamiltonian input and the separate bounded-interval projective branch; and D5 adds fixed-cap stationarity, the locally Lorentzian \(d=4\) scaling regime, and the tensor-upgrade side conditions. The gauge chain stays on the compact paper's D7--D9 surface: D7 reconstructs a compact group only under the explicit loop-coherence criterion \textbf{T1}, symmetric braiding \textbf{T4}, and the transportable refinement-directed sector colimit with faithful bosonic fiber functor supplied by \textbf{T5}--\textbf{T6}; D8 then uses Axiom 5 together with the same ordinary or central-defect bosonic branch, a connected positive-dimensional Lie admissible class, one connected abelian factor, and faithful action on the minimal coupled carrier; D9 stays on the realized one-generation chiral matter plus one Higgs package with asymptotic-freedom and MAR admissibility premises, yielding the realized Standard Model quotient, exact hypercharge lattice, and the counting chain \(N_g=3\) then \(N_c=3\). The worldsheet/string material and the flavor, neutrino, hadron, dark-sector, and spectroscopy branches are kept separate from the recovered core.
...
Later theorem statements use only the canonical technical-premise labels \textbf{T1}--\textbf{T6}. Fixed-cutoff regulator bookkeeping, quasi-local propagation, endpoint-Lipschitz control, collar decomposition, and related branch-internal statements are cited directly from axioms and earlier theorems rather than through a second premise-label system.
...
\textbf{D4} & Section 5.2 null modular bridge with exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line families, the weak tail generator, and the positive half-line null-translation generator & Borchers--Wiesbrock; distribution theory for half-line differentiation & the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed for the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses of Section 5.2, plus T2; the same-half-line local modular-Hamiltonian input is imported on the continuum side, the endpoint/propagation controls are internal to the local finite-constraint MaxEnt branch, and bounded-interval formulas still require the separate interval-preserving projective branch & Phase I conditional bridge theorem \\
