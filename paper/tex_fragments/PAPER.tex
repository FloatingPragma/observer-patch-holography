...
fixed-cutoff OPH consensus theorem stack: asynchronous confluence for the declared
...
The compact Higgs/top forward seed downstream of the D10 gauge core is a declared calibration-surface theorem on the same D10/D11 running, matching, and threshold surface used by the emitted electroweak carrier. Its one-scalar seed law is \(\sigma_{D11,\mathrm{HT}}=\alpha_U\cos(2\theta_{W0})/\sqrt{\pi}\), and the live forward path satisfies the exact fixed-ray factorization \(\delta y_t/y_t^{\mathrm{core}}=-(9/16)\,\delta\lambda/\lambda^{\mathrm{core}}=\sigma_{D11,\mathrm{HT}}\) on that declared surface. The D11 Jacobian then emits the public forward Higgs/top pair. A separate compare-only inverse slice on the same D11 Jacobian can be solved to hit the canonical Higgs/top reference pair exactly, but that sidecar is not the live forward branch and does not change the theorem status of the public D11 rows.
