...
Observer-Patch Holography is a reconstruction program for fundamental physics. Physical data live on a horizon screen \(S^2\), and reality is encoded by the mutual consistency of overlapping patch descriptions. This paper works with five OPH axioms, one explicit external input used only in the cosmological branch, one Phase-II pixel variable fixed on a later outer/inner closure branch, and theorem-local technical premises stated where needed for the scaling, gauge, and transport arguments below.

\paragraph{Canonical premise key.}
The six labeled technical items used across the paper corpus are introduced up front here so the overview can use them without forcing the reader to hunt for a later table. \textbf{T1} is loop coherence on the ordinary compact-gauge route. \textbf{T2} is the controlled refinement/scaling-limit scope clause for the Lorentz/null-modular/Einstein branch. \textbf{T3} is internalized on the compact surface as the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family used in the D5 Einstein branch. \textbf{T4}--\textbf{T6} are the bosonic gauge-reconstruction premises: symmetric braiding, a faithful bosonic fiber functor or explicit super-Tannakian fork, and the directed colimit of transportable edge sectors with objectwise finite-dimensional fibers. Section~\ref{14-assumptions-and-external-inputs} states the same list in formal form.

\subsection{Overview}\label{overview}

The recovered core of the paper is the D1--D5 and D7--D9 chain on the declared fixed-cutoff and recovered-core branches: overlap-consistency normal form, collar and edge-center structure, the conditional Lorentz and Einstein branches, compact gauge reconstruction, and the realized Standard Model branch with exact hypercharge structure and the counting chain \(N_g=3\) then \(N_c=3\). Here and throughout the synthesis, labels such as D1--D12 are documentary node labels for the dependency ledger; the local key appears in Section~\ref{16-summary-complete-axiom-set}, the SM/GR derivation paper \emph{Recovering Relativity and the Standard Model from the OPH Package Rooted in Observer Consistency} gives the same ledger in shorter form~\cite{ophcompact}, and the public paper ledger has no separate D11 row. D6 is not part of the recovered core because it needs total screen capacity as an external input, but it is the input-dependent local/global theorem stack for the same Einstein branch: the local null-data route leaves the metric term undetermined, the external screen-capacity identification closes that same branch, and the same D6 stack emits the de Sitter static-patch parameter package. D12 collects phenomenological continuations kept distinct from the recovered core.
Within D1, the consensus paper classifies the declared fixed-cutoff status sharply: exact
normal-form computation is finite-state and decidable, approximate stability is only collar-local
through the splice and record estimates, no global approximate-consensus theorem for long
asynchronous noisy repair is claimed, and no theorem-grade universality claim is made for a
uniform family of growing patch nets and repair laws. The imported D1 package is the
fixed-cutoff consensus theorem stack: asynchronous confluence for the declared
recovery-derived repair law, the cycle-obstruction and higher-gauge defect hierarchy,
gauge-quotient descent, observable-level confluence on the declared physical observable algebras,
and the record-algebra theorem on the observer-accessible surface. The imported remaining D1
burdens are repair completeness and the stated Petz support/CPTP clause where that branch is
used. If the finite-candidate law-selection model is mentioned, its status stays exactly as in the
consensus paper: a monotonicity theorem on a finite candidate space, not a universality theorem
and not a literal cosmological dynamics claim. The scaling-limit bridge from that fixed-cutoff
patch-net package to the later Lorentz, Einstein, and compact-gauge branches stays separate.

The Lorentz/null-modular/Einstein chain is a scaling-limit statement on the declared recovered-core branch. It depends on the controlled refinement/scaling limit, the theorem-local realized transported geometric cap-local system with its derived carried-collar schedule above the transported fixed-local-collar Markov/faithfulness datum, and the later ordered cut-pair rigidity step. On that branch the scaling-limit observer algebra may leave the finite type-I regulator class, so the theorem is automorphism-level there and \(K_C=2\pi B_C\) is only the special type-I limit form. The null bridge also uses the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed to inherit the spatial-collar-type tensor decomposition, the exact-or-controlled Markov hypotheses, and the geometric scaling action on the null half-line blow-up net that derives the half-sided modular pair and the explicit positive null-translation generator on its Stone domain with the affine half-line modular relation, while the same-half-line local modular-Hamiltonian input enters only in the later generator/charge identification. The Einstein step then adds the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, the internal small-ball bridge from the geometric cap generator together with the separate bounded-interval projective branch, the locally Lorentzian \(d=4\) scaling regime, the small-ball constancy and D4 carried-remainder hypotheses, and the all-directions/all-reference-states tensor-upgrade premise. The gauge chain stays on the compact paper's recovered Standard Model surface: compact gauge reconstruction needs explicit loop coherence, symmetric braiding, and the transportable refinement-directed sector colimit with a faithful bosonic fiber functor; the realized one-generation plus one-Higgs branch then yields the realized Standard Model quotient, exact hypercharge lattice, and the counting chain \(N_g=3\) then \(N_c=3\). The controlled large-\(N_{\mathrm{edge}}\) worldsheet effective-description branch, together with flavor, neutrino, hadron, dark-sector, and spectroscopy branches, is kept separate from the recovered core.

One external continuous input and one Phase-II pixel variable appear only where declared. The
pixel area \(P \equiv a_{\rm cell}/\ell_P^2\) is fixed on the outer/inner closure branch summarized
in Section~\ref{sec:p-closure}, and the screen capacity \(N_{\mathrm{scr}} \equiv \log \dim \mathcal
H_{\mathrm{tot}}\) enters the cosmological-capacity branch. They are not additional axioms.
...
This paper uses five OPH axioms. The only labeled technical premises used below are the canonical premises \textbf{T1}--\textbf{T6} introduced above and stated formally in Section 1.4. Other theorem-local hypotheses are written out in prose where needed rather than assigned a separate label family.
...
Section~\ref{sec:p-closure}, while \(N_{\mathrm{scr}}\) remains the external screen-capacity input
used by the cosmological-capacity branch. Any retained capacity-level neutrino side estimate on
that input is legacy bookkeeping rather than the live weighted-cycle theorem lane. These are
quantitative-branch quantities, not extra axioms.

The list below introduces the only labeled technical premises used later, namely \textbf{T1}--\textbf{T6}. Fixed-cutoff regulator bookkeeping, quasi-local propagation, endpoint-Lipschitz control, collar decomposition, and related branch-internal statements are cited directly from axioms and earlier theorems rather than through a second premise-label system.
...
\textbf{Geometric-subnet BW lift.} The continuum Lorentz statement is asked only on the extracted prime geometric subnet for caps, together with the controlled collar/refinement limit that carries the Markov and recovery remainders explicitly. The finite cap regulators are type-I algebras, but the scaling-limit observer algebra may leave that class, so the theorem is automorphism-level on the emitted geometric cap pair and \(K_C=2\pi B_C\) is only the special type-I limit form. In the canonical numbered list below, the only global item here is the scaling-limit scope clause \textbf{T2}; the theorem-local UV scaffold is a realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum, followed by ordered cut-pair rigidity on the emitted scaling-limit geometric cap pair.
...
\item \textbf{T3}: the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family used in the D5 Einstein branch.
...
For reference, the paper uses the five OPH axioms above, the closure-selected local pixel ratio \(P\), the external capacity input \(N_{\mathrm{scr}}\), and the theorem-local technical premises declared in Section 1.4.
...
\(P\) & Pixel area \(a_{\mathrm{cell}}/\ell_P^2\); use: closure-selected local quantitative scale on the particle/gravity branch; status: fixed on the outer/inner closure branch. \\
...
\textbf{T3} & Derived fixed-cap generalized-entropy stationarity for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family; use: D5 Einstein branch; status: internal theorem-level criterion on the compact surface. \\
...
\textbf{D3} & Theorems 4.2--4.3: geometric modular flow and Lorentz branch & Axioms 1--4 together with SO(3)-invariant constraint data & Bisognano--Wichmann modular geometry;\newline conformal classification on \(S^2\) & T2 together with the theorem-local realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum, and ordered cut-pair rigidity on the emitted scaling-limit geometric cap pair; the scaling-limit observer algebra may leave the finite type-I regulator class, so the theorem is automorphism-level there and \(K_C=2\pi B_C\) is only the special type-I limit form & conditional scaling-limit\newline theorem \\
\textbf{D4} & Section 5.2: null modular bridge with exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line families, the weak tail generator, the derived half-sided modular inclusion, and the explicit positive half-line null-translation generator on its Stone domain with affine half-line modular relations & D2+D3 & Borchers--Wiesbrock positive-generator theorem for standard half-sided inclusions; Stone's theorem; endpoint differentiation & the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed for the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses of Section 5.2, plus T2; quasi-local propagation and endpoint-Lipschitz control are supplied internally by the local finite-constraint MaxEnt branch, the geometric scaling action on the null half-line blow-up net derives the half-sided modular pair, the same-half-line local modular-Hamiltonian formula is imported only for the later generator/charge identification, and bounded-interval formulas still require the separate interval-preserving projective branch & conditional bridge theorem \\
\textbf{D5} & Theorem 5.1: Einstein branch & D3+D4 & fixed-volume area-variation identity for small geodesic balls; local quadratic-polarization argument for the tensor upgrade & the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, together with the internal small-ball bridge from the geometric cap generator, the separate interval-preserving projective branch used to transport the D4 half-line identification to bounded intervals, the locally Lorentzian \(d=4\) scaling regime, small-ball constancy assumptions, D4 carried remainder negligible at order \(\ell^4\), and the all-directions/all-reference-states premise used in the tensor upgrade & conditional scaling-limit theorem \\
\textbf{D6} & local/global theorem stack for \(\Lambda\): null-invisible metric ambiguity, screen-capacity closure of the same Einstein branch, and the derived de Sitter static-patch parameter package, plus a legacy capacity-level neutrino side estimate & D5 local Einstein recovery leaves \(+\Lambda g_{ab}\) undetermined & de Sitter entropy relation, static-patch radius/time formulas, and dimensional analysis & external input \(N_{\mathrm{scr}}\) and the screen-capacity identification & input-dependent global closure / estimate \\
...
\textbf{D12} & downstream flavor,\newline dark-sector,\newline baryogenesis,\newline spectroscopy, and\newline string continuations & various subsets of D6 and D9 & branch-specific EFT\newline and phenomeno\-logical\newline manipulations & explicit additional ans\"atze & phenomeno\-logical\newline continuation \\
\end{longtable}
}
\endgroup

In particular, D3--D5 are scaling-limit branches rather than fixed-cutoff matrix-algebra theorems; D7--D9 are realized-branch structural outputs in the bosonic internal-gauge sector; D6 is the input-dependent global closure of the D5 Einstein branch together with its derived de Sitter static-patch parameter package; and D12 remains phenomenological.
...
MaxEnt therefore selects the unique finite-stage invariant state compatible with those conserved charges. What it does \emph{not} prove by itself is that the scaling-limit branch lands in the BW / canonical cap phase with standard geometric modular action. The stronger statement is the \textbf{T2} scaling-limit clause together with the theorem-local extraction of a realized geometric cap pair and the later ordered cut-pair rigidity step. The corresponding shorthand in this manuscript means only the case in which such a geometric cap pair has been emitted; if the limit algebra is non-type-I, the geometric modular action on that branch is outer.
...
\textbf{Definition 4.2a (Conditional BW-branch observer-relative time reading).}
...
The effective-theory input here is no longer a second operator added on top of the null bridge. In the canonical normalization of the preceding null-half-line construction, the positive null generator is the endpoint derivative of the renormalized half-line modular family:
\[
\langle\psi,P_\Omega\phi\rangle
=
-\frac{1}{2\pi}\frac{d}{da}\Big|_{a=0^+}
...
spectroscopy template, or Page-curve/island closure remains continuation-level and uses extra
...
This branch therefore remains a phenomenological continuation above D6 and D12 premises. It contains structural ingredients and an IR benchmark, but it does not yet derive the precise galaxy-scale response from the recovered core.
...
The imported premises are exactly the external input \(N_{\mathrm{scr}}\), the D6 identification
\(N_{\mathrm{scr}}=S_{\mathrm{dS}}\), the de Sitter entropy relation, and the standard static-patch
formulas. What remains external is the screen-capacity identification itself: the local null-data
...
\textbf{D3} & Theorems 4.2--4.3 Lorentz branch & Bisognano--Wichmann modular geometry; conformal-group classification & rotationally invariant constraints together with T2 and the theorem-local realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum, and ordered cut-pair rigidity on the emitted scaling-limit geometric cap pair; the scaling-limit observer algebra may leave the finite type-I regulator class, so the theorem is automorphism-level there and \(K_C=2\pi B_C\) is only the special type-I limit form & Phase I conditional scaling-limit theorem \\
\textbf{D4} & Section 5.2 null modular bridge with exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line families, the weak tail generator, the derived half-sided modular inclusion, and the explicit positive half-line null-translation generator on its Stone domain with affine half-line modular relations & Borchers--Wiesbrock positive-generator theorem for standard half-sided inclusions; Stone's theorem; distribution theory for half-line differentiation & the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed for the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses of Section 5.2, plus T2; the same-half-line local modular-Hamiltonian input is imported on the continuum side only for the later generator/charge identification, the endpoint/propagation controls are internal to the local finite-constraint MaxEnt branch, the geometric scaling action on the null half-line blow-up net derives the half-sided modular pair, and bounded-interval formulas still require the separate interval-preserving projective branch & Phase I conditional bridge theorem \\
\textbf{D5} & Theorem 5.1 rest-frame Einstein relation plus the conditional tensor upgrade of Corollary 5.1 & fixed-volume area-variation identity for small geodesic balls; local quadratic-polarization argument for the tensor upgrade & the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, the internal small-ball bridge from the geometric cap generator, the locally Lorentzian \(d=4\) scaling regime, the separate interval-preserving projective branch used in the small-ball step, small-ball constancy assumptions, D4 carried remainder negligible at order \(\ell^4\), and the all-directions/all-reference-states premise used in the tensor upgrade & Phase I conditional scaling-limit theorem/corollary \\
\textbf{D6} & local/global theorem stack for \(\Lambda\): null-invisible metric ambiguity, screen-capacity closure of the same Einstein branch, and the derived de Sitter static-patch parameter package, plus a legacy capacity-level neutrino side estimate & de Sitter entropy relation, static-patch radius/time formulas, and dimensional analysis & external input \(N_{\mathrm{scr}}\) and the screen-capacity identification & Phase II input-dependent global closure / estimate \\
...
\textbf{D10} & forward gauge-coupling closure and target-free source-only electroweak repair theorem of the D10 quantitative-closure branch & RG running and matching conventions & pixel ratio \(P\), pixel closure, the source-only forward transmutation solve \(\mathcal F(\alpha_U;P)=0\), the fixed-cutoff edge heat-kernel / Casimir theorem on the microphysics surface together with the compact-group / Peter--Weyl lift, and the printed threshold conventions & Phase II quantitative-closure sector \\
\textbf{D12} & charged-lepton centered-readback / centered-to-affine frontier, textures, dark-sector, baryogenesis, spectroscopy, controlled large-\(N_{\mathrm{edge}}\) string/worldsheet effective descriptions, and other downstream continuations & branch-specific EFT and phenomenological manipulations & explicit additional ans\"atze beyond D6, D9, or D10, including a distinct large-\(N_{\mathrm{edge}}\) regime with fixed \(\tau=tN_{\mathrm{edge}}\) window and uniform genus-remainder control on the worldsheet lane & Phase III phenomenological continuation / program branch \\
\end{longtable}
}
\endgroup
...
\item
Phase II contains the input-dependent global closure D6 of the D5 Einstein branch and the D10 quantitative-closure branch. Failure there does not by itself erase Phase I.
\item
...
\item \textbf{Existence / strange-loop continuations}: self-simulation or strange-loop closure stories are continuation-level epilogues only. The internally available ingredient is a theorem-usable OPH state-and-law habitat built from overlap-consistent patch states together with finite Axiom-3 law coordinates. The recovered core does \emph{not} derive an existence theorem, an internal closure map on an invariant admissible observer-supporting sector, or any uniqueness/stability result for such a map.
\end{enumerate}
...
\]
which becomes an actual defect count only after the extra sixfold-uniform collapse compatible with the realized \(\mathbb Z_6\) quotient. The intrinsic neutrino lane factors directly through the same-label scalar certificate, the quark mass-side continuation compresses to \((\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})\) and the emitted ray \(\mathcal R_{D12}^{ud}\), and the charged lane fixes centered logs once a charged source pair exists while the common-shift quotient is closed negatively. The exact lane-dependent theorem objects therefore sit above the dictionary: \(\widehat C_e^{\mathrm{cand}}\to\widehat C_e\) and then the post-promotion lift whose descended scalar is \(\mu_{\mathrm{phys}}(Y_e)\), with the physical identity-mode equalizer beneath that scalar, for charged leptons; the light-quark overlap-defect scalar \(\Delta_{ud}^{\mathrm{overlap}}=\frac16\log(c_d/c_u)\) closes target-free, and on the emitted D12 mass ray this is equivalently the downstream one-scalar law \ophid{quark_d12_t1_value_law}, with \ophid{intrinsic_scale_law_D12} as the derived wrapper; a separate direct public descent theorem closes on the public physical quark frame class selected by \(P\), where the exact physical sigma datum descends uniquely to target-free public data on that selected class, the affine mean law emits \((g_u,g_d)\) algebraically, and the exact forward construction emits the exact sextet together with explicit exact forward Yukawas \(Y_u\) and \(Y_d\); this is selected-class closure only and does not claim a global classification of all quark frame classes; a stronger class-uniform public descent theorem is separate and is not claimed here; while the weighted-cycle neutrino lane fixes the bridge-rigidity invariant \(C_\nu=G^2Q S^{-1/2}=0.9994295999075177\) above the emitted proxy \(P_\nu=6.699825740519345\), with \(B_\nu=P_\nu C_\nu=6.696004159297337\) retained only as the paper-facing amplitude parameterization. The scalar benchmark \(\varepsilon=1/6\) survives only as the extra uniform sixfold collapse of this richer dictionary, so exponents \(-\log(y_f)/\log 6\) are compare-only summaries of excitation action, not recovered-core outputs or independent flavor inputs.

\paragraph{No gauge-mediated proton decay (Section 6.11).} If the gauge group is genuinely a product (from sector factorization), any retained coupling-unification discussion must be geometric/calibration-level rather than simple-group embedding. On the current D10 lane that means a shared edge-diffusion / heat-kernel calibration branch rather than a recovered-core simple-group theorem. This allows unification-like coupling relations \emph{without} GUT leptoquark bosons, hence no gauge-mediated proton decay. The combination "unification-like running without gauge-mediated proton decay" is a crisp discriminator against classic GUT predictions once the separate D10 calibration premises are granted.
...
The lane-specific readout boundary above that dictionary is explicit. On the lower D12 selector surface the same-label selector is fixed to \(\sigma_{\mathrm{ref}}\), and the mass side emits the one-parameter ray \(D12_{ud}^{\mathrm{mass}}\). The target-free mass bridge closes as \(\Delta_{ud}^{\mathrm{overlap}}=\tfrac16\log(c_d/c_u)\), equivalently \(\Theta_{ud}^{\mathrm{mass}}=\text{\texttt{quark\_d12\_t1\_value\_law}}\), with \(t_1=\tfrac56\ell_{ud}\), \(\eta_Q^{\mathrm{centered}}=-\tfrac{1-x_2^2}{27}t_1\), \(\kappa_Q=-t_1/54\), and the odd source package functorially, with \texttt{intrinsic\_scale\_law\_D12} as the derived wrapper. A separate direct public descent theorem closes on the public physical quark frame class selected by \(P\): the exact physical sigma datum descends uniquely to target-free public data on that selected class, the affine mean law emits \((g_u,g_d)\) algebraically, and the exact forward construction emits the exact sextet together with explicit exact forward Yukawas \(Y_u\) and \(Y_d\). That selected-class theorem hits the official PDG 2025 API running-quark surface exactly, with the top coordinate taken from the PDG cross-section entry. The bridge to the auxiliary direct-top PDG row remains open and is tracked in \href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}. Supporting exact surfaces are the same-family witness on \texttt{current\_family\_only} and the restricted common-refinement transport-frame exact chain. The theorem does not claim a global classification of all quark frame classes. A stronger class-uniform public descent theorem is separate and is not claimed here. The charged-lepton lane carries exact centered readback, a closed common-shift no-go, the live same-label \(q_e\) readback, a source-side determinant character for any fixed formal exponent vector \(S_M=\sum_e M_e^{\mathrm{ch}}\log q_e\), a conditional determinant-line lift on theorem-grade physical charged data, and an algebraic mass readout from theorem-grade \(A_{\mathrm{ch}}(P)\). The theorem lane does not emit a theorem-grade sector-isolated charged determinant exponent vector, and it does not identify a source-side determinant character with the physical charged determinant line. The charged source theorem is the determinant trace-lift attachment \(3\mu(r)=S_M(r)\) on the charged determinant channel. The weighted-cycle neutrino theorem boundary is carried by the bridge-rigidity invariant \(C_\nu=G^2Q S^{-1/2}=0.9994295999075177\) with \(G=\mathrm{sum\_gap}\), \(Q=\mathrm{prod\_qbar}\), and \(S=\mathrm{solar\_response\_over\_mstar}\), above the emitted proxy \(P_\nu=6.699825740519345\); \(B_\nu=P_\nu C_\nu=6.696004159297337\) is retained only as the paper-facing amplitude parameterization, and absolute attachment fixes \(\lambda_\nu=(m_{\star,\mathrm{eV}}/q_{\mathrm{mean}}^{p_\nu})P_\nu C_\nu=1.7237014208357415\), which emits the absolute family and splittings on that branch. The exact positive-segment adapter, the bridge corridor, and the reduced-correction audit remain diagnostic-only beneath this bridge-rigidity surface. The \(\varepsilon = 1/6\) texture language is the extra uniform-center-label collapse of this richer dictionary and is not a recovered-core theorem.
\item \textbf{Hadron masses and resonances}: the particle branch supplies the seeded-family, correlator, and stable-channel readout bridge, but promotable hadron rows remain execution-contract-frozen. The declared particle roadmap does not include hadron-mass closure, because the remaining step is one production backend export bundle followed by real nonperturbative production computation and production systematics rather than an additional symbolic theorem alone, and that execution burden is not a realistic target within the current project compute scope.
\item \textbf{Neutrino data}: beyond any legacy, input-dependent capacity-level side estimate tied to $N_{\mathrm{scr}}$, the recovered core does not derive neutrino masses or mixings. A continuation branch emits intrinsic masses, splittings, and PMNS data; its weighted-cycle branch reaches the physical PMNS/hierarchy regime, the normalized same-label overlap-defect weight section closes beneath that branch, and the weighted-cycle bridge-rigidity theorem emits \(C_\nu=G^2Q S^{-1/2}=0.9994295999075177\) above the emitted proxy \(P_\nu=6.699825740519345\). Absolute attachment then fixes the paper-facing amplitude parameterization \(B_\nu=P_\nu C_\nu=6.696004159297337\), \(\lambda_\nu=(m_{\star,\mathrm{eV}}/q_{\mathrm{mean}}^{p_\nu})P_\nu C_\nu=1.7237014208357415\), and therefore the absolute family \(m_i=\lambda_\nu\hat m_i\), \(\Delta m_{ij}^2=\lambda_\nu^2\widehat{\Delta m_{ij}^2}\). Because the positive selector segment is explicit, a compare-only two-parameter continuation adapter still hits the representative central splittings exactly on the same family, but it is strictly diagnostic-only beneath the bridge-rigidity invariant \(C_\nu\) and its induced paper-facing amplitude \(B_\nu\). Together with the exact \(W/Z\) sidecar pair, the exact Higgs/top sidecar, the exact charged same-family witness, and the selected-class quark theorem with its exact supporting surfaces, the exact non-hadron output lane is complete even though the theorem-grade charged lane remains open and the quark theorem is stated only on the selected public frame class \(f_P\). The attached weighted-cycle stack factors through \(q_e = q_{\mathrm{mean}} qbar_e\), and the best closed constructive local object beneath that bridge-rigidity surface is the defect-weighted same-label edge family \(q_e=\sqrt{g_e d_e}\) together with its induced \(\mu_e\) family; that family is constructive beneath the bridge-rigid theorem surface and does not replace it.
...
\item \textbf{String/worldsheet reorganization}: the compact carrier proves the OPH-to-2D-Yang--Mills partition theorem itself, namely \(Z_{\mathrm{edge}}(t)=\sum_R d_R^2 e^{-tC_2(R)}=K_t(1)\) on the compact-group heat-kernel branch, with the Chapman--Kolmogorov law as the matching collar-sewing rule. It then gives a theorem-level criterion for when the standard Gross--Taylor large-\(N\) rewriting upgrades the large-\(N_{\mathrm{edge}}\) branch to a controlled effective description once one fixes a distinct large-\(N_{\mathrm{edge}}\) sequence, a compact fixed-\(\tau=tN_{\mathrm{edge}}\) window, and uniform genus-remainder control. Any further lift to critical superstring structure remains a conjectural extension requiring extra worldsheet ingredients.
...
\item \textbf{External inputs}: \(N_{\mathrm{scr}}\) remains external, and the numerical value of \(\Lambda\) is not a local recovered prediction because local null data determine the Einstein equation only up to a metric term \(\phi g_{ab}\); fixing \(\Lambda\) requires the global capacity input \(N_{\mathrm{scr}}\). The local pixel ratio \(P\) is fixed only on the Phase-II outer/inner closure program summarized in Section~\ref{sec:p-closure}; it is not part of Phase I.
\item \textbf{Quantitative-closure branch}: the D10 numerical implementation depends on printed running, matching, and threshold conventions together with the integrated heat-kernel quantitative appendices. Its internal transmutation data are fixed forward from the OPH pixel ratio \(P\) through the source-only pixel-closure solve, not read back from measured low-energy couplings. The displayed carrier closes its own exact \(W/Z\) chart, and the promoted target-free source-only repair theorem emits the public electroweak quintet on the Phase II quantitative-closure branch. The freeze-once coherent repair surface is retained only as compare-only validation; on that frozen authoritative surface the canonical \(W/Z\) pair is hit exactly, but it remains below the target-free theorem.
\item \textbf{Hadron execution boundary}: the hadron lane is not blocked by one small symbolic gap. The next object is one production backend export bundle with publication-complete manifest provenance and real stable-channel correlator arrays; the normalized production dump and then production statistical/systematic control only follow after that bundle lands. Because that remaining nonperturbative execution burden is not a realistic target within the current project compute scope, hadron rows remain outside the declared particle roadmap and outside the derived output surface.
\end{itemize}

The compact Higgs/top forward seed downstream of the D10 gauge core is a declared theorem on the
same D10/D11 quantitative surface used by the emitted electroweak carrier. Its one-scalar seed law
is \(\sigma_{D11,\mathrm{HT}}=\alpha_U\cos(2\theta_{W0})/\sqrt{\pi}\), and the live forward path
satisfies the exact fixed-ray factorization
\(\delta y_t/y_t^{\mathrm{core}}=-(9/16)\,\delta\lambda/\lambda^{\mathrm{core}}=\sigma_{D11,\mathrm{HT}}\)
on that declared surface. The D11 Jacobian then emits the public forward Higgs/top pair. A
separate compare-only inverse slice on the same D11 Jacobian can be solved to hit the canonical
Higgs/top reference pair exactly, but that sidecar is not the live forward branch and does not
change the theorem status of the public D11 rows.

Failures localize by tier. A failure in a Phase-III flavor, dark-sector, baryogenesis, spectroscopy, string, strong-CP, proton-spin, or similar continuation retracts that continuation only. A failure in D10 challenges the corresponding quantitative-closure branch. Only a failure in Phase I would overturn the recovered-core claim set.
...
\emph{Resolution:} The framework combines the MaxEnt-selected fixed-cap generalized-entropy stationarity theorem, the derived K\_C = 2\ensuremath{\pi}B\_C structure with rigid normalization, the internal null modular bridge identifying the half-line generator with the local null-stress charge, and the internal small-ball bridge from the geometric cap generator. The remaining D5 hypotheses are the explicit scaling-limit regularity, bounded-interval transport, and tensor-upgrade premises. It does not rely only on "assume a UV CFT."
...
\emph{Resolution:} The retained D10 discussion is geometric/entropic only at the calibration level: shared edge diffusion data, heat-kernel weights, printed running conventions, and extra calibration assumptions can mimic unification-style running without embedding into a simple Lie group. If the reconstructed gauge group genuinely factorizes as a product (sector factorization selector), there are no mixed generators playing the X/Y role. "Unification-like couplings" and "group unification" therefore come apart.
