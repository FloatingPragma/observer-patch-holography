...
The recovered core of the paper is the D1--D5 and D7 chain: overlap-consistency normal form, collar and edge-center structure, the conditional Lorentz and Einstein branches, and compact gauge reconstruction in the bosonic branch. D8--D9 record additional realized-branch Standard Model consequences, but in the present manuscript those consequences remain conditional on an admitted one-generation chiral/Yukawa-capable matter package rather than being outputs of bosonic gauge reconstruction alone. D6 is a separate input-dependent corollary tied to total screen capacity. D10 and D11 are quantitative branches tied to the pixel-area input and the running and matching conventions stated later in the manuscript. D12 collects phenomenological continuations.
...
\textbf{D8} & Section 6.2: product gauge structure up to finite quotient on the admitted chiral matter branch & D7 + Axiom 5 (MAR) & compact Lie representation classification; Schur's lemma & connected admissible class, one connected abelian factor, \([z]=0\), and admitted chiral/Yukawa-capable matter data & conditional realized-branch theorem \\
\textbf{D9} & Theorem 6.13, Proposition 6.9, Theorem 6.14, Proposition 6.6: hypercharge lattice, the realized pair \((N_g,N_c)=(3,3)\), and the \(\mathbb Z_6\) quotient on that admitted matter branch & D8 & anomaly algebra; Witten anomaly; CKM CP counting & realized one-generation/one-Higgs package and MAR admissibility premises & conditional realized-branch theorem/corollary chain \\
\textbf{D10} & gauge/electroweak calibration closure documented in Part III and the Technical Supplement & D9 + \(P\) & RG running and matching conventions recorded in the supplement & pixel closure and the printed threshold conventions & calibration sector \\
\textbf{D11} & Section 6.22 Higgs/top critical-surface branch & D10 & RG transport recorded in Part III and the Technical Supplement & critical-surface condition \(\lambda=\beta_\lambda=0\) at \(M_U\) together with the UV synchronization scale \(\mu_{\mathrm{sync}}\) used in the transport layer & quantitative branch \\
...
In particular, D3--D5 are scaling-limit branches rather than fixed-cutoff matrix-algebra theorems; D7 is the bosonic internal-gauge reconstruction theorem; D8--D9 are further conditional realized-branch structural outputs once the admitted chiral/Yukawa package is supplied; D10--D11 are quantitative branches; and D12 remains phenomenological.
...
\subsubsection{Generation number from CP violation and the admissibility window (derived from MAR)}\label{64-generation-number-from-cp-violation-and-refinement-stability-derived-from-mar}
...
\textbf{Proposition 6.9 (The number of generations is N\_g = 3).} Under (i) intrinsic CP violation in the quark sector, (ii) UV-completability of SU(2)\_L (asymptotic freedom at one loop), (iii) MaxEnt/refinement stability selecting minimal viable spectrum, and (iv) the color-type lower bound \(N_c \ge 3\) from the admitted matter package, the generation number is
...
\item
  Use only the color-type lower bound \(N_c \ge 3\), not the later sharp value \(N_c = 3\).
\end{enumerate}
...
Using only the color-type lower bound \(N_c \ge 3\), we have \(N_c + 1 \ge 4\), so:
...
\subsubsection{Mass predictions from symmetry}\label{612-mass-predictions-from-symmetry}
...
\item
  gauge-as-gluing (premise GG) identifying U(1)\_em as a genuine redundancy on overlaps,
\end{itemize}
...
\textbf{Experimental status}: PDG compiles upper limits of order \(10^{-18}\) eV (approximately \(10^{-27}\) GeV). Existing bounds are consistent with the exact-zero consequence at available experimental sensitivity.
...
\textbf{Experimental status}: PDG 2025 lists m\_g \ensuremath{\leq} 1.76 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{2}\textsuperscript{3} eV/c\textsuperscript{2} (90\% credibility) from gravitational wave dispersion analysis. The GW speed bound from GW170817 constrains (c\_GW - c)/c to \textasciitilde10\textsuperscript{-}\textsuperscript{1}\textsuperscript{5}. Existing bounds are consistent with the exact-zero consequence at available experimental sensitivity.
...
\subsubsection{Particle mass extraction from spectroscopy}\label{615-particle-mass-extraction-from-spectroscopy}
...
where \textbar n\ensuremath{\rangle} is the lowest excited eigenstate with \ensuremath{\langle}n\textbar O\textbar0\ensuremath{\rangle} \(\neq\) 0 and E\textsubscript{0} is the ground state energy.

\textbf{Plaquette channel.} For the Z\textsubscript{3} model, using O = \ensuremath{\Sigma}\_p Re(B\_p), the extracted masses m\_plaq are shown in Section 6.14. This is the standard spectroscopy definition: the lowest pole in the two-point correlator of a local gauge-invariant operator.

\textbf{Dimensional transmutation.} In lattice units, both g\_ent and m\_plaq are dimensionless numbers. The physical mass scale emerges through dimensional transmutation once the coupling is matched to a continuum scheme. The ratio m\_plaq / g\_ent\textsuperscript{2} is a pure number that can be compared across different bare couplings to check scaling.

\subsubsection{Composite masses and the path to predictions}\label{616-composite-masses-and-the-path-to-predictions}
...
where \ensuremath{\Lambda}\_QCD is the dimensional transmutation scale and F is a dimensionless nonperturbative function.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{SU(2) quantum link model}: Measure boundary p\_j and fit slope vs j(j+1) to extract g\textsubscript{2},ent(\ensuremath{\mu}).
\item
  \textbf{SU(3) quantum link model}: Measure boundary p\_(p,q) and fit slope vs C\textsubscript{2}(p,q) to extract g\textsubscript{3},ent(\ensuremath{\mu}).
\item
  \textbf{Scheme matching}: One-time match from entanglement scheme to MS-bar, then RG-run to predict \ensuremath{\alpha}\_s(M\_Z), sin\textsuperscript{2}\ensuremath{\theta}\_W(M\_Z).
\item
  \textbf{Mass scale}: With g\textsubscript{3}(\ensuremath{\mu}) fixed in physical units (gravity side supplies the absolute scale via entanglement equilibrium), compute \ensuremath{\Lambda}\_QCD as the first real mass-scale prediction.
\end{enumerate}
...
A 0.1\% uncertainty in \ensuremath{\alpha}\_s becomes approximately 0.7\% uncertainty in the hadronic mass scale.

\subsubsection{Gauge unification and spectrum constraints}\label{617-gauge-unification-and-spectrum-constraints}
...
\textbf{Scope note.} The unification analysis below uses standard GUT techniques that predate this framework. The results for MSSM-like spectra are well-known in the GUT literature. The framework contributes two ingredients: (1) a \emph{mechanism} for why couplings might unify (shared geometric origin), and (2) a product group structure that forbids proton decay. The numerical \ensuremath{\alpha}\_s consistency is a \emph{consistency check} against known physics. It does not stand as a novel prediction of this framework.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Canonical GUT normalization}: \ensuremath{\alpha}\textsubscript{1} \ensuremath{\equiv} (5/3)\ensuremath{\alpha}\_Y, the standard convention for comparing to RG coefficients.
\item
...
\item
  ...
  \tightlist
  \item
    \^{\ensuremath{\alpha}}\textsuperscript{(}\textsuperscript{5}\textsuperscript{)}(M\_Z\textsuperscript{2})\textsuperscript{-}\textsuperscript{1} = 127.930 \ensuremath{\pm} 0.008 (MS-bar)
  \item
    \^{s}\textsuperscript{2}\_Z \ensuremath{\equiv} sin\textsuperscript{2}\^{\ensuremath{\theta}}\_W(M\_Z\textsuperscript{2}) = 0.23122 \ensuremath{\pm} 0.00006 (MS-bar)
  \item
    \ensuremath{\alpha}\_s(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009 (from EW fit; world average 0.1180)
  \end{itemize}

  \emph{Note on sin\textsuperscript{2}\ensuremath{\theta}\_W schemes:} PDG lists multiple definitions with different values. The MS-bar value \^{s}\textsuperscript{2}\_Z = 0.23122 differs from the effective leptonic angle \={s}\textsuperscript{2}\_\ensuremath{\ell} = 0.23154 and the on-shell value s\textsuperscript{2}\_W = 0.22342. Since we compute from running couplings \ensuremath{\alpha}\textsubscript{1}, \ensuremath{\alpha}\textsubscript{2}, the natural comparison is to the MS-bar definition.
\item
  ...
  \tightlist
  \item
    SM-only: (b\textsubscript{1}, b\textsubscript{2}, b\textsubscript{3}) = (41/10, -19/6, -7)
  \item
    MSSM-like: (b\textsubscript{1}, b\textsubscript{2}, b\textsubscript{3}) = (33/5, 1, -3)
  \end{itemize}
\end{enumerate}

\textbf{Derived couplings at M\_Z.} From \ensuremath{\alpha}\_em and sin\textsuperscript{2}\ensuremath{\theta}\_W:
...
\tightlist
\item
  A\textsubscript{1} \ensuremath{\equiv} \ensuremath{\alpha}\textsubscript{1}\textsuperscript{-}\textsuperscript{1}(M\_Z) \ensuremath{\approx} 59.00
\item
  A\textsubscript{2} \ensuremath{\equiv} \ensuremath{\alpha}\textsubscript{2}\textsuperscript{-}\textsuperscript{1}(M\_Z) \ensuremath{\approx} 29.59
\item
  A\textsubscript{3} \ensuremath{\equiv} \ensuremath{\alpha}\_s\textsuperscript{-}\textsuperscript{1}(M\_Z) \ensuremath{\approx} 8.47
\end{itemize}

\textbf{Analytic prediction formula.} Define A\_i = \ensuremath{\alpha}\_i\textsuperscript{-}\textsuperscript{1}(M\_Z) and L = ln(M\_U/M\_Z). The RG equations give A\_i = A\_U + (b\_i/2\ensuremath{\pi})L. Taking differences to eliminate A\_U:
...
Once beta coefficients are fixed, \ensuremath{\alpha}\_s(M\_Z) is completely determined by electroweak data. This is the hard numerical constraint.
...
with M\_U \ensuremath{\approx} 1.0 \ensuremath{\times} 10\textsuperscript{1}\textsuperscript{3} GeV and \ensuremath{\alpha}\_U\textsuperscript{-}\textsuperscript{1} \ensuremath{\approx} 42.4.

\textbf{Comparison to measurement}: The PDG 2025 EW-fit value is \ensuremath{\alpha}\_s(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009. The SM-only prediction misses by \ensuremath{\Delta}\ensuremath{\alpha}\_s \ensuremath{\approx} 0.047, a \textasciitilde52\ensuremath{\sigma} discrepancy, far too large to be rescued by two-loop corrections or thresholds.
...
with M\_U \ensuremath{\approx} 2.0 \ensuremath{\times} 10\textsuperscript{1}\textsuperscript{6} GeV and \ensuremath{\alpha}\_U\textsuperscript{-}\textsuperscript{1} \ensuremath{\approx} 24.34 \ensuremath{\pm} 0.01.

\textbf{Comparison to measurement}: The PDG 2025 EW-fit value is \ensuremath{\alpha}\_s(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009. The mismatch is \ensuremath{\Delta}\ensuremath{\alpha}\_s \ensuremath{\approx} -0.0011, about 1.2\ensuremath{\sigma}. This is within the expected range of two-loop corrections, threshold effects, and scheme matching.

\textbf{Significance}: SM-only unification predicts \ensuremath{\alpha}\_s(M\_Z) \ensuremath{\approx} 0.071, catastrophically wrong. The MSSM-like prediction is within 1.5\ensuremath{\sigma} of experiment. This validates the spectrum constraint but is not a novel prediction (MSSM GUT analyses from the 1990s obtained similar results).
...
\textbf{Threshold analysis.} The preceding analysis assumes UV degrees of freedom are active all the way down to M\_Z. If the \ensuremath{\Delta}b only turns on above some threshold M*, the running becomes piecewise:
...
The predicted \ensuremath{\alpha}\_s(M\_Z) depends sensitively on M\_*:

\begin{itemize}
\tightlist
\item
  \emph{\emph{M\_} = M\_Z}*: \ensuremath{\alpha}\_s(M\_Z) = 0.1166, M\_U = 2.0\ensuremath{\times}10\textsuperscript{1}\textsuperscript{6} GeV
\item
  \emph{\emph{M\_} = 1 TeV}*: \ensuremath{\alpha}\_s(M\_Z) = 0.1100, M\_U = 9.6\ensuremath{\times}10\textsuperscript{1}\textsuperscript{5} GeV
\item
  \emph{\emph{M\_} = 10 TeV}*: \ensuremath{\alpha}\_s(M\_Z) = 0.1043, M\_U = 4.9\ensuremath{\times}10\textsuperscript{1}\textsuperscript{5} GeV
\end{itemize}

This quantifies what the "scheme matching" step must accomplish: if UV physics only turns on at multi-TeV scales, the matching correction must shift \ensuremath{\alpha}\_s\textsuperscript{-}\textsuperscript{1} by \textasciitilde0.6 to reach the experimental value.

\textbf{Inverted problem: derive M\_S from measured couplings.} With three measured couplings (A\textsubscript{1}, A\textsubscript{2}, A\textsubscript{3}) and three unknowns (M\_S, M\_U, \ensuremath{\alpha}\_U), the system is exactly determined. Define \(x = \ln(M_S/M_Z)\) and \(y = \ln(M_U/M_S)\). Taking differences to eliminate \ensuremath{\alpha}\_U gives a 2\ensuremath{\times}2 linear system whose solution is:
...
\tightlist
\item
  \textbf{M\_S \ensuremath{\approx} 57 GeV} (42--77 GeV at 1\ensuremath{\sigma})
\item
  \textbf{M\_U \ensuremath{\approx} 2.27 \ensuremath{\times} 10\textsuperscript{1}\textsuperscript{6} GeV}
\item
  \textbf{\ensuremath{\alpha}\_U\textsuperscript{-}\textsuperscript{1} \ensuremath{\approx} 24.0}
\end{itemize}
\end{quote}

The uncertainty is dominated by the experimental error on \ensuremath{\alpha}\_s(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009. The central value is sensitive to the precise \ensuremath{\alpha}\_s input: \ensuremath{\alpha}\_s = 0.1175 gives M\_S \ensuremath{\approx} 67 GeV, while \ensuremath{\alpha}\_s = 0.1166 (the MSSM prediction) gives M\_S \ensuremath{\approx} 91 GeV. The qualitative conclusion (effective threshold near the electroweak scale) remains stable across the 1\ensuremath{\sigma} range.

\textbf{Physical interpretation.} This is a striking result: internal consistency of one-loop unification pushes the effective onset of MSSM-like \ensuremath{\Delta}b down to the \textbf{electroweak scale}. The new charged degrees of freedom cannot all live at some ultra-high scale; their \emph{net effect} on beta functions must turn on around \textasciitilde10\textsuperscript{2} GeV.

If the framework requires unification but the UV spectrum only turns on well above M\_Z (say, at multi-TeV), then the gap must be filled by one of: (i) additional running effects at intermediate scales, (ii) non-degenerate particle thresholds that mimic low-scale onset, or (iii) two-loop corrections providing effective \ensuremath{\Delta}b at lower scales.
...
\textbf{Significance.} This provides a "spectrum selector": the framework must produce an effective \ensuremath{\Delta}b in the above direction (from new bulk fields or propagating collar/edge modes), or it cannot match precision gauge couplings. This is a hard, quantitative constraint on possible UV completions, derived before attempting to predict masses.
...
\textbf{Argument.} Standard Grand Unified Theories (SU(5), SO(10)) achieve coupling unification by embedding SU(3) \ensuremath{\times} SU(2) \ensuremath{\times} U(1) into a simple Lie group (Georgi and Glashow, 1974). This embedding necessarily introduces X and Y bosons (leptoquarks) that mediate baryon-number-violating processes like p \ensuremath{\to} e\textsuperscript{+}\ensuremath{\pi}\textsuperscript{0}.
...
\textbf{Proposition (Factorization \ensuremath{\leftrightarrow} additive boundary Laplacian).} Suppose the edge Hamiltonian governing boundary sector weights takes the form:
...
Conversely, if the reconstructed sector category is Rep(G) and the edge weights satisfy this factorization for all caps and scales, then G \ensuremath{\cong} G\textsubscript{1} \ensuremath{\times} G\textsubscript{2} \ensuremath{\times} G\textsubscript{3} (up to finite quotient).
...
\textbf{Experimental status.} Minimal SU(5) GUTs predict \ensuremath{\tau}\_p \textasciitilde{} 10\textsuperscript{3}\textsuperscript{1} years; Super-Kamiokande has pushed limits to \ensuremath{\tau}\_p \textgreater{} 10\textsuperscript{3}\textsuperscript{4} years, excluding minimal GUTs. The model\textquotesingle s prediction of proton stability is consistent with all observations.

\textbf{Distinguishing signature.} The combination of \textbf{precision gauge unification} (MSSM-like \ensuremath{\alpha}\_s consistency) with \textbf{proton stability} is characteristic of this framework. Standard SUSY GUTs predict both unification \emph{and} proton decay; this model predicts unification \emph{without} proton decay, if sector factorization holds.

\textbf{Chain summary}: Edge-sector probabilities \ensuremath{\to} gauge couplings at UV scale \ensuremath{\to} one-loop RG \ensuremath{\to} consistency check for \ensuremath{\alpha}\_s(M\_Z) \ensuremath{\to} spectrum constraint from mismatch with SM-only running. The product group structure (derived from MAR) separately implies proton stability.

\textbf{Precision of the pixel-area relation.} There are two distinct precision questions for a\_cell:

\textbf{(A) In Planck units (a\_cell/\ensuremath{\ell}\_p\textsuperscript{2}).} Once the dimensionless entropy density \={\ensuremath{\ell}} is fixed, the dimensionless pixel area is a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} = 4\={\ensuremath{\ell}}. This ratio is \textbf{independent of the experimental uncertainty in G}, because \ensuremath{\ell}\_p\textsuperscript{2} \ensuremath{\propto} G cancels. The limiting precision is whatever uncertainty remains in \={\ensuremath{\ell}}, i.e., in the inputs used to determine t(\ensuremath{\mu}) (in practice gauge couplings). Since we feed in SM couplings to get \={\ensuremath{\ell}}, precision is limited by those inputs, not by G.

\textbf{(B) In SI units (a\_cell {[}m\textsuperscript{2}{]}).} If \={\ensuremath{\ell}} were known exactly, then a\_cell in SI units would inherit the uncertainty of G:
...
Using CODATA values: G = 6.67430 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{1}\textsuperscript{1} m\textsuperscript{3} kg\textsuperscript{-}\textsuperscript{1} s\textsuperscript{-}\textsuperscript{2} with relative uncertainty \ensuremath{\approx} 2.2 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{5}, so the best-case SI precision is:

\begin{itemize}
\tightlist
\item
  Relative precision of a\_cell: \ensuremath{\delta}a\_cell/a\_cell \ensuremath{\approx} 2.2 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{5}
\item
  Relative precision of \ensuremath{\sqrt{}}a\_cell: \ensuremath{\approx} 1.1 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{5}
\end{itemize}

With the derived value a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} \ensuremath{\approx} 1.63094, this gives a\_cell \ensuremath{\approx} 4.26 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{7}\textsuperscript{0} m\textsuperscript{2}, with irreducible CODATA uncertainty \ensuremath{\delta}a\_cell \ensuremath{\approx} 9.5 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{7}\textsuperscript{5} m\textsuperscript{2}.

\textbf{Reverse-engineering \ensuremath{\alpha}\_s from the pixel-area scale.} The pixel-area relation (Section 5.4) provides an independent route to \ensuremath{\alpha}\_s(M\_Z). The cell area in Planck units is:
...
\textbf{RG mechanism from Markov collar structure.} The running of gauge couplings is a structural consequence of the Markov collar plus symmetry and does not require an extra assumption. Consider a nested family of caps C(\ensuremath{\delta}) and the operation "thicken the collar by \ensuremath{\Delta}":
...
At the level of the classical distribution over sectors p\_\ensuremath{\alpha}(\ensuremath{\delta}), thickening the collar acts by a stochastic kernel:

\[
p(\delta + \Delta) = \mathsf{K}_\Delta \cdot p(\delta).
\]

Imposing gauge-class invariance (kernel depends only on conjugacy class) and rotational invariance (same kernel along the cut), the kernel K\_\ensuremath{\Delta} must be a central (class) convolution kernel on the group.
...
This is exactly the heat-kernel form p\_R(t) \ensuremath{\propto} d\_R\^{}\ensuremath{\kappa} exp(-t C\textsubscript{2}(R)) with \ensuremath{\kappa} = 1 in the simplest MaxEnt edge state.
...
so the coupling parameter t is \textbf{additive} under stacking collar layers. Additive in the "RG-time" variable is exactly what one-loop running requires: if the physical scale changes multiplicatively, the number of collar layers changes additively, hence t runs linearly in ln \ensuremath{\mu}.

\textbf{Normalization from modular geometry.} The BW\_\{S\textsuperscript{2}\} theorem gives modular flow on a cap as the conformal dilation preserving the cap and fixing its boundary, with KMS normalization \(\beta=2\pi\) on the OPH geometric branch. At finite collar scale the discrepancy is the carried remainder from the Markov/recovery analysis; in the controlled refinement limit that remainder vanishes. Near an entangling surface, modular flow is a boost; in Euclidean continuation it becomes an angular coordinate with period \(2\pi\). The dilation parameter is \(s\sim \ln(\mu/\mu_0)\).
...
In base-10 decades: log\textsubscript{1}\textsubscript{0}(e\^{}\{2\ensuremath{\pi}\}) = 2\ensuremath{\pi}/ln(10) \ensuremath{\approx} 2.728 decades. If earlier analysis required a normalization factor around \textasciitilde2.9, the interpretation is that this is the conversion between "per modular period" and "per decade of \ensuremath{\mu}," fixed by the BW\_\{S\textsuperscript{2}\} tangent-limit normalization rather than being a tunable parameter.

Using the measured \(a_{\mathrm{cell}}/\ell_p^2 = 1.63094\) and \ensuremath{\alpha}\textsubscript{2}(M\_Z) \ensuremath{\approx} 0.0338 (giving t\textsubscript{2} \ensuremath{\approx} 1.334), we compute \(\bar{\ell}_{\mathrm{SU}(2)} \ensuremath{\approx} 0.3946\). The SU(3) contribution is then forced:

\[
\bar{\ell}_{\mathrm{SU}(3)} = \frac{1.63094}{4} - 0.3946 \approx 0.0131.
\]

Inverting the monotone function \(\bar{\ell}_{\mathrm{SU}(3)}(t_3)\) gives t\textsubscript{3} \ensuremath{\approx} 4.657, hence:

\textbf{\ensuremath{\alpha}\_s(M\_Z)\textbar\_pixel \ensuremath{\approx} 0.1175}

This is consistent with the PDG EW-fit value \ensuremath{\alpha}\_s(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009 to within \textasciitilde5 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{5}. The agreement is striking but \textbf{conditional}: it becomes a genuine prediction only if \(a_{\mathrm{cell}}/\ell_p^2\) can be fixed independently (from the gravity side) rather than computed from \ensuremath{\alpha}\_s.

\textbf{Proton mass estimate.} Using standard 4-loop \(\overline{\mathrm{MS}}\) running with n\_f = 5 at M\_Z, the above \ensuremath{\alpha}\_s corresponds to \ensuremath{\Lambda}\_\{\textbackslash overline\{\textbackslash mathrm\{MS\}\}\}\^{}\{(5)\} \ensuremath{\approx} 0.208 GeV. With the lattice-motivated ansatz m\_p \ensuremath{\approx} 4.47 \ensuremath{\Lambda}\_QCD:
...
\textbf{\ensuremath{\alpha}\_s(M\_Z) = 0.1175, sin\textsuperscript{2}\^{\ensuremath{\theta}}\_W(M\_Z) = 0.2311}
\end{quote}
...
\tightlist
\item
  \ensuremath{\alpha}\_s(M\_Z): predicted 0.1175 vs PDG 0.1177 \ensuremath{\pm} 0.0009, difference 2\ensuremath{\times}10\textsuperscript{-}\textsuperscript{4} (within 1\ensuremath{\sigma})
\item
  sin\textsuperscript{2}\ensuremath{\theta}\_W(M\_Z): predicted 0.2311 vs PDG \^{s}\textsuperscript{2}\_Z = 0.23122 \ensuremath{\pm} 0.00006 (MS-bar), difference \textasciitilde1\ensuremath{\times}10\textsuperscript{-}\textsuperscript{4} (\textasciitilde2\ensuremath{\sigma})
\end{itemize}

\textbf{Important caveat on sin\textsuperscript{2}\ensuremath{\theta}\_W.} In \emph{percentage} terms, 0.05\% looks impressive. But the MS-bar experimental uncertainty is \ensuremath{\pm}0.00006, so the difference \textasciitilde1\ensuremath{\times}10\textsuperscript{-}\textsuperscript{4} is \textasciitilde2\ensuremath{\sigma} in experimental units.

This residual discrepancy is not a calculation bug. For MSSM one-loop coefficients, unification implies a tight relation between \ensuremath{\alpha}\_s and sin\textsuperscript{2}\ensuremath{\theta}\_W once \^{\ensuremath{\alpha}} is fixed:

\[
\sin^2\hat{\theta}_W(M_Z) = \frac{1}{5} + \frac{7}{15}\frac{\hat{\alpha}(M_Z)}{\alpha_s(M_Z)}.
\]

Once the pipeline outputs \ensuremath{\alpha}\_s \ensuremath{\approx} 0.1175, the weak mixing angle is essentially locked near 0.231. The \textasciitilde2\ensuremath{\sigma} residual reflects missing theoretical corrections, not a flaw in the core calculation.

The required correction is small: shifting sin\textsuperscript{2}\ensuremath{\theta}\_W by \textasciitilde1\ensuremath{\times}10\textsuperscript{-}\textsuperscript{4} corresponds to only \textasciitilde0.04\% change in \ensuremath{\alpha}\textsubscript{2}(M\_Z). Effects at this level include:
...
All of these are O(10\textsuperscript{-}\textsuperscript{4}) corrections that the one-loop treatment used here omits. The claim is therefore: the framework produces sin\textsuperscript{2}\ensuremath{\theta}\_W within \textasciitilde2\ensuremath{\sigma} of the MS-bar measurement using only EW inputs plus the pixel constraint. This is a genuine parameter reduction, though precision claims require a proper theory error budget.
...
\bottomrule\noalign{}
\endlastfoot
Two-loop running & O(10\textsuperscript{-}\textsuperscript{4}) in sin\textsuperscript{2}\ensuremath{\theta}\_W \\
Threshold corrections (edge\ensuremath{\to}4D matching) & Unknown; is O(10\textsuperscript{-}\textsuperscript{3}) \\
A\_eff normalization ambiguity & Factor \textasciitilde6; affects absolute t, not ratios \\
Scheme conversion (entanglement \ensuremath{\to} MS-bar) & O(10\textsuperscript{-}\textsuperscript{4}) expected \\
Missing U(1) mixing effects & O(10\textsuperscript{-}\textsuperscript{4}) \\
\end{longtable}
}

Without a full two-loop treatment and proper threshold matching, the framework cannot claim precision better than \textasciitilde0.1\% on sin\textsuperscript{2}\ensuremath{\theta}\_W. The \textasciitilde2\ensuremath{\sigma} agreement with MS-bar data is encouraging but not definitive evidence.

\textbf{Input elimination.} This represents a genuine reduction in free parameters: the standard unification story requires both \(\hat{\alpha}(M_Z)\) \textbf{and} sin\textsuperscript{2}\ensuremath{\theta}\_W(M\_Z) as inputs. The pixel-area constraint eliminates sin\textsuperscript{2}\ensuremath{\theta}\_W as an input, predicting it instead.

\textbf{SM measurement contradiction.} Repeating with SM-only beta coefficients (b\textsubscript{1}, b\textsubscript{2}, b\textsubscript{3}) = (41/10, -19/6, -7) gives \ensuremath{\alpha}\_s \ensuremath{\approx} 0.096 and sin\textsuperscript{2}\ensuremath{\theta}\_W \ensuremath{\approx} 0.216, both far from observation. The pixel-area constraint strongly disfavors SM-only running.

\textbf{Edge-mode derivation of \ensuremath{\beta}-coefficients via Peter-Weyl structure.} The key insight comes from the Peter-Weyl decomposition of L\textsuperscript{2}(G):

\[
L^2(G) \simeq \bigoplus_R V_R \otimes V_R^*
\]

A representation R corresponds to a block of size d\_R\textsuperscript{2}. However, entropy and vacuum polarization "see" different parts of this structure:

\begin{itemize}
\item
  \textbf{Entropy (MaxEnt selection)} traces over one side of the entanglement cut, giving the factor d\_R in the probability p\_R \ensuremath{\propto} d\_R exp(-t C\textsubscript{2}(R)).
\item
  \textbf{Vacuum polarization loops} run over both indices of the V\_R \ensuremath{\otimes} V\_R* block, restoring the second d\_R factor.
\end{itemize}
...
\textbf{Edge sector weights.} For the SM product group with Z\textsubscript{6} quotient, the superselection weight for sector (R\textsubscript{3}, R\textsubscript{2}, n) is:

w(R\textsubscript{3}, R\textsubscript{2}, n) = d\textsubscript{3}(R\textsubscript{3}) exp(-t\textsubscript{3} C\textsubscript{2}(R\textsubscript{3})) \ensuremath{\cdot} d\textsubscript{2}(R\textsubscript{2}) exp(-t\textsubscript{2} C\textsubscript{2}(R\textsubscript{2})) \ensuremath{\cdot} exp(-t\_Y n\textsuperscript{2})

with the Z\textsubscript{6} selection rule n \ensuremath{\equiv} -2\ensuremath{\tau} - 6j (mod 6), where \ensuremath{\tau} is SU(3) triality and j is SU(2) spin. The probability is p = w/Z (normalized).
...
\textbf{Representation bookkeeping.} Complex representations R and their conjugates \={R} are counted separately in the sum. For SU(3), the fundamental \textbf{3} and antifundamental \textbf{\={3}} both contribute with d = 3, C\textsubscript{2} = 4/3, and T = 1/2. Real representations (like the adjoint \textbf{8}) appear once. This is standard QFT bookkeeping: each chiral fermion species contributes independently to vacuum polarization.

\textbf{Why U(1) uses a different formula.} The hypercharge formula differs from SU(2)/SU(3) because U(1) has no Dynkin index structure; all irreps are 1-dimensional. The contribution to b\textsubscript{1} comes from Y\textsuperscript{2} (the charge squared), with the factor 3/5 from GUT normalization. This is the standard form in unified theories, not a framework-specific choice. The different structure is why the U(1) prediction (5\% error) is less precise than the non-Abelian ones (\textless1\% error).

\textbf{Numerical result at unification.} At t\_U \ensuremath{\approx} 1.64 (corresponding to \ensuremath{\alpha}\_U\textsuperscript{-}\textsuperscript{1} \ensuremath{\approx} 24.1):
...
\bottomrule\noalign{}
\endlastfoot
\ensuremath{\Delta}b\textsubscript{1} & 2.49 & 2.50 & -0.3\% \\
\ensuremath{\Delta}b\textsubscript{2} & 4.38 & 4.17 & +5.1\% \\
\ensuremath{\Delta}b\textsubscript{3} & 3.97 & 4.00 & -0.7\% \\
\end{longtable}
}

This achieves MSSM-like beta shifts without inserting MSSM by hand. The \textasciitilde5\% tension in \ensuremath{\Delta}b\textsubscript{2} is resolved by two-loop corrections, threshold effects, or refinements to the U(1) sector weighting.

\textbf{What makes this non-trivial.} The key test is the \textbf{ratio} \ensuremath{\Delta}b\textsubscript{3}/\ensuremath{\Delta}b\textsubscript{2}, rather than matching individual \ensuremath{\Delta}b values (which can be achieved by adjusting an overall normalization). The MSSM requires \ensuremath{\Delta}b\textsubscript{3}/\ensuremath{\Delta}b\textsubscript{2} = 4.00/4.17 = 0.959. The Peter-Weyl calculation gives 3.97/4.38 = 0.906, about 6\% low. This ratio is fixed by the heat-kernel distribution and representation theory, with no free parameters to adjust. Getting within 6\% of a non-trivial ratio like 0.96 from first principles is significant, though the remaining discrepancy suggests the mechanism is not yet complete.
...
At t\_U \ensuremath{\approx} 1.64, the expected Dynkin indices are:
...
\tightlist
\item
  \ensuremath{\Delta}b\textsubscript{2} \ensuremath{\approx} 4\ensuremath{\pi} \ensuremath{\times} 0.330 = 4.15 (vs MSSM target 25/6 = 4.17, error -0.4\%)
\item
...
\textbf{Exploratory threshold estimates.} Using the Peter-Weyl-derived beta shifts and measured \ensuremath{\alpha}\_em(M\_Z), sin\textsuperscript{2}\ensuremath{\theta}\_W(M\_Z) to fix \ensuremath{\alpha}\textsubscript{1}, \ensuremath{\alpha}\textsubscript{2}, one obtains the following threshold-study numbers:

\begin{itemize}
\tightlist
\item
  \ensuremath{\alpha}\_s(M\_Z) \ensuremath{\approx} 0.1168
\item
  M\_U \ensuremath{\approx} 2.0 \ensuremath{\times} 10\textsuperscript{1}\textsuperscript{6} GeV
\item
  \ensuremath{\alpha}\_U\textsuperscript{-}\textsuperscript{1} \ensuremath{\approx} 24.3
\end{itemize}

These numbers belong to an exploratory edge-threshold exercise rather than to the D10 calibration branch discussed in Section 6.19.

Using 4-loop M\={S} running with n\_f = 5, this \ensuremath{\alpha}\_s corresponds to:

\ensuremath{\Lambda}\_M\={S}\textsuperscript{(}\textsuperscript{5}\textsuperscript{)} \ensuremath{\approx} 195 MeV

This is the first genuinely "mass-like" scale output once \ensuremath{\beta}-coefficients are internally derived. The proton mass remains blocked by the nonperturbative conversion constant C\_p (essentially what lattice QCD computes), but the upstream RG machinery is closed.

\textbf{Full UV \ensuremath{\beta}-vector from edge modes.} The edge-derived shifts can be combined with SM coefficients to obtain a complete UV running law without importing MSSM by hand:

\[
b^{\rm UV} = b^{\rm SM} + \Delta b_{\rm edge} \approx (6.59, 1.22, -3.03).
\]

\textbf{Threshold constraint.} If this UV content were active from M\_Z upward, one-loop unification with measured \ensuremath{\alpha}\textsubscript{1}, \ensuremath{\alpha}\textsubscript{2} would predict \ensuremath{\alpha}\_s(M\_Z) \ensuremath{\approx} 0.157, far above the measured \textasciitilde0.118. This forces a threshold/decoupling scale M\_S above which the edge spectrum contributes to running, with SM running below.
...
This should be read as an exploratory threshold scenario rather than as part of the D10 calibration branch.

\textbf{What remains.} Currently M\_S \ensuremath{\approx} 100 TeV is what the model needs to match data. To convert this into a genuine prediction requires deriving M\_S from the edge physics itself (the gap/decoupling scale of edge excitations in the collar Hamiltonian), rather than solving for it from measured \ensuremath{\alpha}\_s. This is the sharpest remaining target for closing the precision prediction chain.

\subsubsection{\texorpdfstring{The \(Z_6\) quotient: edge-sector selection rules and center-label entropy}{The Z6 quotient: edge-sector selection rules and center-label entropy}}\label{618-the-zux2086-quotient-edge-sector-selection-rules-and-center-label-entropy}

The SM global gauge group is the quotient (SU(3) \ensuremath{\times} SU(2) \ensuremath{\times} U(1))/Z\textsubscript{6}, rather than the direct product (Proposition 6.6). The quotient gives exact edge-sector selection rules. In the later flavor discussion we also use a phenomenological center-label entropy ansatz tied to this sixfold quotient structure.

\textbf{The Z\textsubscript{6} congruence rule.} The identified element is (exp(2\ensuremath{\pi}i/3), -1, exp(i\ensuremath{\pi}/3)) \ensuremath{\in} SU(3) \ensuremath{\times} SU(2) \ensuremath{\times} U(1). Label edge sectors by SU(3) triality \ensuremath{\tau} \ensuremath{\in} \{0,1,2\}, SU(2) spin j, and hypercharge Y = n/6 with n \ensuremath{\in} Z. For the representation to descend to the quotient group, the identified element must act trivially:

exp(2\ensuremath{\pi}i\ensuremath{\tau}/3) \ensuremath{\cdot} (-1)\^{}\{2j\} \ensuremath{\cdot} exp(i\ensuremath{\pi}n/3) = 1

This gives the exact selection rule:

\textbf{n \ensuremath{\equiv} -2\ensuremath{\tau} - 6j (mod 6)}
...
\tightlist
\item
  Q\_L: (3, 2, Y=1/6) =\textgreater{} (\ensuremath{\tau}=1, j=1/2, n=1) \ensuremath{\checkmark}
\item
  L\_L: (1, 2, Y=-1/2) =\textgreater{} (\ensuremath{\tau}=0, j=1/2, n=-3) \ensuremath{\checkmark}
\item
  u\^{}c: (\={3}, 1, Y=-2/3) =\textgreater{} (\ensuremath{\tau}=2, j=0, n=-4) \ensuremath{\checkmark}
\item
  d\^{}c: (\={3}, 1, Y=1/3) =\textgreater{} (\ensuremath{\tau}=2, j=0, n=2) \ensuremath{\checkmark}
\item
  e\^{}c: (1, 1, Y=1) =\textgreater{} (\ensuremath{\tau}=0, j=0, n=6) \ensuremath{\checkmark}
\end{itemize}

\textbf{Heat-kernel slopes at M\_Z.} The general relation is t = g\textsuperscript{2}A\_eff/2 where A\_eff is the effective "Euclidean thickness" of the collar. Section 6.14\textquotesingle s SU(3) lattice analysis finds A\_eff \ensuremath{\to} 2.004 \ensuremath{\pm} 0.012 as g\textsuperscript{2} \ensuremath{\to} 0.

Using g\textsuperscript{2} = 4\ensuremath{\pi}\ensuremath{\alpha} and defining the \textbf{normalization convention} A\_eff = 4\ensuremath{\pi} (which differs from the lattice extrapolation by a factor of \textasciitilde2\ensuremath{\pi}; see normalization note below), we have:

t\_i = (g\_i\textsuperscript{2} \ensuremath{\cdot} 4\ensuremath{\pi})/2 = 2\ensuremath{\pi} \ensuremath{\cdot} g\_i\textsuperscript{2} = 4\ensuremath{\pi}\textsuperscript{2} \ensuremath{\alpha}\_i

With the electroweak inputs and PDG 2025 EW-fit value \ensuremath{\alpha}\_s(M\_Z) = 0.1177:
...
\emph{(Note: Using the MSSM unification prediction \ensuremath{\alpha}\_s = 0.1166 instead would give t\_3 = 4.605, a 1.2\% shift that negligibly affects the residue-class distributions below.)}

For the U(1) factor with Y = n/6, the effective slope is t\_Y = t\textsubscript{1}/36 = 0.0186.

\emph{Normalization note:} The A\_eff = 4\ensuremath{\pi} convention gives t = 4\ensuremath{\pi}\textsuperscript{2}\ensuremath{\alpha}, a clean relation used throughout. However, Section 6.14\textquotesingle s lattice extrapolation gives A\_eff \ensuremath{\to} 2, not 4\ensuremath{\pi} \ensuremath{\approx} 12.6. This factor-of-\textasciitilde6 discrepancy indicates either: (1) the toy UV model\textquotesingle s "g\textsuperscript{2}" differs from the continuum MS-bar convention by a factor of \textasciitilde2\ensuremath{\pi}, or (2) additional physics (spin-statistics, vertex factors) enters the lattice\ensuremath{\leftrightarrow}continuum matching. This normalization ambiguity affects \emph{absolute} t values but not ratios between gauge groups, so predictions depending only on ratios (hypercharge selection rules and center-label entropy estimates) remain stable. Predictions depending on absolute t (like the \ensuremath{\alpha}\_s pixel-area extraction) require this normalization to be resolved from first principles; in this branch it is fixed by convention.

\textbf{Full edge-sector probability law.} A sector (R\textsubscript{3}, R\textsubscript{2}, n) has weight

w(R\textsubscript{3}, R\textsubscript{2}, n) = d\textsubscript{3}(R\textsubscript{3}) exp(-t\textsubscript{3} C\textsubscript{2}(R\textsubscript{3})) \ensuremath{\cdot} d\textsubscript{2}(R\textsubscript{2}) exp(-t\textsubscript{2} C\textsubscript{2}(R\textsubscript{2})) \ensuremath{\cdot} exp(-t\_Y n\textsuperscript{2})

but only if the congruence n \ensuremath{\equiv} -2\ensuremath{\tau} - 6j (mod 6) holds; otherwise w = 0 exactly. The probability is p = w/Z with Z summing over allowed sectors.
...
At M\_Z, because SU(3) is strongly coupled (t\textsubscript{3} large), triality-zero sectors dominate, so most weight sits in residues 0 and 3 (integer and half-integer hypercharge). The "quark-like residues" (1, 2, 4, 5) are suppressed at the 10\textsuperscript{-}\textsuperscript{3} level.
...
\textbf{log\textsubscript{2} 6 = 2.584962500721156... bits}

This is a parameter-free constant once the uniform sixfold center-label ansatz is assumed; the quotient itself fixes only the six allowed label classes.
...
\subsubsection{Electroweak scale as a calibration-sector output}\label{619-electroweak-scale-from-dimensional-transmutation}

In the D10 calibration branch, the electroweak scale is not presented as an independent theorem-level prediction. Once the pixel input \(P\), the gauge-sector closure, and the printed running and matching conventions are fixed, the same fixed-point problem returns the low-energy electroweak scale and tree-level electroweak masses as consistency outputs.

The scale-setting formulas used in this branch are
\[
...
\]
using the gauge-closure value \(\alpha_U^{-1}\approx 24.32\). Under the conventions used here this gives
\[
v\approx 246.77\ \mathrm{GeV}.
\]

Accordingly, \(v\), \(m_W\), and the running \(Z\)-mass relation should be read as calibration-sector consistency checks of this branch rather than as independent confirmations of the recovered core. Their role is to show that the D10 closure is internally coherent once \(P\) and the printed conventions are imposed.

\subsubsection{Heavy-top heuristic and its status}\label{620-top-quark-mass-from-order-one-yukawa}

A simple order-one-Yukawa estimate,
\[
m_t\sim \frac{v}{\sqrt 2},
\]
explains why the top sector is naturally much heavier than the rest of the Yukawa spectrum, but it is not the D11 result quoted later in the paper.

The top/Higgs numbers belong to the critical-surface transport branch of Section 6.22. That branch, not the order-one heuristic, is the secondary quantitative output discussed later in the paper. This subsection should therefore be read only as motivation for why the top channel is the natural least-suppressed Yukawa mode.

\subsubsection{\texorpdfstring{Yukawa hierarchy from \(Z_6\) defect suppression}{Yukawa hierarchy from Z6 defect suppression}}\label{621-yukawa-hierarchy-from-zux2086-defect-suppression}

The Z\textsubscript{6} quotient structure provides a possible phenomenological route to the fermion mass hierarchy once one adds the uniform center-label ansatz and the defect-suppression hypothesis, without introducing continuous Yukawa parameters.
...
\ensuremath{\varepsilon} = exp(-ln 6) = 1/6
...
\textbf{Definition (Defect number).} If the direct intertwiner is forbidden by the Z\textsubscript{6} congruence rule, it can be generated by inserting defect operators that shift the grading. Define:

\textbf{n\_f := min\{n \ensuremath{\in} Z\ensuremath{\geq}0 : neutral intertwiner exists after n defect insertions\}}

This is a minimal path length in the overlap groupoid, automatically an integer.

\textbf{Suppression from entropy.} Each defect insertion resolves one unit of the Z\textsubscript{6} restriction, removing a factor of 6 in available microstates. MaxEnt weighting then gives:

\textbf{y\_f \ensuremath{\propto} \ensuremath{\varepsilon}\^{}n\_f = 6\^{}(-n\_f), where \ensuremath{\varepsilon} = 1/6}
...
\textbf{Extraction of defect charges.} Using \(y_f=\sqrt2\,m_f/v\) with the calibration-sector value of \(v\) from Section 6.19, and \(n_f=-\ln(y_f)/\ln(6)\):
...
d & 2.7\ensuremath{\times}10\textsuperscript{-}\textsuperscript{5} & 5.87 & 6 & 1.27 \\
u & 1.3\ensuremath{\times}10\textsuperscript{-}\textsuperscript{5} & 6.30 & 6 & 0.59 \\
\ensuremath{\tau} & 0.010 & 2.55 & 3 & 2.23 \\
\ensuremath{\mu} & 0.00061 & 4.13 & 4 & 0.80 \\
e & 3.0\ensuremath{\times}10\textsuperscript{-}\textsuperscript{6} & 7.10 & 7 & 0.83 \\
\end{longtable}
}
...
\tightlist
\item
  The logarithms are close to integers in base 6, the "Z\textsubscript{6} controls hierarchy" fingerprint.
\item
...
This reproduces the observed hierarchy with integer charges and \textbf{no continuous parameters beyond \ensuremath{\varepsilon} = 1/6}.
...
\textbf{Computational note} (January 2026): using the calibration-sector value \(v\approx 246.77\) GeV leaves the base-6 hierarchy pattern close to integer defect counts, but the branch remains phenomenological regardless of that small numerical shift.

\subsubsection{Higgs/top critical-surface branch}\label{622-higgs-mass-from-critical-surface-constraint}

The D11 branch uses the refinement-stability condition
\[
...
The appendix supplies only the low-scale SM piece of the one-loop \(\overline{\mathrm{MS}}\) transport for the coupled \((y_t,\lambda,g_Y,g_2,g_3)\) system. The D11 branch used for the displayed numbers runs with SM coefficients from \(m_Z\) up to a UV synchronization scale \(\mu_{\mathrm{sync}}\approx 6.8\times 10^{11}\,\mathrm{GeV}\), then with the same MSSM-like coefficients as D10 from \(\mu_{\mathrm{sync}}\) to \(M_U\), and only afterward applies order-one-GeV low-scale matching corrections. The literal all-SM appendix flow by itself lands much lower, near
\[
m_t^{\mathrm{pole}}\approx 164.2\ \mathrm{GeV},
\qquad
m_H\approx 115.2\ \mathrm{GeV}.
\]
The piecewise-running core before those final matching shifts instead yields
\[
m_t^{\mathrm{pole,core}}\approx 170.3\ \mathrm{GeV},
\qquad
m_H^{\mathrm{core}}\approx 126.6\ \mathrm{GeV},
\]
and after the residual matching conventions documented in the spectrum supplement the displayed D11 outputs are
\[
...
\]
The appendix records only the low-scale SM part of the flow, while the D11 outputs above also use the UV synchronization scale, the MSSM-like UV coefficients inherited from D10, and the shared matching, low-scale extraction, and pole-mass conventions stated in this part. The appendix equations alone are not meant as a standalone reproduction recipe for these displayed numbers.

These numbers are the D11 outputs used in this paper. They add no new continuous fit once the D10 gauge trajectory and scale-setting branch are fixed, but they remain a secondary quantitative branch rather than part of the recovered-core theorem package.

The critical-surface branch can therefore fail even when the electroweak calibration succeeds. Conversely, agreement here supports only the supplement-backed D11 implementation analyzed here, not the entire OPH program.

\subsubsection{Rigorous derivation chain: axioms to predictions}\label{623-rigorous-derivation-chain-axioms-to-predictions}
...
The first four steps below record the structural chain. The later threshold and two-input numerics are retained only as an illustrative edge-running sketch, not as the D10/D11 branch used elsewhere in the manuscript.
...
This is Theorem 2.6. The Lagrange multipliers \ensuremath{\lambda}\_a are determined by constraint values, not derived by MaxEnt itself.
...
\textbf{Step 2: The t--\ensuremath{\alpha} bridge (rigorous).}

In 2D Yang-Mills / heat-kernel language, the weight is exp(-(g\textsuperscript{2}A/2) C\textsubscript{2}(R)), giving t = g\textsuperscript{2}A/2. The modular/Euclidean-regularity constraint fixes the collar\textquotesingle s effective area to A = 2\ensuremath{\pi} (the Rindler angle period), yielding:
...
In Planck units (\ensuremath{\ell}\_p\textsuperscript{2} \ensuremath{\equiv} G):

\[
\frac{a_{\rm cell}}{\ell_p^2} = 4\bar{\ell}_{\rm tot}(t_2, t_3)
\]

This is a derived relation, not an assumption. However, the \textbf{numerical value} of a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} depends on the t\_i values, which depend on the couplings.

\textbf{Step 4: Edge-derived beta functions via Z\textsubscript{6} quotient structure (new).}
...
The entropy weights by log d\_R; vacuum polarization loops see d\_R\textsuperscript{2} (both indices of the Peter-Weyl block V\_R \ensuremath{\otimes} V\_R*).

\textbf{Hypercharge via Z\textsubscript{6} quotient.} For (SU(3) \ensuremath{\times} SU(2) \ensuremath{\times} U(1))/Z\textsubscript{6}, the allowed hypercharge lattice is constrained. Writing y = 6Y:
...
\textbf{Result.} At t\_U \ensuremath{\approx} \ensuremath{\pi}\textsuperscript{2}/6 \ensuremath{\approx} 1.645 (corresponding to \ensuremath{\alpha}\_U\textsuperscript{-}\textsuperscript{1} \ensuremath{\approx} 24), with one overall normalization fixed by demanding \ensuremath{\Delta}b\textsubscript{2} matches MSSM:

\[
\Delta b_{\rm pred} \approx (2.49, \, 4.17, \, 4.01)
\]

Compare to MSSM--SM shift: \ensuremath{\Delta}b\_MSSM = (2.5, 4.17, 4.0). Agreement is \textless1\% for all three coefficients.
...
\textbf{Step 5: Historical inverse-problem illustration for threshold and unification scales.}

With edge-derived \ensuremath{\Delta}b and measured electroweak inputs at M\_Z:

\begin{itemize}
\tightlist
\item
  \^{\ensuremath{\alpha}}\textsuperscript{-}\textsuperscript{1}(M\_Z) = 127.951 \ensuremath{\pm} 0.009
\item
  \^{s}\textsuperscript{2}\_Z = sin\textsuperscript{2}\^{\ensuremath{\theta}}\_W(M\_Z) = 0.23122 \ensuremath{\pm} 0.00004
\item
  \ensuremath{\alpha}\_s(M\_Z) = 0.1180 \ensuremath{\pm} 0.0009
\end{itemize}
...
\textbf{Step 6: Historical two-input illustration.}
...
\tightlist
\item
  Pixel constant: a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} = 1.631 (treat as fundamental)
\item
  One electroweak datum: \^{\ensuremath{\alpha}}\textsuperscript{-}\textsuperscript{1}(M\_Z) = 127.951
\end{enumerate}
...
\tightlist
\item
  Pixel: a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} = 4(\={\ensuremath{\ell}}\textsubscript{2} + \={\ensuremath{\ell}}\textsubscript{3})
\item
...
\tightlist
\item
  \ensuremath{\alpha}\_s(M\_Z): predicted 0.1175 vs measured 0.1180 \ensuremath{\pm} 0.0009 \ensuremath{\to} 0.6\ensuremath{\sigma} low
\item
  sin\textsuperscript{2}\ensuremath{\theta}\_W: predicted 0.2310 vs measured 0.23122 \ensuremath{\pm} 0.00004 \ensuremath{\to} \textasciitilde2\ensuremath{\sigma}
\end{itemize}

The \ensuremath{\alpha}\_s agreement is excellent. The sin\textsuperscript{2}\ensuremath{\theta}\_W tension (\textasciitilde2\ensuremath{\sigma}) is where precision threshold/two-loop effects matter.

\textbf{Step 7: Consistency check, beta\_EW from v.}

If the pixel constant P and electroweak VEV v are both treated as inputs, we can solve for the transmutation coefficient \ensuremath{\beta}\_EW that reproduces v:

\[
v = \frac{E_p}{\sqrt{P}} \exp\left(-\frac{2\pi}{\beta_{\rm EW} \cdot \alpha_U}\right)
\]

Using P = 1.63094, v = 246.22 GeV, E\_p = 1.22089 \ensuremath{\times} 10\textsuperscript{1}\textsuperscript{9} GeV:

\[
\beta_{\rm EW}^{\rm req} = 3.997
\]

This is \ensuremath{\beta}\_EW = 4 to within 0.1\%. The integer 4 = N\_c + 1 (number of SU(2) doublets per generation) emerges from fitting, but also has a structural rationale from the Witten anomaly constraint.
...
G = a\_cell/(4\={\ensuremath{\ell}}) relation & Derived from generalized entropy matching \\
Numerical value of a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} & \textbf{Not derived}; requires fixing t (hence \ensuremath{\alpha}) \\
...
\ensuremath{\beta}\_EW = 4 & Structural (N\_c + 1) or fitted; ambiguous status \\
\end{longtable}
}
...
\item
  Treating a\_cell/\ensuremath{\ell}\_p\textsuperscript{2} as fundamental input (replaces one coupling)
\item
  Using measured v to fix the transmutation chain
\end{enumerate}

A two-input mode of this kind is a useful illustration of how one could trade one electroweak datum for the pixel constant. The branch analyzed elsewhere in the manuscript instead follows the D10/D11 construction summarized above; full closure (option 1) awaits a UV completion that specifies the MaxEnt constraint values.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Open Questions}\label{7-open-questions}
...
\item
  Derive the nonperturbative conversion \(C_p = m_p/\Lambda_{QCD}\) from first principles (the benchmark analyzed here uses lattice QCD\textquotesingle s \(C_p \approx 4.47\))
\item
...
\subsection{Critical Evaluation}\label{8-critical-evaluation}
...
\textbf{D8} & product gauge structure up to finite quotient on the admitted chiral matter branch & compact Lie representation classification; Schur's lemma & MAR, connected admissible class, one connected abelian factor, \([z]=0\), and admitted chiral/Yukawa-capable matter data & Phase I conditional realized-branch theorem \\
\textbf{D9} & realized \(\SU(3)\times\SU(2)\times\U(1)/\mathbb Z_6\), exact hypercharges, the realized pair \((N_g,N_c)=(3,3)\), and no gauge-mediated proton decay as a product-group corollary on that admitted matter branch & anomaly algebra; Witten anomaly; CKM CP counting & realized one-generation/one-Higgs package and MAR admissibility premises & Phase I conditional realized-branch theorem/corollary chain \\
\textbf{D10} & gauge/electroweak numerical closure of the D10 calibration branch & RG running and matching conventions & external input \(P\), pixel closure, and the printed threshold conventions & Phase II calibration sector \\
\textbf{D11} & Higgs/top critical-surface branch & RG transport and pole-mass conversion & critical-surface condition \(\lambda=\beta_\lambda=0\) at \(M_U\), plus the UV synchronization scale \(\mu_{\mathrm{sync}}\) used in the D11 transport; no new continuous fit once D10 is fixed & Phase II quantitative branch \\
...
\def\labelenumi{\arabic{enumi}.}
\item
Phase I, the recovered core carried by this manuscript, is D1--D5 together with D7. D8--D9 are additional conditional realized-branch consequences recorded in the same manuscript, but they require the admitted one-generation chiral/Yukawa package stated in their premise columns and should not be read back as bosonic-gauge reconstruction outputs.
\item
Phase II contains the separate input-dependent capacity corollary D6 and the D10--D11 quantitative branches. Failure there does not by itself erase Phase I.
\item
...
\item
  \textbf{Gauge structure}: The bosonic gauge group is reconstructed from sector fusion, and the anomaly/gluing link is precise. On the admitted chiral/Yukawa matter branch, the Selection Axiom MAR fixes the SM factors; the CP/UV admissibility window and MAR give \(N_g = 3\), and Witten's parity constraint with that realized generation count gives \(N_c = 3\). Proposition 6.1a gives DHR transportability \(\Leftrightarrow [z]=0\). Justifying the refinement-stability selector for chirality in explicit models remains a target for future work.
\item
...
Failures localize by tier. A failure in a Phase-III flavor, dark-sector, baryogenesis, spectroscopy, string, strong-CP, proton-spin, or similar continuation retracts that continuation only. A failure in D10 or D11 challenges the corresponding quantitative branch. Only a failure in Phase I would overturn the recovered-core claim set.
