...
OPH is an observer-centric reconstruction program for fundamental physics. Physical data live on a horizon screen \(S^2\), and reality is encoded by the mutual consistency of overlapping patch descriptions. This paper works with five OPH axioms, one explicit external input used only in the cosmological branch, one Phase-II pixel variable fixed on a later outer/inner closure branch, and theorem-local technical premises stated where needed for the scaling, gauge, and transport arguments below.
...
\item
Phase II contains the separate input-dependent capacity corollary D6 and the D10 quantitative-closure branch. Failure there does not by itself erase Phase I.
\item
