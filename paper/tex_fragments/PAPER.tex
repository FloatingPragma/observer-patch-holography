...
declared physical quotient. Certified read-from provenance generates a finite informational poset
and its exact longest authenticated-parent-chain height. Independently, the exact
rank-three source Gram quotient and a real axis define a four-dimensional
ambient target carrier with Lorentz inertia \((1,3)\); source height enters
only through the temporal coordinate of a scaled event placement. This
is not an intrinsic dimension estimate for the finite poset, and the source
unit directions are exactly the celestial \(S^2\) and future-null-ray boundary. This removes a
freely declared rank-four chart and fitted signature from the finite precursor. Modular flow on
caps independently supplies Lorentz-frame kinematics and the three-dimensional observer-frame
hyperboloid. Neither result constructs physical spacetime. A smooth effective Lorentzian manifold
requires one source-selected spatial placement and refinement family carrying exact two-way physical
cone/order agreement, count--volume calibration, independent dimension/manifoldlikeness and
topology tests, and uniqueness control. Smooth Einstein promotion further
requires same-family tensor-curvature convergence or the independent continuum
small-ball/null-balance identification; scalar-curvature convergence alone is diagnostic.
...
enter only on their declared quantitative branches. The source-causal continuum and Einstein theorems do
...
Identifying it with a physical Riemannian bulk or with Lorentzian/Einstein spacetime (via the source-causal continuum certificate of Theorem 4.3e, under its physical-order, refinement, count--volume, independent-dimension, manifoldlikeness, topology, and curvature receipts) requires the
...
The executable causet audit tests authenticated histories themselves.
Independent generation and replay at every cutoff prove exact induced-prefix custody for the
\(4,8,16,32,64\)-round family, from 24 to 384 events. Width is exactly two
and ordering fraction rises from \(0.8841\) to \(0.9922\), so this is an
exact custody family rather than a spacetime-density refinement. On the
24-event control, one prescribed placement puts records at authenticated
icosahedral port anchors and readback/feedback events at arithmetic
consumed-record barycentres, with all carrier identities expressed in one
stipulated reference port frame. Eight
same-source-height pairs collide and eight different-source-height incomparable pairs have zero
spatial separation; the causal lower scale is \(1.129775730952839\) while the
spacelike upper scale is zero. No admissible scale makes the map injective and
order-reflecting. Producer, independent
verifier, receipt, and negative controls agree~\cite{ophfpe}. This proves
source-order custody and falsifies that single-frame/barycentre ansatz. It
does not derive inter-carrier gluing or exclude a different physical
refinement that passes the continuum certificate.
The same 24-event order has 30 Hasse links, all authenticated direct reads.
Every link carries a source-native carrier-local port label and, in the
receipt's hidden reference presentation, a normalized future-null
representative \((1,n)\). The used-port ranks are \(2,1,2\), so no carrier
populates rank three at this cutoff. Independent \(A_5\) relabeling changes
the pooled rank from three to two while preserving each local rank, and naive
shared-template path integration is path-dependent. This exact positive
decoration statement supplies no local rank-three Lorentz cone, frame gluing,
integrated event placement, order reflection, or physical attachment.
...
A separate post-hoc exploratory interval comparison, frozen after evaluation
for exact replay, compares the current \(2{,}304\)-event local-domain cut with
Poisson-sprinkled \(3+1\) Minkowski, de~Sitter flat-patch, and nonmanifold
controls. All \(736\) qualifying intervals of size \(32\)--\(72\) lie outside
the frozen exploratory four-dimensional
ordering-fraction band. This is current-cutoff dissimilarity for the captured
event semantics, not manifoldlikeness or a physical no-go; no certified OPH
density-refinement family or held-out confirmation is present. The full RER
mirror and checker are \path{evidence/causet_likeness/causet_likeness_receipt.json}
and \path{evidence/causet_likeness/verify_causet_likeness_receipt.py}. The exact
underlying local-domain package and its archive checker are under
\path{evidence/local_domain/}.
That receipt also contains a positive grammar-compatibility control. For four
seeds and nested target means \(64,128,256\), Poisson sprinklings of one fixed
Minkowski \(3+1\) Alexandrov interval compile through declared-parent-list-free single-writer
provenance. At all 12 levels the full four-clause authentication holds,
rederived direct edges equal the geometric Hasse links, closure equals the
seeded order, coordinate-map injectivity, sampled-interval membership, and the
fixed-volume Poisson count channel are independently rechecked, jointly forming
a construction-seeded causal-set faithful-embedding control, and
carrier/order restrictions are induced and event material is
regulator-specific. Counts lie within three Poisson
standard deviations. The seed-averaged Myrheim--Meyer diagnostic estimates are
\(4.02824,3.84595,3.90346\)~\cite{reid2003}.
Geometry, density, event population, read pattern, and placement are imported
controls. This proves source-grammar
compatibility with a \(3+1\) causal-set carrier and order, not OPH selection,
similarity, manifoldlikeness, dimension, volume, or a continuum.
...
the required source-causal continuum, scale, and curvature-convergence receipts, the standard GR
...
observer-facing geometry, effective Lorentzian spacetime, and classical Einstein branch. None of these
...
smooth Einstein equation on the physical effective-spacetime and curvature/stress
...
event-record stitch theorem. It consumes affine event supports under the effective-spacetime certificate
...
\emph{Resolution:} The Lorentz result has two exact inputs that meet before any continuum assumption. On the extracted geometric subnet, BW scaling and cap-normal reconstruction give \(q(\Omega)=(1,\Omega)\), \(n_{gC}=\Lambda_g n_C\), \(\mathrm{Conf}^{+}(S^2)\cong\mathrm{SO}^{+}(3,1)\), and the three-dimensional observer-frame space \(H^3\). Independently, authenticated semantic read-from provenance generates the finite event poset. Its parent relation determines canonical source height: roots have height zero and every nonroot has one plus the maximum direct-parent height. The exact source Gram quotient supplies a rank-three positive carrier. Its direct sum with an independent real axis, \(\mathbb R\oplus V_{\mathrm{src}}\), with \(Q(t,x)=t^2-g_{\mathrm{src}}(x,x)\), is a four-dimensional ambient target carrier with inertia \((1,3)\); source-unit directions are exactly the future-null rays, hence form the same \(S^2\) celestial cone. A positive \texttt{timeScale} multiplies source height only when defining the temporal coordinate of an event placement in this carrier. This constructs the target-carrier dimension; it does not estimate the finite poset's intrinsic dimension, and source height is not an operational clock. No Lorentz signature, free precedence relation, or fitted cone is postulated at this finite stage. What remains conditional is physical spacetime: a source-selected spatial placement and inter-carrier gluing, exact two-way physical order/cone agreement, calibrated event-count density, manifoldlikeness and independent dimension tests, stable topology, and convergence to a unique distinguishing Lorentzian limit. The source-history family proves exact induced-prefix custody but has bounded width and is not a density family; one prescribed single-frame port-anchor/barycentre placement of its 24-event control is neither injective nor order-reflecting. That failure rejects this ansatz, not other source-selected placements. Thus the finite ambient \(1+3\) carrier is derived, while the effective manifold claim is tied to an explicit, falsifiable continuum certificate rather than to supplied-atlas gluing assumptions.
