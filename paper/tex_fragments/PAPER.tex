...
\(\mathbb Z_6\) kernel, and the maximal faithful matter image. This implication uses no Minimal
Admissible Realization premise. The cover and its \(\mathbb Z_2\), \(\mathbb Z_3\), and
...
CKM/weak-sector clauses give the window \(3\le N_g\le5\); the generation count inside that window is an open physical attachment, and the former least-value selection is withdrawn from its
declared branch. Family multiplicity, extra-sector completeness, and the charged-lepton and D10
