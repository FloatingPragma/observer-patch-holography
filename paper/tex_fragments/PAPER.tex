...
This imports the D6 static-patch scale into the continuation surface. Any galaxy-scale continuation built from the anomaly term would therefore be an IR phenomenon, appearing only when accelerations are small and distances are large.
...
\Lambda=\frac{3\pi}{G N_{\mathrm{scr}}}.
\]
If the screen Hilbert space has finite dimension, the natural semiclassical interpretation is a cosmological horizon with finite entropy. On the same D6 branch, this fixes the static-patch package
\[
...
\]
while the observed cosmic age remains a downstream FLRW benchmark rather than an additional D6 theorem output. The local/global stack is therefore: local null-modular reconstruction leaves the metric ambiguity, global screen capacity fixes the remaining constant and its de Sitter readout package. This logic is compatible with the BW scaling branch but does not depend on the scaling-limit algebra remaining type I.
...
\textbf{D12} & charged-lepton centered-readback / centered-to-affine frontier, textures, dark-sector, baryogenesis, spectroscopy, string/worldsheet, and other downstream continuations & branch-specific EFT and phenomenological manipulations & explicit additional ans\"atze beyond D6, D9, or D10 & Phase III phenomenological continuation / program branch \\
\end{longtable}
}
\endgroup
