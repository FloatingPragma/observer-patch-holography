...
This turns ``local Gibbs is an assumption'' into ``local Gibbs is a corollary of what constraints we allow.''
...
\textbf{Refinement-stable multiplier branch.} Because the same finite constraint family is preserved under coarse-graining, the regulator states lie in one common finite-dimensional multiplier family rather than in unrelated state spaces at different cutoffs. The ``refinement-stable'' language used later therefore means persistence along the stable or fixed branch of this multiplier family. In particular, whenever the gauge derivation speaks of a refinement-stable directed colimit of sectors, the refinement-stable qualifier is supplied by the third OPH axiom; only the transportability, symmetry, bosonic, and fiber-functor clauses remain extra.
...
\textbf{Proposition 2.6 (Geometric modular action on the BW/canonical cap branch).} On the \textbf{T1}+\textbf{T3} branch, let \(\alpha_{\lambda_C(s)}\) be the automorphism of the scaling-limit cap net induced by the standard cap-preserving conformal subgroup \(\lambda_C(s)\). Then
\[
...
This shows the modular-covariance condition serves as a regularity condition on how the geometric labeling matches the limit algebra net. It does not add extra physics and is required if locality is to be meaningful.
...
MaxEnt therefore selects the unique finite-stage invariant state compatible with those conserved charges. What it does \emph{not} prove by itself is that the scaling-limit state is already the vacuum or canonical cap state. The stronger statement is exactly \textbf{T1}+\textbf{T3}: one refinement-stable branch of the same finite-dimensional multiplier family converges to the cap state whose modular automorphism group is the standard geometric one. If the limit algebra is non-type-I, that modular action is outer.

\textbf{QNEC internalization.} The Quantum Null Energy Condition has rigorous QFT proofs in broad settings~\cite{bousso16}. On the \textbf{T1}+\textbf{T3} branch, the Recoverable Generalized Entropy axiom can be supported internally by:
\begin{itemize}
\item
...
The \textbf{T1}+\textbf{T3} geometric-modular scaling branch is therefore a phase statement: the refinement-stable MaxEnt fixed point must lie in the geometric modular action class. That is a concrete condition on the UV completion rather than an automatic consequence of MaxEnt alone.
The smallest honest internal extension beyond this boundary is a two-step chain, not a monolithic BW repair theorem: first, a scaling-limit cap-pair extraction theorem that extracts \((\mathcal A_\infty(C),\omega_\infty^C)\) as local weak-\(*\) limits of transported cap marginals with vanishing carried collar errors; second, an ordered null cut-pair rigidity theorem that collapses the residual cap-preserving conformal freedom on those extracted cap pairs to the unique BW hyperbolic subgroup, after which modular KMS fixes the \(2\pi\) normalization. That chain is not part of the proved theorem surface.
...
\]
(ii) MaxEnt selection with rotationally invariant local constraints on the fixed-cutoff regulator net; and (iii) the refinement-stable scaling-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\) lies in the Bisognano--Wichmann geometric modular class, i.e.\ for each cap \(C\) the scaling-limit modular automorphism group is the standard cap-preserving conformal subgroup \(\lambda_C(s)\), with \(s\) normalized so that the null blow-up near a smooth cut acts by \(v\mapsto e^{-s}v\). Then
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(2\pi t)}
...
\textbf{Scope note.} The theorem proves the Bisognano--Wichmann conclusion \emph{on the OPH refinement branch lying in the stated BW geometric modular class}. It does not prove that MaxEnt alone selects that branch, and it does not claim that the continuum algebra remains type I.
...
Using the collar decomposition and the collar double-scaling hypothesis, established at regulator level via Theorem 2.3 or alternatively via the local-Gibbs form together with exponential mixing, the state takes the Markov normal form. MaxEnt selection maximizes entropy within each edge sector, producing
...
\item
on the explicit \textbf{T1}+\textbf{T3} branch, the refinement-limit cap modular action is geometric, and in the continuum case of interest it may be outer on a non-type-I observer algebra.
\end{enumerate}
...
\textbf{Yukawa continuation benchmark (Section 6.21).} A continuation-level question is whether a defect-mediated ansatz \(y_f \propto 6^{-n_f}\) with integer defect charges leaves the extracted exponents \(-\ln(y_f)/\ln(6)\) unusually close to integers across the charged-fermion spectrum. The small parameter \(\varepsilon = 1/6\) is motivated by the same \( \mathbb{Z}_6 \) structure that underlies the conditional \(\log_2 6\) center-label entropy ansatz; this ties the benchmark hierarchy to a quotient-label mechanism rather than to a chosen Froggatt-Nielsen parameter. It is not a recovered-core prediction across all fermions, and its interpretation still depends on the extra flavor ans\"atze, scheme choices, and running/matching conventions identified in the tier ledger.
...
\begin{itemize}
\item \textbf{Flavor and Yukawa data}: Yukawa matrices, flavor hierarchies, charged-lepton/Koide fits, quark-texture exponents, CKM/PMNS closure, and related downstream flavor fits. These require extra ans\"atze such as a uniform \(\mathbb Z_6\) center-label ensemble, circulant generation-space structure, discrete exponent assignments, or further external numerical machinery. The light-quark split remains a D12 continuation frontier rather than a recovered-core selector, the charged-lepton lane fixes only the centered hierarchy until an absolute-normalization transport scalar is emitted, and the neutrino continuation branch reaches the physical PMNS/hierarchy pattern while still leaving one positive absolute-scale scalar open.
\item \textbf{Hadron masses and resonances}: the present particle branch supplies the seeded-family, correlator, and stable-channel readout bridge, but promotable hadron rows remain execution-bound. They require real nonperturbative production computation and production systematics rather than an additional symbolic theorem alone, so they are not derived outputs of the current particle-spectrum paper.
\item \textbf{Neutrino data}: beyond the input-dependent, order-of-magnitude capacity estimate tied to $N_{\mathrm{scr}}$, the recovered core does not derive neutrino masses or mixings. A continuation branch emits intrinsic masses, splittings, and PMNS data; its repaired weighted-cycle branch reaches the physical PMNS/hierarchy regime, but the absolute spectrum remains one positive normalization scalar short of theorem-grade closure.
...
\item \textbf{Hadron execution boundary}: the current hadron lane is not blocked by one small symbolic gap. The honest next object is a production correlator dump together with production statistical and systematic control, so hadron rows remain outside the present derived output surface.
\end{itemize}
