...
Planck-\(\Lambda\) central capacity is \(N_\Lambda\simeq3.313\times10^{122}\), a mismatch of
about \(6.6\) percent. Capacity-level
...
controls. The complete capacity-indexed A1--A3 packet lift is absent. A
positive result needs that lift, a source law selecting a unique continuation
...
\textbf{Theorem 4.2 (Finite cap-net support-visible \(\mathrm{BW}_{S^2}\) scaling theorem).} Let
\(\widehat C=(C,n_C,p_C^-,p_C^+)\) be a nondegenerate BW-framed cap. For every OPH-realized
observer-supporting refinement branch satisfying Axioms 1--3, the named fixed-cutoff
regulator, collar, consensus, and optimizer-pushforward interfaces, the finite cap-normal BW certificate above, and independently
the complete multiresolution reference-tower and algebra-state comparison package of Theorems 2.6c--2.6f on that same
branch, the
support-visible geometric cap net admits a weak-\(*\)/GNS scaling-limit cap pair
\((\mathcal A_\infty^{\mathrm{geo,sv}}(C),\omega_\infty^{\mathrm{geo},C})\). Then the
scaling-limit modular automorphism group is
\[
\sigma_t^{\omega_\infty^{\mathrm{geo},C}}
=
\alpha_{\lambda_{\widehat C}(2\pi t)}.
...
category. The complete capacity-indexed A1--A3 source packet is absent, and
no unique physical fixed point is supplied. Horizon
...
differ by about \(6.6\) percent; that comparison becomes physical only
after a positive direct result and both bridges share a certified carrier.
