...
This paper uses five OPH axioms. The only labeled technical premises used below are the canonical premises \textbf{T1}--\textbf{T7} stated in Section 1.4. Other theorem-local hypotheses are written out in prose where needed rather than assigned a separate label family.
...
Later theorem statements use only the canonical technical-premise labels \textbf{T1}--\textbf{T7}. Fixed-cutoff regulator bookkeeping, quasi-local propagation, endpoint control, collar decomposition, and related branch-internal statements are cited directly from axioms and earlier theorems rather than through a second premise-label system.
...
\textbf{Bisognano--Wichmann geometric branch.} The refinement-stable OPH branch for caps lies in the geometric modular class, together with the controlled collar/refinement limit that carries the Markov and recovery remainders explicitly. In the canonical numbered list below, this is the content of \textbf{T1} together with the scaling-limit scope clause \textbf{T3}.
...
\textbf{Fixed-cutoff realized presentation.} At a UV scale \(\ell_{\mathrm{UV}}\), local patch algebras are type-I with finite-dimensional Hilbert spaces, and gauge-as-gluing is realized as a boundary group action whose fixed-point algebra defines physical observables. Section 2.3 proves the resulting fixed-cutoff collar package directly from overlap consistency on this realized presentation.
...
\begin{itemize}
\tightlist
\item \textbf{T1}: the branch condition that the refinement-stable cap net lies in the Bisognano--Wichmann geometric modular class for caps and their conformal images.
\item \textbf{T2}: vanishing of the relevant gluing obstruction class whenever global sector transportability is invoked, written \([z]=0\) on the central branch or \(q_\Sigma=0\) on the genuinely noncentral branch.
...
\textbf{T1} & Bisognano--Wichmann geometric branch for caps and their conformal images; use: Theorem 4.2 and the Lorentz branch; status: canonical technical premise. \\
\textbf{T2} & Vanishing relevant transport obstruction; use: gauge reconstruction and realized product-group branch; status: canonical technical premise when invoked. \\
\textbf{T3} & Controlled refinement/scaling-limit scope clause; use: Lorentz/null-modular/Einstein branch; status: canonical technical premise. \\
\textbf{T4} & Fixed-cap generalized-entropy stationarity; use: Jacobson branch; status: canonical technical premise. \\
\textbf{T5} & Symmetric braiding in the \(3+1\)D EFT branch; use: compact gauge reconstruction; status: canonical technical premise. \\
\textbf{T6} & Bosonic Tannakian fiber functor or explicit super-Tannakian fork; use: compact gauge reconstruction; status: canonical technical premise. \\
\textbf{T7} & Directed colimit of transportable edge sectors with objectwise finite-dimensional fibers; use: compact gauge reconstruction; status: canonical technical premise. \\
\end{longtable}
}
...
\textbf{D5} & Theorem 5.1: Einstein branch & D3+D4 & Jacobson small-ball mathematics; null-to-tensor reconstruction modulo a metric term & T4 together with the locally Lorentzian \(d=4\) scaling regime & conditional scaling-limit theorem \\
...
\textbf{D7} & Theorem 6.1: compact gauge reconstruction & Axioms 1--4 & Doplicher--Roberts / Tannaka reconstruction & T2 and T5--T7 & conditional structural theorem \\
\textbf{D8} & Section 6.2: product gauge structure up to finite quotient & D7 + Axiom 5 (MAR) & compact Lie representation classification; Schur's lemma & connected admissible class, one connected abelian factor, and vanishing relevant transport obstruction & realized-branch theorem \\
...
This is the standard clustering/mixing condition for local Gibbs states, equivalent to assuming
that the MaxEnt state lies in a Dobrushin uniqueness regime or has a uniform spectral gap. It is a
natural UV-state condition and is not derived from Axiom 3 alone.
...
\textbf{What this example does and does not close.} The quantum-link realization makes the fixed-cutoff matrix/fixed-point bookkeeping concrete. What it does \textbf{not} automatically guarantee is that modular flow on caps becomes geometric conformal dilation with the \(2\pi\) KMS normalization, that is, the continuum modular-covariance regime encoded by \textbf{T1} and \textbf{T3}. That requires the state to sit in a regime that is effectively relativistic/QFT-like in the continuum limit. Viable architectures for this include holographic quantum error-correcting codes~\cite{pastawski15} and quantum double / string-net Hamiltonians~\cite{levinWen05}.

\subsubsection{Conformal-modular fixed-point microphysics}\label{27-conformal-modular-fixed-point-microphysics-cmfp}

After Section 2.3, the remaining gap is narrower. On the local finite-constraint MaxEnt branch of the third OPH axiom, the logarithm of the selected state is a quasi-local UV generator, and the refinement-stable branch lies inside one common finite-dimensional multiplier family. The unresolved question is not whether the fixed-cutoff branch is local; that part is internal. The unresolved question is which algebraic phase the refinement limit realizes. At each regulator stage the patch and cap algebras are type-I matrix algebras. The scaling-limit observer algebra may leave that class. OPH therefore proves a local-Gibbs/refinement-stable branch with propagation and endpoint control, but it does \emph{not} prove that every such branch lands in the Bisognano--Wichmann geometric phase or that MaxEnt alone already selects a vacuum or canonical cap state. The honest conditional boundary is \textbf{T1}+\textbf{T3}: they identify the BW/geometric branch boundary, while the internal extraction of a realized scaling-limit cap pair still waits on the derived carried-collar schedule above the transported fixed-local-collar Markov/faithfulness datum and the later weak-\(*\) extraction plus GNS gluing step.
...
\textbf{Proposition 2.6 (Geometric modular action on caps in the BW/canonical cap phase).} On the \textbf{T1}+\textbf{T3} branch, for any realized scaling-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\), let \(\alpha_{\lambda_C(s)}\) be the automorphism induced by the standard cap-preserving conformal subgroup \(\lambda_C(s)\). Then
\[
...
\textbf{Proof.} \textbf{T1} identifies the scaling-limit cap modular group with the standard geometric cap subgroup up to normalization. The modular KMS condition fixes the normalization to \(2\pi\). If the limit algebra is type I, the automorphism statement may be represented by a modular Hamiltonian \(K_C\); otherwise the automorphism statement is the full content. QED.

\textbf{Alternative derivation via net regularity.} The same modular-covariance property can also be read off from a scaling-limit support map when the net satisfies the outer-regularity / minimal-support condition used later. This does \emph{not} choose the BW branch; it clarifies how the geometric labeling of the limit net is read once that branch is present.
...
\textbf{Null-surface modular structure.} On the same \textbf{T1}+\textbf{T3} branch, the null-surface modular machinery narrows as follows:
\begin{itemize}
\item
...
MaxEnt therefore selects the unique finite-stage invariant state compatible with those conserved charges. What it does \emph{not} prove by itself is that the scaling-limit state is already the vacuum or canonical cap state. The stronger statement is exactly \textbf{T1}+\textbf{T3}: they identify the BW/geometric branch boundary, while the internal extraction of a realized scaling-limit cap pair still waits on the vanishing carried-collar schedule and the later weak-\(*\) extraction plus GNS gluing step. If the limit algebra is non-type-I, the geometric modular action on that branch is outer.

\textbf{QNEC internalization.} QNEC has rigorous QFT proofs in broad settings~\cite{bousso16}. On the \textbf{T1}+\textbf{T3} branch, the Recoverable Generalized Entropy axiom can be supported internally by:
\begin{itemize}
\item
...
\begin{itemize}
\item
\textbf{Internal to OPH:} the local-Gibbs form, quasi-local propagation, bounded-interval endpoint control, and the refinement-stable notion of sector persistence, all from the local finite-constraint MaxEnt branch.
\item
\textbf{Theorem-external branch statement:} that the scaling limit lands in the Bisognano--Wichmann geometric modular class. On that branch the limit algebra may be non-type-I, the modular action may be outer, and the \(2\pi\) normalization and later null half-sided-inclusion bridge follow.
\end{itemize}

The present best internal extension route is a two-step chain, and the current pressure point is the first object. First, one needs a canonical scaling-limit observer cap-pair realization from transported marginals. The theorem contract already visible in the current corpus is that the realized transported cap-local system packages the cap-local test family, the projectively compatible transported marginal family, and the asymptotic transport-equivalence certificate. The remaining emitted witness is a derived vanishing carried-collar schedule on fixed local collars, now explicitly decomposed into the constructive-recovery remainder and the faithful modular-defect term, namely convergence of the carried quantity
\[
...
\]
to zero after transport to each fixed local collar model; only then can local weak-\(*\) extraction and GNS gluing emit the realized scaling-limit cap pair. Beneath that derived schedule, the sharper raw datum is the transported fixed-local-collar Markov/faithfulness package. Second, one needs ordered cut-pair rigidity on that realized limit. That stage remains symbolic until the realized scaling-limit cap pair exists. The weak-\(*\) extraction plus rigidity route is therefore an explicit internal extension route, not part of the proved theorem surface.
...
The finite collar technology works on the finite type-I regulator net. The Lorentz statement is instead about the scaling-limit cap pair carried by the controlled refinement clause \textbf{T3}. Nothing in local MaxEnt alone proves that this limit stays type I or that it lands in the Bisognano--Wichmann branch. The theorem therefore has one honest conditional boundary: if the realized refinement-stable branch lies in the BW geometric modular class of \textbf{T1}, then the scaling-limit modular automorphism group is geometric, and the only normalization fixed internally is the standard \(2\pi\) one.

\textbf{Theorem 4.2 (\(\mathrm{BW}_{S^2}\) on the OPH Bisognano--Wichmann branch).} Let \(C\subset S^2\) be a cap and let \((A_\delta,B_\delta,D_\delta)\) be a shrinking collar family around \(\partial C\). Assume: (i) the derived collar-control/fixed-cutoff regulator package used throughout Section 4, so that exact collar formulas are read either at exact Markovity or along a controlled family with
\[
\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta)\to 0;
\]
(ii) the controlled scaling-limit scope clause \textbf{T3}; and (iii) the realized refinement-stable scaling branch for caps lies in the Bisognano--Wichmann geometric modular class, i.e.\ for each realized scaling-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\) the modular automorphism group is the standard cap-preserving conformal subgroup \(\lambda_C(s)\), with \(s\) normalized so that the null blow-up near a smooth cut acts by \(v\mapsto e^{-s}v\). Then
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(2\pi t)}
\]
on the scaling-limit cap net, where \(\alpha_{\lambda_C(s)}\) is the automorphism induced by \(\lambda_C(s)\).

At finite collar scale the regulator modular Hamiltonian \(K_C^{(\delta)}\) exists because the cap algebra is type I, and replacing its action by the geometric cap-dilation action incurs only the carried errors
\[
r_{\mathrm{FR}}(\varepsilon_\delta)
\qquad\text{and}\qquad
4\lambda_\ast^{-1}\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta).
\]
If the scaling-limit cap algebra happens to remain type I, the same statement may be written as
\[
K_C=2\pi B_C.
\]
In the generic continuum case, the scaling-limit cap algebra has left the regulator class and the modular action is generally outer rather than inner.

\textbf{Proof.} Markov locality does one precise job: it reduces the modular problem to the shrinking collar, with the finite-stage deviation from an exact Markov reference carried explicitly by \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(4\lambda_\ast^{-1}\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta)\). Assumption (iii) then identifies the scaling-limit modular automorphism group with the standard cap-preserving conformal subgroup up to normalization:
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(\kappa_C t)}
\]
for some \(\kappa_C>0\). The modular group is KMS by definition, and because \(s\) is normalized by unit null dilation on the BW branch, the usual Bisognano--Wichmann relation fixes \(\kappa_C=2\pi\). Thus
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(2\pi t)}.
...
\textbf{Scope note.} The theorem proves the Bisognano--Wichmann conclusion \emph{on the OPH refinement branch lying in the stated BW geometric modular class}. It does not prove that MaxEnt alone selects that branch, and it does not claim that the continuum algebra remains type I. When the manuscript informally uses the phrase ``BW/canonical cap phase,'' it means only this \textbf{T1}-selected branch, not an independently derived vacuum or canonical-state theorem.

\textbf{Current internal extension route.} The remaining burden is an explicit two-object scaffold. First, one needs a realized scaling-limit cap-pair extraction from transported marginals. The current theorem contract is already sharper than a placeholder: the realized transported cap-local system packages the cap-local test family, the projectively compatible transported marginal family, and the asymptotic transport-equivalence certificate. The remaining emitted witness is a derived vanishing carried-collar schedule on fixed local collars, now explicitly decomposed into the constructive-recovery remainder and the faithful modular-defect term, namely the convergence to zero of the carried quantity
\[
r_{\mathrm{FR}}(\varepsilon_{n,m,\delta})
+
4\lambda_\ast^{-1}\,
...
\bigl(\varepsilon_{n,m,\delta}\bigr)
\]
after transport to each fixed local collar model; only then can local weak-\(*\) extraction and GNS gluing emit the realized scaling-limit cap pair. Second, one needs ordered cut-pair rigidity on that realized limit. The rigidity calculation remains symbolic until the realized cap pair exists.
...
Section 4.2 fixes the cap modular statement first at the automorphism level. On the \textbf{T1} branch, the scaling-limit cap modular flow is geometric with the standard \(2\pi\) normalization,
\[
\sigma_t^{\omega_\infty^C}
=
\alpha_{\lambda_C(2\pi t)}.
...
\end{itemize}
This is the route from null-strip factorization to the later Jacobson branch, without identifying
the null modular bridge itself with the downstream density-upgrade and null-stress steps.
...
\item
on the explicit \textbf{T1}+\textbf{T3} branch, the realized scaling-limit cap modular action is geometric and may be outer on a non-type-I observer algebra.
\end{enumerate}

\textbf{Current BW-side status.} The Lorentz side remains at the same explicit conditionality boundary recorded elsewhere in the manuscript. The current best internal extension route is still a two-object scaffold: first a realized scaling-limit cap-pair extraction from transported marginals, then ordered cut-pair rigidity on that realized limit. The present pressure point is the first object. Until that realized scaling-limit cap pair exists, the BW/canonical language remains theorem-external shorthand for the \textbf{T1}+\textbf{T3} branch rather than a fully internalized static-patch theorem.
...
At any fixed UV cutoff, edge-center completion provides finite tensor categories of transportable edge charges. The local MaxEnt / collar-mixing package established earlier controls fixed-cutoff recoverability, modular-support localization, and carried error terms on the realized branch. It does \emph{not} by itself prove that transportable edge sectors persist as nontrivial objects under refinement, and it does \emph{not} by itself determine whether the refinement limit lies in a trivial or nontrivial gauge phase. Whenever compact gauge reconstruction is invoked, that transportable refinement-directed sector structure is exactly the content of \textbf{T7}.
...
for the directed colimit retaining the sectors and intertwiners that persist in that transportable system, with objectwise finite-dimensional fibers. This gives a tensor category \(\mathsf{Sect}_\infty\) of edge charges:
...
\textbf{Corollary.} On the central branch, the "DHR transportability" condition in Corollary 6.1 is internal to the gluing framework: it is equivalent to requiring that the central obstruction class vanishes. This is a constraint on the allowed sector structure, not an external physical assumption. On the genuinely noncentral branch, Corollary 3.4a gives the parallel criterion in terms of the higher-gauge class \(q_\Sigma\).
...
The complete formal statement, admissibility definitions, and proof route are given in the rest of Section 6.2 and in the dedicated gauge-derivation surface.
...
The full proof route is given in the remainder of Section 6.2 and in the dedicated gauge-derivation surface.
...
\textbf{Theoretical derivation.} The heat-kernel law can be derived from the axioms once the edge-sector generator is known to take the quasi-local local-Gibbs form derived in Theorem 2.6.

\textbf{Theorem 6.20 (Heat-kernel law from MaxEnt + gauge structure).} Under the OPH axioms, the fixed-cutoff overlap-gauge realization of Sections 2.3 and 3.2, and the quasi-local local-Gibbs form supplied by Theorem 2.6, the edge-sector probability distribution satisfies:
...
\textbf{Status.} The derivation is complete up to the factorwise Laplacian choice just stated. The quasi-local local-Gibbs generator used here is derived from Theorem 2.6: if MaxEnt constraints are expectations of finitely many quasi-local operators, the entropy maximizer is automatically a Gibbs state with a quasi-local generator. What remains is the specific \emph{Laplacian form} of that generator; this follows from gauge invariance plus the factorwise uniqueness of the bi-invariant second-order operator on each compact simple factor.
...
\textbf{D3} & Theorems 4.2--4.3 Lorentz branch & Bisognano--Wichmann modular geometry; conformal-group classification & rotationally invariant constraints together with T1 and T3 & Phase I conditional scaling-limit theorem \\
...
\textbf{D8} & product gauge structure up to finite quotient under MAR & compact Lie representation classification; Schur's lemma & MAR, connected admissible class, one connected abelian factor, and vanishing relevant transport obstruction & Phase I realized-branch theorem \\
...
\textbf{D10} & gauge-coupling numerical closure and target-free electroweak repair theorem of the D10 calibration branch & RG running and matching conventions & external input \(P\), pixel closure, and the printed threshold conventions & Phase II calibration sector \\
...
\paragraph{Shared excitation dictionary benchmark (Section 6.21).} The current family continuations are organized by one common excitation dictionary rather than by a primitive one-number suppression law. The proof-facing data are the same-label gap/overlap scalars
\[
(g_e,\omega_e),\qquad
d_e=1-\omega_e,\qquad
...
\]
equivalently the centered eta-class \([\eta_e]\). The intrinsic neutrino lane factors exactly through this dictionary; the quark mass-side continuation compresses transported same-label data to \((\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})\) and on the compact same-family branch to the ray \(\mathcal R_{D12}^{ud}\), while the CKM/CP frontier is now sharper than a generic wrong-sheet memo in a different way: the local same-label left-handed orbit closes to the singleton \(\sigma_{\mathrm{ref}}\), so the selected branch is fixed but remains the current wrong-branch D12 sheet. The mass side is equally sharp: an explicit underdetermination theorem says that the present same-family D12 branch emits only that one-parameter ray and not a theorem-grade modulus on it. The next exact theorem-side quark object is therefore the intrinsic scale law on the emitted D12 mass ray, not another selector theorem. The charged lane has an exact same-carrier readback once the charged sector emits its source pair; its theorem-grade waiting set is the promotion \(\widehat C_e^{\mathrm{cand}}\to \widehat C_e\) and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), from which the uncentered lift, determinant-line section, and affine anchor \(A_{\mathrm{ch}}\) follow canonically. The older scalar benchmark \(\varepsilon = 1/6\) survives only as the extra uniform sixfold center-label collapse of that richer dictionary, so exponents \(-\log(y_f)/\log 6\) are compare-only summaries of excitation action rather than recovered-core outputs or independent flavor inputs.
...
\begin{itemize}
\item \textbf{Flavor and Yukawa data}: Yukawa matrices, flavor hierarchies, charged-lepton/Koide fits, quark-texture exponents, CKM/PMNS closure, and related downstream flavor fits are not Phase-I outputs. The common proof-facing family base is the same-label excitation dictionary
\[
(g_e,\omega_e)\longmapsto d_e=1-\omega_e,\qquad q_e=\sqrt{g_e d_e},\qquad [\eta_e].
\]
What remains open is not a fresh set of primitive flavor parameters but the lane-specific readout closures above that dictionary: on the emitted local quark surface the same-label selector is already fixed to \(\sigma_{\mathrm{ref}}\), which leaves the physical CKM-shell no-go intact on the current D12 sheet, and an explicit same-family underdetermination theorem says that the mass side emits only the one-parameter ray \(D12_{ud}^{\mathrm{mass}}\). So the next exact quark object is the intrinsic scale law on the emitted D12 mass ray rather than another selector witness. The charged-lepton lane waits on promotion of \(\widehat C_e^{\mathrm{cand}}\) and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), from which the uncentered lift, determinant-line section, and affine anchor \(A_{\mathrm{ch}}\) follow canonically; and the neutrino continuation branch waits only on the reduced bridge-correction invariant \(C_\nu\) above the emitted proxy, with \(B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}\) retained only as the paper-facing amplitude parameterization. The older \(\varepsilon = 1/6\) texture language is only the extra uniform-center-label collapse of this richer dictionary and is not a recovered-core theorem.
\item \textbf{Hadron masses and resonances}: the present particle branch supplies the seeded-family, correlator, and stable-channel readout bridge, but promotable hadron rows remain execution-contract-frozen. They require one production backend export bundle, then real nonperturbative production computation and production systematics rather than an additional symbolic theorem alone, so they are not derived outputs of the current particle-spectrum paper.
\item \textbf{Neutrino data}: beyond the input-dependent, order-of-magnitude capacity estimate tied to $N_{\mathrm{scr}}$, the recovered core does not derive neutrino masses or mixings. A continuation branch emits intrinsic masses, splittings, and PMNS data; its weighted-cycle branch reaches the physical PMNS/hierarchy regime, the normalized same-label overlap-defect weight section closes beneath that branch, and the absolute spectrum remains one reduced bridge-correction invariant \(C_\nu\) short of theorem-grade closure above the emitted proxy, with the paper-facing amplitude \(B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}\) and the positive normalization scalar \(\lambda_\nu\) induced only after that reduced bridge law is fixed. The attached weighted-cycle stack factors through \(q_e = q_{\mathrm{mean}} qbar_e\), so the present corpus does not derive a \(qbar_e\)-only bridge collapse law. The best closed constructive local object beneath that bridge is the defect-weighted same-label edge family \(q_e=\sqrt{g_e d_e}\) together with its induced \(\mu_e\) family, but that family does not emit \(C_\nu\) or \(B_\nu\) by itself.
...
\item \textbf{Calibration sector}: the D10 numerical implementation depends on supplement-backed running, matching, and threshold conventions. The displayed carrier closes its own exact \(W/Z\) chart, and the promoted target-free source-only repair theorem emits the public electroweak quintet on the Phase II calibration branch. The freeze-once coherent repair surface is retained only as compare-only validation.
\item \textbf{Hadron execution boundary}: the current hadron lane is not blocked by one small symbolic gap. The honest next object is one production backend export bundle with publication-complete manifest provenance and real stable-channel correlator arrays; the normalized production dump and then production statistical/systematic control only follow after that bundle lands, so hadron rows remain outside the present derived output surface.
\end{itemize}

A compact Higgs/top forward seed exists downstream of the D10 gauge core, but it remains a secondary quantitative descendant rather than a recovered-core theorem or a completed particle-spectrum closure.
...
\emph{OPH route:} The Markov collar condition does heavy lifting: approximate Markov implies approximate modular additivity, with the defect controlled by conditional mutual information. This makes "modular locality" a controlled approximation. It is not treated as an assumption. On the stated Bisognano--Wichmann geometric branch of Theorem 4.2, the controlled tangent-half-space comparison then locks modular flow to geometric dilations with rigid \(2\pi\) normalization.
...
\textbf{Structural pattern.} OPH treats locality, Lorentz invariance, gauge symmetry, and
