...
\textbf{Proof.} Markov locality localizes the modular defect to the shrinking collar, with carried finite-stage errors \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(\eta_\delta^{\mathrm M}\). By \textbf{T3}, the theorem concerns only scaling-limit cap pairs. By \textbf{T1}, on the selected branch the scaling-limit modular automorphism group is the standard cap-preserving conformal flow up to normalization:
\[
\sigma_t^{\omega_\infty^C}
=
...
\textbf{Scope note.} The theorem fixes the \(2\pi\) normalization and the carried refinement limit on the BW branch. It does \emph{not} prove that MaxEnt alone selects that branch. The remaining UV-side scaffold is exactly a realized transported cap-local system whose carried-collar schedule
\[
r_{\mathrm{FR}}(\varepsilon_{n,m,\delta})
+
...
\bigl(\varepsilon_{n,m,\delta}\bigr)
\]
is derived from transported CMI together with an eventual fixed-local-collar modular-transport common floor, followed by ordered cut-pair rigidity on the emitted scaling-limit cap pair. No separate cap-isotropy or Euclidean-regularity premise is used, and no second comparison-state faithfulness clause is missing.
...
The effective-theory input here is no longer a second operator added on top of the null bridge. In the canonical normalization of the preceding null-half-line construction, the positive OPH null generator is already the endpoint derivative of the renormalized half-line modular family:
\[
\langle\psi,P_\Omega\phi\rangle
=
-\frac{1}{2\pi}\frac{d}{da}\Big|_{a=0^+}
...
\]
But the generator/charge identification itself is already fixed inside the OPH null bridge.
...
In the d=4 scaling regime, the small-ball bridge is internal to the OPH gravity chain. The only extra standard geometric input is the fixed-volume area-variation identity for a small geodesic ball. The modular kernel itself is not imported from a separate EFT law: on the BW branch, a sufficiently small cap is the tangent causal diamond \(D_\ell\), whose preserving conformal Killing field is
\[
\xi_{D_\ell}
=
\frac{1}{2\ell}
...
\]
The bounded-interval kernel from the null modular bridge gives the same weight on each null generator of the diamond, so the D3 geometric branch and the D4 stress bridge together yield
\[
...
\textbf{Theorem 5.1 (Observer-consistency implies a conditional scaling-limit Einstein branch).} Under the five OPH axioms, the collar and null-strip hypotheses of Sections 2 and 5, the canonical technical premises \textbf{T1}, \textbf{T3}, and \textbf{T4}, the half-line generator/charge identification of Section 5.2 together with the bounded-interval projective branch used there, and admissible first variations about a maximally symmetric reference state, the cap equilibrium condition implies the rest-frame first-variation relation
...
\]
which becomes an actual defect count only after the extra sixfold-uniform collapse compatible with the realized \(\mathbb Z_6\) quotient. The intrinsic neutrino lane factors directly through the same-label scalar certificate, the quark mass-side continuation compresses to \((\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})\) and the emitted ray \(\mathcal R_{D12}^{ud}\), and the charged lane fixes centered logs once a charged source pair exists. The remaining blockers therefore sit above the dictionary: \(\widehat C_e^{\mathrm{cand}}\to\widehat C_e\) and the post-promotion lift whose descended scalar is \(\mu_{\mathrm{phys}}(Y_e)\), with the physical identity-mode equalizer beneath that scalar, for charged leptons; the one-scalar law \texttt{quark\_d12\_t1\_value\_law} on the emitted quark ray, with \texttt{intrinsic\_scale\_law\_D12} as the derived wrapper, for quarks; and the neutrino absolute-scale correction scalar \(C_\nu\). The scalar benchmark \(\varepsilon=1/6\) survives only as the extra uniform sixfold collapse of this richer dictionary, so exponents \(-\log(y_f)/\log 6\) are compare-only summaries of excitation action, not recovered-core outputs or independent flavor inputs.
