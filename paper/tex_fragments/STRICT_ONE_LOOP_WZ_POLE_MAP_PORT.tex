...
Slavnov--Taylor controls. This is an external-model validation surface. An
artifact-resolved complex-ball contour and Laurent packet for the \(W\) and
\(Z\), dressed-current residues, an independent-\(\xi\) cross-check, the
remaining Slavnov--Taylor controls, and complete independent replay are work
in progress. The harness therefore establishes no physical \(W/Z\) pole and
supplies no OPH-native source packet.
