...
stability are separate determinacy tests on the resulting equation.
...
few-parts-per-million level as a source-map result.

The direct global proposal is
\[
N=\log M_0(\mathfrak U_N).
\]
The completed finite source class does not select a unique cosmic value, so
...
branch the finite-presence
candidate is \(3.2921\times10^{122}\), about \(0.63\) percent below the
Planck base-\(\Lambda\)CDM comparison coordinate. The exponential branch is
about \(0.39\) percent below it. Both comparisons were exposed before the
branch choice and carry no predictive weight.
