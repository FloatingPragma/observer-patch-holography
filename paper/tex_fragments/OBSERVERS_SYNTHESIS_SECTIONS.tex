...
Quark family & continuation branch & transport and family structure; emitted quark rows remain continuation-level & the current quark sheet is wrong-branch, so the next exact object is a finite left-handed same-label relative-sheet orbit and selector, then a selected-branch mass-scale law \\
Charged leptons & continuation branch & centered charged-shape class and same-carrier readback & theorem-grade waiting set is branch-generator splitting plus the uncentered determinant-line lift, then one affine-covariant absolute anchor \(A_{\mathrm{ch}}\) \\
Neutrinos & weighted-cycle continuation & PMNS/hierarchy shape and the scale-free mass normal form & one positive neutrino mass-normalization scalar \(\lambda_\nu\) remains open; no further discrete branch object remains on that lane \\
