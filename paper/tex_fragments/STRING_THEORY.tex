...
On the \(T1/T3/T4\) branch, together with the stated downstream null-stress identification, OPH yields the target-space equation
\[
...
\textbf{Geometric modular flow:} On the explicit \textbf{T1}+\textbf{T3} BW/geometric branch, cap modular flow is the standard conformal dilation with the \(2\pi\) normalization. OPH does not derive that branch from MaxEnt alone, and Euclidean regularity is not part of the theorem contract.
...
\item
  Conditional geometric modular flow with \(2\pi\) normalization on the explicit \textbf{T1}+\textbf{T3} BW branch, and Lorentz symmetry from Conf\((S^2)\) on that branch
\item
