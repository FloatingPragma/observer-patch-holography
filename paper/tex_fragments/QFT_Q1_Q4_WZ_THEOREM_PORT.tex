...
The Standard Model field-theory implications split into a perturbative branch
and an observable-reconstruction branch. Their dependency structure is:
\begingroup\small
\[
\begin{aligned}
\mathsf{source\ matter\ and\ declared\ action}
  &\longrightarrow \mathsf{finite\ local\ classical\ action},\\
\mathsf{finite\ local\ classical\ action}
  &\longrightarrow \mathsf{finite\ chiral\ quantum\ object},\\
\mathsf{finite\ local\ classical\ action}
  &\longrightarrow \mathsf{formal\ BV/ST\ restoration}
  \longrightarrow \mathsf{strict\ finite\!-\!order\ }W/Z,\\
\mathsf{nonperturbative\ observable\ tower}
  &\longrightarrow \mathsf{observable\ reconstruction}
  \longrightarrow \mathsf{continued\!-\!sheet\ resonance}.
\end{aligned}
\]
\endgroup
Source provenance for the classical action additionally requires the finite
matter result and a source/action-identity receipt. A finite chiral measure or
Hamiltonian does not follow from perturbative restoration, and perturbative
restoration does not follow from either finite quantum construction. Strict
finite-order poles do not supply a nonperturbative observable tower or a
continued-sheet resonance theorem.
...
\midrule
Finite matter packet & finite representation, charge, anomaly, lattice, and selector consequences on the declared packet
& physical source selection and attachment
& exact finite result; physical attachment is work in progress \\
Local classical action & finite local classical \(G_6\) action is gauge invariant and local; the standard tree kernel also needs canonical continuum normalization
& source-selected action, coefficients, regulator, normalization, and ancestry
& conditional implication; source production is work in progress \\
Finite quantum object & an equivariant determinant-line section or a noncollapsing constrained Hamiltonian supplies an exact finite quantum object
& full operator, measure/current or Hamiltonian/nonvacuum packet, and refinement controls
& criteria proved; constructions are work in progress \\
Perturbative renormalization and poles & stable anomaly-free BV/ST theory is formally restorable order by order; strict \(W/Z\) poles follow from the complete finite-order packet
& counterterm basis, matching and Fleischer--Jegerlehner (FJ) engines, identities, currents, and numerical freeze
& implications proved; external validation and OPH source production are work in progress \\
Observable reconstruction & Osterwalder--Schrader reconstruction and a separate continued-sheet resonance theorem
& reflection-positive tower and analytic-continuation packet
& implications proved; constructions are work in progress \\
\bottomrule
\end{tabular}
\end{center}

\begin{theorem}[Finite local classical \(G_6\) action]
...
\begin{remark}[Classical-action boundary]
...
\begin{theorem}[Equivariant determinant-line criterion]
...
\begin{theorem}[Finite Hamiltonian soundness]
...
\begin{remark}[Finite-quantum boundary]
The determinant-line and Hamiltonian results are criteria and soundness
theorems. Neither complete \(G_6\) construction is supplied. Anomaly
arithmetic alone instantiates neither theorem.
\end{remark}

\begin{theorem}[Formal Slavnov--Taylor restoration]
...
\begin{remark}[Perturbative boundary]
This is a formal power-series theorem. It supplies no convergence theorem,
finite chiral measure, finite constrained Hamiltonian, or Wightman
construction. A numerical implementation must produce its regulator-specific
restoration transcript and verify the Ward, Slavnov--Taylor, and Nielsen
identities.
\end{remark}
...
\begin{theorem}[Nielsen control and physical current pole]
...
\begin{theorem}[Gauge-invariant observable reconstruction]
...
\begin{theorem}[Resonance and residue stability]
...
These results specify conditional implications. A target-free
shared-\(\xi\) external Standard Model harness supplies a declared action and
active census, a frozen effective-field-theory interval, dual rule tables,
direct and converted FJ blocks, and payload-replayed Ward and
Slavnov--Taylor controls. Separate interval receipts exclude scalar zeros on
...
neither chart with the physical resonance sheet and supply no unique
continuation identity or sign bridge, no full-matrix Laurent packet or
dressed-current amplitude, and no independent numerical replay. The
independent-\(\xi\) cross-check and complete identity-control transcript are
also work in progress. The external fixture is not composed with the OPH
electroweak chart. The source-selected classical action, either complete
finite quantum object, the OPH matching and current packet, and the observable
tower with continued-sheet data are work in progress.
\end{remark}
