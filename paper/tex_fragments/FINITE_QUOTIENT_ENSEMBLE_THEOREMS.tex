\paragraph{Finite quotient ensemble theorem.}
...
N_r=n_r(Q_r).
\]
The map \(n_r\) is a normal-form map, not a probability law. Any physical branch inherits
...
\paragraph{Physical-vacuum requirement.}
...
A physical-vacuum interpretation requires source Euclidean slab data
\[
...
\(\mu_r^{\rm vac}\).  A continuum interpretation additionally requires reflection positivity or
...
A screen covariance contains incomplete radial information. The complete
one-shell map
\[
C_\ell
=
...
\]
has an infinite-dimensional kernel that persists under positivity. OPH
physical primordial interpretation requires the source-only stress, single-clock,
entropy-repair, curvature-evolution, adiabatic-mode, phase-coherence, physical
mode, radial-null-space, and forward-projection receipts together with a
scale-natural physical dilation intertwiner or complete radial cross-covariance
tomography. A finite radial prior produces a conditional continuation.
Observable CMB comparison also requires declared source, solver,
dataset, covariance, nuisance, data-use, and pooled-reducer provenance.

\paragraph{Interpretation classes and required receipts.}
Every ensemble-facing run records its ensemble id, interpretation class, regulator, representative
