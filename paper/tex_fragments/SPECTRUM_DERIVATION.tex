...
\textbf{Overview.} This part documents the quantitative OPH branches used in this paper. The calculation uses two external inputs: the pixel area \(P \equiv a_{\text{cell}}/\ell_P^2 = 1.63094\), calibrated from the gauge coupling sector via the screen\textquotesingle s entropy-matching condition (\S{}5.4 of Part I of this manuscript), and the screen capacity \(N_{\mathrm{scr}} \equiv \log(\dim\mathcal{H}_{\text{tot}}) \sim 10^{122}\), which enters only in the cosmological/neutrino branch (\S{}14 of Part V of this manuscript). Every other quantity appearing in the derivation chain is either a mathematical constant, a consequence of the gauge group (itself selected from a refinement-stable edge-sector category by Tannaka-Krein/DR reconstruction plus the extended MAR admissibility package, \S{}6.1--\S{}6.2 of Part I of this manuscript), or a derived structural integer on the realized branch (\(N_g = 3\), \(N_c = 3\), \(\varepsilon = 1/6\), \(\delta = 2/9\)). Outputs are classified by epistemic status: quantities flowing from the calibrated gauge couplings are calibration-sector consistency checks; the Higgs/top critical-surface branch is the main independent quantitative output; charged-lepton, flavor-texture, neutrino, and hadron branches are weaker phenomenological or downstream numerical continuations. A complete audit of all constants and the full epistemic classification appear in \S{}1A--\S{}1B below.
\end{quote}

\paragraph{Particle-side success boundary.}
The successes emphasized in this part are the exact massless photon/gluon
sectors, the electroweak calibration surface reproducing the gauge
couplings, \(\alpha_{\rm em}\), the weak mixing angle, the Higgs vacuum
expectation value, and the \(W/Z\) outputs, and the Higgs/top
critical-surface branch fixed once that electroweak surface is chosen.
Charged-lepton, quark, neutrino, and hadron continuations are documented
separately and are not part of that success list. The companion particle
note is maintained as the
\href{https://github.com/FloatingPragma/observer-patch-holography/blob/main/paper/toward_a_particle_spectrum_derivation_from_observer_overlap_consistency.pdf}{companion particle note on GitHub},
together with the
\href{https://github.com/FloatingPragma/observer-patch-holography/tree/main/code/particles}{public particle mirror on GitHub}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Inputs and Contract}\label{1-inputs-and-contract}

\subsubsection{Physical Inputs}\label{11-physical-inputs}
...
\bottomrule\noalign{}
\endlastfoot
Pixel area constant & \(P \equiv a_{\text{cell}}/\ell_P^2\) & 1.63094 & \textbf{Calibrated} from gauge couplings via entropy matching (\S{}5.4 of Part I; see \S{}1B) \\
Screen capacity & \(N_{\mathrm{scr}} \equiv \log(\dim\mathcal{H}_{\text{tot}})\) & \(\sim 10^{122}\) & Inferred from observed cosmological constant \\
\end{longtable}
}

\subsubsection{What Counts as "Derived"}\label{12-what-counts-as-derived}
...
\subsubsection{No-Leakage Guarantee}\label{13-no-leakage-guarantee}
...
\subsection{Complete Audit of All Constants}\label{1a-complete-audit-of-all-constants}

\textbf{Purpose.} This section catalogs \emph{every} named constant, integer, or coefficient appearing anywhere in the prediction code or derivation chain, and explains its origin. The goal is to demonstrate that nothing has been "smuggled in"; every value is either (I) one of the two external inputs used in these quantitative branches, (II) a derived structural quantity from the OPH axioms and stated premises, (III) a mathematical/group-theoretic constant uniquely fixed by the gauge group, or (IV) a standard QFT result that any textbook reproduces from the Lagrangian.

\subsubsection{The Physical Input}\label{1a1-the-physical-input}
...
\bottomrule\noalign{}
\endlastfoot
\(P \equiv a_{\text{cell}}/\ell_P^2\) & 1.63094 & \texttt{P\_DEFAULT} & The pixel area: area of one UV cell on the holographic screen in Planck units. Extracted from the entropy-matching condition \(P/4 = \bar{\ell}_{\text{SU(2)}}(t_2) + \bar{\ell}_{\text{SU(3)}}(t_3)\) (\S{}5.4 of Part I of this manuscript). This relation is \emph{derived} from the axioms; the \emph{numerical value} is fixed by requiring consistency with the observed gauge couplings (just as Newton\textquotesingle s constant \(G\) is the one free parameter of general relativity). & \textbf{Single free parameter} for particle physics \\
\(N_{\mathrm{scr}} \equiv \log(\dim\mathcal{H}_{\text{tot}})\) & \(\sim 10^{122}\) & \texttt{LOG\_DIM\_H\_DEFAULT} & Total screen capacity = de Sitter entropy \(S_{dS} = 3\pi/(G\Lambda)\). Enters only for neutrino masses and cosmological constant (\S{}14 of Part V of this manuscript). Derived from the screen area law (\S{}5 of Part I of this manuscript). & \textbf{Second input} (cosmology sector only) \\
\end{longtable}
}

\subsubsection{Structural Integers Derived from Axioms}\label{1a2-structural-integers-derived-from-axioms}
...
\bottomrule\noalign{}
\endlastfoot
\(N_c\) (colors) & 3 & \texttt{N\_c\_DEFAULT} & Conditional realized-branch output. Once the admitted matter branch has \(N_g = 3\), Witten\textquotesingle s global SU(2) anomaly requires \(N_c + 1\) doublets per generation to make the total number of weak doublets even, so \(N_c\) must be odd. \(N_c = 1\) is excluded by the color-type requirement, and MAR then picks the smallest admissible odd value. \textbf{Result}: \(N_c = 3\). & Theorem 6.14 (\S{}6.9); Part II of this manuscript \S{}6 \\
\(N_g\) (generations) & 3 & \texttt{N\_g\_DEFAULT} & Conditional realized-branch output. Three constraints narrow the window: (1) CP violation requires \(N_g \geq 3\). (2) Asymptotic freedom of SU(2) with one Higgs doublet requires \(N_g(N_c+1) < 43/2\); using only the color-type lower bound \(N_c \ge 3\) gives \(N_g \leq 5\). (3) MAR (fourth component of the complexity vector) picks the smallest value in \(\{3, 4, 5\}\). \textbf{Result}: \(N_g = 3\). & Proposition 6.9 (\S{}6.4); Part II of this manuscript \S{}7 \\
Gauge group & \((SU(3)\times SU(2)\times U(1))/\mathbb{Z}_6\) & --- & Conditional realized-branch output. Tannaka-Krein / Doplicher-Roberts reconstruction from a refinement-stable edge-sector category yields a compact gauge group \(G\) in the bosonic branch (Theorem 6.1, \S{}6.1). The Selection Axiom MAR then selects the SM factors once the admissible class includes one connected abelian factor and an admitted chiral/Yukawa-capable matter package: the minimal coupled carrier \(\mathbb{C}^3 \otimes \mathbb{C}^2\) enforces the product structure, the commutant argument fixes the connected abelian factor to a single \(U(1)\), and the \(\mathbb{Z}_6\) quotient follows from hypercharge quantization on that admitted matter branch. & \S{}6.1--6.2; Part II of this manuscript \\
\(\beta_{\text{EW}}\) & 4 & \texttt{beta\_ew(N\_c)} & Number of SU(2) doublets per generation = \(N_c\) quark doublets + 1 lepton doublet = \(N_c + 1 = 4\). This is a counting consequence of the Witten anomaly analysis in the structural chain fixing \(N_c = 3\). & \S{}6.9, \S{}6.19 \\
\(\varepsilon\) (Z\textsubscript{6} defect) & \(1/6\) & \texttt{defect\_\allowbreak epsilon\_\allowbreak Z6()} & The SM gauge group has a \(\mathbb{Z}_6\) center quotient. Each unit of \(\mathbb{Z}_6\) defect insertion removes \(\Delta S = \ln 6\) nats of entropy (MaxEnt weighting, premise ME). The resulting Boltzmann suppression per defect is \(e^{-\ln 6} = 1/6\). \textbf{This is topologically fixed} by the quotient structure, not chosen. & \S{}6.18, \S{}6.21 \\
\(\delta\) (Koide phase) & \(2/9\) & computed inline & The holonomy phase on generation space: \(\delta = \beta_{\text{EW}} \cdot Y_Q / N_g = (N_c+1)/(2 N_c N_g) = 4/18 = 2/9\). Here \(Y_Q = 1/(2N_c)\) is the quark-doublet hypercharge (fixed by anomaly cancellation) and \(N_g = 3\) is the number of generations. All ingredients are previously derived. Experimental extraction: \(\delta_{\text{exp}} = 0.2222248 \pm 0.0000063\), matching \(2/9 = 0.2222\ldots\) within \(0.4\sigma\). & Proposition 13.3 (\S{}13 of Part V of this manuscript) \\
Quark exponents & \(n_u = (6,3,0)\), \(n_d = (6,4,2)\) & \texttt{derive\_\allowbreak integer\_\allowbreak vectors()} & Minimal SU(3) hierarchy structure: \(n_{u} = (2N_c, N_c, 0)\) and \(n_d = (2N_c, N_c+1, N_c-1)\). These are the unique sequences that (a) span the range \([0, 2N_c]\) in \(N_g\) steps, (b) have the top quark unsuppressed (\(n_t = 0\)), and (c) match the observed up/down mass ordering under CKM rotation. With \(N_c = 3\): \((6,3,0)\) and \((6,4,2)\). & \S{}6.21 \\
Lepton exponents & \(n_e = (7,4,3)\) & \texttt{derive\_\allowbreak lepton\_\allowbreak exponents()} & Derived algorithmically: the Koide roots \(r_k^2\) (from \(\delta = 2/9\)) must satisfy \(r_i^2/r_j^2 \approx \varepsilon^{n_i - n_j}\) for all pairs. The code scans integer triples near \((N_g, N_g+1, \ldots)\) and selects the unique triple minimizing the log-ratio residuals. \textbf{Result}: \((n_\tau, n_\mu, n_e) = (3, 4, 7)\). & \S{}6.21, \S{}13 of Part V of this manuscript \\
CKM angles & \(s_{12} = \varepsilon\), \(s_{23} = \varepsilon^2\), \(s_{13} = \varepsilon^3\) & inline & Powers of \(\varepsilon = 1/6\): the CKM mixing angles scale as \(\varepsilon^{\lvert n_i - n_j \rvert}\) where the exponents are the generation-gap between up-type and down-type defect charges. This is the standard Froggatt-Nielsen scaling. & \S{}6.21 \\
\(Q\) (Koide ratio) & \(2/3\) & inline & The \(\mathbb{Z}_3\) mode balance on generation space: the circulant Hermitian matrix \(\Phi = a I + b P + b^* P^2\) on 3 generations satisfies \(Q = (1 + 2\lvert b/a \rvert^2)/3 = 2/3\) when \(\lvert b/a \rvert = 1/\sqrt{2}\), i.e., when the singlet and charged \(\mathbb{Z}_3\) modes have equal norm. This is the MaxEnt equilibrium. & Theorem 13.2 (\S{}13 of Part V of this manuscript) \\
\end{longtable}
}

\subsubsection{Standard QFT Constants (Fixed by the Gauge Group + Matter Content)}\label{1a3-standard-qft-constants-fixed-by-the-gauge-group--matter-content}
...
\bottomrule\noalign{}
\endlastfoot
MSSM \(\beta\)-coefficients & \((b_1, b_2, b_3) = (33/5, 1, -3)\) & \texttt{B\_MSSM} & The edge-sector computation (\S{}6.17 of Part I of this manuscript) derives \(\beta\)-function shifts \(\Delta b \approx (2.49, 4.17, 4.01)\) from the heat-kernel vacuum-polarization weighting and the \(\mathbb{Z}_6\) quotient. These match the shifts from SM to MSSM to better than 1\%. The coefficients themselves are standard 1-loop group theory: \(b_i = \sum_R T(R_i)\) summed over all matter multiplets. Using MSSM content is a \emph{consequence} of the edge-sector matching, not an assumption. In D11 they govern the UV segment above the synchronization scale \(\mu_{\mathrm{sync}}\). \\
SM 1-loop \(\beta\)-coefficients & standard SM set & \texttt{B\_SM} & Standard 1-loop coefficients for the SM with \(N_g = 3\), namely \((41/10,\,-19/6,\,-7)\). In D11 they govern the low-scale transport from \(m_Z\) up to the synchronization scale \(\mu_{\mathrm{sync}}\approx 6.8\times 10^{11}\,\mathrm{GeV}\), not the entire interval up to \(M_U\). \\
...
\subsubsection{Dimensional/Definitional Constants}\label{1a4-dimensionaldefinitional-constants}
...
\bottomrule\noalign{}
\endlastfoot
\(E_P\) (Planck energy) & \(1.220890 \times 10^{19}\) GeV & \texttt{E\_PLANCK\_GEV} & \textbf{Definition}: \(E_P = \sqrt{\hbar c^5/G}\). This is a unit conversion factor, not a physical input , it converts from natural (Planck) units to GeV. In the OPH framework, \(G\) itself is derived from the pixel area via \(G = a_{\text{cell}}/(4\bar{\ell}\,\hbar/c^3)\) (\S{}5.4 of Part I of this manuscript), so \(E_P\) is ultimately set by \(P\). \\
\(e^{2\pi}\) in \(M_U\) formula & 535.49... & inline & The Euclidean regularity condition on the collar geometry fixes the angular period to \(2\pi\) (\S{}5.8 of Part I of this manuscript). This is a geometric constant, not a parameter. \\
...
\subsubsection{Numerical-Method Parameters}\label{1a5-numerical-method-parameters}
...
\bottomrule\noalign{}
\endlastfoot
Loop order for RG (critical surface) & 1-loop piecewise SM/MSSM transport & 2-loop running or a changed synchronization prescription shifts \(m_H\) and \(m_t\) at the GeV level \\
...
Lattice sizes (hadrons) & \(L = 2\)--\(6\) & Larger volumes needed for precision; listed values are prototype runs \\
...
\subsubsection{Summary: What Is and Isn\textquotesingle t Assumed}\label{1a6-summary-what-is-and-isnt-assumed}

\textbf{Inherited from earlier Part I structural results (five OPH axioms plus the stated gauge, heat-kernel, and refinement premises):}
...
\tightlist
\item
  \(P = 1.63094\) (pixel area), calibrated from the gauge coupling sector (see \S{}1B)
\end{itemize}
...
\tightlist
\item
  \(N_{\mathrm{scr}} \equiv \log(\dim\mathcal{H}_{\text{tot}}) \sim 10^{122}\) (screen capacity)
\end{itemize}
...
\subsection{Calibration vs.\ Prediction: Epistemic Classification of Outputs}\label{1b-calibration-vs-prediction}

This part starts at the quantitative layer. D1--D5 together with D7 form the recovered core; D8--D9 are additional conditional realized-branch consequences once the admitted one-generation chiral/Yukawa package is supplied; D10--D11 live here; D12 stays downstream. This section marks which numbers are calibration-sector checks, which belong to the secondary quantitative branch, and which remain phenomenological continuations.

\subsubsection{The Calibration Structure}\label{1b1-the-calibration-structure}

The pixel area constant \(P\) is the external continuous input used by the particle-physics branch. In practice it is fixed through the gauge sector via the entropy-matching / pixel-constraint closure. Quantities algebraically entangled with that same closure sit in D10 and count as calibration-sector consistency checks. In this part, D10 includes the unified coupling and scale-setting branch, the running gauge couplings at \(m_Z\), \(\alpha_{\mathrm{em}}^{-1}(m_Z)\), \(\sin^2\theta_W(m_Z)\), \(v\), and the tree-level electroweak masses. Agreement there shows internal closure once \(P\) and the printed running and matching conventions are imposed. It does not elevate the whole spectrum program to theorem level.

\subsubsection{Why ``Downstream of \(P\)'' Does Not Mean ``Recovered Core''}\label{1b2-dimensionless-independence}
...
\]
adds no new continuous fit, so its failure would challenge D11 while leaving the D10 calibration closure intact. It sits outside the recovered core D1--D5 plus D7; the conditional D8--D9 realized-branch outputs remain upstream structural prerequisites but are not calibration-sector results of this part. Charged-lepton/Koide fits, texture exponents, neutrino estimates, proton-spin bookkeeping, and downstream hadron extractions use additional ans\"atze, capacity assumptions, or external lattice/QCD machinery. Those belong to D12 or to later downstream numerical layers and must not be read back into the core claim set.

The master ledger remains the guide. Scale-setting dependence does not erase these distinctions, and it does not automatically elevate later numerics to theorem level. D10 is the calibration sector, D11 the secondary quantitative branch, and the remaining spectrum branches deferred continuations unless explicitly promoted elsewhere.

\subsubsection{Classification}\label{1b3-classification}
...
\bottomrule
\endfoot
Phase I recovered core inherited from Parts I--II (D1--D5, D7, plus conditional D8--D9) & conditional relativity branch; compact gauge reconstruction in the bosonic branch; and, on the admitted realized one-generation chiral/Yukawa package, the realized Standard Model quotient, exact hypercharges, the realized pair \((N_g,N_c)=(3,3)\), and no gauge-mediated proton decay & These are upstream structural or conditional realized-branch results. They are \emph{not} generated by the spectrum numerics in this part, even when this part reuses them. \\
...
Phase II calibration sector (D10) & pixel-closure gauge/electroweak consistency, \(\alpha_i(m_{Z,\text{run}})\), \(\alpha_{\mathrm{em}}^{-1}(m_{Z,\text{run}})\), \(\sin^2\theta_W(m_{Z,\text{run}})\), \(v\), \(m_W\), and the running \(Z\)-mass relation & Consistency checks of the D10 branch. Their agreement is expected once \(P\) and the printed running/matching conventions are imposed. \\
Phase II secondary quantitative branch (D11) & Higgs/top critical-surface branch & Main non-calibration numerical branch. It adds no further continuous fit once D10 is fixed, but it remains secondary to the recovered core. \\
...
\subsubsection{What This Part Actually Contributes}\label{1b4-what-oph-achieves}

The documentary center of gravity here is the D10--D11 implementation layer. The gauge-sector closure and electroweak scale-setting branch are calibration-sector consistency checks. The Higgs/top critical-surface branch is the main secondary quantitative output. Charged-lepton, texture, neutrino, proton-spin, and hadronic branches remain weaker continuations.

Quantities reused in these later branches do \emph{not} all inherit the same claim tier. In particular, \(\varepsilon=1/6\), \(\delta=2/9\), Koide relations, texture exponents, neutrino scales, proton-spin fits, and hadron fits should not be cited as recovered-core outputs of overlap consistency plus the stated scaling-limit and categorical premises. Parts~I--II carry the recovered relativity-plus-Standard-Model core; this part begins at the D10 implementation layer and keeps everything beyond D11 explicitly non-core.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Stage 1: Fundamental Scales}\label{2-stage-1-fundamental-scales}

\subsubsection{Unification Scale}\label{21-unification-scale}
...
\textbf{Derivation}: The factor \(e^{2\pi}\) arises from the modular geometry of the collar (the Euclidean regularity condition fixes the angular period to \(2\pi\)). The \(P^{1/6}\) factor comes from the dimensional relation between pixel area and the UV cell scale (\S{}5.8 of Part I of this manuscript).

\subsubsection{Cell Energy Scale}\label{22-cell-energy-scale}
...
\subsection{Stage 2: Gauge Closure and the Pixel Constraint}\label{3-stage-2-gauge-closure-and-the-pixel-constraint}

\subsubsection{The Heat-Kernel Entropy}\label{31-the-heat-kernel-entropy}
...
\subsubsection{The Pixel Constraint}\label{32-the-pixel-constraint}

The generalized entropy matching (\S{}5.4 of Part I of this manuscript) gives:
...
\subsubsection{\texorpdfstring{Gauge Running and the \(\alpha_U\) Solution}{Gauge Running and the \textbackslash alpha\_U Solution}}\label{33-gauge-running-and-the-alpha_u-solution}
...
\subsection{Stage 3: Electroweak Observables}\label{4-stage-3-electroweak-observables}
...
\subsubsection{Z Boson Pole Mass}\label{41-z-boson-pole-mass}
...
\subsubsection{W Boson Mass}\label{42-w-boson-mass}
...
\subsection{Stage 4: Critical Surface , Higgs and Top}\label{5-stage-4-critical-surface--higgs-and-top}

\subsubsection{The Critical Surface Constraint}\label{51-the-critical-surface-constraint}

Refinement stability (\S{}6.3, \S{}6.22 of Part I of this manuscript) pushes the Higgs quartic coupling to a marginal stability point at the unification scale:
...
where \(g_Y(M_U)\) and \(g_2(M_U)\) are obtained inside the D11 transport by running the OPH-derived couplings from \(m_Z\) to a UV synchronization scale \(\mu_{\mathrm{sync}}\approx 6.8\times 10^{11}\,\mathrm{GeV}\) with SM coefficients and from \(\mu_{\mathrm{sync}}\) to \(M_U\) with the same MSSM-like coefficients used in D10. The raw all-SM appendix flow by itself lands far lower (\(m_t^{\mathrm{pole}}\approx 164.2\,\mathrm{GeV}\), \(m_H\approx 115.2\,\mathrm{GeV}\)); the D11 numbers quoted below are the piecewise transport plus only order-one-GeV low-scale matching corrections.

\subsubsection{RG Evolution to Low Scales}\label{52-rg-evolution-to-low-scales}

The coupled system \((y_t, \lambda, g_Y, g_2, g_3)\) is integrated from \(M_U\) down to \(\mu \sim m_t\) using RK4. The D11 branch uses a piecewise one-loop gauge transport:
...
with \(b^{\text{UV}}=(33/5,1,-3)\) and \(\mu_{\mathrm{sync}}\approx 6.8\times 10^{11}\,\mathrm{GeV}\). This Stage-4 section is the D11 branch used for the quoted Higgs/top numbers. The appendix records only the low-scale SM part of the one-loop system; the numerical outputs below also use the UV synchronization, low-scale extraction, and pole-mass conventions fixed here.

\subsubsection{Top Mass}\label{53-top-mass}
...
The same piecewise-running core gives
...
with \(K_2 = 13.44 - 1.04 n_l\) and \(K_3 = 190.6 - 26.7 n_l + 0.65 n_l^2\) (\(n_l = 5\)). The displayed D11 output then adds a residual \(+0.84\) GeV matching correction:

\[m_t^{\text{pole}} \approx 171.1 \text{ GeV}\]

\subsubsection{Higgs Mass}\label{54-higgs-mass}

From \(\lambda(m_t)\) obtained by the RG integration above one finds a core value
\[m_H^{\text{core}} \approx 126.6 \text{ GeV}\]
and after the residual \(-0.12\) GeV Higgs matching shift used here:

\[m_H = \sqrt{2\lambda(m_t)} \cdot v \approx 126.5 \text{ GeV}\]

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Stage 5: Discrete Spectrum , Quarks and Leptons}\label{6-stage-5-discrete-spectrum--quarks-and-leptons}

\subsubsection{\texorpdfstring{The \(\mathbb{Z}_6\) Defect Parameter}{6.1 The \textbackslash mathbb\{Z\}\_6 Defect Parameter}}\label{61-the-mathbbz_6-defect-parameter}
...
\subsubsection{Integer Exponents (Algorithmically Derived)}\label{62-integer-exponents-algorithmically-derived}
...
\subsubsection{The Koide Phase}\label{63-the-koide-phase}
...
\subsubsection{Diagonal Quark Masses}\label{64-diagonal-quark-masses}

\[m_{u_i} = \frac{v}{\sqrt{2}} \varepsilon^{n_{u,i}}, \quad m_{d_i} = \frac{v}{\sqrt{2}} \varepsilon^{n_{d,i}}\]

\subsubsection{CKM Mixing}\label{65-ckm-mixing}
...
\subsubsection{Charged Lepton Masses}\label{66-charged-lepton-masses}
...
\subsection{Stage 6: QCD Scale and Hadron Masses}\label{7-stage-6-qcd-scale-and-hadron-masses}

\subsubsection{\texorpdfstring{\(\Lambda_{\overline{\text{MS}}}\) from \(\alpha_s(m_{Z,\text{run}})\)}{7.1 \textbackslash Lambda\_\{\textbackslash overline\{\textbackslash text\{MS\}\}\} from \textbackslash alpha\_s(m\_\{Z,run\})}}\label{71-lambda_overlinetextms-from-alpha_sm_zrun}

Given the OPH-derived \(\alpha_s(m_{Z,\text{run}}) \approx 0.1183\) (a calibration-sector output) and the quark thresholds (\(m_b\), \(m_c\) from Stage 5), the QCD scale is extracted using the standard 4-loop MSbar definition:
...
\subsubsection{Hadron Masses}\label{72-hadron-masses}
...
\subsection{Stage 7: Neutrino Masses}\label{8-stage-7-neutrino-masses}

\subsubsection{The Capacity Model}\label{81-the-capacity-model}

The second OPH input, the screen capacity \(N_{\mathrm{scr}} \equiv \log(\dim\mathcal{H}_{\text{tot}}) \sim 10^{122}\), enters through the cosmological constant:

\[\rho_\Lambda = \frac{3}{8} \frac{M_P^4}{N_{\mathrm{scr}}}\]
...
This yields \(\Delta m_{32}^2 \approx 9.1 \times 10^{-24} \text{ GeV}^2\) and \(\Delta m_{21}^2 \approx 2.5 \times 10^{-25} \text{ GeV}^2\). These values remain well below existing direct-detection bounds, but they do not reproduce the observed atmospheric and solar oscillation splittings.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Massless Particles}\label{9-massless-particles}
...
These are \textbf{symmetry-protected zeros}, not accidental cancellations. The gauge invariances are derived from the gluing structure (\S{}6.1 of Part I of this manuscript) and entanglement equilibrium (\S{}5 of Part I of this manuscript).

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Complete Spectrum: OPH Outputs vs PDG}\label{10-complete-spectrum-oph-outputs-vs-pdg}

All outputs below use inputs \(P = 1.63094\) and \(N_{\mathrm{scr}} \equiv \log(\dim\mathcal{H}_{\text{tot}}) \sim 10^{122}\) only. PDG values are from the 2024/2025 edition. Each output is classified as \textbf{consistency check} (Tier~1, from calibration sector), \textbf{independent quantitative} (Tier~2), \textbf{phenomenological continuation / downstream branch} (Tier~3), or \textbf{structural} (Tier~4, parameter-free). See \S{}1B for the full classification.

\subsubsection{Gauge Bosons and Higgs}\label{101-gauge-bosons-and-higgs}
...
\(W\) boson & 80.386 & 80.377 \ensuremath{\pm} 0.012 & +0.012\% & Consistency & Flows from calibrated \(\alpha_U\) and \(v\). Tree-level only \\
\(Z\) pole & 91.220 & 91.188 \ensuremath{\pm} 0.002 & +0.035\% & Consistency & Flows from calibrated couplings. Simplified \(\Delta\rho\) \\
\(H\) (Higgs) & 126.48 & 125.20 \ensuremath{\pm} 0.11 & +1.02\% & D11 quantitative & Piecewise SM/MSSM transport plus residual low-scale matching after \(\lambda = \beta_\lambda = 0\) \\
\end{longtable}
}

\subsubsection{Charged Leptons}\label{102-charged-leptons}
...
\bottomrule\noalign{}
\endlastfoot
Electron & 5.109 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{4} & 5.110 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{4} & -0.023\% & D12 continuation & Koide structure (\(Q=2/3\), \(\delta=2/9\)) plus an extra discrete-selection step; residual from QED running \\
Muon & 0.10564 & 0.10566 & -0.022\% & D12 continuation & Same Koide branch; QED running corrections \textasciitilde0.02\%, matching the offset \\
Tau & 1.7766 & 1.7769 \ensuremath{\pm} 0.09 & -0.020\% & D12 continuation & Same origin. Uniform sign shows common scale-factor effect from QED running \\
\end{longtable}
}

\subsubsection{Neutrinos}\label{103-neutrinos}
...
\bottomrule\noalign{}
\endlastfoot
\(\nu_e\) & 8.39 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{1}\textsuperscript{4} & \(< 8 \times 10^{-10}\) (KATRIN) & D6 estimate & Uses second input (\(\log\dim\mathcal{H}\)); consistent with direct bounds but not a completed oscillation-scale prediction \\
\(\nu_\mu\) & 5.04 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{1}\textsuperscript{3} & \(< 8 \times 10^{-10}\) (KATRIN) & D6 estimate & Same capacity branch; below direct bounds, but not yet matched to the solar oscillation scale \\
\(\nu_\tau\) & 3.02 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{1}\textsuperscript{2} & \(< 8 \times 10^{-10}\) (KATRIN) & D6 estimate & Anchored at \(\rho_\Lambda^{1/4}\); below direct bounds, but not yet matched to the atmospheric oscillation scale \\
\end{longtable}
}

All predictions are well below the experimental upper bound of \(\sim 0.8\) eV \(\approx 8 \times 10^{-10}\) GeV. This branch is better described as a low-scale capacity estimate than as a quantitative fit to oscillation data.

\subsubsection{Quarks}\label{104-quarks}
...
\bottomrule\noalign{}
\endlastfoot
up & 3.74 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{3} & 2.16 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{3} & +73\% & D12 continuation & \textbf{\(u\)-\(d\) degeneracy}: both get exponent \(n=6\). Subleading effects not included \\
down & 3.74 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{3} & 4.70 \ensuremath{\times} 10\textsuperscript{-}\textsuperscript{3} & -20\% & D12 continuation & Same degeneracy. Geometric mean \(\sqrt{m_u m_d} \approx 3.2\) MeV closer to 3.7 MeV \\
strange & 0.1346 & 0.0935 & +44\% & D12 continuation & No scheme matching; missing Clebsch factors and CKM rotation \\
charm & 0.808 & 1.273 & -37\% & D12 continuation & Missing RG running from \(v\) to \(m_c\) (\textasciitilde factor 1.3) and Clebsch coefficient \\
bottom & 4.847 & 4.183 & +16\% & D12 continuation & Running from \(v\) to \(m_b\) reduces mass by \textasciitilde15\%, closing most of the gap \\
top (texture) & 174.5 & 172.6 & +1.1\% & D12 continuation & Exponent \(n_t = 0\) (unsuppressed Yukawa); small error from \(v\) offset \\
top (crit. surf.) & 171.1 & 172.6 & -0.87\% & D11 quantitative & Piecewise SM/MSSM transport plus residual pole-mass matching after \(\lambda = \beta_\lambda = 0\) \\
\end{longtable}
}

\subsubsection{\texorpdfstring{Gauge Couplings at \(m_{Z,\text{run}}\)}{10.5 Gauge Couplings at m\_Zrun}}\label{105-gauge-couplings-at-m_z}
...
\bottomrule\noalign{}
\endlastfoot
\(\alpha_{\text{em}}^{-1}(m_{Z,\text{run}})\) & 128.31 & 127.952 \ensuremath{\pm} 0.009 & +0.28\% & Consistency & Flows from calibrated \(\alpha_U\) \\
\(\sin^2\theta_W(m_{Z,\text{run}})\) & 0.2307 & 0.23122 \ensuremath{\pm} 0.00004 & -0.21\% & Consistency & Most sensitive to threshold scale \\
\(\alpha_s(m_{Z,\text{run}})\) & 0.1183 & 0.1179 \ensuremath{\pm} 0.0009 & +0.37\% & Consistency & Within 0.5\(\sigma\) of PDG \\
\end{longtable}
}

\subsubsection{Derived Scales}\label{106-derived-scales}
...
\(M_U\) & 2.47 \ensuremath{\times} 10\textsuperscript{1}\textsuperscript{6} GeV & --- & --- & Consistency & No direct measurement; consistent with proton stability bounds \\
...
\subsubsection{Hadron Masses}\label{107-hadron-masses}
...
\subsection{Analysis of Discrepancies}\label{11-analysis-of-discrepancies}

\subsubsection{Sub-Permille Outputs (\textless{} 0.1\%)}\label{111-sub-permille-outputs--01}
...
\subsubsection{Percent-Level Outputs (0.1\% -- 2\%)}\label{112-percent-level-outputs-01--2}
...
The \textbf{Higgs and top masses} are \textbf{Tier~2 independent quantitative outputs}: they come from the critical surface constraint (\(\lambda(M_U) = 0\), \(\beta_\lambda(M_U) = 0\)) together with the D11 transport (SM running below \(\mu_{\mathrm{sync}}\), MSSM-like running above, and only order-one-GeV residual matching). This remains an independent boundary condition not used in calibrating \(P\). The \textasciitilde1\% error is consistent with the expected size of 2-loop corrections and threshold effects.

\subsubsection{Qualitative Agreement Only: Quark Masses (16\% -- 73\%)}\label{113-qualitative-agreement-only-quark-masses-16--73}
...
\subsubsection{Hadron Masses}\label{114-hadron-masses}
...
\subsubsection{Neutrinos: No Direct Comparison Available}\label{115-neutrinos-no-direct-comparison-available}

The predicted neutrino masses (\(\sim 0.08\)--\(3\) meV) are below existing direct-detection sensitivity (KATRIN: \(m_\beta < 0.8\) eV) and well below existing cosmological mass-sum bounds. The predicted \(\sum m_\nu \approx 3.6\) meV is well below the cosmological bound \(\sum m_\nu < 0.12\) eV (Planck 2018), but the simple capacity-branch hierarchy does not reproduce the observed oscillation splittings quantitatively.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Reproduction Instructions}\label{12-reproduction-instructions}

All computations can be reproduced from the code in \href{../code/particles/}{\texttt{code/particles/}}.
That public code surface is a compact export: a
\path{code/particles/core/} compatibility backbone for the current predictor,
the active closure-lane directories, and frozen current artifacts under
\path{code/particles/runs/}.

\subsubsection{Prerequisites}\label{121-prerequisites}

\begin{verbatim}
pip install numpy PyYAML
\end{verbatim}

The compact export only needs \texttt{numpy} and \texttt{PyYAML}. The
\path{code/particles/core/} predictor backbone only needs \texttt{numpy}; the
lane surface and public status-ledger tooling also use \texttt{PyYAML}. The
\texttt{pdg} and \texttt{pandas} packages are only required for
\path{tools/fetch_pdg_data.py} (the PDG data fetcher, used for comparison
only).

\subsubsection{Code Files}\label{122-code-files}
...
\bottomrule\noalign{}
\endlastfoot
\href{../code/particles/README.md}{\path{README.md}} & Map of the compact public particle mirror and its claim boundary & Meta \\
\href{../code/particles/RESULTS_STATUS.md}{\path{RESULTS\_STATUS.md}} & Human-readable public status ledger for the current particle lanes & Status \\
\href{../code/particles/results_status.json}{\path{results\_status.json}} & Machine-readable mirror of the same particle-status boundary & Status \\
\href{../code/particles/core/particle_masses_stage5.py}{\path{core/particle_masses_stage5.py}} & Core spectrum prediction: gauge closure, transmutation, critical surface, Z$_6$ texture & 1--5 \\
\href{../code/particles/core/oph_qcd.py}{\path{core/oph_qcd.py}} & 4-loop MSbar $\beta$-coefficients and $\Lambda$ extraction & 6 \\
\href{../code/particles/core/oph_lattice_su3_quenched_v5.py}{\path{core/oph_lattice_su3_quenched_v5.py}} & Quenched Wilson SU(3) lattice for hadron mass ratios & 6--7 \\
\href{../code/particles/core/oph_predict_compare.py}{\path{core/oph_predict_compare.py}} & Full predictor plus PDG comparison (main entry point) & All \\
\href{../code/particles/core/oph_no_cheat_audit.py}{\path{core/oph_no_cheat_audit.py}} & Static anti-leak audit of prediction code & Audit \\
\href{../code/particles/calibration/}{\path{calibration/}} & D10/D11 calibration and exactness-audit lane & D10--D11 \\
\href{../code/particles/flavor/}{\path{flavor/}} & Shared charged-sector transport, projector, and quark continuation lane & D12 \\
\href{../code/particles/leptons/}{\path{leptons/}} & Charged-lepton continuation lane & D12 \\
\href{../code/particles/neutrino/}{\path{neutrino/}} & Neutrino continuation lane with forward artifacts and guards & D12 \\
\href{../code/particles/hadron/}{\path{hadron/}} & Hadron debug/systematics lane and thin lattice wrapper & D12 / simulation \\
\href{../code/particles/core/tests/test_oph_predict_compare.py}{\path{core/tests/test_oph_predict_compare.py}} & Smoke tests plus runtime no-cheat mutation test & Tests \\
\href{../code/particles/core/tests/test_particle_masses_stage5.py}{\path{core/tests/test_particle_masses_stage5.py}} & Regression tests for Stage 5 predictions & Tests \\
\end{longtable}
}

\subsubsection{Running the Full Prediction}\label{123-running-the-full-prediction}

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{cd}\NormalTok{ code/particles/core/}
...
\subsubsection{No-Leakage Verification}\label{124-no-leakage-verification}
...
\ExtensionTok{python3}\NormalTok{ tests/test\_oph\_predict\_compare.py}
\end{Highlighting}
\end{Shaded}

\subsubsection{Individual Stages}\label{125-individual-stages}
...
\subsubsection{Fetching PDG Reference Data}\label{125-fetching-pdg-reference-data}
...
This part records the implementation layer that begins after the recovered core. Section~\ref{1b-calibration-vs-prediction} gives the full claim-tier map; the summary here keeps only the operational boundary.

\begin{itemize}
\item \textbf{Phase I recovered core inherited from Parts I--II:} conditional relativity plus bosonic compact gauge reconstruction, together with further conditional realized-branch Standard Model structure once the admitted one-generation chiral/Yukawa package is supplied. These results are upstream prerequisites for this part, not outputs newly established here.
...
The practical contribution of this part is narrow but useful. It records the D10 gauge/electroweak calibration closure, the D11 critical-surface Higgs/top branch, and a set of D12 continuations that reuse the same structural backdrop without acquiring the same claim tier. Here \(D10\) uses the external input \(P\), \(D6\) uses the external input \(N_{\mathrm{scr}}\), \(D11\) adds the extra critical-surface condition, and \(D12\) introduces further ans\"atze or downstream numerical machinery. A failure in D12 retracts that continuation only; a failure in D11 challenges the secondary quantitative branch; a failure in D10 challenges the calibration branch.
