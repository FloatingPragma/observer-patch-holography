...
\bottomrule
\endfoot
Phase I recovered core inherited from Parts I--II (D1--D5, D7--D9) & conditional relativity branch; compact gauge reconstruction in the bosonic branch; realized Standard Model quotient, exact hypercharges, the counting chain \(N_g=3\) then \(N_c=3\), and no gauge-mediated proton decay & These are structural or realized-branch outputs proved upstream. They are \emph{not} generated by the present spectrum numerics, even when this part reuses them. \\
...
Phase II current implementation: calibration sector (D10) & pixel-closure gauge/electroweak consistency, \(\alpha_i(m_{Z,\text{run}})\), \(\alpha_{\mathrm{em}}^{-1}(m_{Z,\text{run}})\), \(\sin^2\theta_W(m_{Z,\text{run}})\), \(v\), \(m_W\), and the running \(Z\)-mass relation & Supplement-backed consistency checks of the current implementation. Their agreement is expected once \(P\) and the printed running/matching conventions are imposed. \\
...
\subsubsection{\texorpdfstring{7.1 \(\Lambda_{\overline{\text{MS}}}\) from \(\alpha_s(m_{Z,\text{run}})\)}{7.1 \textbackslash Lambda\_\{\textbackslash overline\{\textbackslash text\{MS\}\}\} from \textbackslash alpha\_s(m\_\{Z,run\})}}\label{71-lambda_overlinetextms-from-alpha_sm_zrun}
...
\(H\) (Higgs) & 126.48 & 125.20 ± 0.11 & +1.02\% & D11 quantitative & UV-synchronized SM/MSSM transport plus residual low-scale matching after \(\lambda = \beta_\lambda = 0\) \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
Electron & 5.109 × 10⁻⁴ & 5.110 × 10⁻⁴ & −0.023\% & D12 continuation & Koide structure (\(Q=2/3\), \(\delta=2/9\)) plus an extra discrete-selection step; residual from QED running \\
...
Tau & 1.7766 & 1.7769 ± 0.09 & −0.020\% & D12 continuation & Same origin. Uniform sign shows common scale-factor effect from QED running \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\(\nu_e\) & 8.39 × 10⁻¹⁴ & \(< 8 \times 10^{-10}\) (KATRIN) & D6 estimate & Uses second input (\(\log\dim\mathcal{H}\)); consistent with direct bounds but not a completed oscillation-scale prediction \\
\(\nu_\mu\) & 5.04 × 10⁻¹³ & \(< 8 \times 10^{-10}\) (KATRIN) & D6 estimate & Same capacity branch; below direct bounds, but not yet matched to the solar oscillation scale \\
\(\nu_\tau\) & 3.02 × 10⁻¹² & \(< 8 \times 10^{-10}\) (KATRIN) & D6 estimate & Anchored at \(\rho_\Lambda^{1/4}\); below direct bounds, but not yet matched to the atmospheric oscillation scale \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
up & 3.74 × 10⁻³ & 2.16 × 10⁻³ & +73\% & D12 continuation & \textbf{\(u\)-\(d\) degeneracy}: both get exponent \(n=6\). Subleading effects not included \\
down & 3.74 × 10⁻³ & 4.70 × 10⁻³ & −20\% & D12 continuation & Same degeneracy. Geometric mean \(\sqrt{m_u m_d} \approx 3.2\) MeV closer to 3.7 MeV \\
...
top (crit. surf.) & 171.1 & 172.6 & −0.87\% & D11 quantitative & UV-synchronized SM/MSSM transport plus residual pole-mass matching after \(\lambda = \beta_\lambda = 0\) \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\(\alpha_{\text{em}}^{-1}(m_{Z,\text{run}})\) & 128.31 & 127.952 ± 0.009 & +0.28\% & Consistency & Flows from calibrated \(\alpha_U\) \\
\(\sin^2\theta_W(m_{Z,\text{run}})\) & 0.2307 & 0.23122 ± 0.00004 & −0.21\% & Consistency & Most sensitive to threshold scale \\
\(\alpha_s(m_{Z,\text{run}})\) & 0.1183 & 0.1179 ± 0.0009 & +0.37\% & Consistency & Within 0.5\(\sigma\) of PDG \\
\end{longtable}
}
