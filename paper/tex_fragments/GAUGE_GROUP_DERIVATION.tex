...
\textbf{Theorem (Standard Model closure from the gauge-selection hypotheses).} Under the ordinary or central-defect bosonic gauge-reconstruction hypotheses of Theorem 6.1, admissibility conditions (i)--(vi), and Axiom 5 (MAR):
...
On the central branch, the explicit loop-coherence premise \([z]=0\) is the condition under which those transported sectors glue globally, so the reconstructed compact group acts as a genuine gauge symmetry rather than as local bookkeeping only (Proposition 6.1a). The genuinely noncentral fixed-cutoff branch is handled separately by the crossed-module criterion \(q_\Sigma=0\) of Corollary 3.4a and is not here upgraded to the ordinary DR field-algebra route. \(\square\)
...
\(+ \; [z]=0\) on the central branch \(\xrightarrow{\text{Prop 6.1a}}\) DHR transportable, global gauge symmetry
