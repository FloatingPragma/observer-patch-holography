\section{Gauge Group Derivation: Standard Model Closure under Extended MAR}\label{gauge-group-derivation-standard-model-closure-under-extended-mar}
...
This proves \emph{existence} of a compact gauge group but does not identify \emph{which} compact group is realized. The remainder of this part determines the connected Lie realization selected by the extended admissibility package.
...
An \textbf{OPH-realizable gauge sector package} \(\mathfrak S = (G, \text{matter content})\) consists of a compact gauge group \(G\) reconstructed via Tannaka-Krein from the refinement-stable edge-sector category (Theorem 6.1) together with an \emph{admitted} low-energy chiral matter spectrum on which the reconstructed group acts. In this fragment the bosonic Tannaka/DR step reconstructs only the compact gauge group. The chiral matter package and its Yukawa-capable realization are additional realized-branch inputs rather than outputs of the bosonic reconstruction theorem. A sector package \(\mathfrak S\) is \textbf{admissible} if it satisfies all of the following conditions:
...
\subsection{Selection Axiom MAR}\label{3-selection-axiom-mar}

\textbf{Selection Axiom MAR (Minimal Admissible Realization).} Among all OPH-realizable sector packages \(\mathfrak S\) that satisfy admissibility conditions (i)--(vi), the realized low-energy package is the \textbf{lexicographically minimal} one under the complexity vector
...
\textbf{Theorem (Conditional Standard Model branch under extended admissibility).} Under the extended gauge-selection package \(T_{\mathrm{ext}}\), together with the refinement-stable reconstruction hypotheses of Theorem 6.1 and an admissible imported chiral/Yukawa-completable matter package on the realized branch:

\[G_{\mathrm{phys}} = \frac{SU(3) \times SU(2) \times U(1)}{\mathbb{Z}_6}, \qquad N_g = 3, \qquad N_c = 3.\]

The compact group reconstruction itself is the bosonic DR/Tannaka statement of Theorem 6.1. The hypercharge, counting, and quotient statements below are conditional on the admitted chiral matter package rather than outputs of bosonic reconstruction alone. Within that admitted package, the realized counting chain is sequential rather than joint: the CP/UV admissibility window and MAR first select \(N_g = 3\), and Witten's \(\mathrm{SU}(2)\) anomaly parity then fixes \(N_c = 3\).
...
Since the color-type role forces \(N_c \ge 3\), one has \(N_c + 1 \ge 4\). Therefore \(4N_g < \frac{43}{2}\), so \(N_g \leq 5\). This step uses only the lower bound \(N_c \ge 3\) coming from the admitted color-type role, not the later sharp value \(N_c = 3\).
...
\(+ \;\text{apply MAR on the admissible class with an imported chiral/Yukawa package}\) \(\xrightarrow{\text{Thm (this doc)}}\) connected SM factors with the realized chain \(N_g = 3\) then \(N_c = 3\), and \(G_{\mathrm{phys}} = SU(3) \times SU(2) \times U(1)/\mathbb{Z}_6\)

\(\xrightarrow{\text{Thm 6.13}}\) hypercharges fixed \(\xrightarrow{\text{Cor.}}\) no gauge-mediated proton decay
\end{quote}

Under \(T_{\mathrm{ext}}\) together with the admitted chiral/Yukawa-completable matter package, the following are the conditional realized-branch results:
...
\tightlist
\item
  \textbf{Fermion mass hierarchy and Yukawa couplings} , see Part V of this manuscript \S{}12
\item
