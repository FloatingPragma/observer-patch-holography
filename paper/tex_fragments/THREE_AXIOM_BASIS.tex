...
resolutions. The axiom constrains accepted shared data only. It does not
imply global state extension, repair termination, confluence, unique normal
forms, record durability, or commutation of state optimization with
...
strictly positive weights \(w_{r,P}\), the realized state is the
information projection
\[
\rho_r=\operatorname*{arg\,min}_{\rho\in\mathcal K_r}
\sum_{P\in\mathcal G_r} w_{r,P}\,D(\rho_{r,P}\Vert\tau_{r,P}),
\]
equivalent to weighted local entropy maximization when the reference is
identity-proportional. "Random" means least informative relative to the
