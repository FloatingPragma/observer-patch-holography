...
Observer-relative modular time & Declared-branch corollary & On the BW\(_{S^2}\) geometric branch, observer-relative time is read from the cap modular automorphism parameter of the observer's accessible algebra/state pair. This is the D3 reading only. \\
...
D6 cosmological-capacity closure at the cosmic record fixed point & Conditional corollary & Conditional on a constructed \(F\), CP-1--CP-3, and the observed-branch identification \(N_{\mathrm{CRC}}=S_{\mathrm{dS}}\), the fixed point \(N_{\mathrm{CRC}}=F(N_{\mathrm{CRC}})\) gives \(\Lambda_{\mathrm{CRC}}\ell_\star^2=3\pi/N_{\mathrm{CRC}}\). The static-patch SI display additionally uses the selected OPH scale certificate. The 6.6 percent central-value mismatch with the Lambda-located capacity is not a contradiction or significance test before those premises close and the joint cosmological posterior is propagated. \\
