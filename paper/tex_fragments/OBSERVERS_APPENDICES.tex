...
Problem of time & Interpretive or organizational synthesis & Modular flow from the BW\(_{S^2}\) geometric branch provides a physical time parameter for each observer patch. It is an organizational principle rather than a separate recovered-core theorem. \\
...
Cosmological constant and de Sitter parameter package & Phase II input-dependent global closure & local null data leave the Einstein equation fixed only up to a metric term, and the additional screen-capacity identification fixes that remaining branch constant by \(\Lambda=3\pi/(G N_{\mathrm{scr}})\) and therefore the same branch's static-patch package \(S_{\mathrm{dS}}=N_{\mathrm{scr}}\), \(r_{\mathrm{dS}}=\sqrt{3/\Lambda}\), and \(t_\Lambda=r_{\mathrm{dS}}/c\). The observed cosmic age remains a downstream FLRW benchmark, and the whole package still lies outside the local recovered-core theorem because it requires the external input \(N_{\mathrm{scr}}\). \\
...
String/worldsheet reorganization & Phase III deferred continuation & Requires a controlled large-\(N_{\mathrm{edge}}\) regime distinct from the physical color rank \(N_c=3\). Any further lift to critical superstring structure remains a conjectural extension requiring extra worldsheet ingredients not derived in the OPH core. \\
