...
This appendix is interpretive. It records only the OPH state-and-law habitat available inside the
...
\begin{enumerate}
\item an actual OPH closure map \(T\) built from internal operations such as overlap repair,
...
canonical OPH self-map, it does not identify any nonempty invariant observer-supporting subset,
...
\bottomrule
\endfoot
Quantum gravity consistency & Phase I recovered core (conditional scaling-limit) & Conditional Lorentz branch on the BW scaling surface of the extracted prime geometric subnet, with cap-pair extraction and ordered cut-pair rigidity still explicit; conditional Jacobson-type Einstein branch from the null bridge, the separate bounded-interval projective branch, and fixed-cap stationarity. \\
UV completion / microscopic uniqueness & Mixed: fixed-cutoff theorem package / conditional scaling-limit branch & OPH gives a finite-range interacting fixed-cutoff branch, a unique physical UV branch only modulo OPH-stable equivalence, and a fixed-cutoff higher-gauge closure of the genuinely noncentral topological branch. Open items are a unique microscopic representative, the continuum BW lift on the extracted geometric subnet behind T2 plus cap-pair extraction and ordered cut-pair rigidity, and the refinement-limit transportable compact-gauge lift behind T1/T4--T6. \\
Measurement problem & Mixed: fixed-cutoff theorem package / interpretive synthesis & Ref.~\cite{ophmicrophysics} proves a fixed-cutoff central-record / Born-L\"uders package for observer-accessible events. Broader philosophical closure claims remain interpretive here. \\
