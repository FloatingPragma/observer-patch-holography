...
Cosmological-capacity closure & Conditional corollary & Conditional on an additional source law selecting one physical zero of the finite-size slack, a universe-level carrier attachment, stable whole-fiber closure \(\mathfrak F_{r,0}(D_\star)=\{D_\star\}\), and horizon--record identification \(N_{\mathrm{CRC}}=S_{\mathrm{dS}}\), one gets \(\Lambda_{\mathrm{CRC}}\ell_\star^2=3\pi/N_{\mathrm{CRC}}\). The fixed-\(D=24\) packet is exact inside its declared finite source category. The complete capacity-indexed A1--A3 packet lift retains incompatible fixed sets and proves that the complete declared source class does not entail one physical zero. The static-patch SI display additionally uses the selected OPH scale certificate. The uncorrected electroweak/Higgs bridge differs from the weighted Planck base-\(\Lambda\)CDM coordinate by \(6.6\) percent. Separately premised finite-presence and Poisson carriers reduce the retrospective residuals to \(-0.63\) and \(-0.39\) percent. None of these comparisons establishes the common-carrier or horizon receipt. \\
...
Magnetic cocharacter and simple-GUT monopole boundary & Conditional matter-image and global-form arithmetic & Complete reversible response and endogenous overlap transport force the local Standard Model gauge Lie algebra. Incidence and target-blind port readback derive the signed response. Under the conditional matrix current and rank-15 matter representation, the common central kernel is \(\mathbb Z_6\) and the quotient by it is the maximal faithful image. The cover and its \(\mathbb Z_2\), \(\mathbb Z_3\), and \(\mathbb Z_6\) quotients carry the same local tensors. The six-axis menu matches the \(\mathbb Z_6\) quotient only after its coefficient relations are declared, so it does not select the physical global form. The cocharacter \((1,1,1/6)\) and its electromagnetic multiple apply to the declared image. The Dirac-pairing commutant theorem selects the electric polarization uniquely inside the declared line lattice. Theta periodicity requires four-dimensional instanton-sector data. The product adjoint removes the simple-GUT X/Y production route, not all possible 't Hooft lines. \\
