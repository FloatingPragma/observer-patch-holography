...
D6 cosmological-capacity closure at the cosmic record fixed point & Proved corollary of earlier theorems & At the fixed point \(N_{\mathrm{CRC}}=F(N_{\mathrm{CRC}})\), with observed-branch readout \(N_{\mathrm{CRC}}=S_{\mathrm{dS}}\), the same Einstein branch fixes \(\Lambda_{\mathrm{CRC}}=3\pi/(G N_{\mathrm{CRC}})\) and the static-patch readout; \(N_\star\) is the count-density representation of the same closure. \\
