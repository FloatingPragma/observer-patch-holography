...
\begin{enumerate}[(P1)]
  \item \textbf{One-vote-per-view.}
        ...
        If the protocol has prepare and commit phases, its messages in both
        phases refer to that same value.  A node that has supported a value in
        view~$v$ ignores any later conflicting request in view~$v$.
        View participation is monotone: after entering view \(w\), a
        nonfaulty node never signs a prepare or commit message for \(v<w\).
  \item \textbf{Certificate semantics.}
        A prepared certificate
        \(\operatorname{PC}(v,x)\) contains \(q\) distinct, unforgeable,
        authenticated prepare votes for value \(x\) in view \(v\).  A decision
        certificate contains \(q\) distinct authenticated commit votes, each
        referring to the same valid \(\operatorname{PC}(v,x)\).  In the
  ...
  \item \textbf{Prepared-certificate lock and justified new view.}
        On accepting \(\operatorname{PC}(v,x)\), and before signing a commit
        vote for it, a nonfaulty validator records the lock \((v,x)\).
        Its authenticated view-change message carries its highest valid
        prepared certificate, if any.  A new leader collects \(q\) such
        messages.  If it issues a proposal, that proposal carries the value
        of their highest-view prepared certificate; it may carry a fresh
        ...
        prepared certificate.  Otherwise it votes only for its current lock
        value or for the value of a valid prepared certificate in a strictly
        higher view than its lock and a lower view than the current proposal,
        updating its lock monotonically before committing.
  \item \textbf{Post-\textrm{GST} nonfaulty progress.}
        ...
        receives a valid proposal promptly sends its prepare vote; after
        accepting a valid prepared certificate it records its lock and sends
        its commit vote; after accepting a valid decision certificate it
...
\begin{theorem}[QBFT Safety Bound
  ...
  certificate, overlap, authentication, and lock rules]}]
...
\(v<w\). Every nonfaulty signer in the commit quorum \(Q_x\) accepted
\(\operatorname{PC}(v,x)\), locked \(x\), and then committed. Hence its
nonfaulty subset \(H_x\) has size at least \(q-f\). Every size-\(q\)
prepare quorum \(P\) intersects that set because
\[
 |H_x\cap P|
 \ge (q-f)+q-n
 =2q-n-f
...
Assume for contradiction that a prepared certificate for a value different
from \(x\) exists above view \(v\), and choose one with least view \(r>v\).
Let \(O^*\in H_x\) be a nonfaulty member of its prepare quorum. If \(O^*\)
prepared the view-\(r\) value before its view-\(v\) commit, monotone view
participation in (P1) forbids that later lower-view commit. If it prepared
after the commit, it was locked on \(x\). Rule (P4) permits a different value
only when justified by a valid conflicting prepared certificate whose view
lies strictly between \(v\) and \(r\), contradicting the minimality of \(r\).
...
supply all \(q\) new-view, prepare, and commit messages without Byzantine
participation. The highest valid certificate selected by (P4) is compatible
with every nonfaulty lock by the safety invariant. Rule (P5) therefore
executes the proposal, prepare, commit, and decision-relay phases, so every
...
\[
 2(n-f)\ge n+f+1,
\]
hence \(n\ge3f+1\) (with the integer convention already present in (A6)).
...
\((n,f,q)=(1,0,1),(4,1,3),(7,2,5)\).  It checks both the nonfaulty
certificate-overlap witness and transfer of at least one nonfaulty prepared
lock into every new-view quorum.  It also executes the next-view lock
transition against every candidate prepare quorum at those parameter points,
parses actual certificate records before selecting the highest valid report,
and runs finite post-\textrm{GST} proposal/prepare/commit/relay traces for
...
bound, authentication, prepared locking, highest-certificate leader
selection, partial synchrony, quorum availability, quorum connectivity,
...
from \(\{B,H_1,H_2\}\) in view zero and \(y\ne x\) from
\(\{B,H_2,H_3\}\) in view one while respecting one-value-per-validator in
