...
\author{B. M\"uller \and Kai Xue \and Peter Nguyen \and Mario Poneder \and Kale Arnav Anirudha}
...
\begin{abstract}
Observer-Patch Holography starts from a simple question: if every observer sees only part of the world, what kind of physics follows from partial descriptions that agree where they overlap? This synthesis paper organizes the current OPH corpus on one surface. On the declared fixed-cutoff branch, the companion consensus and microphysics papers supply the normal-form, defect, quotient, record, measurement, Bell/CHSH, and checkpoint/restoration theorem packages together with their explicit remaining burdens and non-claims. On the declared recovered-core branch, the compact paper carries the D1--D5 and D7--D9 ledger: conditional Lorentz and Einstein recovery, conditional compact gauge reconstruction, and the realized Standard Model quotient chain with its exact hypercharge and counting surface. Phase II collects the separate capacity and calibration sectors; Phase III collects downstream particle, dark-sector, worldsheet, and other continuations. One pixel input organizes the live quantitative branches on their stated surfaces. Charged-lepton absolute masses and the hadron spectrum remain open derivation lanes.
\end{abstract}
...
realized geometric cap-pair extraction and ordered cut-pair rigidity on the extracted prime
geometric subnet; its D4 side still uses the theorem-local inherited-strip and
exact-or-controlled Markov hypotheses, the same-half-line local modular-Hamiltonian input, and
the separate bounded-interval projective branch; and its D5 side adds fixed-cap stationarity,
...
electromagnetic transport family whose Thomson endpoint gives
\(\alpha^{-1}(0)=137.035999177\), and a compare-only frozen validation pair, its internal
...
UV burden is the realized geometric cap-pair extraction, ordered cut-pair rigidity on that
extracted prime pair, and the refinement-limit transportable-sector lift.
