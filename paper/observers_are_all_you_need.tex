...
microphysics and observer lane. It shows, in one place, what is closed, what is conditional, and what sits on an
...
This synthesis paper reads the OPH suite on one synchronized theorem and status surface. It brings
...
Section~7 keeps the worldsheet lane at its conditional status. Section~8 gives the
...
\item \emph{Deriving the Particle Zoo from Observer Consistency}~\cite{ophparticles}, which carries the structural-to-family particle route and the per-sector status ledger.
\end{enumerate}
