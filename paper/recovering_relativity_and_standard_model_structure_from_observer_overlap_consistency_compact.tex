...
\frac{\SU(3)\times\SU(2)\times\U(1)}{\mathbb Z_6},
\]
let the collar edge-center entropy operator be
\[
L_C=\log d_R
\]
on a cut sector carrying representation
\[
...
\begin{proof}
For a product-group representation \(R=R_3\boxtimes R_2\boxtimes q\),
\[
...
\]
This proves \(L_C=L_C^{(3)}+L_C^{(2)}\) at the operator level. On the product heat-kernel branch, expectation values add, so
\[
\ellbar_{\mathrm{shared}}
=
\langle L_C\rangle
