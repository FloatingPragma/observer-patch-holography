...
\alpha=0.007297352564331425030245795264691683\ldots,\qquad
...
endpoint ledger in Section~\ref{sec:claim-tiers} records the source-side audit
...
\]
on the Ward-projected \(\U(1)_Q\) lane is declared conditionally through the
...
fine-structure comparisons enter only as explicit compare metadata.
...
Phase II quantitative-closure branch (D10) & forward transmutation data \(\alpha_U(P)\), \(t_U(P)\), \(t_{\mathrm{tr}}(P)\), pixel-closure gauge-coupling consistency, \(\alpha_i(m_Z)\), the source-locked anchor \(a_0(P)=\alpha_{\mathrm{em}}^{-1}(m_Z^2;P)\), the declared electroweak transport family \((W,Z,\alpha_{\mathrm{em}}^{-1}(q^2),\sin^2\theta_W(q^2),v)\), and the Thomson endpoint \(\alpha_{\mathrm{Th}}^{-1}=137.035999177(21)\). The source-side audit trunk emits \(\alpha_{\mathrm{cand}}^{-1}=136.994835164621649457949994585787193262029\) at \(P_{\mathrm{cand}}=1.63097209569432901817967892561191884270169\). The endpoint residual ledger records \(0.041465861005223389053448715357314044\ldots\) inverse-alpha units at the CODATA-mapped pixel, with \(S_{\mathrm{required}}=0.895400132647658797805800283181670641\ldots\) and \(c_Q=0.658025759927155435638230170232360050\ldots\) & Failure of the integrated quantitative closure once \(P\) and the printed running/matching/scheme conventions are imposed, or failure of the explicit source spectral payload, same-scheme remainder, and interval-certificate disclosures \\
Phase II input-dependent corollary (D6) & \(\Lambda=\frac{3\pi}{G N_{\mathrm{scr}}}\), \(S_{\mathrm{dS}}=N_{\mathrm{scr}}\), \(r_{\mathrm{dS}}=\sqrt{3/\Lambda}\), and \(t_\Lambda=r_{\mathrm{dS}}/c\) once the screen-capacity identification is made, while any retained capacity-level neutrino side estimate on that input is legacy bookkeeping rather than the weighted-cycle neutrino theorem lane; the observed cosmic age is a downstream FLRW benchmark & Failure of the screen-capacity identification or of the specific global capacity relation. Such a failure would not by itself erase the recovered core \\
...
\]
by the declared D11 Jacobian, again with no inverse readback of the internal transmutation data from measured low-energy couplings. The electroweak \(W/Z\), running-family, and Thomson rows stay on their declared surface until the source spectral measure payload, same-scheme remainder, and interval certificate are supplied. The same D11 Jacobian then emits the Higgs/top branch. On the gravity side, the same D10 pixel law packages
\[
\ellbar_{\mathrm{SU(2)}}(t_{2,\mathrm{run}})
\;+\;
\ellbar_{\mathrm{SU(3)}}(t_{3,\mathrm{run}})
