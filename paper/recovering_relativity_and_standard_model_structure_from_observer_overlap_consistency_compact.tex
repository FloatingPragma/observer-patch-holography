...
The paper also states the present boundaries clearly. The quantitative implementation uses two
...
only the labels defined in the ledger below. The table below is the local key for this compact
paper, and the companion papers reuse the same labels when they point back to this ledger. The
...
This section states the premise ledger and dependency map for the compact paper. The OPH basis used below consists of the five core axioms, the two external continuous inputs listed next, and the external technical premises \(T1\)--\(T7\) collected afterward. On the fixed-cutoff local finite-constraint MaxEnt branch, the local-Gibbs form, quasi-local dynamics, Lieb--Robinson propagation control, endpoint-Lipschitz interval control, and refinement-stable branch persistence are internal consequences rather than theorem-external regularity selectors. Theorem~\ref{thm:bw} fixes the cap generator and its \(2\pi\) normalization only on the Bisognano--Wichmann geometric branch; Corollary~\ref{cor:n2} derives the null half-sided modular pair only after the later null blow-up step; and the remaining theorem-external items are therefore the scaling-limit scope clause, the geometric-branch selection burden, the density-upgrade burden, fixed-cap stationarity, the bosonic/fiber-functor fork, the relativistic null-stress identification premise, and the vanishing obstruction premise where invoked.
...
N_{\mathrm{scr}} &\equiv \log \dim \mathcal H_{\mathrm{tot}} \sim 10^{122}.
\end{align}
In the quantitative implementation used here, \(P\) is calibrated from the gauge sector and \(N_{\mathrm{scr}}\) is inferred from the observed cosmological constant. The first sets the particle-physics scale. The second enters the cosmological-capacity branch and a separate order-of-magnitude neutrino estimate.
...
This local-Gibbs/Lieb--Robinson package should not be read as a proof that the refinement limit lands in a trivial gauge phase or that OPH has selected a unique microscopic realization. Its theorem-level role is collar-local mixing and the carried support/error control used by the later scaling-limit arguments.
...
The compact paper uses one canonical numbered list of external technical premises. The fixed-cutoff realized presentation, the local-Gibbs form, quasi-local Lieb--Robinson propagation, endpoint-Lipschitz interval control, and the induced finite-dimensional refinement branch are not listed here because they are already established internally from Axioms~\ref{ax:screen}--\ref{ax:maxent}. The seven external items are:
...
\item[\textbf{T7}] whenever compact gauge reconstruction is invoked, the sector category means the directed colimit of transportable edge sectors with objectwise finite-dimensional fibers.
\end{itemize}
\end{assumption}
...
The table below is the dependency map for the compact paper. The ``Immediate OPH ingredients'' column records only parents internal to the OPH program. Imported standard mathematics and extra non-axiom physical premises or external inputs are stated separately rather than hidden inside the word ``derived.''
...
\textbf{D3} & Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}: geometric modular flow and Lorentz branch & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) & T1 together with T3, with the standard cap-dilation identification carried directly by the BW geometric branch in T1 & conditional scaling-limit theorem \\
...
\textbf{D12} & Sections~8--10 phenomenological continuations, including charged-lepton/Koide structure, proton-spin and non-gauge proton-lifetime estimates together with the string/worldsheet branch & various subsets of D6, D9, and D10 & branch-specific EFT and phenomenological manipulations & additional ans\"atze such as the uniform \(\mathbb Z_6\) center-label ensemble, texture choices, dark-sector response assumptions, discrete-horizon assumptions, or extra worldsheet/CFT premises & phenomenological continuation branch \\
\end{longtable}
\endgroup
...
Under the present OPH axiom language, the UV invariant determined by the theory is the class \([\mathfrak U]_{\mathrm{OPH}}\), not a unique microscopic regulator presentation. Literal microscopic UV uniqueness is therefore not an OPH invariant.
\end{corollary}
...
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, the derived fixed-cutoff regulator/collar package established above, and the controlled scaling-limit scope clause \textbf{T3} of Assumption~\ref{ass:technical}. Assume in addition the \textbf{T1} branch condition that the refinement-stable scaling-limit cap net lies in the Bisognano--Wichmann geometric modular class for caps and their conformal images. Then the following hold.
...
\item On the scaling-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\), the modular automorphism group is geometric:
\[
...
K_C=2\pi B_C.
\]
In the generic continuum case, where \(\mathcal A_\infty(C)\) has left the regulator class, item \textup{(ii)} is the correct statement and the modular action is generally outer rather than inner.
\end{enumerate}
...
Step 2: passage to the scaling-limit cap pair. Theorem~\ref{thm:bw} is a scaling-limit statement by \textbf{T3}, so the theorem concerns the limit pair \((\mathcal A_\infty(C),\omega_\infty^C)\), not the finite regulator matrices themselves. Nothing in this step assumes that the algebraic type survives the limit.

Step 3: branch identification. By \textbf{T1}, the scaling-limit modular automorphism group of the cap net is the standard cap-preserving conformal flow \(\alpha_{\lambda_C(s)}\) for some geometric parameter \(s\). Therefore there is a constant \(\kappa_C>0\) such that
\[
...
This theorem has one honest conditionality boundary. OPH derives the finite-range MaxEnt family and its carried collar control. It does \emph{not} derive that every refinement-stable MaxEnt branch lands in the Bisognano--Wichmann phase, nor that MaxEnt by itself already selects the vacuum/canonical cap state. What the compact note informally calls the ``BW/canonical cap branch'' is precisely the refinement branch singled out by \textbf{T1}: its scaling-limit modular automorphism group is the standard cap-preserving conformal dilation. Once \textbf{T1} holds, the theorem fixes the \(2\pi\) normalization and the correct automorphism-level statement of geometric modular flow, while explaining why the finite type-I regulator need not survive the refinement limit.

The remaining BW-side burden splits into two objects. First, a \emph{canonical scaling-limit cap-pair realization theorem} would extract \((\mathcal A_\infty(C),\omega_\infty^C)\) for each round cap \(C\) as a local weak-\(*\) limit of transported cap marginals along the realized refinement-stable branch, with vanishing carried collar errors and with no assumption that type-I structure survives. Second, an \emph{ordered null cut-pair rigidity theorem} on those extracted cap pairs would collapse the residual cap-preserving conformal freedom to the unique hyperbolic subgroup \(\lambda_C(s)\); the modular KMS normalization would then fix \(s=2\pi t\). The weak-\(*\) extraction plus rigidity route is an explicit internal extension route, not part of the proved theorem surface.
\end{remark}
...
\Conf^+(S^2)\cong \mathrm{PSL}(2,\mathbb C)\cong \mathrm{SO}^+(3,1).
\]
By Theorem~\ref{thm:bw}, on the \textbf{T1} branch the scaling-limit cap modular automorphism groups are the standard cap-preserving conformal dilations with the \(2\pi\) normalization fixed internally. Hence the local kinematics induced by modular flow is the connected Lorentz group.
\end{proof}
...
over regulator refinements $r$, retaining only transportable sectors and intertwiners that persist
under refinement. Tannaka/Doplicher--Roberts reconstruction is applied to this category.
...
The pixel area $P$ calibrates the separate gauge-coupling sector used here. Quantities algebraically entangled with that calibration are therefore consistency checks for that implementation rather than part of the recovered-core claim set.

Operationally, changing $P$ moves the calibration family $(\alpha_U,\alpha_i(m_Z),\sin^2\theta_W(m_Z))$. It does not alter the parameter-free structural outputs such as the gauge quotient or hypercharge lattice. Downstream matter-sector continuations, including Koide-style relations, remain outside the compact paper's recovered-core theorem package.
...
K=-\log\rho
\]
are literal finite-dimensional objects. The Lorentz branch is \emph{not} a claim that the continuum observer algebra stays in that class. The honest refinement picture is
\[
...
In the Bisognano--Wichmann branch of Theorem~\ref{thm:bw}, the scaling-limit cap algebras may be non-type-I and, in the continuum-QFT case of interest, are expected to be type III. Then there is generally no cap density matrix \(\rho_C\in \mathcal A_\infty(C)\) and no bounded operator \(K_C=-\log\rho_C\) inside the limit algebra. The correct object is the modular automorphism group of the pair \((\mathcal A_\infty(C),\omega_\infty)\), which on that branch acts geometrically and typically outerly.

There is therefore no contradiction between using type-I regulators and aiming at a type-III continuum. The finite regulator provides the collar decomposition, recovery control, and carried errors. The additional burden is not to keep type I forever, but to show that the selected refinement branch has a controlled scaling limit and, if one wants Lorentz kinematics, that this limit lies in the Bisognano--Wichmann geometric modular class. Those are exactly the theorem-external roles of \textbf{T3} and \textbf{T1}.

This is analogous to ordinary lattice field theory: one computes at finite regulator in a type-I algebra and then asks which continuum algebraic phase the scaling limit realizes. OPH adds one extra structural point. Because the realized states across cutoffs remain inside one common finite-dimensional MaxEnt family, the regulator branch is controlled. What MaxEnt by itself does \emph{not} prove is the algebraic type of the continuum limit or that the limit state is already the vacuum/canonical BW state. That stronger identification is precisely the extra branch statement encoded in Theorem~\ref{thm:bw}.
...
The local MaxEnt branch gives a finite-range interacting fixed-cutoff dynamics, Propositions~\ref{prop:gauge} and \ref{prop:ancillauv} fix uniqueness of the UV branch only modulo OPH-stable equivalence, and Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} with Proposition~\ref{prop:hginteracting} close the genuinely noncentral topological branch at fixed cutoff. The open items on the UV side are the continuum BW/geometric lift of \textbf{T1}+\textbf{T3} and the fuller transportable compact-gauge lift when \textbf{T2} and \textbf{T5}--\textbf{T7} are invoked. The smallest honest internal extension route to the BW/geometric lift is the two-step chain of a scaling-limit cap-pair extraction theorem that extracts \((\mathcal A_\infty(C),\omega_\infty^C)\) as local weak-\(*\) limits of transported cap marginals with vanishing carried collar errors, followed by an ordered null cut-pair rigidity theorem that collapses the residual cap-preserving conformal freedom on those extracted cap pairs to the unique BW hyperbolic subgroup. That two-step chain is not part of the proved theorem surface stated here.

The D10 calibration sector gives supplement-backed gauge-coupling closure on the displayed carrier. That carrier closes its own exact \(W/Z\) chart, and the promoted target-free source-only repair theorem emits the public electroweak quintet on the Phase II calibration branch; the freeze-once coherent repair surface serves only as compare-only validation. The separate D6 cosmological-capacity relation remains an input-dependent corollary. The charged-lepton and Koide sectors remain phenomenological continuations rather than theorem-level spectra. What is exact there is narrower: the charged same-carrier readback law is closed, the charged absolute-scale underdetermination theorem is closed, and the theorem-grade waiting set is the charged sector-response / declaration functor \(\widehat C_e\) together with the common-shift-breaking affine anchor \(A_{\mathrm{ch}}\). A minimal operator-side extension route is a centered Schur-type \(P\!\to\!Q\!\to\!P\) transfer theorem for the descended commutator after the central split; even if that route is internalized, the affine anchor \(A_{\mathrm{ch}}\) remains a separate object. Its clean future realization is an uncentered charged response lift carrying a determinant line, with \(A_{\mathrm{ch}}=\tfrac13\log\det(Y_e)=\tfrac13\operatorname{tr}(\log Y_e)\). On the flavor side, the structural output fixed before those continuations is the realized counting chain \(N_g=3\) then \(N_c=3\).

The light-quark and hadron sectors remain weaker. A recovered-core nonzero light-quark pure-\(B\) selector is not derived, so the light-quark split remains a D12 continuation branch. On the CKM side the D12 sheet is a wrong-branch no-go, so the remaining quark branch object is one finite left-handed same-label relative-sheet orbit and its discrete selector; a smaller local chirality-basis orbit is diagnostic-only because it leaves the ordered same-label left-eigenframe domain. The weighted-cycle neutrino continuation fixes the PMNS/hierarchy shape and leaves one positive absolute-scale scalar open; no further discrete neutrino branch object appears on that lane. The hadron lane depends on nonperturbative runtime completion and stable-channel convergence rather than a closed theorem-level mass surface.

The paper does not address simulation narratives, observer-continuation mechanisms, or complete closure of all low-energy observables.\footnote{Questions outside this theorem package do not alter the specific claim set established here. Even a separate interpretive habitat theorem for OPH state-and-law data would not enlarge the compact paper's Phase-I scope unless the actual closure map, invariant admissible sector, and stability hypotheses were also proved at the same tier.}

\section{Conclusion}

Observer-Patch Holography starts from overlap consistency of observer patches and yields, within one framework, several effective structures usually treated separately in modern fundamental physics. The compact paper supports a unique schedule-independent normal form for overlap repair and hence a unique physical UV branch only modulo boundary redundancy, implementation hiding, and inert ancillary stabilization, a conditional Lorentz branch in the assumed geometric modular phase, a conditional Jacobson-type Einstein branch under fixed-cap stationarity and the null-stress premise, the Standard Model gauge quotient with exact hypercharges on the realized matter package under the MAR admissibility package in the bosonic internal-gauge branch, the realized counting chain \(N_g=3\) then \(N_c=3\) on that branch, a separate supplement-backed gauge-calibration sector, and a separate input-dependent cosmological-capacity corollary. Phenomenological continuations are discussed separately from the recovered core.
