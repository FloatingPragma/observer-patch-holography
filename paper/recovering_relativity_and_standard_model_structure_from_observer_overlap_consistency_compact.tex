...
The collar analysis proves a fixed-cutoff statement on the finite type-I regulator net: the reduced cap state has a literal density matrix, its modular Hamiltonian exists, and its nonadditive part is confined to a shrinking collar up to carried errors. The Lorentz claim is \emph{not} that these finite matrix algebras are the continuum observer algebras. The finite type-I regulator class is not refinement-closed. The target is the refinement-limit cap pair. Under the controlled scaling clause \textbf{T3}, write
\[
(\mathcal A_\infty(C),\omega_\infty^C)
\]
for any realized scaling-limit cap algebra/state pair that the realized refinement-stable branch emits for a round cap \(C\subset S^2\). Axiom~\ref{ax:maxent} controls branch persistence inside one common finite-dimensional MaxEnt family; it does \emph{not} by itself force \(\mathcal A_\infty(C)\) to remain type I or prove that the emitted scaling-limit pair lands in the Bisognano--Wichmann geometric phase. The role of \textbf{T1} is exactly to identify the branch on which the scaling-limit modular action is geometric. On such a branch the limit local algebra may leave the regulator class, and the modular action is then generally outer rather than inner.
...
\item No separate cap-isotropy/\(\mathrm{SO}(2)\)-equivariance selector or Euclidean-regularity premise is used in this theorem. Once \textbf{T1} has fixed the geometric branch and \(\lambda_C(s)\) is normalized by the null blow-up \(v\mapsto e^{-s}v\), the modular KMS normalization fixes the coefficient \(2\pi\). No claim is made that \(\mathcal A_\infty(C)\) remains type I. If the scaling-limit cap algebra happens to remain type I, item \textup{(ii)} may be written as the operator identity
\[
K_C=2\pi B_C.
\]
...
Step 2: scaling-limit claim tier. Theorem~\ref{thm:bw} is a scaling-limit statement by \textbf{T3}, so the theorem concerns only scaling-limit cap pairs furnished by the controlled refinement/scaling package, not the finite regulator matrices themselves. Nothing in this step assumes that type-I structure survives the limit.
...
Step 4: normalization. Because \textbf{T1} identifies the standard cap-preserving conformal subgroup, no separate cap-isotropy or Euclidean-regularity selector enters here. The modular group is KMS at inverse temperature \(1\) by definition. Since \(\lambda_C(s)\) is normalized by unit null dilation under the blow-up \(v\mapsto e^{-s}v\), the standard Bisognano--Wichmann normalization gives
\[
\kappa_C=2\pi,
\]
hence
...
\]
after transport to each fixed local collar model. Only after that witness is supplied can local weak-\(*\) extraction and GNS gluing emit the realized scaling-limit cap pair. Beneath that derived schedule, the sharper raw datum is the transported fixed-local-collar Markov/faithfulness package, and inside that datum the single missing clause is the eventual common floor on the finite modular-transport family feeding the faithful modular-defect term.
No second comparison-state faithfulness clause is missing there: on one fixed collar model, the exact-Markov replacements inherit the same eventual floor once \(\delta^{\mathrm M}_{m,\delta}(\varepsilon_{n,m,\delta})\to0\).
...
This shared dictionary feeds the continuation lanes. The intrinsic neutrino lane factors through the same-label scalar certificate \((g_e,\omega_e)\) and its centered descendants \((q_e,\eta_e,\mu_e)\); the weighted-cycle branch therefore uses no extra primitive flavor labels. The quark mass-side continuation compresses the same dictionary to
\[
(\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})
\]
and, on the compact same-family branch, to the emitted ray \(\mathcal R_{D12}^{ud}\). On the CKM/CP side the same-label left-handed orbit collapses to the singleton \(\sigma_{\mathrm{ref}}\), so the selected D12 sheet is fixed and is the wrong CKM sheet. The emitted branch therefore carries one named exact quark object, namely the intrinsic scale law on the emitted ray. The charged sector is equally tied to the same backbone: once a charged source pair \((\eta_{\mathrm{ch}},\sigma_{\mathrm{ch}})\) exists on the ordered charged carrier \((-1,x_2,1)\), the centered charged logs are fixed algebraically by
\[
\begin{aligned}
...
\]
and then the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\). For quarks, the next exact object is the intrinsic scale law on the emitted D12 mass ray. For neutrinos, the same-label scalar certificate closes beneath the weighted-cycle branch, and the irreducible exact missing object is the reduced absolute-scale correction invariant \(C_\nu\). Thus the shared excitation dictionary is not the missing theorem object. It is the common OPH family base, while old base-6 exponents survive only as compare-only compressions of its excitation action.
...
The light-quark lane remains a D12 continuation branch and is not a recovered-core theorem. Its frontier is sharper than an open selector problem. On the emitted same-label left-handed solver surface the local orbit collapses to the singleton \(\sigma_{\mathrm{ref}}\) (\ophid{sigma_ref}). This is a real but negative closure, because \(\sigma_{\mathrm{ref}}\) is the wrong D12 CKM sheet. No theorem-grade branch-changing route from the emitted D12 data to the physical CKM shell is derived. Same-sheet overlap scans and any other non-sector-attached orbit improvement remain compare-only diagnostics unless they are promoted as sector-attached left-handed same-label transport, so they do not alter the selected-sheet statement.

The mass side is therefore the next theorem surface. The same-family branch emits only the one-parameter ray
\[
D12_{ud}^{\mathrm{mass}}
\equiv
\mathcal R_{D12}^{ud}
\]
(paper id \ophid{D12_ud_mass_ray}), not a distinguished modulus on that ray. The exact next quark object on the emitted branch is the one-scalar law \ophid{quark_d12_t1_value_law} on \ophid{D12_ud_mass_ray}, with \ophid{intrinsic_scale_law_D12} as the derived wrapper. CKM and CP closure are not a second independent selector task on the emitted surface; they remain downstream of the wrong-sheet no-go until a genuine physical branch-changing transport is derived.

A target-anchored same-family exact witness reproduces the six quark reference masses exactly on the same ordered three-point family, and within that same-family scope the selected-sheet exact readout closes through \ophid{oph_quark_current_family_selected_sheet_exact_closure}, built on the ordered three-point quadratic readout theorem \ophid{oph_quark_current_family_quadratic_readout_theorem}. That exact-hit sidecar does not move the selected D12 sheet and does not emit \ophid{quark_d12_t1_value_law}. Separately, the builder-facing pure-\(B\) payload pair beneath the reported quark values remains open.
...
\]
This weighted-cycle branch repairs the oscillation-shape output to the physical PMNS/hierarchy regime on this continuation surface. It emits
\[
...
What it does \emph{not} yet do is emit the absolute normalization as a theorem. The normalized same-label overlap-defect weight section closes beneath that weighted-cycle shape, and the paper-facing amplitude parameterization is
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
...
\]
The best emitted proxy above the closed normalizer is
\[
P_\nu:=I_\nu^{1/2}\,\widehat{\mathrm{ratio}}^{\,1/2}\,\mathrm{sum\_defect}^{-1}.
\]
The attached stack factors through \(q_e=q_{\mathrm{mean}}qbar_e\), so a \(qbar_e\)-only bridge collapse does not follow from the corpus. The selected-point scalar built from the weighted-cycle state is internal to the emitted \(qbar_e\), \(\psi_{\mathrm{wc}}\), and \(\psi_\ast\) data and therefore is not the missing bridge-external object. Above that emitted proxy, the exact missing bridge object is smaller:
\[
C_\nu:=\frac{B_\nu}{P_\nu}.
\]
Equivalently,
...
\lambda_\nu=\frac{m_{\star,\mathrm{eV}}}{q_{\mathrm{mean}}^{p_\nu}}\,P_\nu\,C_\nu.
\]
Because the proxy is internal to the emitted stack, this reduced correction invariant \(C_\nu\) is the irreducible exact missing object for the absolute lane, while \(B_\nu\) is retained only as the paper-facing amplitude parameterization.

The bridge audit is therefore stated at the correct object. The live compare-only value is
\[
C_\nu^\ast=0.9995473560,
\]
and the target-containing reduced window is
\[
C_\nu\in[0.9994296,1.0015784].
\]
This induces the sharpened paper-facing corridor
\[
B_\nu\in[6.69600,6.71040]
\]
with midpoint \(6.70320\). The older three-route corridor \([6.69315,6.71259]\) remains only as a wider comparison envelope, and the narrower shortlist-consensus window \([6.70484,6.71259]\) remains diagnostic-only because it misses the live compare-only target.
The strongest compare-only continuation adapter on the explicit positive selector segment still solves one parameter against the representative central ratio and one against \(\Delta m_{32}^2\): solving \(\tau_\nu\) on \(D_\tau=(1-\tau_\nu)\chi+\tau_\nu(1+\gamma_{1/2})\) against the representative central ratio and then solving \(\lambda_\nu\) against \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\) yields \(\tau_\nu=0.49717\), \(\lambda_\nu=1.72396\),
\[
(m_1,m_2,m_3)=
(0.01745663295,\ 0.01948419960,\ 0.05308139066)\,\mathrm{eV},
\]
...
\]
but this remains strictly compare-only and non-promotable because the theorem lane waits on \(C_\nu\), not on another selector or on a bare amplitude fit.
