...
The collar analysis proves a fixed-cutoff statement on the finite type-I regulator net: the reduced cap state has a literal density matrix, its modular Hamiltonian exists, and its nonadditive part is confined to a shrinking collar up to carried errors. The Lorentz claim is therefore not a literal statement about the finite regulator matrices. Under the controlled scaling clause \textbf{T3}, the target object is a realized scaling-limit cap pair
\[
(\mathcal A_\infty(C),\omega_\infty^C)
\]
for each round cap \(C\subset S^2\). Axiom~\ref{ax:maxent} controls the state-side realized branch across refinement through one common finite-dimensional MaxEnt family; it does \emph{not} by itself prove that the scaling-limit cap algebra remains type I, that a canonical scaling-limit cap pair has been extracted, or that the realized branch lands in the Bisognano--Wichmann geometric modular class. The theorem below therefore does the precise job available on the present theorem surface: it proves the cap-modular statement on the \textbf{T1}-selected branch, makes the \(2\pi\) normalization and carried refinement limit explicit, and leaves the remaining UV burden exposed rather than hidden.
...
\item No separate cap-isotropy/\(\mathrm{SO}(2)\)-equivariance selector or Euclidean-regularity premise is used in this theorem. Once \textbf{T1} has fixed the geometric branch and \(\lambda_C(s)\) is normalized by the null blow-up \(v\mapsto e^{-s}v\), the modular KMS normalization fixes the coefficient \(2\pi\).
...
\begin{remark}[Exact BW-side UV scaffold, and the only nonlatent lower input on the faithfulness side]
Theorem~\ref{thm:bw} proves the Bisognano--Wichmann conclusion only on the \textbf{T1}-selected geometric branch. To internalize that branch entirely within OPH one still needs exactly two further objects, and neither of them is an independent cap-isotropy selector or a Euclidean-regularity premise.

First, one needs a \emph{realized transported cap-local system} for caps and their conformal images. The relevant package is the cap-local test family, the projectively compatible transported marginal family, and the asymptotic transport-equivalence certificate. On each fixed local collar model \((m,\delta)\), let \(\varepsilon_{n,m,\delta}\) be the transported collar conditional mutual information. If the transported physical marginals satisfy an eventual common floor
\[
...
\begin{proof}
Lemma~\ref{lem:translation} already proves the first displayed identity: in the canonical normalization of Corollary~\ref{cor:n1}, the weak endpoint derivative of the renormalized half-line family is exactly the Borchers generator. Passing to the effective spacetime description does not change the operator family; it rewrites the same \(\widetilde K_a(\Omega)\) in local continuum variables appropriate to the local Lorentzian regime. In that description, the local null-stress charge is precisely the right endpoint derivative
\[
-\frac{1}{2\pi}\frac{d}{da}\Big|_{a=0^+}\widetilde K^{\mathrm{eff}}_a(\Omega)
\]
...
P_\Omega = Q_{kk}(\Omega).
\]
The endpoint-supported central term does not alter the noncentral generator; in the canonical blockwise normalization it has already been absorbed into the central part fixed in Corollary~\ref{cor:n1}. This proves the identification.
\end{proof}
...
\end{enumerate}
So the gap closed here is the generator/charge identification itself: inside the null bridge and on the OPH half-line family, \(P_\Omega\) is already exactly the local null-stress charge.
...
Assume the hypotheses of Theorem~\ref{thm:stress}. Let $p$ be the cap center and let $u^a$ be the future-directed unit tangent of the local diamond rest frame at $p$. In local inertial coordinates $(t,x^i)$ adapted to $u^a$, let $D_\ell$ be the causal diamond whose $t=0$ slice is the Euclidean ball
\[
B_\ell=\{t=0,\ r<\ell\},
\qquad
...
\begin{proof}
Work in the local Lorentzian scaling regime around $p$. On the OPH Bisognano--Wichmann branch, Theorem~\ref{thm:bw} identifies the cap modular flow with the geometric flow of the diamond-preserving conformal Killing field. In the tangent diamond $D_\ell$, that field is
\[
\xi_{D_\ell}
=
\frac{1}{2\ell}
...
The same kernel is the bounded-interval kernel selected by the D4 null bridge. Along each null generator of the diamond, Theorem~\ref{thm:stress} gives the affine-covariant interval weight that vanishes at the two diamond endpoints, and on the $t=0$ slice that weight is exactly $(\ell^2-r^2)/(2\ell)$. Therefore the null-stress bridge reconstructs the bulk modular charge of the geometric generator as
\[
...
G_{ab}+\Lambda g_{ab}=8\pi G\,\langle T_{ab}\rangle
\]
throughout that branch. The only local ambiguity is the metric term already isolated by Lemma~\ref{lem:nullreconstruct}, and that ambiguity is exactly the same branch constant $\Lambda$.
\end{corollary}
...
Observer-Patch Holography starts from overlap consistency of observer patches and yields, within one framework, several effective structures usually treated separately in modern fundamental physics. This SM/GR derivation paper supports a unique schedule-independent normal form for overlap repair and hence a unique physical UV branch only modulo boundary redundancy, implementation hiding, and inert ancillary stabilization, a conditional Lorentz branch in the assumed geometric modular phase, a conditional Jacobson-type Einstein branch under fixed-cap stationarity and the null modular bridge together with the bounded-interval projective branch, the Standard Model gauge quotient with exact hypercharges on the realized matter package under the MAR admissibility package in the bosonic internal-gauge branch, the realized counting chain \(N_g=3\) then \(N_c=3\) on that branch, a separate supplement-backed gauge-calibration sector, and a separate input-dependent cosmological-capacity corollary. Phenomenological continuations are discussed separately from the recovered core.
