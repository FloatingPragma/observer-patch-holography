...
\begin{abstract}
This paper is the compact statement of what Observer-Patch Holography (OPH) claims at
its most defensible core. The goal is not to explain all of particle physics and gravity at once.
The goal is to isolate the smallest theorem-bearing package in which relativity and the structural
part of the Standard Model can be recovered from overlap consistency among local observer
descriptions on a finite-capacity screen.

Starting from five axioms and an explicit scaling-limit/categorical premise ledger, the paper
derives a schedule-independent normal form for overlap consistency, a conditional Lorentz branch,
a conditional Jacobson-type Einstein branch, and a conditional compact-gauge reconstruction route.
Once the Minimal Admissible Realization axiom is imposed on the admissible low-energy class, the
realized gauge structure on that branch is
\[
...
N_c=3,
\]
together with the exact rational hypercharge lattice for the realized one-generation Standard
Model chiral matter package with one Higgs doublet. The realized counting chain is sequential:
first \(N_g=3\), then \(N_c=3\).

The paper also states the boundary conditions clearly. The quantitative implementation uses two
external continuous inputs, the pixel area
\[
...
\]
Here \(P\) feeds the supplement-backed gauge-coupling calibration sector and \(N_{\mathrm{scr}}\)
feeds the capacity branch for \(\Lambda\); neither numerical branch belongs to the recovered core.
At fixed cutoff, the topological regulator package is closed on the ordinary, central-defect, and
genuinely noncentral branches. What remains open sits where it should: in the continuum
Bisognano--Wichmann/geometric lift, the refinement-limit compact-gauge reconstruction lane, and
the further phenomenological continuations.
\end{abstract}
...
\item Downstream matter-sector continuations, including charged-lepton/Koide structure, remain visible but explicitly outside this SM/GR derivation paper's recovered-core theorem package.
...
\text{Phase III}=D12.
\]
Phase I is the recovered-core claim set. Phase II contains the separate input-dependent capacity corollary and the supplement-backed gauge-calibration sector. Phase III collects phenomenological continuations. The canonical five-axiom basis, the two external continuous inputs, and the technical-premise labels used below are fixed in Section~\ref{sec:premise-ledger}. Each row names a node in the reconstruction program, records its immediate OPH-side parents, separates imported standard mathematics from extra non-axiom physical premises or external inputs, and states the resulting claim tier.
...
\caption{Dependency checklist for the compact OPH program.}\\
\toprule
...
\textbf{D3} & Lorentz branch \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) (Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; conformal-group classification & T1 together with T3, with the standard cap-dilation identification carried directly by the BW geometric branch in T1 & Phase I conditional scaling-limit theorem \\
\textbf{D4} & Null modular bridge to \(T_{kk}\) (Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Theorem~\ref{lem:density}, Lemma~\ref{lem:translation}; Theorem~\ref{thm:stress}) & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock half-sided modular inclusion; distributional differentiation on half-lines & the theorem-local inherited-strip decomposition and exact-or-controlled Markov hypotheses of Proposition~\ref{prop:nullec} and Corollary~\ref{cor:n1}, plus T3 together with the density-upgrade hypotheses of Theorem~\ref{lem:density} and the relativistic null-stress identification premise; quasi-local propagation and endpoint control are supplied internally by Axiom~\ref{ax:maxent} on the local finite-constraint branch & Phase I conditional bridge theorem \\
\textbf{D5} & Jacobson-type Einstein branch: rest-frame relation plus conditional tensor upgrade (Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}) & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Jacobson small-ball first law; null-to-tensor reconstruction modulo a metric term & T4 fixed-cap stationarity; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions; D4 carried remainder negligible at order \(\ell^4\); all local directions/reference states for the tensor upgrade & Phase I conditional scaling-limit theorem/corollary \\
...
\textbf{D7} & Compact gauge reconstruction (Theorem~\ref{thm:compactgauge}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Doplicher--Roberts / Tannaka reconstruction & T2 and T5--T6 together with T7's directed-colimit/fiber premise for transportable edge sectors; the local-Gibbs/Dobrushin-type collar package supplies only fixed-cutoff support and recoverability control and does not by itself prove existence, persistence, triviality, or nontriviality of the refinement-limit sector colimit & Phase I conditional structural theorem \\
\textbf{D8} & Product gauge structure up to finite quotient (Lemmas~\ref{lem:mincontent}--\ref{lem:commutant}, Theorem~\ref{thm:sm}) & D7 + Axiom~\ref{ax:mar} & compact Lie representation classification; Schur's lemma & connected positive-dimensional Lie admissible class; one connected abelian factor; faithful action on the minimal coupled carrier; vanishing relevant transport obstruction & Phase I realized-branch theorem \\
...
\textbf{D10} & Gauge-coupling numerical closure of the supplement-backed implementation & D9 + external input \(P\) & RG evolution and matching conventions supplied in the supplement & pixel constraint; edge heat-kernel closure; supplement-backed beta-function and threshold conventions & Phase II calibration sector \\
\textbf{D12} & Charged-lepton/Koide, texture, dark-sector, baryogenesis, black-hole spectroscopy, proton-spin, proton-lifetime estimates beyond the gauge-channel exclusion, conditional string/worldsheet reorganizations, conjectural critical-superstring extensions, and other continuations & various subsets of D6, D9, and D10 & branch-specific EFT and phenomenological manipulations & additional ans\"atze such as the uniform \(\mathbb Z_6\) center-label ensemble, discrete texture choices, dark-sector response assumptions, discrete-horizon assumptions, or extra worldsheet/CFT premises & Phase III phenomenological continuation branch \\
\end{longtable}
\endgroup

The recovered core of this SM/GR derivation paper is D1--D5 together with D7--D9. D6 is a separate input-dependent corollary, D10 is the supplement-backed calibration sector, and D12 collects phenomenological continuations. The modular-anomaly dark-sector branch belongs to D12 and does not derive MOND/RAR-style response laws or galaxy/cluster phenomenology. The only external continuous inputs are \(N_{\mathrm{scr}}\) in D6 and \(P\) in D10, together with descendants built from those branches.
...
This section states the premise ledger and dependency map for this SM/GR derivation paper. The OPH basis used below consists of the five core axioms, the two external continuous inputs listed next, and the external technical premises \(T1\)--\(T7\) collected afterward. On the fixed-cutoff local finite-constraint MaxEnt branch, the local-Gibbs form, quasi-local dynamics, Lieb--Robinson propagation control, and endpoint-Lipschitz interval control are internal consequences rather than theorem-external regularity selectors. Any Dobrushin/local-Gibbs/mixing language used below names only a sufficient fixed-cutoff collar-recoverability mechanism on that branch. It is not an infinite-volume uniqueness claim for the refinement-limit theory, and it does not by itself constrain whether a transportable refinement-directed edge-sector colimit exists or whether, if it exists, it is trivial or nontrivial. What Axiom~\ref{ax:maxent} fixes under refinement is the realized state-side MaxEnt branch itself. Theorem~\ref{thm:bw} fixes the cap generator and its \(2\pi\) normalization only on the Bisognano--Wichmann geometric branch; Corollary~\ref{cor:n2} derives the null half-sided modular pair only after the later null blow-up step; and the remaining theorem-external items are therefore the scaling-limit scope clause, the geometric-branch selection burden, the density-upgrade burden, fixed-cap stationarity, the relativistic null-stress identification premise, the vanishing obstruction premise where invoked, and, whenever compact gauge reconstruction is invoked, the existence of a transportable refinement-directed sector colimit together with its bosonic/symmetric/fiber-functor structure.
...
\]
which is precisely the endpoint-Lipschitz / finite-variation control used later in the null modular chain. The compact note does \emph{not} separately derive a second independent microscopic generator beyond this quasi-local branch generator; if a UV completion carries one, the only later requirement is that it lie in the same bounded-support algebraic closure so that the same propagation constants control the selected branch.

This local-Gibbs/Lieb--Robinson package, together with any Dobrushin-type estimate used later, should be read only as fixed-cutoff collar-local recoverability, support control, and carried-error bookkeeping on the selected branch. It is not a proof that the refinement limit lands in a trivial thermodynamic phase, not a proof that transportable edge sectors persist under refinement, not a proof that the relevant sector colimit exists, and not a proof that such a colimit is nontrivial. Its theorem-level role is precisely the collar-local mixing and support/error control carried into the later fixed-cutoff and scaling-limit arguments.
...
This SM/GR derivation paper uses one canonical numbered list of external technical premises. The fixed-cutoff realized presentation, the local-Gibbs form, quasi-local Lieb--Robinson propagation, endpoint-Lipschitz interval control, and the induced finite-dimensional refinement branch are not listed here because they are established internally from Axioms~\ref{ax:screen}--\ref{ax:maxent}. The seven external items are:
\begin{itemize}[leftmargin=2.3em]
\item[\textbf{T1}] the branch condition that the refinement-stable cap net lies in the Bisognano--Wichmann geometric modular class for caps and their conformal images: for each round cap \(C\subset S^2\), the scaling-limit modular automorphism group of \(\mathcal A(C)\) is implemented by the standard cap-preserving conformal one-parameter subgroup \(\lambda_C(s)\subset \Conf^+(S^2)\), with geometric parameter \(s\) normalized so that the null blow-up near a smooth cut acts by \(v\mapsto e^{-s}v\); no separate cap-isotropy / \(\mathrm{SO}(2)\)-equivariance selector is imposed;
\item[\textbf{T2}] vanishing of the relevant gluing obstruction class whenever global sector transportability is invoked: \([z]=0\) on the central branch, or its higher-gauge analogue \(q_\Sigma=0\) on the genuinely noncentral branch;
\item[\textbf{T3}] all Lorentz/null-modular/Einstein statements are claims about a controlled refinement/scaling limit of the regulator net, not literal exactness at fixed finite cutoff;
\item[\textbf{T4}] fixed-cap generalized-entropy stationarity for the admissible first-variation class used in the Jacobson branch;
\item[\textbf{T5}] symmetric braiding in the \(3+1\)D EFT branch;
\item[\textbf{T6}] a bosonic Tannakian fiber-functor premise, or else an explicit super-Tannakian fork;
\item[\textbf{T7}] whenever compact gauge reconstruction is invoked, the sector category means the directed colimit of transportable edge sectors with objectwise finite-dimensional fibers; the existence of that directed system, the transportability maps, the fiber functor, and the possible triviality or nontriviality of the colimit are all part of the categorical premise rather than consequences of Axiom~\ref{ax:maxent} or of any fixed-cutoff local-Gibbs/Dobrushin-type collar estimate.
\end{itemize}
\end{assumption}
...
\textbf{D3} & Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}: geometric modular flow and Lorentz branch & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry;\newline conformal classification on \(S^2\) & T1+T3; the BW branch carries the standard cap-dilation identification & conditional scaling-limit theorem \\
\textbf{D4} & Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Theorem~\ref{lem:density}, Lemma~\ref{lem:translation}; Theorem~\ref{thm:stress}: null modular bridge to \(T_{kk}\) & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock half-sided modular inclusion; distributional differentiation on half-lines & the theorem-local inherited-strip decomposition and exact-or-controlled Markov hypotheses of Proposition~\ref{prop:nullec} and Corollary~\ref{cor:n1}, plus T3 together with the density-upgrade hypotheses of Theorem~\ref{lem:density} and the relativistic null-stress identification premise; quasi-local propagation and endpoint control are supplied internally by Axiom~\ref{ax:maxent} on the local finite-constraint branch & conditional bridge theorem \\
\textbf{D5} & Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}: Jacobson-type Einstein branch via a rest-frame relation plus conditional tensor upgrade & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Jacobson small-ball first law; Lemma~\ref{lem:nullreconstruct} for the null-to-tensor upgrade modulo a metric term & T4 fixed-cap stationarity; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions; D4 carried remainder negligible at order \(\ell^4\); all local directions/reference states for the tensor upgrade & conditional scaling-limit theorem/corollary \\
\textbf{D6} & Proposition~\ref{prop:lambda}, Corollary~\ref{cor:lambda}: capacity branch for \(\Lambda\), plus the separate neutrino estimate & D5, which leaves the \(+\Lambda g_{ab}\) ambiguity & de Sitter entropy relation and dimensional analysis & external input \(N_{\mathrm{scr}}\) and the screen-capacity identification & input-dependent corollary / estimate \\
\textbf{D7} & Theorem~\ref{thm:compactgauge}: compact gauge reconstruction & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Doplicher--Roberts / Tannaka reconstruction & T2 and T5--T6 together with T7's directed-colimit/fiber premise for transportable edge sectors; the local-Gibbs/Dobrushin-type collar package supplies only fixed-cutoff support and recoverability control and does not by itself prove existence, persistence, triviality, or nontriviality of the refinement-limit sector colimit & conditional structural theorem \\
\textbf{D8} & Lemmas~\ref{lem:mincontent}--\ref{lem:commutant}, Theorem~\ref{thm:sm}: product gauge structure up to finite quotient & D7 + Axiom~\ref{ax:mar} & compact Lie representation classification; Schur's lemma & connected positive-dimensional Lie admissible class; one connected abelian factor; faithful action on the minimal coupled carrier; vanishing relevant transport obstruction & realized-branch theorem \\
...
\textbf{D12} & Sections~8--10 continuations:\newline charged leptons, proton spin/lifetime,\newline and string/worldsheet branches & various subsets of D6, D9, and D10 & branch-specific EFT and phenomenological manipulations & additional ans\"atze such as the uniform \(\mathbb Z_6\) center-label ensemble, texture choices, dark-sector response assumptions, discrete-horizon assumptions, or extra worldsheet/CFT premises & phenomenological continuation branch \\
\end{longtable}
\endgroup
...
\]
\item under the explicit transportable sector-colimit premise of \textbf{T7}, the edge-sector category reconstructs a compact gauge group, and the realized connected gauge structure has the form
\[
...
\begin{proof}
Items (i)--(iii) are collected from Theorem~\ref{thm:confluence}, Corollary~\ref{cor:lorentz}, Theorem~\ref{thm:einstein}, and Corollary~\ref{cor:einstein}. The compact gauge reconstruction is Theorem~\ref{thm:compactgauge}, applied only after the transportable sector colimit, its fiber functor, and whatever trivial or nontrivial content it has are supplied by \textbf{T7}; the preceding MaxEnt/local-Gibbs/collar-mixing package is not used there to prove existence or nontriviality of that colimit. The product gauge structure up to finite quotient is Theorem~\ref{thm:sm}; and the final identification of the exact Standard Model quotient is Theorem~\ref{thm:hypercharge} together with Corollaries~\ref{cor:ng}, \ref{cor:nc}, and Proposition~\ref{prop:z6}.
\end{proof}
...
\begin{proof}
Termination is assumed. Local confluence plus termination imply confluence by Newman's lemma~\cite{newman42}. The resulting normal form is unique. By assumption it is a globally consistent state.
\end{proof}
...
\end{enumerate}
Items 1 and 4 are the fixed-cutoff realized presentation used later; they are not extra axioms beyond the finite regulator presentation plus overlap redundancy.
\end{proposition}
...
The collar analysis proves a fixed-cutoff statement on the finite type-I regulator net: the reduced cap state has a literal density matrix, its modular Hamiltonian exists, and its nonadditive part is confined to a shrinking collar up to carried errors. The Lorentz claim is \emph{not} that these finite matrix algebras are the continuum observer algebras. The target is the refinement-limit cap pair. Under the controlled scaling clause \textbf{T3}, write
\[
(\mathcal A_\infty(C),\omega_\infty^C)
\]
for any realized scaling-limit cap algebra/state pair along the realized refinement-stable branch for a round cap \(C\subset S^2\). Nothing in Axiom~\ref{ax:maxent} by itself forces \(\mathcal A_\infty(C)\) to remain type I, and nothing in the fixed-cutoff MaxEnt package by itself proves that the scaling-limit branch lands in the Bisognano--Wichmann geometric phase. The role of \textbf{T1} is exactly to identify the branch on which the scaling-limit modular action is geometric. On such a branch the limit local algebra may leave the regulator class, and the modular action is then generally outer rather than inner.
...
\]
where \(\lambda_\ast>0\) is the lower spectral bound used to compare modular Hamiltonians. As in the collar section, every later ``exact'' collar formula is to be read in one of two ways:
\begin{enumerate}[leftmargin=2em]
...
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, the derived fixed-cutoff regulator/collar package established above, and the controlled scaling-limit scope clause \textbf{T3} of Assumption~\ref{ass:technical}. Assume in addition the \textbf{T1} branch condition that the realized refinement-stable scaling branch for caps and their conformal images lies in the Bisognano--Wichmann geometric modular class. Then the following hold.
...
\]
has nonadditive part confined to the shrinking collar up to the carried errors measured by \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(\eta_\delta^{\mathrm M}\).

\item For every realized scaling-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\) on that branch, the modular automorphism group is geometric:
\[
...
\item No claim is made that \(\mathcal A_\infty(C)\) remains type I. If the scaling-limit cap algebra happens to remain type I, item \textup{(ii)} may be written as the operator identity
\[
K_C=2\pi B_C.
\]
...
\begin{proof}
Step 1: fixed-cutoff collar control. On the finite type-I regulator net, Propositions~\ref{prop:splice}, \ref{prop:modulartransport}, and \ref{prop:gensplit} localize the modular defect to the shrinking collar and quantify the discrepancy between the physical collar state and its exact-Markov reference by the carried errors \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(\eta_\delta^{\mathrm M}\).

Step 2: scaling-limit claim tier. Theorem~\ref{thm:bw} is a scaling-limit statement by \textbf{T3}, so the theorem concerns the refinement-limit cap pair \((\mathcal A_\infty(C),\omega_\infty^C)\), not the finite regulator matrices themselves. Nothing in this step assumes that type-I structure survives the limit.

Step 3: branch identification. By \textbf{T1}, on the selected branch the scaling-limit modular automorphism group of each cap pair is the standard cap-preserving conformal flow \(\alpha_{\lambda_C(s)}\). Therefore there is a constant \(\kappa_C>0\) such that
\[
\sigma_t^{\omega_\infty^C}=\alpha_{\lambda_C(\kappa_C t)}.
\]

Step 4: normalization. The modular group is KMS at inverse temperature \(1\) by definition. Since \(\lambda_C(s)\) is normalized by unit null dilation under the blow-up \(v\mapsto e^{-s}v\), the standard Bisognano--Wichmann normalization gives
\[
\kappa_C=2\pi,
\]
hence
...
Step 5: operator versus automorphism form. If the limit cap algebra remains type I, one can represent the modular automorphism group by a modular Hamiltonian \(K_C=-\log\rho_C\) and recover \(K_C=2\pi B_C\). If the limit algebra leaves the regulator class, the modular automorphism group remains well defined while the inner operator representative need not exist inside \(\mathcal A_\infty(C)\). In that case the automorphism identity is the full theorem statement. The final sentence is exactly the carried-error conclusion from Step 1 transported along the controlled scaling family of \textbf{T3}.
\end{proof}

\begin{remark}[What is proved, what the ``BW/canonical cap phase'' means, and what remains open]
This theorem has one conditional boundary. OPH derives the finite-range MaxEnt family and its carried collar control. It does \emph{not} derive that every refinement-stable MaxEnt branch lands in the Bisognano--Wichmann phase, nor that MaxEnt by itself selects a vacuum or canonical cap state. When the manuscript informally speaks of the ``BW/canonical cap phase,'' it means exactly the \textbf{T1}-selected refinement branch whose realized scaling-limit cap pair satisfies Theorem~\ref{thm:bw}; in the generic non-type-I case this is an outer geometric modular action, not an inner density-matrix formula.

The remaining BW-side burden is an explicit two-object UV scaffold, and the present pressure point is the first object, not the symbolic rigidity calculation. First, one would need a \emph{canonical scaling-limit observer cap-pair realization theorem} from transported marginals. The current theorem contract for that object is sharp: the realized transported cap-local system packages the cap-local test family, the projectively compatible transported marginal family, and the asymptotic transport-equivalence certificate. The single remaining emitted witness is a derived vanishing carried-collar schedule on fixed local collars, explicitly decomposed into the constructive-recovery remainder and the faithful modular-defect term, namely the convergence to zero of the carried quantity
\[
...
\]
after transport to each fixed local collar model. Only after that witness is supplied can local weak-\(*\) extraction and GNS gluing emit the realized scaling-limit cap pair. Beneath that derived schedule, the sharper raw datum is the transported fixed-local-collar Markov/faithfulness package, and on the current branch the single live missing clause inside that datum is the eventual positive lower spectral bound feeding the faithful modular-defect term.

Second, one would need an \emph{ordered cut-pair rigidity theorem} on those realized cap pairs to collapse the residual cap-preserving conformal freedom to the unique hyperbolic subgroup \(\lambda_C(s)\); modular KMS normalization would then fix \(s=2\pi t\). Ordered cut-pair rigidity therefore remains symbolic until the realized scaling-limit cap pair exists. The weak-\(*\) extraction plus rigidity route is an explicit internal extension route, not part of the proved theorem surface.
\end{remark}
...
Beyond the derived half-sided-inclusion bridge, the remaining burden is precisely the density upgrade of Theorem~\ref{lem:density} and the relativistic identification of that density with continuum null stress.
\end{theorem}
...
This is the Jacobson-type fixed-cap stationarity premise used in the compact note; it is not
...
G_{ab}+\Lambda g_{ab}=8\pi G\,\langle T_{ab}\rangle
\]
modulo the metric-term ambiguity isolated by Lemma~\ref{lem:nullreconstruct}.
\end{corollary}
...
\]
for the directed colimit retaining the transportable sectors and intertwiners that persist in that system, with objectwise finite-dimensional fibers. This is the category on which Doplicher--Roberts / Tannaka reconstruction is applied. The theorem below is neutral about whether \(\mathsf{Sect}_\infty\) is trivial or nontrivial: if \textbf{T7} furnished only the tensor unit, the reconstructed compact group would be the trivial group.

\begin{theorem}[Conditional compact gauge reconstruction in the bosonic branch]\label{thm:compactgauge}
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, the relevant parts of Assumption~\ref{ass:technical}, and that the transportable edge-sector colimit furnished by \textbf{T7} in the EFT regime is a rigid symmetric \(C^*\)-tensor category with a faithful bosonic fiber functor to finite-dimensional Hilbert spaces. Then
\[
...
The forward transmutation certificate on the live D10 lane makes that logical order explicit: the source-only basis reconstructs the same \(\alpha_U(P)\), unified diffusion parameter
\[
...
\]
as the pixel-closure solve itself. It also separates the paper-side transmutation factor \(\beta_{\mathrm{transmutation,EW}}=N_c+1\) from the overloaded compare-only validation ratio that appears on older diagnostic readouts.
...
v(P):=v(\alpha_U(P),P).
\]
Thus the branch runs from OPH input \(P\) to the internal \(t_i\) and only then to the low-energy couplings. It does not use measured \(\alpha_i(m_Z)\) to infer \(t_i\).

Every quoted D10 electroweak number is downstream of this forward map. In particular, \(\alpha_{\mathrm{em}}^{-1}(m_Z)\), \(\sin^2\theta_W(m_Z)\), \(m_W\), and the stage-3 \(m_Z\) readout are compare-only consequences of the chosen D10 running/matching package. Their numerical agreement with observation therefore validates the printed implementation only in the calibration-sector sense recorded in the premise ledger. What remains outside theorem level is a general existence/uniqueness or monotonicity proof for the scalar root selected by the printed D10 package under arbitrary convention changes.
...
\]
namely the relativity chain together with the realized Standard Model structural chain. D6 is a separate input-dependent capacity corollary, D10 is the supplement-backed calibration sector, and D12 collects phenomenological continuations.
...
Phase II calibration sector (D10) & Pixel-closure gauge-coupling consistency, \(\alpha_i(m_Z)\), \(\alpha_{\mathrm{em}}^{-1}(m_Z)\), \(\sin^2\theta_W(m_Z)\), and the target-free source-only electroweak quintet \((W,Z,\alpha_{\mathrm{em}}^{-1},\sin^2\theta_W,v)\) on the D10 calibration branch & Failure of the supplement-backed closure once \(P\) and the printed running/matching conventions are imposed \\
...
The pixel area \(P\) calibrates the separate gauge-coupling sector used here, and the circular step that read the internal D10 transmutation parameter back from measured low-energy couplings is omitted from the compact-paper presentation. Within the printed D10 implementation, \(P\) first fixes \(M_U(P)\) and \(E_{\mathrm{cell}}(P)\), then a one-dimensional pixel-closure solve fixes \(\alpha_U(P)\), and only then do \(t_2(P)\), \(t_3(P)\), \(v(P)\), and the running electroweak outputs appear. Quantities algebraically entangled with that calibration are compare-only consistency checks for that implementation and do not belong to the recovered-core claim set.

Operationally, changing \(P\) moves the calibration family \((\alpha_U,\alpha_i(m_Z),\sin^2\theta_W(m_Z))\) through that forward solve. It does not alter the parameter-free structural outputs such as the gauge quotient or hypercharge lattice. Downstream matter-sector continuations, including Koide-style relations, remain outside this SM/GR derivation paper's recovered-core theorem package.
...
The remaining UV-side burden on the Lorentz branch is therefore the explicit branch-selection and scaling-limit package \textbf{T1}+\textbf{T3}, not the mere existence of a finite cell size.
...
\]
and the limit can leave the regulator class.

On the \textbf{T1} branch of Theorem~\ref{thm:bw}, the scaling-limit cap algebras may be non-type-I and, in the continuum-QFT case of interest, are expected to be type III. Then there is generally no cap density matrix \(\rho_C\in \mathcal A_\infty(C)\) and no bounded operator \(K_C=-\log\rho_C\) inside the limit algebra. The correct continuum object is the modular automorphism group of the pair \((\mathcal A_\infty(C),\omega_\infty^C)\), which on that branch acts geometrically and typically outerly.

There is therefore no contradiction between using type-I regulators and aiming at a type-III continuum. The finite regulator provides the collar decomposition, recovery control, and carried errors. What the fixed-cutoff MaxEnt package adds is control of the realized branch through one common finite-dimensional multiplier family under refinement. What it still does \emph{not} prove is the algebraic type of the continuum limit or that the limit state is a vacuum or canonical BW cap state. The phrase ``BW/canonical cap phase,'' when used in this compact note, means only the refinement-stable branch singled out by \textbf{T1}: the branch whose realized scaling-limit cap pair carries the standard geometric modular automorphism group. In the generic non-type-I case that is an outer geometric modular action, not an inner density-matrix identity.

The clean internal extension route remains a two-step chain, and the present pressure point is the first object. First, one needs a canonical scaling-limit observer cap-pair realization from transported marginals. The current theorem contract for that object is explicit: the realized transported cap-local system packages the cap-local test family, the projectively compatible transported marginal family, and the asymptotic transport-equivalence certificate. The remaining emitted witness is a derived vanishing carried-collar schedule on fixed local collars, explicitly decomposed into the constructive-recovery remainder and the faithful modular-defect term, namely the convergence to zero of the carried quantity
\[
r_{\mathrm{FR}}(\varepsilon_{n,m,\delta})
+
4\lambda_\ast^{-1}\,
...
The local MaxEnt branch gives a finite-range interacting fixed-cutoff dynamics, Propositions~\ref{prop:gauge} and \ref{prop:ancillauv} fix uniqueness of the UV branch only modulo OPH-stable equivalence, and Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} with Proposition~\ref{prop:hginteracting} close the genuinely noncentral topological branch at fixed cutoff. The open items on the UV side are the continuum BW/geometric lift of \textbf{T1}+\textbf{T3} and the fuller transportable compact-gauge lift when \textbf{T2} and \textbf{T5}--\textbf{T7} are invoked. The smallest internal extension route to the BW/geometric lift is the two-step chain of a scaling-limit cap-pair extraction theorem that extracts \((\mathcal A_\infty(C),\omega_\infty^C)\) as local weak-\(*\) limits of transported cap marginals with vanishing carried collar errors, followed by an ordered null cut-pair rigidity theorem that collapses the residual cap-preserving conformal freedom on those extracted cap pairs to the unique BW hyperbolic subgroup. Beneath the derived carried-collar witness, the sharper raw datum is a transported fixed-local-collar Markov/faithfulness package: collarwise conditional mutual information vanishing together with an eventual positive lower spectral bound on each fixed local collar model. Within that lower datum, the single missing clause is the eventual spectral floor, which feeds the faithful modular-defect term. The present corpus does not derive a refinement-uniform lower spectral floor for the transported fixed-local-collar marginals; this single clause remains external to the emitted theorem chain. That two-step chain and its transported fixed-local-collar datum are not part of the proved theorem surface stated here.

\paragraph{Shared excitation dictionary and flavor continuation status.}
The D10 calibration sector gives supplement-backed gauge-coupling closure on the displayed carrier. That carrier closes its own exact \(W/Z\) chart, and the promoted target-free source-only repair theorem emits the public electroweak quintet on the Phase II calibration branch; the freeze-once coherent repair surface serves only as compare-only validation. On that frozen authoritative repair surface the canonical \(W/Z\) reference pair is hit exactly, but it remains a compare-only sidecar below the target-free theorem. The separate D6 cosmological-capacity relation remains an input-dependent corollary. On the flavor side the manuscript treats one shared OPH excitation dictionary as the only proof-facing family datum; lane-specific integers are not introduced independently. For each realized same-label refinement arrow \(e\) on the three-generation bundle, let \(\omega_e\) be the same-label overlap holonomy scalar, let
\[
d_e:=1-\omega_e
\]
...
What earlier D12 language called a ``texture exponent'' or ``defect count'' is therefore not an extra primitive beyond the structural branch. For any lane-specific readout \(\gamma\) on the realized same-label transport graph, let
\[
m_{\gamma,e}\in\mathbb Z_{\ge 0}
\]
be the multiplicity with which \(\gamma\) consumes the elementary same-label excitation arrow \(e\). The associated excitation action is
\[
A_\gamma
:=
...
\]
So overlap holonomy and edge defects determine the suppression law before any base choice is made. The primitive flavor-side integers are the transport multiplicities \(m_{\gamma,e}\). A one-number exponent in ``units of \(\log 6\)'' is only the compressed summary
\[
...
This shared dictionary feeds the live continuation lanes. The intrinsic neutrino lane factors through the same-label scalar certificate \((g_e,\omega_e)\) and its centered descendants \((q_e,\eta_e,\mu_e)\); the current weighted-cycle branch therefore uses no extra primitive flavor labels. The quark mass-side continuation compresses the same dictionary to
\[
(\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})
\]
and, on the compact same-family branch, to the emitted ray \(\mathcal R_{D12}^{ud}\). On the CKM/CP side the selector frontier is closed: the same-label left-handed orbit collapses to the singleton \(\sigma_{\mathrm{ref}}\), so the selected D12 sheet is fixed and the next exact quark objects are the distinct \(u/d\) excitation-gap map and the intrinsic scale law on the emitted ray, not a fresh selector or a fresh flavor integer. The charged sector is equally tied to the same backbone: once a charged source pair \((\eta_{\mathrm{ch}},\sigma_{\mathrm{ch}})\) exists on the ordered charged carrier \((-1,x_2,1)\), the centered charged logs are fixed algebraically by
\[
\begin{aligned}
...
The remaining burdens sit strictly above the shared dictionary. For charged leptons, the theorem-grade waiting set is the promotion
\[
\widehat C_e^{\mathrm{cand}}\longrightarrow \widehat C_e
\]
and then the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\). For quarks, the next exact objects are the distinct \(u/d\) excitation-gap map and the intrinsic scale law on the emitted D12 mass ray. For neutrinos, the same-label scalar certificate closes beneath the weighted-cycle branch, and the irreducible exact missing object is the reduced absolute-scale correction invariant \(C_\nu\). Thus the shared excitation dictionary is not the missing theorem object. It is the common OPH family base, while old base-6 exponents survive only as compare-only compressions of its excitation action.
...
\]
on the frozen authoritative repair surface, but that exact pair remains compare-only beneath the target-free source-only theorem. The Higgs lane has an exact compare-only D11 inverse slice with
\[
...
\]
on the same ordered eigenvalue family, but it remains same-family-only beneath the open theorem chain \(\widehat C_e^{\mathrm{cand}}\to \widehat C_e \to \mu_{\mathrm{phys}}(Y_e)\). The quark lane has an exact same-family witness
\[
(u,d,s,c,b,t)=
(0.00216,\ 0.00470,\ 0.0935,\ 1.273,\ 4.183,\ 172.3523553288311)\,\mathrm{GeV},
\]
again same-family-only, because the selected D12 sheet is the wrong CKM branch and the emitted ray waits on the intrinsic scale law. The neutrino lane has a compare-only exact two-parameter adapter on the explicit positive selector segment with
\[
(m_1,m_2,m_3)=
(0.01745663295,\ 0.01948419960,\ 0.05308139066)\,\mathrm{eV},
\]
and exact representative central splittings \(\Delta m_{21}^2=7.49\times10^{-5}\,\mathrm{eV}^2\), \(\Delta m_{31}^2=2.5129\times10^{-3}\,\mathrm{eV}^2\), \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\), but it remains non-promotable while \(C_\nu\) stays open. The exact non-hadron output lane is complete, on a mixed theorem/compare-only/same-family surface rather than one uniform theorem tier.
...
Second, the common-shift quotient surface is closed negatively: the equalizer/common-shift route does not determine an absolute charged scale from centered data alone. The irreducible post-promotion object is the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\). If a theorem-grade \(\widehat C_e\) and refinement-stable physical \(Y_e\) are obtained, then the uncentered lift, determinant-line section, and affine anchor follow canonically from that one scalar:
\[
...
\]
Hence the public claim boundary is exact same-carrier readback plus the closed common-shift no-go, with the remaining burdens carried by the splitting package \ophid{oph_generation_bundle_branch_generator_splitting} and the descent to \(\mu_{\mathrm{phys}}(Y_e)\). A target-anchored same-family exact witness reproduces the charged reference triple exactly on the same ordered eigenvalue family, but its scope is same-family-only and it does not promote the charged theorem lane. Any balanced-circulant / \(Q=2/3\) / \(\delta=2/9\) relation remains compare-only continuation and is not an emitted mass theorem.

\paragraph{Quark continuation status.}
The light-quark lane remains a D12 continuation branch and is not a recovered-core theorem. Its local frontier is sharp. On the emitted same-label left-handed solver surface the selector problem is closed in the negative sense: the local orbit collapses to the singleton \(\sigma_{\mathrm{ref}}\) (\ophid{sigma_ref}). That selected sheet is the wrong D12 CKM sheet, so no theorem-grade branch-changing route from the emitted D12 data to the physical CKM shell is derived. Compare-only orbit moves do not change that statement because they are not promoted as sector-attached left-handed same-label transport. The mass side is equally sharp: the same-family branch emits only the one-parameter ray \(D12_{ud}^{\mathrm{mass}}\) (equivalently \(\mathcal R_{D12}^{ud}\), paper id \ophid{D12_ud_mass_ray}), not a distinguished modulus on that ray. Hence the theorem-side quark burden on the emitted branch is the single intrinsic ray law \ophid{intrinsic_scale_law_D12} on the emitted mass ray \ophid{D12_ud_mass_ray}. A target-anchored same-family exact witness reproduces the six quark reference masses exactly on the same ordered three-point family, but its scope is same-family-only and it does not remove the wrong-branch CKM no-go or emit the intrinsic ray law. Separately, the builder-facing pure-\(B\) payload pair beneath the reported quark values remains open.

\subsection{Neutrino and hadron continuation status}

The isotropic intrinsic neutrino branch is ruled out by the exact atmospheric-splitting cap, so the continuation lane is the weighted-cycle branch. On that branch the standard-math-fixed balanced transport-load selector is the arithmetic midpoint on the positive segment between \(\chi=1+\varepsilon\) and \(1+\gamma/2\):
\[
D_\nu=\frac{\chi+1+\gamma/2}{2},
\qquad
p_\nu(\gamma,\varepsilon)=1+\gamma+\frac{\varepsilon}{D_\nu}.
\]
This weighted-cycle branch reaches the PMNS window and the correct splitting hierarchy on this continuation surface, and no hidden extra discrete branch is left on that lane.

What it does \emph{not} yet do is emit the absolute normalization as a theorem. The normalized same-label overlap-defect weight section closes beneath the weighted-cycle shape, and the paper-facing remaining freedom is one positive absolute-scale bridge scalar
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
above the closed normalizer. Equivalently,
\[
\lambda_\nu=\frac{m_{\star,\mathrm{eV}}}{q_{\mathrm{mean}}^{p_\nu}}\,B_\nu.
\]
The attached stack factors through \(q_e=q_{\mathrm{mean}}qbar_e\), so a \(qbar_e\)-only bridge collapse does not follow from the corpus. The selected-point scalar built from the weighted-cycle state is internal to the emitted \(qbar_e\), \(\psi_{\mathrm{wc}}\), and \(\psi_\ast\) data and therefore is not the missing bridge-external object. Above the best emitted proxy, the exact missing bridge object is smaller: the near-unity positive correction
\[
C_\nu:=\frac{B_\nu}{I_\nu^{1/2}\,\widehat{\mathrm{ratio}}^{\,1/2}\,\mathrm{sum\_defect}^{-1}},
\]
whose compare-only value is \(C_\nu^\ast=0.9995473560\). Because the proxy is internal to the emitted stack, this reduced correction invariant \(C_\nu\) is the irreducible exact missing object, while the paper-facing amplitude \(B_\nu\) is reconstructed above it. The direct correction audit gives the target-containing reduced window \(C_\nu\in[0.9994296,1.0015784]\), which induces the sharpened compare-only bridge window \(B_\nu\in[6.69600,6.71040]\) with midpoint \(6.70320\), improving on the earlier three-route corridor. A narrower shortlist-consensus window \([6.70484,6.71259]\) is also on disk, but it remains diagnostic-only because it does not contain the compare-only target.
The positive selector segment also supports a stronger compare-only continuation adapter: solving \(\tau_\nu\) on \(D_\tau=(1-\tau_\nu)\chi+\tau_\nu(1+\gamma_{1/2})\) against the representative central ratio and then solving \(\lambda_\nu\) against \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\) yields \(\tau_\nu=0.49717\), \(\lambda_\nu=1.72396\), \((m_1,m_2,m_3)=(0.01745663,\,0.01948420,\,0.05308139)\,\mathrm{eV}\), and exact representative central splittings \(\Delta m_{21}^2=7.49\times10^{-5}\,\mathrm{eV}^2\), \(\Delta m_{31}^2=2.5129\times10^{-3}\,\mathrm{eV}^2\), \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\). That adapter remains strictly compare-only and non-promotable because the theorem lane waits on \(C_\nu\).
Taken together with the exact \(W/Z\) frozen repair pair, the exact Higgs/top inverse slice, and the exact charged/quark same-family witnesses, the exact non-hadron output lane is complete, even though the theorem-grade charged, quark, and neutrino lanes remain open at their sharper remaining objects.

The best closed constructive local object beneath that scalar is the defect-weighted same-label edge family
\[
q_e=\sqrt{g_e d_e},
\qquad
\mu_e=\mu_\nu\,\frac{e^{\eta_e}}{\frac13\sum_f e^{\eta_f}},
\]
but that family does not emit \(C_\nu\) or the induced \(B_\nu\) by itself. The sharpest local interface for the missing bridge remains the Majorana overlap-defect scalar route, yet the actual open object above the normalizer is the reduced correction invariant \(C_\nu\), not another hidden branch selector. The strongest closed-form residual clue remains compare-only and must stay that way until derived.

The hadron lane remains separate. It depends on one production backend export bundle with publication-complete manifest provenance, real stable-channel correlator arrays, and later nonperturbative runtime/systematics completion rather than on a closed theorem-level mass surface, so the neutrino weighted-cycle improvement does not promote hadron observables.
...
Observer-Patch Holography starts from overlap consistency of observer patches and yields, within one framework, several effective structures usually treated separately in modern fundamental physics. This SM/GR derivation paper supports a unique schedule-independent normal form for overlap repair and hence a unique physical UV branch only modulo boundary redundancy, implementation hiding, and inert ancillary stabilization, a conditional Lorentz branch in the assumed geometric modular phase, a conditional Jacobson-type Einstein branch under fixed-cap stationarity and the null-stress premise, the Standard Model gauge quotient with exact hypercharges on the realized matter package under the MAR admissibility package in the bosonic internal-gauge branch, the realized counting chain \(N_g=3\) then \(N_c=3\) on that branch, a separate supplement-backed gauge-calibration sector, and a separate input-dependent cosmological-capacity corollary. Phenomenological continuations are discussed separately from the recovered core.

The framework therefore presents general relativity and the Standard Model as effective descriptions arising once the stated scaling-limit and categorical premises are realized. Open items are sharper quantitative control, derivation or removal of the transportability-side premises on the refinement-limit compact-gauge branch, a fuller fermionic gauge branch, and explicit microscopic realizations whose continuum lift lands in the required modular/geometric phase.
