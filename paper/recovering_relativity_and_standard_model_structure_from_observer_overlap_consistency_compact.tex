...
The imported D1 package is the fixed-cutoff OPH consensus theorem stack: asynchronous confluence
...
\[
\alpha_{\mathrm{Th}}^{-1}(P)=\lim_{q^2\to0}\alpha_{\mathrm{em}}^{-1}(q^2;P)=137.035999177
\]
...
implementation in the calibration-sector sense recorded in the premise ledger. What remains
...
Phase II calibration sector (D10) & forward transmutation data \(\alpha_U(P)\), \(t_U(P)\), \(t_{\mathrm{tr}}(P)\), pixel-closure gauge-coupling consistency, \(\alpha_i(m_Z)\), the source-locked anchor \(a_0(P)=\alpha_{\mathrm{em}}^{-1}(m_Z^2;P)\), the electroweak transport family \((W,Z,\alpha_{\mathrm{em}}^{-1}(q^2),\sin^2\theta_W(q^2),v)\), and the Thomson endpoint \(\alpha_{\mathrm{Th}}^{-1}(P)=137.035999177\) on the Ward-projected \(\U(1)_Q\) lane & Failure of the integrated calibration closure once \(P\) and the printed running/matching conventions are imposed \\
...
Operationally, changing \(P\) moves the calibration family
\[
...
The exact lane-dependent theorem objects sit strictly above the shared dictionary. For charged leptons, the centered common-shift quotient is closed negatively, and the theorem-grade waiting set is the promotion
\[
\widehat C_e^{\mathrm{cand}}\longrightarrow \widehat C_e
\]
and then the post-promotion lift whose descended scalar is \(\mu_{\mathrm{phys}}(Y_e)\), with the physical identity-mode equalizer as the exact smaller forcing object beneath that scalar. For quarks, the mass bridge is internalized separately: the light-quark overlap-defect scalar
\[
\Delta_{ud}^{\mathrm{overlap}}=\frac16\log\frac{c_d}{c_u}
\]
closes target-free, and on the emitted D12 mass ray this is equivalently the downstream one-scalar law \ophid{quark_d12_t1_value_law}, with \ophid{intrinsic_scale_law_D12} as the derived wrapper. A separate direct public descent theorem closes on the public physical quark frame class selected by \(P\): the exact physical sigma datum descends uniquely to target-free public data on that selected class, the affine mean law emits \((g_u,g_d)\) algebraically, and the exact forward construction emits the exact sextet together with explicit exact forward Yukawas \(Y_u\) and \(Y_d\). This is selected-class closure only. It does not claim a global classification of all quark frame classes. For neutrinos, the same-label scalar certificate closes beneath the weighted-cycle branch, the bridge-rigidity theorem emits
\[
C_\nu
=
G^2Q S^{-1/2}
...
\]
on the frozen authoritative repair surface, but that exact pair remains compare-only beneath the target-free source-only theorem. The D11 Higgs/top branch splits into one exact Higgs calibration theorem and one companion D11 top-side calibration output. On the declared D10/D11 calibration surface,
\[
m_H=125.1995304097179~\mathrm{GeV},
\qquad
m_t^{D11}=172.388646~\mathrm{GeV}.
\]
The Higgs theorem uses the lambda-side seed
\[
...
\]
together with the readout law
\[
\delta\lambda(\mu_t)
=
-\frac{16}{9}\sigma_{D11,H}^{\mathrm{exact}}\lambda^{\mathrm{core}}(\mu_t),
\]
which fixes the Higgs row exactly on that declared surface. The companion D11 top-side row continues to use the common one-scalar seed
\[
\sigma_{D11,\mathrm{HT}}=\frac{\alpha_U\cos(2\theta_{W0})}{\sqrt{\pi}},
\]
with the fixed-ray factorization
\[
\frac{\delta y_t(\mu_t)}{y_t^{\mathrm{core}}(\mu_t)}
=-\frac{9}{16}\frac{\delta\lambda(\mu_t)}{\lambda^{\mathrm{core}}(\mu_t)}
=\sigma_{D11,\mathrm{HT}}.
\]
The exact inverse slice on the same D11 Jacobian remains compare-only and does not define the theorem surface.
...
\]
again same-family-only on \texttt{current\_family\_only}. This sextet matches the official PDG 2025 API running-quark target surface exactly; its top coordinate is the PDG 2025 cross-section mass entry rather than the auxiliary direct-top entry. A separate target-free mass bridge closes
\[
\Delta_{ud}^{\mathrm{overlap}}=\frac16\log\frac{c_d}{c_u},
\]
equivalently the downstream one-scalar law \ophid{quark_d12_t1_value_law}, on the emitted D12 ray. A separate restricted current-family common-refinement transport-frame theorem surface emits a sector-attached element of \(\Sigma_{ud}^{\mathrm{phys}}\), the exact physical sigma datum, the restricted absolute sector readout, reconstructs the same running sextet exactly on that declared carrier, and emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\) on that carrier. A separate direct public descent theorem closes on the public physical quark frame class selected by \(P\): the exact physical sigma datum descends uniquely to target-free public data on that selected class, the affine mean law emits \((g_u,g_d)\) algebraically, and the exact forward construction emits the same exact sextet together with explicit exact forward Yukawas \(Y_u\) and \(Y_d\). This is selected-class closure only. It does not claim a global classification of all quark frame classes. The neutrino lane has the weighted-cycle bridge-rigid absolute family
\[
(m_1,m_2,m_3)=
...
\paragraph{Charged-lepton theorem boundary.}
...
\paragraph{Quark selected-class theorem.}
The light-quark lane carries a theorem-grade selected-class closure on the public physical quark frame class chosen by \(P\). The historical emitted same-label left-handed solver surface closes to the singleton \(\sigma_{\mathrm{ref}}\) (\ophid{sigma_ref}). That closure is a lower-level negative selector statement on the selected D12 sheet. Same-sheet overlap scans, chirality-swapped basis diagnostics, and other non-sector-attached orbit improvements are compare-only and do not alter the selected-class theorem.
...
\]
and on that emitted ray the same scalar is equivalently the one-scalar law
\[
\Theta_{ud}^{\mathrm{mass}}
:=
\text{\ophid{quark_d12_t1_value_law}}.
\]
On the declared current-family common-refinement transport-frame carrier, a separate restricted theorem chain emits a sector-attached element of \(\Sigma_{ud}^{\mathrm{phys}}\), the exact physical sigma datum, the restricted absolute sector readout, the same PDG 2025 sextet, and explicit exact forward Yukawas \(Y_u\) and \(Y_d\). A separate direct public descent theorem closes on the public physical quark frame class selected by \(P\), represented on the realized lane by the explicit same-label common-refinement transport-frame class \([F_0^\dagger F_1]\).
...
\]
The full D12 mass-side package is therefore functorial:
\[
...
\]
The mass bridge is not part of the remaining exact frontier on this code surface. The builder-facing pure-\(B\) payload pair remains a lower implementation object beneath the reported forward quark values, and \ophid{intrinsic_scale_law_D12} remains the derived wrapper above \(\Theta_{ud}^{\mathrm{mass}}\).

The direct public theorem on the selected public physical quark frame class closes the route that the historical physical-sheet contract isolated. Write the same-label left-handed physical carrier as
\[
...
\]
for diagonal \(D_u,D_d\in U(1)^3\). On the selected public quark frame class \(f_P\), represented on the realized lane by the explicit common-refinement transport-frame class \([F_0^\dagger F_1]\), the restricted sector-attached lift emits a class in \(\Sigma_{ud}^{\mathrm{phys}}\) together with the exact physical sigma datum. The realized section on that selected class is unique up to the defining diagonal quotient, so the emitted \(\Sigma_{ud}^{\mathrm{phys}}\) element and the exact sigma datum depend only on \(f_P\) and not on the declared representative. Therefore the exact sigma readout descends uniquely to target-free public data on \(f_P\).
...
\]
Then the affine mean law emits
\[
g_u
=
g_{\mathrm{ch}}
...
\exp\!\bigl(-(A_{ud}\sigma_{\mathrm{seed}}^{ud}+B_{ud}\eta_{ud})\bigr).
\]
The ordered three-point quadratic readout then emits the exact running sextet, and the exact forward construction emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\) on that selected public class. The historical algebraic-collapse theorem remains the explanation for why the absolute readout is algebraic once the exact physical sigma datum is public.

The selected-class public theorem can therefore be written as
\[
f_P
\Longrightarrow
...
\]
together with explicit exact forward Yukawas \(Y_u\) and \(Y_d\). This is selected-class closure only. It does not claim a global classification of all quark frame classes.
...
\end{aligned}
\]
Then the corpus theorem-emits exactly the following quark-side package:
\[
\textbf{(i)}\quad
D12_{ud}^{\mathrm{mass}}
=
...
\]
A separate target-free theorem pair closes the mass bridge:
\[
P\vdash
\Delta_{ud}^{\mathrm{overlap}}=\frac16\log\frac{c_d}{c_u},
\qquad
...
\]
On the lower theorem-emitted package and the mass bridge alone, the selected-class public theorem does not follow. The separate direct public descent theorem is the exact additional theorem on the selected public class \(f_P\). It factors the exact \(\Sigma_{ud}^{\mathrm{phys}}\) element and the exact sigma datum through \(f_P\), so the affine mean law emits \((g_u,g_d)\) algebraically and the ordered three-point quadratic readout emits
\[
(m_u,m_d,m_s,m_c,m_b,m_t).
\]
The exact forward construction then emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\) on that selected public class.

\paragraph{Historical extension pair and selected-class closure theorem.}
Above the lower theorem-emitted package and the internalized mass bridge, the historical physical-sheet extension pair remains
\[
H_{\mathrm{phys}}
\;:\;
...
\]
The closed selected-class theorem is the direct descent statement on \(f_P\): the exact physical sigma datum is target-free public on the selected public class, the affine mean law emits \((g_u,g_d)\) algebraically, the ordered three-point quadratic readout emits the exact sextet, and the exact forward construction emits explicit exact forward Yukawas \(Y_u\) and \(Y_d\). This is selected-class closure only. It does not claim a global classification of all quark frame classes.
...
\]
and within that same-family scope the selected-sheet exact readout closes through \ophid{oph_quark_current_family_selected_sheet_exact_closure}, built on the ordered three-point quadratic readout theorem \ophid{oph_quark_current_family_quadratic_readout_theorem}. A separate restricted current-family common-refinement transport-frame theorem surface emits a sector-attached element of \(\Sigma_{ud}^{\mathrm{phys}}\) together with a unique exact sigma target on that declared carrier, and the affine mean law reconstructs the same running sextet exactly on that carrier. These exact supporting surfaces realize the same exact sextet that the selected-class public theorem emits on \(f_P\). The builder-facing pure-\(B\) payload pair beneath the historical forward branch remains a separate lower-level object.
A separate D12 internal backread sidecar is on disk. On that continuation-only surface, feeding the emitted reference-free forward light-quark pair through the coefficient-ratio identification \(c_u/c_d=y_u/y_d=m_u/m_d\) and the D12 normalization \(\Delta_{ud}^{\mathrm{overlap}}=\tfrac16\log(c_d/c_u)\) fixes
\[
...
\]
That sidecar closes the D12 mass-side scalar package numerically on its own continuation surface. It does not replace the selected-class public theorem and it does not by itself classify all quark frame classes.

\subsection{Neutrino and hadron theorem boundaries}
...
Taken together with the exact \(W/Z\) frozen repair pair, the exact Higgs calibration theorem together with its companion D11 top-side calibration output and compare-only exact inverse Higgs/top slice, the exact charged witness, the selected-class exact quark theorem together with its same-family and restricted transport-frame supporting surfaces, and the weighted-cycle bridge-rigid absolute neutrino family, the exact non-hadron output lane is complete. The theorem-grade charged lane remains open at its sharper remaining objects, while the quark theorem is selected-class closure rather than a global classification of all quark frame classes.
...
The hadron lane is separate. It depends on one production backend export bundle with publication-complete manifest provenance, real stable-channel correlator arrays, and later nonperturbative runtime/systematics completion rather than on a closed theorem-level mass surface. The declared particle roadmap does not include hadron-mass closure, because that nonperturbative execution burden is not a realistic target within the current project compute scope, so the neutrino weighted-cycle improvement does not promote hadron observables.
