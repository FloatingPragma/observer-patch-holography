...
This paper presents the compact reconstruction program based on Observer-Patch Holography (OPH). Physical data are organized patchwise: each observer has access only to a patch algebra, neighboring observers must agree on overlaps, and the reconstruction is driven by those consistency conditions. Under the five axioms and the additional branch premises stated below, that structure feeds a Lorentzian modular-phase classification, a Jacobson-type Einstein branch conditional on the derived fixed-cap generalized-entropy stationarity theorem for the realized cap-label-preserving MaxEnt family together with the null modular bridge and the bounded-interval projective branch, compact gauge reconstruction in a bosonic internal-gauge sector, charge quantization, and the low-energy architecture selected by the admissibility conditions used in the gauge branch. The retained black-hole structural package on the companion synthesis surfaces is narrower: on the fixed-cutoff edge-center collar carrier in the exact-Markov or idealized exact-recoverability limit, together with the geometric modular branch, the manuscript retains the edge-center obstruction to naive factorization, the recoverability-style interior-encoding reading at small collar conditional mutual information, Hawking/KMS normalization, the discrete area spectrum, and the Schwarzschild transition identity for actual sector changes, while discrete-horizon combs, page-curve/island claims, and spectroscopy templates remain in the continuation lane and do not sit inside the recovered relativity-plus-Standard-Model core.
...
\item The same Einstein branch closes globally once the external screen-capacity input is supplied, tying the cosmological-constant branch to global screen capacity rather than local vacuum energy.
...
compact-gauge branches remains separate.
...
The labeled premises used in the dependency table are introduced here before the ledger appears later in the paper. \textbf{T2} is the controlled refinement/scaling-limit scope clause for the Lorentz/null-modular/Einstein branch. \textbf{T3} is internalized below as the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family used in the D5 Einstein branch. The bosonic gauge-reconstruction premises corresponding to \textbf{T1} and \textbf{T4}--\textbf{T6} are stated separately in Assumption~\ref{ass:gaugeprem} in Section~\ref{sec:premise-ledger}.
...
The recovered core of this SM/GR derivation paper is D1--D5 together with D7--D9. D6 is not part of that recovered core because it requires the external screen-capacity input \(N_{\mathrm{scr}}\), but it is the input-dependent global closure of the same Einstein branch together with its derived de Sitter static-patch parameter package. D10 is the integrated quantitative-closure branch, and D12 collects phenomenological continuations. The local/global handoff is exactly the D5\(\to\)D6 handoff: D5 recovers the local Einstein branch only modulo a metric term, and D6 fixes that remaining constant only after the global screen-capacity input \(N_{\mathrm{scr}}\) is supplied. The modular-anomaly dark-sector branch belongs to D12 and does not derive MOND/RAR-style response laws or galaxy/cluster phenomenology. The only external continuous input is \(N_{\mathrm{scr}}\) in D6; the local pixel ratio \(P\) belongs to the separate Phase-II closure branch together with descendants built from it.
...
declared quantitative branch. The current code surface includes a numerical fixed-point witness
...
\(P=1.630968209403959\ldots\) from \(\alpha^{-1}=137.035999177\). This is not yet an
interval-arithmetic proof of uniqueness; the companion local numerical contraction certificate
records the current machine-checkable hardening step. A separate pending hardware note reports an
...
This SM/GR derivation paper uses one canonical numbered list for the remaining theorem-external scaling items outside the gauge branch. The fixed-cutoff realized presentation, the local-Gibbs form, quasi-local Lieb--Robinson propagation, endpoint-Lipschitz interval control, the induced finite-dimensional refinement branch, the fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, and the operational/geometric split of Theorem~\ref{thm:geosubnet} are not listed here because they are established internally from Axioms~\ref{ax:screen}--\ref{ax:maxent}. The remaining external item outside the gauge branch is:
...
\end{itemize}
The former \textbf{T3} item is internalized below as Theorem~\ref{thm:equilibrium}. The bosonic ordinary/central gauge premises are stated separately in Assumption~\ref{ass:gaugeprem}.
\end{assumption}

\begin{assumption}[Bosonic gauge reconstruction premises]\label{ass:gaugeprem}
Whenever compact gauge reconstruction is invoked on the ordinary or central-defect branch, the following additional premises are kept explicit. Together they are the compact-paper form of \textbf{T1} and \textbf{T4}--\textbf{T6}.
...
\]
is half-sided modular. Equivalently, after the harmless convention change \(t\mapsto -t\), one recovers the standard positive-time half-sided-inclusion form. The former half-sided-inclusion premise is therefore derived from the null-interval structure, isotony, and the scaling-limit geometric action rather than imported as an additional modular-QFT ingredient.
\end{corollary}
...
The branch quantity \(t_\Lambda\) is the de Sitter timescale fixed by the same D6 closure. The observed cosmic age \(t_0\) is not an additional D6 theorem output. It remains a downstream FLRW comparison quantity once one chooses a cosmological model above the local/global D5\(\to\)D6 stack. On the flat \(\Lambda\)CDM benchmark,
\[
...
Proposition~\ref{prop:lambda} and Corollaries~\ref{cor:lambda}, \ref{cor:lambda-global}, \ref{cor:lambda-parameters} are the intended local/global D6 stack: local null-modular reconstruction leaves the Einstein equation determined only up to a metric term, the global screen-capacity identification fixes the remaining constant, and the same Einstein branch then closes globally with its de Sitter static-patch readout once \(N_{\mathrm{scr}}\) is declared. This is a structural aspect of the framework. The vacuum-energy problem is reorganized by separating local null data from the global capacity closure.
...
contraction certificate for the implemented map. What remains outside theorem level is an
...
\Lambda=\frac{3\pi}{G N_{\mathrm{scr}}}
\]
fits naturally with a de Sitter static patch of finite entropy via $N_{\mathrm{scr}}=A_{\mathrm{dS}}/(4G)$. On the same D6 branch, this fixes the static-patch package
\[
S_{\mathrm{dS}}=N_{\mathrm{scr}},
\qquad
r_{\mathrm{dS}}=\sqrt{\frac{3}{\Lambda}},
...
t_\Lambda=\frac{r_{\mathrm{dS}}}{c},
\]
while the observed cosmic age remains a downstream FLRW benchmark rather than an additional D6 theorem output. The local null-modular reconstruction leaves the metric ambiguity, while screen capacity supplies the global datum that fixes \(\Lambda\) and its de Sitter readout package. The compact paper is therefore formulated in a de Sitter-first language. The positive cosmological constant is tied to finite capacity rather than to a deformation of a negative-$\Lambda$ starting point.
...
branch remain on the Phase-II implementation surface: they are forward-emitted there, and
...
\]
and the limit can leave the regulator class. Axiom~\ref{ax:maxent} controls the realized state-side branch across refinement; it does \emph{not} by itself prove that the refinement limit remains type I or that the emitted scaling-limit pair lies in the BW / canonical cap phase with standard geometric modular action.
...
family-side input beneath those continuations. Old base-6 exponents remain compare-only
...
\]
on the frozen authoritative repair surface, but that exact pair remains compare-only beneath the target-free source-only theorem. The D11 Higgs/top branch closes as one source-only split theorem on the declared D10/D11 quantitative surface. On the emitted D10 repair tuple \((\eta_{\mathrm{source}},\beta_{EW},\lambda_{EW},\tau_{2,\mathrm{tree}}^{\mathrm{exact}},\delta n_{\mathrm{tree}}^{\mathrm{exact}})\), define
\[
...
direct-top PDG row remains open; see \href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}.
The old one-scalar seed
\[
\sigma_{D11,\mathrm{HT}}=\frac{\alpha_U\cos(2\theta_{W0})}{\sqrt{\pi}}
\]
remains on disk only as the lower-rank fixed-ray companion branch with \(\pi_y=\pi_\lambda=\sigma_{D11,\mathrm{HT}}\). The exact inverse slice on the same D11 Jacobian remains compare-only and does not define the theorem surface.
