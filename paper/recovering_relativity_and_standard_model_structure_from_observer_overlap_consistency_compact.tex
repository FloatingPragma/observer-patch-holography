...
\]
on the frozen authoritative repair surface, but that exact pair remains compare-only beneath the target-free source-only theorem. The D11 Higgs/top branch closes as one source-only split calibration theorem on the declared D10/D11 calibration surface. On the emitted D10 repair tuple \((\eta_{\mathrm{source}},\beta_{EW},\lambda_{EW},\tau_{2,\mathrm{tree}}^{\mathrm{exact}},\delta n_{\mathrm{tree}}^{\mathrm{exact}})\), define
\[
...
\]
again same-family-only on \texttt{current\_family\_only}. This sextet matches the official PDG 2025 API running-quark target surface exactly; its top coordinate is the PDG 2025 cross-section mass entry. The bridge to the auxiliary direct-top row remains open; see \href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}. A separate target-free mass bridge closes
\[
\Delta_{ud}^{\mathrm{overlap}}=\frac16\log\frac{c_d}{c_u},
\]
