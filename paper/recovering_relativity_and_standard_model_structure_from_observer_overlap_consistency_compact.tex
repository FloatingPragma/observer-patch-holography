...
reconciliation square, and the record-algebra theorem on the observer-accessible surface. The imported D1
...
engineering evidence. The conditional endpoint package localizes the remaining
fine-structure displacement: the implemented fixed-point report is short by
\(0.041163974743011720681199819215801821\) inverse-alpha units, while the
CODATA-mapped pixel value \(P=1.630968209403959324879279847782648941\ldots\)
requires a source residual
\(0.041465835978928681137459869870016982\ldots\) beyond the exact one-loop
continuation. In quark-screening form, the required factor is
\(S_{\mathrm{required}}=0.895400127551185647132725678585532880\ldots\), or
\(c_Q=0.658025360816792342502465198049036592\ldots\) in
\(S=1-x+c_Qx^2\). This scalar is a theorem target for the Ward-projected QCD
screening and endpoint-remainder map, not a source-only derivation of that map.
...
\textbf{D4} & Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Theorem~\ref{lem:density}, Lemma~\ref{lem:translation}; Theorem~\ref{thm:stress}: null modular bridge to \(T_{kk}\), including exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line families, the weak tail generator, the derived half-sided modular inclusion, the explicit positive half-line null-translation generator on its Stone domain with affine half-line modular relations, and the exact half-line generator/charge identification with the local null-stress charge & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock positive-generator theorem for standard half-sided inclusions; Stone's theorem; distributional differentiation on half-lines & the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed for the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses of Proposition~\ref{prop:nullec} and Corollary~\ref{cor:n1}, plus T2; quasi-local propagation and endpoint-Lipschitz control are supplied internally by Axiom~\ref{ax:maxent} on the local finite-constraint branch, the geometric scaling action on the null half-line blow-up net derives the half-sided modular pair, the effective local modular-Hamiltonian formula is used only to read the same renormalized half-line family in continuum variables, and bounded-interval formulas require the separate interval-preserving projective branch & conditional bridge theorem \\
...
The forward transmutation certificate on the D10 lane makes that logical order explicit: the source-only basis reconstructs the same \(\alpha_U(P)\), hence the same unified diffusion parameter
\[
...
The UV regulator is placed on the screen algebra. It is not placed on an emergent bulk spacetime lattice. Lorentz kinematics arise from the continuum modular action on caps, summarized in Corollary~\ref{cor:lorentz}, without relying on microscopic invariance of a bulk discretization.
...
until a source-side extraction-response kernel is emitted.
...
\]
Detailed particle-spectrum continuation statements are carried by Ref.~\cite{ophparticles}. That companion paper develops the quark selected-class theorem, the charged-lepton determinant-line boundary, the weighted-cycle neutrino theorem surface, and the hadron execution boundary. The hadron backend tasks \#153/\#157 are closed as out-of-scope/computationally blocked, not solved; the compact paper therefore promotes no hadron masses and reopens that branch only after a working OPH hadron backend emits production spectral data and systematics. These particle-continuation statements do not belong to the compact-paper claim surface.
