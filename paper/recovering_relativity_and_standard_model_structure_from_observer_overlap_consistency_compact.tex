...
The numbered premises used in the dependency table are introduced here before the ledger appears later in the paper. \textbf{T1} is the controlled refinement/scaling-limit scope clause for the Lorentz/null-modular/Einstein branch. \textbf{T2} is the nontrivial realized MAR-occupancy boundary for the compact-gauge branch. The fixed-cap generalized-entropy stationarity result is Theorem~\ref{thm:equilibrium}, not a labeled premise. Transportability, the fixed-cutoff bosonic-category construction, and refinement/fiber descent are theorem-produced by Theorems~\ref{thm:transport-from-gluing}, \ref{thm:fixed-cutoff-bosonic-sector-category}, and \ref{thm:refinement-functor-fiber-descent}, so they do not appear in the numbered premise list.
...
This section states the premise ledger and dependency map for this SM/GR derivation paper. The basis used below consists of the five core axioms, the quantitative quantities listed next, the scaling/continuum premises collected afterward, and the theorem-produced bosonic gauge-reconstruction chain. This is an algebraic-information basis: the screen-net, state, trace/probability, and generalized-entropy structures are starting ingredients of the OPH framework. The purpose is to test whether this basis supports a consistent and comprehensive theory-of-everything reconstruction of the observed effective universe. On the fixed-cutoff local finite-constraint MaxEnt branch, the local-Gibbs form, quasi-local dynamics, Lieb--Robinson propagation control, and endpoint-Lipschitz interval control are internal consequences, separate from theorem-external regularity selectors. Any Dobrushin/local-Gibbs/mixing language used below names only a sufficient fixed-cutoff collar-recoverability mechanism on that branch. It is not an infinite-volume uniqueness claim for the refinement-limit theory, and it does not by itself decide nontrivial realized gauge occupancy. What Axiom~\ref{ax:maxent} fixes under refinement is the realized state-side MaxEnt branch itself. Theorem~\ref{thm:geosubnet} fixes the operational/geometric split at fixed cutoff. On that geometric subnet, Proposition~\ref{prop:cap-extraction} emits the extracted scaling-limit geometric cap pair by local weak-\(*\) / GNS extraction, Proposition~\ref{prop:round-cap-rigidity} fixes the round-cap conformal subgroup from the surviving cut data, and Theorem~\ref{thm:bw} then fixes the cap generator and its \(2\pi\) normalization. The old common-floor collapse remains relevant only as a warning against a stronger raw regulator-level modular-Hamiltonian statement on directions that disappear in the extracted GNS quotient; Theorem~\ref{thm:regularized-modular-transport} supplies the support-visible control needed on the theorem target. Corollary~\ref{cor:n2} then derives the null half-sided modular pair after the null blow-up step, and Lemma~\ref{lem:translation} promotes that pair to the explicit positive null-translation generator on its Stone domain with the affine half-line modular relation. Theorem~\ref{thm:equilibrium} then internalizes the fixed-cap generalized-entropy stationarity step for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family. The only numbered theorem-external boundary is therefore \textbf{T2}, realized nontriviality of the MAR gauge branch; \textbf{T1} remains only the declared scaling-limit claim tier. The bounded-interval transport/projective branch remains a theorem-local branch condition only where those interval formulas are invoked. Gluing-side path-independent transport is not theorem-external: it is Theorem~\ref{thm:transport-from-gluing}. On the ordinary/central branch the zero obstruction is \([z]_\Sigma=0\); on the genuinely noncentral branch the zero obstruction is \(q_\Sigma=0\), and \(q_\Sigma\ne0\) is routed to the fixed-cutoff higher-gauge sector rather than to ordinary compact-group reconstruction.
...
\]
for an induced map \(R_{\ell\to L}\) on that multiplier space. The refinement-stable realized branch is therefore the persistent trajectory or invariant subset selected inside this finite-dimensional space, not an imported theorem-external regularity package. This state-side refinement notion is enough to compare realized states across cutoffs. It does not, by itself, prove nontrivial realized gauge occupancy. Any Dobrushin/local-mixing hypothesis used by the compact-gauge branch is logically separate: it supplies a collar estimate on a fixed finite-dimensional model, not information about the refinement-limit gauge phase. Whenever later sections speak of a ``refinement-stable directed colimit'' of zero-obstruction edge sectors, the monoidal-refinement/fiber clause is Theorem~\ref{thm:refinement-functor-fiber-descent}; Axiom~\ref{ax:maxent} supplies only the realized state branch along which one asks whether nontrivial sector occupancy persists.
...
\begin{assumption}[Numbered technical premises]\label{ass:technical}
This SM/GR derivation paper uses one canonical numbered technical-premise list. The fixed-cutoff realized presentation, the local-Gibbs form, quasi-local Lieb--Robinson propagation, endpoint-Lipschitz interval control, the induced finite-dimensional refinement branch, the fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, and the operational/geometric split of Theorem~\ref{thm:geosubnet} are not listed here because they are established internally from Axioms~\ref{ax:screen}--\ref{ax:maxent}.
\begin{itemize}[leftmargin=2.3em]
\item[\textbf{T1}] all Lorentz/null-modular/Einstein statements are claims about a controlled refinement/scaling limit of the regulator net, not literal exactness at fixed finite cutoff;
\item[\textbf{T2}] the compact reconstruction category can be trivial unless the OPH/MAR branch actually occupies a nontrivial zero-obstruction realized low-energy sector. MAR order, minima, and physical-equivalence uniqueness are formalized below; nontrivial realized occupancy is the branch-existence boundary.
\end{itemize}
The fixed-cap generalized-entropy stationarity result is Theorem~\ref{thm:equilibrium}. Transportability, the fixed-cutoff bosonic category, and the refinement/fiber ladder are not premises: they are supplied by Theorems~\ref{thm:transport-from-gluing}, \ref{thm:fixed-cutoff-bosonic-sector-category}, and \ref{thm:refinement-functor-fiber-descent}. The genuinely noncentral fixed-cutoff branch is controlled by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport}: \(q_\Sigma=0\) strictifies to the ordinary compact-group transportable case, while \(q_\Sigma\ne0\) remains a higher-gauge sector and is not itself the ordinary compact-group reconstruction theorem. Fermionic signs and chirality belong to the later fermionic/super-Tannakian matter lift.
\end{assumption}
...
This theorem packages the recovered-core nodes D1--D5 and D7--D9 of the dependency DAG above. Node D6 sits outside this recovered-core theorem because it requires the external screen-capacity input \(N_{\mathrm{scr}}\). It is the input-dependent global closure of the Einstein branch recovered in item (iii); D10 and D12 are outside this theorem's scope. Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Axiom~\ref{ax:mar}, Assumption~\ref{ass:technical}, and the hypotheses of Theorems~\ref{thm:confluence}, \ref{thm:bw}, \ref{thm:stress}, \ref{thm:einstein}, \ref{thm:refinement-functor-fiber-descent}, \ref{thm:sector-colimit}, \ref{thm:compactgauge}, \ref{thm:sm}, and \ref{thm:hypercharge}, together with Corollaries~\ref{cor:ng}, \ref{cor:nc}, and Proposition~\ref{prop:z6}. Then, in the controlled refinement/scaling regime singled out by Assumption~\ref{ass:technical}:
...
transportability is a theorem-level zero-obstruction criterion, not a separate premise.
...
On the genuinely noncentral branch, elementary triangle comparisons are not scalars but
...
\begin{corollary}[No separate transportability premise]\label{cor:no-transportability-premise}
...
is no longer a premise: it is the strict-transport corollary of overlap gluing. If the genuinely
...
\begin{corollary}[No fixed-cutoff category premise]\label{cor:no-fixed-cutoff-category-premise}
...
finite bosonic fibers are supplied by Theorem~\ref{thm:refinement-functor-fiber-descent}. Thus the
remaining gauge-side premise burden is only \textbf{T2}, nontrivial realized low-energy occupancy.
\end{corollary}
...
The collar analysis proves a fixed-cutoff statement on the finite type-I regulator net: the reduced cap state has a literal density matrix, its modular Hamiltonian exists, and its nonadditive part is confined to a shrinking collar up to carried errors. The Lorentz claim is therefore not a literal statement about the finite regulator matrices. Under the controlled scaling clause \textbf{T1}, the target object is a realized scaling-limit geometric cap pair
\[
(\mathcal A_\infty^{\mathrm{geo}}(C),\omega_\infty^{\mathrm{geo},C})
\]
for each round cap \(C\subset S^2\). Axiom~\ref{ax:maxent} controls the state-side realized branch across refinement through one common finite-dimensional MaxEnt family, and Theorem~\ref{thm:geosubnet} removes the overlap-trivial auxiliary/record layers from the BW target. The remaining task is therefore not a raw regulator-level matrix limit, but extraction of the geometric cap pair on the local GNS support quotient seen by the transported marginals themselves. The next proposition packages that extraction from the already-proved support-visible modular transport estimate together with local weak-\(*\) compactness and GNS gluing; the following rigidity proposition then shows that, on round caps, the surviving cut data already determine the cap-preserving conformal subgroup up to normalization. The theorem below can therefore be stated with those two propositions as its only UV inputs.
...
\begin{proposition}[Scaling-limit geometric cap-pair extraction on the local GNS support quotient]\label{prop:cap-extraction}
Fix a round cap \(C\subset S^2\). Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Theorem~\ref{thm:geosubnet}, the carried-collar package above, the projectively compatible transported cap-marginal family on \(\mathcal A_n^{\mathrm{geo}}(C_n)\), and the asymptotic transport-equivalence certificate. Assume further that on every fixed local collar model the transported family satisfies the vanishing carried schedule required by Theorem~\ref{thm:regularized-modular-transport}. Then, after passing to a cofinal subnet:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2em]
\item the transported cap marginals admit local weak-\(*\) limits on the support-visible subalgebra of each fixed collar model;
\item those limits are compatible on nested visible-support subalgebras and therefore glue to a state \(\omega_\infty^{\mathrm{geo},C}\) on a scaling-limit geometric cap algebra \(\mathcal A_\infty^{\mathrm{geo}}(C)\);
\item the resulting GNS pair \((\mathcal A_\infty^{\mathrm{geo}}(C),\omega_\infty^{\mathrm{geo},C})\) is faithful on its local support quotient, and the finite-stage regularized support-visible modular actions converge there to the modular automorphism group of the extracted pair.
\end{enumerate}
\end{proposition}

\begin{proof}
For each fixed collar model, boundedness of the transported states gives weak-\(*\) precompactness on the support-visible algebra. Theorem~\ref{thm:regularized-modular-transport} identifies the correct comparison notion on that visible algebra: the carried modular defect converges there without requiring a refinement-uniform lower floor on spectral directions that disappear in the limit support. Because the transported marginals are projectively compatible and the transport-equivalence certificate identifies overlapping visible-support readouts asymptotically, a diagonal extraction over the collar scale and refinement scale yields compatible local weak-\(*\) limits. Standard GNS gluing for compatible local states then produces the extracted pair. Faithfulness holds on the local support quotient by construction, since null directions have been quotiented out before passing to the modular limit. The modular automorphism group of the extracted pair is the unique limit modular action on that quotient-compatible visible algebra. 
\end{proof}

\begin{proposition}[Round-cap rigidity from surviving cut data]\label{prop:round-cap-rigidity}
Let \(C\subset S^2\) be a round cap, and let \((\mathcal A_\infty^{\mathrm{geo}}(C),\omega_\infty^{\mathrm{geo},C})\) be an extracted scaling-limit geometric cap pair furnished by Proposition~\ref{prop:cap-extraction}. Then the surviving geometric cut observables \(Q_a^{(IJ)}\) descending through Theorem~\ref{thm:geosubnet} determine the ordered boundary circle \(\partial C\) together with its two null-oriented endpoint classes. Any one-parameter conformal subgroup preserving that ordered cut data is therefore the standard cap-preserving hyperbolic subgroup \(\lambda_C(s)\), unique up to reparameterization \(s\mapsto \kappa_C s\) with \(\kappa_C>0\).
\end{proposition}

\begin{proof}
Theorem~\ref{thm:geosubnet} shows that the extracted geometric subnet retains precisely the overlap-sector projectors and geometric boundary observables while removing only overlap-trivial auxiliary and record factors. On a round cap, the ordered cut data therefore still determine the boundary circle and the two distinguished null directions at its endpoints. In \(\Conf^+(S^2)\cong \mathrm{PSL}(2,\mathbb C)\), the connected subgroup preserving a round circle together with its ordered endpoint classes is exactly the standard hyperbolic one-parameter subgroup. Any two such parameterizations differ only by positive rescaling of the flow parameter. 
\end{proof}

\begin{theorem}[Geometric modular flow on caps on the extracted prime geometric subnet]\label{thm:bw}
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Theorem~\ref{thm:geosubnet}, the derived fixed-cutoff regulator/collar package established above, the controlled scaling-limit scope clause \textbf{T1} of Assumption~\ref{ass:technical}, Proposition~\ref{prop:cap-extraction}, and Proposition~\ref{prop:round-cap-rigidity}.
Then:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2em]
...
\item For every realized scaling-limit geometric cap pair \((\mathcal A_\infty^{\mathrm{geo}}(C),\omega_\infty^{\mathrm{geo},C})\) furnished by Proposition~\ref{prop:cap-extraction}, the modular automorphism group is geometric:
\[
...
\item No separate cap-isotropy/\(\mathrm{SO}(2)\)-equivariance selector, Euclidean-regularity premise, or extra rigidity assumption is used in this theorem. Once Proposition~\ref{prop:round-cap-rigidity} has fixed the geometric cap subgroup on the extracted prime geometric pair and \(\lambda_C(s)\) is normalized by the null blow-up \(v\mapsto e^{-s}v\), the modular KMS normalization fixes the coefficient \(2\pi\).
...
Step 2: scaling-limit claim tier. Theorem~\ref{thm:bw} is a scaling-limit statement by \textbf{T1}, so the theorem concerns only scaling-limit cap pairs furnished by the controlled refinement/scaling package, not the finite regulator matrices themselves. Nothing in this step assumes that type-I structure survives the limit.

Step 3: geometric identification on the prime subnet. By Proposition~\ref{prop:cap-extraction} and Theorem~\ref{thm:geosubnet}, the realized limit pair is extracted on the overlap-generated prime geometric subnet, with overlap-trivial auxiliary/record factors removed. By Proposition~\ref{prop:round-cap-rigidity}, the cap-preserving conformal action singled out by the surviving ordered cut data is unique up to normalization. Therefore there is a constant \(\kappa_C>0\) such that
\[
\sigma_t^{\omega_\infty^{\mathrm{geo},C}}=\alpha_{\lambda_C(\kappa_C t)}.
\]

Step 4: normalization. Because Proposition~\ref{prop:round-cap-rigidity} has identified the standard cap-preserving conformal subgroup on the extracted geometric pair, no separate cap-isotropy or Euclidean-regularity selector enters. The modular group is KMS at inverse temperature \(1\) by definition. Since \(\lambda_C(s)\) is normalized by unit null dilation under the blow-up \(v\mapsto e^{-s}v\), the Bisognano--Wichmann normalization fixes
\[
\kappa_C=2\pi,
\]
hence
...
Theorem~\ref{thm:geosubnet} removes the old ambiguity about the target algebra: the continuum Lorentz claim is asked only on the extracted prime geometric subnet, not on the full operational algebra. The UV-side bridge is now explicit and internal on that target.

First, Proposition~\ref{prop:cap-extraction} turns the realized transported geometric cap-local system into a theorem-grade extraction statement on the local GNS support quotient. The relevant package is the cap-local test family on \(\mathcal A_n^{\mathrm{geo}}(C_n)\), the projectively compatible transported marginal family, the asymptotic transport-equivalence certificate, and the support-visible modular control of Theorem~\ref{thm:regularized-modular-transport}. The old common-floor collapse still blocks a stronger raw regulator-level statement for unregularized \(-\log\rho_n\) on directions that disappear in the limit support, but it does not obstruct extraction of the scaling-limit cap pair on the quotient actually seen by the limiting state.

Second, Proposition~\ref{prop:round-cap-rigidity} shows that once the pair is extracted on the geometric subnet, no separate rigidity selector remains on round caps: the surviving geometric cut observables already determine the cap-preserving conformal subgroup up to reparameterization. The geometric modular statement is therefore fully internal on \(\mathcal A_\infty^{\mathrm{geo}}(C)\), modulo the declared scaling-limit claim tier \textbf{T1}.
\end{remark}
...
\]
have \(\lambda_{\min}(\rho_n)\to0\). Tensoring this family with any fixed finite exact-Markov collar factor gives a full-rank exact-Markov collar family, but no refinement-uniform lower spectral floor survives. On an off-diagonal matrix unit the modular generator carries a logarithmic gap of order \(n\), so unregularized modular transport can fail even though every finite stage is faithful and Markov. Thus the common-floor clause is a real theorem burden, not a consequence of finite-stage Markovity.
\end{proposition}
...
\begin{remark}[T1 closure status]
The BW/geometric branch is now closed on the theorem target used in this paper: the extracted scaling-limit geometric cap pair on the local GNS support quotient. Proposition~\ref{prop:commonfloor-collapse} still shows that one cannot strengthen this to a uniform raw regulator-level statement about unregularized modular Hamiltonians on spectral directions that collapse out of the support in the limit. The role of \textbf{T1} is therefore only to mark the scaling-limit claim tier, not an additional unresolved BW-side theorem burden.
\end{remark}
...
sector datum rather than a silent change of the old sector.
...
associativity and bosonic spacelike-exchange identities already proved at fixed cutoff.
...
theorems rather than by a refinement premise. Theorem~\ref{thm:transport-from-gluing} supplies
...
nontrivial realization is a separate MAR/nontrivial-branch theorem burden, not a hidden hypothesis
in this reconstruction theorem.
\end{proof}

If a fermionic sign object is present, the correct reconstruction statement is super-Tannakian
rather than purely Tannakian. This paper does not prove that fermionic/chiral extension or the
nontriviality of the realized transportable bosonic ladder; it works only in the bosonic
internal-gauge branch once the fixed-cutoff sector packages and their monoidal refinement transport
...
The UV-side burden on the Lorentz branch is therefore the explicit scaling-limit plus geometric-subnet extraction-and-rigidity package under \textbf{T1}, not the mere existence of a finite cell size.
...
\]
after transport to each fixed local collar model. Local weak-\(*\) extraction and GNS gluing then emit the realized scaling-limit geometric cap pair on the local support quotient. On round caps, the surviving geometric cut data already collapse the residual cap-preserving conformal freedom to the unique BW hyperbolic subgroup up to reparameterization, after which the modular KMS condition fixes the \(2\pi\) normalization. Stronger raw regulator-level statements about unregularized modular Hamiltonians on collapsing spectral directions remain outside that theorem target.
...
The local MaxEnt branch gives a finite-range interacting fixed-cutoff dynamics, Propositions~\ref{prop:gauge} and \ref{prop:ancillauv} fix uniqueness of the UV branch only modulo OPH-stable equivalence, and Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} with Proposition~\ref{prop:hginteracting} close the genuinely noncentral topological branch at fixed cutoff. On the UV side, the BW/geometric lift is now closed on the extracted prime geometric subnet under the declared scaling-limit tier \textbf{T1}: Proposition~\ref{prop:cap-extraction} extracts \((\mathcal A_\infty^{\mathrm{geo}}(C),\omega_\infty^{\mathrm{geo},C})\) as local weak-\(*\) limits of transported cap marginals on \(\mathcal A_n^{\mathrm{geo}}(C_n)\) on the local GNS support quotient, and Proposition~\ref{prop:round-cap-rigidity} collapses the residual cap-preserving conformal freedom on round caps to the unique BW hyperbolic subgroup up to normalization. Proposition~\ref{prop:commonfloor-collapse} still shows that the corpus does not derive a refinement-uniform raw lower spectral floor on the full finite modular-transport marginals, so stronger regulator-level unregularized modular-Hamiltonian statements remain outside the theorem target. On the bosonic gauge lane, the remaining item is \textbf{T2}, nontrivial realized MAR occupancy after Theorems~\ref{thm:transport-from-gluing}, \ref{thm:fixed-cutoff-bosonic-sector-category}, \ref{thm:refinement-functor-fiber-descent}, \ref{thm:sector-colimit}, and \ref{thm:compactgauge}.

\paragraph{Shared excitation dictionary and flavor theorem boundaries.}
The D10 quantitative-closure branch gives integrated gauge-coupling closure on the displayed carrier. That carrier closes its own exact \(W/Z\) chart, while the public electroweak transport family remains on a declared running/matching/threshold/scheme surface until the source spectral measure payload, same-scheme remainder, and interval certificate are supplied. On the declared runtime surface the running/matching packet is a declared-convention contract rather than a theorem that derives every coefficient, threshold, and conversion. The freeze-once coherent repair surface serves only as compare-only validation. On that frozen authoritative repair surface the canonical \(W/Z\) reference pair is hit exactly, but it is a compare-only sidecar below the public promotion gate. The separate D6 cosmological-parameter package is an input-dependent corollary. On the flavor side the manuscript treats one shared OPH excitation dictionary as the only proof-facing family datum; lane-specific integers are not introduced independently. For each realized same-label refinement arrow \(e\) on the three-generation bundle, let \(\omega_e\) be the same-label overlap holonomy scalar, let
\[
d_e:=1-\omega_e
\]
...
\]
Detailed particle-spectrum continuation statements are carried by Ref.~\cite{ophparticles}. That companion paper develops the quark selected-class theorem, the charged-lepton determinant-line boundary, the weighted-cycle neutrino theorem surface, and the hadron execution boundary. The hadron backend tasks \#153/\#157 are source-backend boundaries with empirical closure policy documented. The compact paper promotes no source-only hadron masses. Empirical hadron closure values use a separate \(e^+e^-\to\mathrm{hadrons}\) payload class. These particle-continuation statements do not belong to the compact-paper claim surface.
