...
In the broader OPH paper stack, this paper is the source ledger later imported by the synthesis
wrapper for D2--D10 and D12. The only upstream fixed-cutoff foundation it assumes as already
established is the D1 consensus package: schedule-independent quotient normal form,
gauge-quotient descent, observable-level confluence, and the fixed-cutoff record layer. This
paper then adds the exact premise ledger and dependency map for the Lorentz/gravity, gauge/matter,
capacity, calibration, and continuation branches rather than paraphrasing those branches on a
separate wrapper surface.
...
The D1 row is included here as an imported foundation row so that downstream wrapper surfaces can
reuse one explicit dependency ledger across the OPH suite. Its source theorem package is the
consensus paper; D2--D10 and D12 are the branches added on top of that imported fixed-cutoff base
in this compact derivation paper.
