...
N_c=3,
\]
once an admitted one-generation Standard Model chiral matter package with one Higgs doublet is supplied on the realized branch. On that admitted matter branch, anomaly constraints and Yukawa invariance fix the exact rational hypercharge lattice, and the admissibility window fixes the realized pair \((N_g,N_c)=(3,3)\). These D8--D9 statements are conditional realized-branch consequences, not outputs of bosonic gauge reconstruction alone. The two external continuous inputs of the present implementation are the pixel area
\[
...
\item On an admitted one-generation chiral matter plus one-Higgs package, anomaly constraints and Yukawa invariance fix the hypercharge lattice, and the same admitted realized branch fixes the pair \((N_g,N_c)=(3,3)\) through the CP/UV admissibility window, MAR, and Witten's \(\SU(2)\) anomaly parity constraint.
...
\text{Phase III}=D12.
\]
Phase I is the recovered-core claim set. D8--D9 are recorded below as further conditional realized-branch consequences once the admitted one-generation chiral/Yukawa package is supplied on the realized branch. Phase II contains the separate input-dependent capacity corollary and the current supplement-backed numerical implementation. Phase III collects deferred continuations. The canonical five-axiom basis, the two external continuous inputs, and the technical-premise labels used below are fixed in Section~\ref{sec:premise-ledger}. Each row names a node in the reconstruction program, records its immediate OPH-side parents, separates imported standard mathematics from extra non-axiom physical premises or external inputs, and states the resulting claim tier.
...
\textbf{D8} & Product gauge structure up to finite quotient on the admitted chiral matter branch (Lemmas~\ref{lem:mincontent}--\ref{lem:commutant}, Theorem~\ref{thm:sm}) & D7 + Axiom~\ref{ax:mar} & compact Lie representation classification; Schur's lemma & connected positive-dimensional Lie admissible class; one connected abelian factor; faithful action on the minimal coupled carrier; vanishing obstruction \([z]=0\); admitted chiral/Yukawa-capable matter data & Phase I conditional realized-branch theorem \\
\textbf{D9} & Realized Standard Model quotient, hypercharges, the realized pair \((N_g,N_c)=(3,3)\), and product-group corollaries on that admitted matter branch (Theorem~\ref{thm:hypercharge}, Corollaries~\ref{cor:ng}, \ref{cor:nc}, Proposition~\ref{prop:z6}) & D8 & anomaly-cancellation algebra; Witten global-anomaly argument; CKM CP counting & realized one-generation chiral matter plus one Higgs package; asymptotic-freedom and MAR admissibility premises & Phase I conditional realized-branch theorem/corollary chain \\
...
\textbf{D8} & Lemmas~\ref{lem:mincontent}--\ref{lem:commutant}, Theorem~\ref{thm:sm}: product gauge structure up to finite quotient on the admitted chiral matter branch & D7 + Axiom~\ref{ax:mar} & compact Lie representation classification; Schur's lemma & connected positive-dimensional Lie admissible class; one connected abelian factor; faithful action on the minimal coupled carrier; vanishing obstruction \( [z]=0 \); admitted chiral/Yukawa-capable matter data & conditional realized-branch theorem \\
\textbf{D9} & Theorem~\ref{thm:hypercharge}, Corollaries~\ref{cor:ng}, \ref{cor:nc}, Proposition~\ref{prop:z6}: realized Standard Model quotient, hypercharges, the realized pair \((N_g,N_c)=(3,3)\), and product-group corollaries on that admitted matter branch & D8 & anomaly-cancellation algebra; Witten global-anomaly argument; CKM CP counting & realized one-generation chiral matter plus one Higgs package; asymptotic-freedom and MAR admissibility premises & conditional realized-branch theorem/corollary chain \\
...
\end{enumerate}
After the hypercharge lattice, generation-count, color-count, and trivial-action quotient steps on the admitted one-generation matter package, the finite quotient is fixed to \(\Gamma=\mathbb Z_6\), so the realized gauge structure on that admitted MAR-admissible branch is
\[
\frac{\SU(3)\times\SU(2)\times\U(1)}{\mathbb Z_6},
\qquad
N_g=3,
...
N_c=3,
\]
with the exact Standard Model hypercharge lattice on that realized matter package. These D8--D9 statements are conditional on the admitted chiral/Yukawa package rather than on bosonic gauge reconstruction alone. Here the realized-branch fixing is sequential: the asymptotic-freedom/CP-capability window and minimal admissibility first yield \(N_g=3\), and then Witten's \(\SU(2)\) anomaly parity constraint together with the same realized-branch selector yield \(N_c=3\).
\end{theorem}
...
Theorem~\ref{thm:master} therefore packages only the recovered relativity-plus-bosonic-gauge core: D1--D5 together with D7. The further D8--D9 Standard Model quotient, hypercharge, and counting statements are conditional realized-branch consequences once the admitted one-generation chiral/Yukawa package is supplied. The D6 capacity relation is a separate input-dependent corollary; the quantitative calibration sector, Higgs/top branch, and phenomenological continuations remain outside its scope.
...
The quantitative program uses the same axiom set together with the two external continuous inputs $P$ and $N_{\mathrm{scr}}$ in its current supplement-backed numerical implementation. In the current implementation, $P$ is calibrated from the gauge sector and $N_{\mathrm{scr}}$ is inferred from the observed cosmological constant. The realized structural discrete data are
\[
...
\item \textbf{Conditional realized-branch structural outputs.} These include the gauge quotient, the exact hypercharge lattice on the admitted realized matter package, and the realized pair $(N_g,N_c)=(3,3)$ on that MAR-admissible branch.
...
These are calibration-sector outputs. Their role in the paper is scale setting, not independent confirmation. A fully reproducible numerical audit requires the beta-function shifts and matching conventions supplied in the repository supplements. The compact note therefore reports the running-$Z$ relation rather than a pole-$Z$ benchmark output.
...
\item string/worldsheet claims need a controlled large-$N_{\mathrm{edge}}$ regime distinct from the physical color rank $N_c=3$; the further lift to critical superstring structure additionally requires worldsheet supersymmetry, critical dimension, modular invariance, and GSO projection, none of which are derived here.
\end{enumerate}
...
\bottomrule
\endfoot
Phase I recovered core (Theorem~\ref{thm:master}; D1--D5, D7, plus conditional D8--D9) & Confluence of overlap repair, conditional Lorentz kinematics, the conditional Jacobson-type Einstein branch in the stated scaling regime, compact gauge reconstruction in the bosonic branch, and the further realized Standard Model quotient chain, exact hypercharge lattice, realized pair \((N_g,N_c)=(3,3)\), and no-gauge-mediated-proton-decay corollary only once the admitted one-generation chiral/Yukawa package is supplied on the realized branch & A mathematical failure in the derivation chain, or data that require a different realized gauge quotient, different realized hypercharges, a different color or generation count on the admitted branch, or gauge-mediated proton decay \\
...
Mirrored interpretive surfaces may discuss a separate OPH-internal state-and-law habitat theorem for the strange-loop wrapper. That result, when stated carefully, still sits outside Phase I and does not change this falsifiability ledger: the missing ingredients remain the actual OPH closure map, an invariant admissible observer-supporting sector for that map, and any uniqueness or stability estimate.
...
N_c=3,
\]
with the exact one-generation hypercharge lattice on the admitted matter package. Here the realized counting chain is sequential: the CP/UV admissibility window and minimal admissibility yield \(N_g=3\), and Witten's \(\SU(2)\) anomaly parity with that realized generation count yields \(N_c=3\).
\[
Y_Q=\frac16,\qquad
Y_L=-\frac12,\qquad
Y_u=-\frac23,\qquad
...
\item the Standard Model gauge quotient chain on the admitted realized matter branch;
\item exact hypercharges on the admitted one-generation matter package with one Higgs doublet;
\item the realized pair \((N_g,N_c)=(3,3)\) on that admitted MAR-admissible branch;
