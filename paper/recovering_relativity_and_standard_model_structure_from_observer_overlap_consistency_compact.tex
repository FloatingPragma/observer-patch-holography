...
The framework is presented as a reconstruction program in which general relativity and the Standard Model appear as effective sectors derived from a smaller axiom set together with an explicit refinement/scaling-limit package.
...
compact-gauge branches is separate.
...
Whenever compact gauge reconstruction is invoked on the ordinary or central-defect branch, the following additional premises are kept explicit. Together they are the compact-paper form of \textbf{T1} and \textbf{T3}--\textbf{T5}.
...
\item a directed colimit \(\mathsf{Sect}_\infty=\varinjlim_r \mathsf{Sect}_r\) of transportable edge-sector categories, built from refinement functors that preserve transportable sector classes on cofinal tails, with objectwise finite-dimensional fibers;
...
Gauge group & model input & conditional compact group reconstruction from edge sectors in the bosonic branch; exact SM quotient selected on the realized MAR-admissible branch with the realized one-generation/one-Higgs package \\
