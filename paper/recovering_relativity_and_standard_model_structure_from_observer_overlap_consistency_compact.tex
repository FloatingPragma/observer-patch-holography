...
\begin{abstract}
We present a falsifiable compact formulation of Observer-Patch Holography (OPH) as a conditional reconstruction program. The framework starts from five axioms, two external continuous configuration inputs in the current quantitative implementation, and an explicit scaling-limit/categorical regularity package. From that data it derives a schedule-independent normal form for overlap consistency, a conditional classification of the geometric-modular Lorentz branch on the screen, a conditional Jacobson-type Einstein branch from fixed-cap generalized-entropy stationarity together with the null modular bridge and the scaling-limit null-stress premise, and conditional compact gauge reconstruction in the bosonic internal-gauge branch from a refinement-stable symmetric edge-sector colimit. After the extended MAR selector is imposed on the connected positive-dimensional Lie admissible class with one connected abelian charge factor, the realized low-energy gauge group on that MAR-admissible branch is
\[
...
\]
In the current implementation, $P$ is calibrated from the gauge sector and $N_{\mathrm{scr}}$ is inferred from the observed cosmological constant. Gauge couplings and tree-level electroweak masses are supplement-backed calibration-sector consistency checks, while the critical-surface Higgs/top branch introduces no additional continuous fit once the gauge trajectory and scale-setting branch are fixed. The charged-lepton, Koide, quark-texture, baryogenesis, dark-sector, string, and black-hole-spectroscopy discussions are retained only as phenomenological continuations or conjectural branches, not as theorem-level outputs of overlap consistency alone. The aim of this compact note is therefore not to claim a complete derivation of all Standard Model and cosmological data from observer-overlap consistency by itself, but to state clearly which reconstruction results are structural, which are scaling-limit conditional, which are supplement-backed numerical branches, and which remain phenomenological.
\end{abstract}
...
This section is the master status ledger for the compact note. The canonical five-axiom basis, the two external continuous inputs, and the technical-premise labels used below are fixed in Section~\ref{sec:premise-ledger}. Each row names a node in the reconstruction program, records its immediate OPH-side parents, separates imported standard mathematics from extra non-axiom physical premises or external inputs, and states the resulting claim tier. A row should not be read as a five-axiom consequence unless the extra-load columns are empty; in practice almost every nontrivial branch below uses stated technical premises or realized-branch assumptions in addition to the canonical five-axiom package.

\begingroup
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.05\textwidth}>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.13\textwidth}@{}}
\caption{Master dependency checklist for the compact OPH program.}\\
\toprule
...
\bottomrule
\endfoot
\textbf{D1} & Unique schedule-independent normal form (Theorem~\ref{thm:confluence}) & overlap-consistency problem on a finite patch net & Newman's lemma for terminating locally confluent rewrite systems & finite state spaces, termination, and local confluence & structural theorem \\
...
\textbf{D3} & Lorentz branch \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) (Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; conformal-group classification & T4, T5, and T8 of Assumption~\ref{ass:technical} & conditional scaling-limit theorem \\
\textbf{D4} & Null modular bridge to \(T_{kk}\) (Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Lemmas~\ref{lem:density}, \ref{lem:translation}; Theorem~\ref{thm:stress}) & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock half-sided modular inclusion; distributional differentiation on half-lines & T1--T3, T6, T8, and the relativistic null-stress identification premise & conditional bridge theorem \\
\textbf{D5} & Jacobson-type Einstein branch (Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}) & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Jacobson small-ball first law; null-to-tensor reconstruction modulo a metric term & T9 fixed-cap stationarity; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions & conditional scaling-limit theorem/corollary \\
\textbf{D6} & Capacity branch for \(\Lambda\) and the associated neutrino estimate (Proposition~\ref{prop:lambda}, Corollary~\ref{cor:lambda}) & D5 leaves a \(+\Lambda g_{ab}\) ambiguity & de Sitter entropy relation and dimensional analysis & external input \(N_{\mathrm{scr}}\) and the screen-capacity identification & input-dependent corollary / estimate \\
\textbf{D7} & Compact gauge reconstruction (Theorem~\ref{thm:compactgauge}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Doplicher--Roberts / Tannaka reconstruction & T7 and T10--T12; rigid symmetric bosonic refinement-stable sector colimit with fiber functor & conditional structural theorem \\
\textbf{D8} & Product gauge structure up to finite quotient (Lemmas~\ref{lem:mincontent}--\ref{lem:commutant}, Theorem~\ref{thm:sm}) & D7 + Axiom~\ref{ax:mar} & compact Lie representation classification; Schur's lemma & connected positive-dimensional Lie admissible class; one connected abelian factor; faithful action on the minimal coupled carrier; vanishing obstruction \( [z]=0 \) & realized-branch theorem \\
...
\textbf{D10} & Gauge/electroweak numerical closure of the present implementation & D9 + external input \(P\) & RG evolution and matching conventions supplied in the supplement & pixel constraint; edge heat-kernel closure; supplement-backed beta-function and threshold conventions & calibration sector \\
\textbf{D11} & Higgs/top critical-surface branch & D10 & RG transport and pole-mass conversion supplied in the supplement & critical-surface condition \(\lambda=\beta_\lambda=0\) at \(M_U\); no new continuous fit once D10 is fixed & supplement-backed quantitative branch \\
\textbf{D12} & Charged-lepton/Koide, texture, dark-sector, baryogenesis, black-hole, and string continuations & various subsets of D6, D9, D10, and D11 & branch-specific EFT and phenomenological manipulations & additional ans\"atze such as the uniform \(\mathbb Z_6\) center-label ensemble, discrete texture choices, dark-sector response assumptions, or discrete-horizon assumptions & phenomenological continuation / program-level branch \\
\end{longtable}
\endgroup

Nodes D1--D9 are the structural reconstruction program. Nodes D10--D11 are the current supplement-backed numerical implementation. Node D12 collects downstream continuations that are intentionally not advertised as theorem-level consequences of the five axioms alone. The only external continuous inputs are \(N_{\mathrm{scr}}\) in D6 and \(P\) in D10, together with descendants built from those branches.
...
\textbf{D4} & Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Lemmas~\ref{lem:density}, \ref{lem:translation}; Theorem~\ref{thm:stress}: null modular bridge to \(T_{kk}\) & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock half-sided modular inclusion; distributional differentiation on half-lines & T1--T3, T6, T8, and the relativistic null-stress identification premise & conditional bridge theorem \\
\textbf{D5} & Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}: Jacobson-type Einstein branch & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Jacobson small-ball first law; Lemma~\ref{lem:nullreconstruct} for the null-to-tensor upgrade modulo a metric term & T9 fixed-cap stationarity; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions & conditional scaling-limit theorem/corollary \\
...
\begin{theorem}[Summary theorem for the conditional reconstruction package]\label{thm:master}
This theorem packages nodes D1--D9 of the authoritative dependency DAG above; it does not erase any of the extra premises already listed there. Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Axiom~\ref{ax:mar}, Assumption~\ref{ass:technical}, and the hypotheses of Theorems~\ref{thm:confluence}, \ref{thm:bw}, \ref{thm:stress}, \ref{thm:einstein}, \ref{thm:compactgauge}, \ref{thm:sm}, and \ref{thm:hypercharge}, together with Corollaries~\ref{cor:ng}, \ref{cor:nc}, and Proposition~\ref{prop:z6}. Then, in the controlled refinement/scaling regime singled out by Assumption~\ref{ass:technical}:
...
\]
\item admissible fixed-cap stationarity together with the null modular bridge and null-stress premise yield a Jacobson-type rest-frame relation, whose tensor upgrade gives
\[
...
Theorem~\ref{thm:master} therefore packages only the structural and conditional branches D1--D9. The quantitative calibration sector, Higgs/top branch, and phenomenological continuations remain outside its scope.
...
Assume a uniform sixfold center-label ensemble on the realized $\mathbb Z_6$ quotient sectors. Then the associated center-label entropy is
\[
...
\]
which is used as a phenomenological flavor-hierarchy ansatz in the remainder of this section. The quotient alone does not force this center-label entropy without the ensemble assumption.
...
Within this continuation, we adopt the phase ansatz motivated by the previously fixed discrete data:
\[
...
\frac29.
\]
This is a rigid continuation ansatz rather than a theorem of the core OPH axioms. It ties the Koide phase to color count, generation count, and the hypercharge lattice inside the flavor continuation.
...
\]
This is within order-one agreement with the empirical MOND/RAR acceleration scale. If no additional infrared scales or residual dimensionless response functions survive in the deep-IR law, dimensional analysis then yields
\[
...
Z_{\mathrm{edge}}(t)=\sum_R d_R^2 e^{-tC_2(R)}.
\]
This is the same structure that appears in two-dimensional Yang-Mills theory. If a large-$N_{\mathrm{edge}}$ regime exists, with $N_{\mathrm{edge}}$ denoting an effective edge-sector rank rather than the physical color rank $N_c=3$, the corresponding free energy admits the Gross-Taylor genus expansion
\[
\log Z = \sum_{h\ge 0} N_{\mathrm{edge}}^{2-2h} F_h(\lambda).
\]
In OPH this is interpreted as a conditional effective worldsheet reorganization of the edge dynamics. String theory therefore enters as an emergent sector description rather than a theorem-level microscopic starting point.
...
\bottomrule
\endfoot
Structural theorems & Confluence of overlap repair, conditional Lorentz kinematics, the conditional scaling-limit Einstein relation/tensor upgrade, conditional bosonic compact gauge reconstruction, Standard Model quotient chain, hypercharge lattice, $N_c=3$, $N_g=3$ on the realized branch & A mathematical failure in the derivation chain, or data that require a different gauge quotient, different hypercharges, a different color count, or gauge-mediated proton decay \\
Calibration sector & $\alpha_i(m_Z)$, $\alpha_{\mathrm{em}}^{-1}(m_Z)$, $\sin^2\theta_W$, $v$, $m_W$ & Inability to close the pixel fixed point or systematic disagreement once the one-input calibration is imposed \\
...
Phenomenological continuations & Charged-lepton masses, Koide relation, quark textures, neutrino estimate & Failure of the Koide/holonomy continuation, wrong texture exponents, or branch retraction once a sharper benchmark formula is supplied \\
...
If the uniform center-label ensemble ansatz of Section~\ref{subsec:z6-suppression} is adopted and the realized gauge group is
\[
\frac{\SU(3)\times\SU(2)\times\U(1)}{\mathbb Z_6},
\]
let $Z$ denote the discrete center/quotient label in the edge-center completion. Then the predicted central-label entropy is
\[
\Delta S_{\mathrm{center}}=\ln 6
\]
or
...
\]
This is a global-structure observable conditioned on the ensemble ansatz. It is not the Shannon entropy of the open-edge irrep weights $p_R(t)$ and is not $t$-dependent. A clean measurement of central-label entropies inconsistent with that value would falsify that continuation.
...
\item the two-input continuous economy of the present quantitative implementation.
\end{enumerate}
