...
genuinely noncentral branches. The current boundary sits where it should: in the continuum
...
\textbf{D5} & Jacobson-type Einstein branch: rest-frame relation plus conditional tensor upgrade (Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}) & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Jacobson small-ball first law; null-to-tensor reconstruction modulo a metric term & T3 fixed-cap stationarity; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions; D4 carried remainder negligible at order \(\ell^4\); all local directions/reference states for the tensor upgrade & Phase I conditional scaling-limit theorem/corollary \\
...
\textbf{D7} & Compact gauge reconstruction (Theorem~\ref{thm:compactgauge}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Doplicher--Roberts / Tannaka reconstruction & T1 and T4--T5 together with T6's directed-colimit/fiber premise for transportable edge sectors; the local-Gibbs/Dobrushin-type collar package supplies only fixed-cutoff support and recoverability control and does not by itself prove existence, persistence, triviality, or nontriviality of the refinement-limit sector colimit & Phase I conditional structural theorem \\
...
This SM/GR derivation paper uses one canonical numbered list of external technical premises. The fixed-cutoff realized presentation, the local-Gibbs form, quasi-local Lieb--Robinson propagation, endpoint-Lipschitz interval control, the induced finite-dimensional refinement branch, and the operational/geometric split of Theorem~\ref{thm:geosubnet} are not listed here because they are established internally from Axioms~\ref{ax:screen}--\ref{ax:maxent}. The six external items are:
\begin{itemize}[leftmargin=2.3em]
\item[\textbf{T1}] vanishing of the relevant gluing obstruction class whenever global sector transportability is invoked: \([z]=0\) on the central branch, or its higher-gauge analogue \(q_\Sigma=0\) on the genuinely noncentral branch;
...
\item[\textbf{T6}] whenever compact gauge reconstruction is invoked, the sector category means the directed colimit of transportable edge sectors with objectwise finite-dimensional fibers; the existence of that directed system, the transportability maps, the fiber functor, and the possible triviality or nontriviality of the colimit are all part of the categorical premise rather than consequences of Axiom~\ref{ax:maxent} or of any fixed-cutoff local-Gibbs/Dobrushin-type collar estimate.
\end{itemize}
\end{assumption}
...
\textbf{D3} & Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}: geometric modular flow and Lorentz branch & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry;\newline conformal classification on \(S^2\) & T2 together with the theorem-local geometric cap-pair extraction and ordered cut-pair rigidity hypotheses of Theorem~\ref{thm:bw}; Theorem~\ref{thm:geosubnet} fixes the target subnet internally & conditional scaling-limit theorem \\
...
\]
\item under the explicit transportable sector-colimit premise of \textbf{T6}, the edge-sector category reconstructs a compact gauge group, and the realized connected gauge structure has the form
\[
...
\end{enumerate}
So the gap closed here is the generator/charge identification itself: inside the null bridge and on the OPH half-line family, \(P_\Omega\) is exactly the local null-stress charge.
...
P_\nu=6.699825740519345,
\]
and the absolute-attachment theorem emits
\[
B_\nu=P_\nu C_\nu=6.696004159297337.
\]
...
\]
with \(\Delta m_{21}^2=7.488059465106851\times10^{-5}\,\mathrm{eV}^2\), \(\Delta m_{31}^2=2.5123118727618473\times10^{-3}\,\mathrm{eV}^2\), and \(\Delta m_{32}^2=2.4374312781107786\times10^{-3}\,\mathrm{eV}^2\). The exact two-parameter positive-segment adapter remains on disk only as a diagnostic sidecar beneath the theorem pair \((C_\nu,B_\nu)\). The exact non-hadron output lane is complete, on a mixed theorem/compare-only/same-family surface rather than one uniform theorem tier.
...
Let
\[
P:=p_a+\text{(Axioms 1--5)}+\text{Assumption 6}
...
\]
\[
\widehat{\Delta m_{21}^2}=2.520259180367675\times 10^{-5}\,\mathrm{eV}^2,\qquad
\widehat{\Delta m_{31}^2}=8.455698156217034\times 10^{-4}\,\mathrm{eV}^2,\qquad
...
\]
\[
\Delta m_{21}^2=7.488059465106851\times10^{-5}\,\mathrm{eV}^2,\qquad
\Delta m_{31}^2=2.5123118727618473\times10^{-3}\,\mathrm{eV}^2,\qquad
...
The framework therefore presents general relativity and the Standard Model as effective descriptions arising once the stated scaling-limit, geometric-subnet, and categorical premises are realized. Open items are sharper quantitative control, derivation or removal of the transportability-side premises on the refinement-limit compact-gauge branch, a fuller fermionic gauge branch, and explicit microscopic realizations that emit the prime geometric cap pair and support ordered cut-pair rigidity.
