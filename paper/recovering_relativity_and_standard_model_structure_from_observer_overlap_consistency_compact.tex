...
\begin{abstract}
Observer-Patch Holography starts from a finite screen and one requirement: neighboring observer patches must agree on their overlap. From that simple rule, the recovered core yields a Lorentzian spacetime sector, a Jacobson-style route to Einstein's equation, and a bosonic compact-gauge reconstruction branch. On the realized MAR-admissible one-Higgs matter branch, the same stack yields the realized Standard Model quotient \(\left(\SU(3)\times\SU(2)\times\U(1)\right)/\mathbb Z_6\) with the exact hypercharge lattice, the realized color triplet \(N_c=3\), and the generation count \(N_g=3\). Fermions and chirality enter through the explicit realized matter-sector premise; this compact paper does not promote them to a super-Tannakian reconstruction theorem. The basis used here is quantum-algebraic: patch algebras, states, trace/Born probabilities where measurement records are invoked, and generalized entropy are part of the starting formalism. OPH's reconstruction claim is not that every mathematical ingredient must be derived from a blank starting point. It is that this algebraic-information basis can support a consistent and comprehensive theory-of-everything program by recovering the observed effective universe from overlap consistency. The broader framework then introduces one local pixel ratio \(P\) by describing the same screen cell from two sides: as an exterior screen cell displaced from perfect self-similar equilibrium and as the electromagnetic observation scale available inside the encoded world. That single scale feeds the downstream particle papers. This compact paper isolates the recovered core and keeps it distinct from the quantitative particle and continuation branches built on top of the same \(P\).
\end{abstract}
...
consistency conditions. Under the five axioms and the additional branch premises stated below,
...
The paper separates exact structural theorems, conditional scaling-limit branches, quantitative outputs, and phenomenological continuations. Several longer arguments are written in compressed proof form so the claim boundary stays readable.

The framework is presented as a reconstruction program in which general relativity and the Standard Model appear as effective sectors derived from the stated quantum-algebraic axiom set together with explicit refinement and scaling-limit premises. This theory-of-everything framing is a claim about recovering the observed effective universe from that basis and its declared branch premises, not a demand that every mathematical ingredient be derived from a blank starting point.
...
\item A conditional Jacobson-type Einstein branch is recovered from observer-overlap modular geometry, the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, the geometric-branch selection burden, and an explicit null-modular scaling branch; in the null modular bridge, quasi-local propagation, exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line control, the resulting weak tail generator, and the half-sided modular pair are supplied internally by the null-interval structure and the scaling-limit geometric action, after which Borchers--Wiesbrock yields the explicit positive null-translation generator on its Stone domain together with the affine half-line modular relation and the later half-line generator/charge identification; the D5 side then adds the internal small-ball bridge from the geometric cap generator together with bounded-interval transport through the separate projective branch, and the later tensor upgrade requires the explicit all-directions/all-reference-states premise on that scaling branch.
...
\item Dark-sector, baryogenesis, string, and spectroscopy branches are kept visible only as phenomenological continuations or, where additional ans\"atze are involved, conjectural phenomenology.
\item Overlap repair admits a unique schedule-independent normal form on the stated finite patch-net hypotheses and induces refinement-limit normal-form and holonomy classes on separated cofinal refinement systems with compatible projections.
\item The fixed-cutoff topological UV package closes on the ordinary branch, the central-defect branch, and the genuinely noncentral branch through a compact crossed-module higher-gauge collar theorem; the UV-side open burdens are the continuum BW/geometric lift and the refinement-limit compact-gauge premises.
\end{enumerate}
...
The imported D1 package is the fixed-cutoff consensus theorem stack: asynchronous confluence
...
and the record-algebra theorem on the observer-accessible surface. The imported D1
burdens are repair completeness and the stated Petz support/CPTP clause where that branch is used.
...
normal forms and holonomy maps.

\section{Overview of Results}

This section summarizes the claim tiers used throughout. The labels \(D1\)--\(D12\) are internal
documentary node labels for the project dependency ledger, not bibliography references or external
notation. The public paper ledger has no separate D11 row; companion papers should use
only the labels defined in the ledger below. The table below is the local key for this
SM/GR derivation paper, and the companion papers reuse the same labels when they point back to this ledger. The
...
\text{Phase III}=D12.
\]
Phase I is the recovered-core claim set. Phase II contains the input-dependent global closure D6 of the D5 Einstein branch together with the integrated D10 quantitative-closure branch. Phase III collects phenomenological continuations. The canonical five-axiom basis, the quantitative quantities listed next, the scaling/continuum premises, and the gauge-side premises used below are fixed in Section~\ref{sec:premise-ledger}. Each row names a node in the reconstruction program, records its immediate internal parents, separates imported standard mathematics from declared non-axiom premises or external inputs, and states the resulting claim tier.

\paragraph{Premise key for the table.}
The labeled premises used in the dependency table are introduced here before the ledger appears later in the paper. \textbf{T2} is the controlled refinement/scaling-limit scope clause for the Lorentz/null-modular/Einstein branch. The fixed-cap generalized-entropy stationarity result is Theorem~\ref{thm:equilibrium}, not a labeled premise. The bosonic gauge-reconstruction premises corresponding to \textbf{T3}--\textbf{T5} are stated separately in Assumption~\ref{ass:gaugeprem} in Section~\ref{sec:premise-ledger}; on the central-defect branch, the loop-coherence criterion \textbf{T1} is carried by the same path-independent transportability hypothesis rather than listed as a second independent gauge premise.
...
\toprule
Node & Output & Immediate internal ingredients & Standard mathematics used & Declared non-axiom premises / external inputs & Claim tier \\
\midrule
\endhead
\bottomrule
\endfoot
\textbf{D1} & Unique schedule-independent normal form on the physical quotient, observable-level confluence on the declared fixed-cutoff physical algebras for the recovery-derived repair relation, and inverse-limit normal-form / holonomy classes on separated cofinal refinement systems & overlap-consistency problem on a finite patch net together with the declared fixed-cutoff collar recovery step, its touched-overlap local-fit acceptance contract, support-local disjoint commutation, restriction-compatible union-collar gluing on the physical quotient, the fixed-point / quotient-local physical observable algebras carried by that same collar package, and cofinal refinement projections compatible with finite-stage normal forms and holonomy maps & local-to-global Lyapunov descent on the inconsistency potential; local-diamond completion from overlap-associative center-sector / higher-gauge union-collar gluing; Newman's lemma; inverse limits of separated finite quotient systems & repair completeness; stated Petz support/CPTP control where that branch is used; normal-form naturality, holonomy naturality, and visible separation for the refinement system & Phase I structural theorem \\
...
\textbf{D3} & Lorentz branch \(\Conf^+(S^2)\cong \mathrm{SO}^+(3,1)\) (Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry; conformal-group classification & T2 together with the theorem-local realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum, and ordered cut-pair rigidity on the emitted scaling-limit geometric cap pair; Theorem~\ref{thm:geosubnet} fixes the target subnet internally; the scaling-limit observer algebra may leave the finite type-I regulator class, so the theorem is automorphism-level there and \(K_C=2\pi B_C\) is only the special type-I limit form & Phase I conditional scaling-limit theorem \\
\textbf{D4} & Null modular bridge to \(T_{kk}\), including exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line families, the weak tail generator, the derived half-sided modular inclusion, and the explicit positive half-line null-translation generator on its Stone domain with affine half-line modular relations (Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Theorem~\ref{lem:density}, Lemma~\ref{lem:translation}; Theorem~\ref{thm:stress}) & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock positive-generator theorem for standard half-sided inclusions; Stone's theorem; distributional differentiation on half-lines & the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed for the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses of Proposition~\ref{prop:nullec} and Corollary~\ref{cor:n1}, plus T2; quasi-local propagation and endpoint-Lipschitz control are supplied internally by Axiom~\ref{ax:maxent} on the local finite-constraint branch, the geometric scaling action on the null half-line blow-up net derives the half-sided modular pair, the same-half-line local modular-Hamiltonian formula is imported only for the generator/charge identification, and bounded-interval formulas require the separate interval-preserving projective branch & Phase I conditional bridge theorem \\
\textbf{D5} & Jacobson-type Einstein branch: rest-frame relation plus conditional tensor upgrade (Theorem~\ref{thm:equilibrium}, Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}) & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & fixed-volume area-variation identity for small geodesic balls; local quadratic-polarization argument for the tensor upgrade & Theorem~\ref{thm:equilibrium}, which fixes the admissible fixed-cap MaxEnt variation class and derives generalized-entropy stationarity on the realized cap-label-preserving MaxEnt family; the internal small-ball bridge of Lemma~\ref{lem:smallball}, which uses the geometric cap generator together with the D4 half-line generator/charge identification and the separate interval-preserving projective branch; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions; D4 carried remainder negligible at order \(\ell^4\); all local directions/reference states for the tensor upgrade & Phase I conditional scaling-limit theorem/corollary \\
...
\textbf{D7} & Construction of the refinement-stable bosonic sector category plus compact gauge reconstruction (Theorem~\ref{thm:sector-colimit}, Theorem~\ref{thm:compactgauge}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & directed-colimit descent for monoidal \(C^*\)-categories; Doplicher--Roberts / Tannaka reconstruction & Assumption~\ref{ass:gaugeprem} on the ordinary or central-defect bosonic branch; the genuinely noncentral fixed-cutoff branch is handled separately by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} and is not itself the ordinary compact-group reconstruction theorem & Phase I conditional structural theorem \\
...
The recovered core of this SM/GR derivation paper is D1--D5 together with D7--D9. Within that package, D7 is the bosonic sector-category construction plus compact-gauge reconstruction stage, while D8--D9 are realized-branch results that keep the realized one-generation chiral matter plus one-Higgs package and MAR admissibility premises explicit. D6 is not part of that recovered core because it requires the external screen-capacity input \(N_{\mathrm{scr}}\), but it is the input-dependent local/global theorem stack for the same Einstein branch: D5 leaves the metric term undetermined, D6 closes that same branch once the external screen-capacity input \(N_{\mathrm{scr}}\) and the screen-capacity identification are declared, and the same D6 stack emits the de Sitter static-patch parameter package. D10 is the integrated quantitative-closure branch, and D12 collects phenomenological continuations. The modular-anomaly dark-sector branch belongs to D12: it imports the D6 static-patch scale only as an IR benchmark and does not derive MOND/RAR-style response laws, a controlled galaxy-scale source/response law, or galaxy/cluster phenomenology. The only external continuous input is \(N_{\mathrm{scr}}\) in D6; the local pixel ratio \(P\) belongs to the separate Phase-II closure branch together with descendants built from it.
\section{Five Axioms, the Phase-II Pixel Variable, the External Capacity Input, and Technical Premises}\label{sec:premise-ledger}

This section states the premise ledger and dependency map for this SM/GR derivation paper. The basis used below consists of the five core axioms, the quantitative quantities listed next, the scaling/continuum premises collected afterward, and the gauge-side premises of Assumption~\ref{ass:gaugeprem}. This is an algebraic-information basis: the screen-net, state, trace/probability, and generalized-entropy structures are starting ingredients of the current OPH framework. The purpose is not to derive every mathematical ingredient from a blank slate, but to test whether this basis supports a consistent and comprehensive theory-of-everything reconstruction of the observed effective universe. On the fixed-cutoff local finite-constraint MaxEnt branch, the local-Gibbs form, quasi-local dynamics, Lieb--Robinson propagation control, and endpoint-Lipschitz interval control are internal consequences rather than theorem-external regularity selectors. Any Dobrushin/local-Gibbs/mixing language used below names only a sufficient fixed-cutoff collar-recoverability mechanism on that branch. It is not an infinite-volume uniqueness claim for the refinement-limit theory, and it does not by itself constrain whether a nontrivial transportable bosonic refinement ladder exists. The paper therefore treats that ladder as explicit gauge-branch data rather than as an entropy-side consequence of the MaxEnt/mixing package. What Axiom~\ref{ax:maxent} fixes under refinement is the realized state-side MaxEnt branch itself. Theorem~\ref{thm:geosubnet} fixes the operational/geometric split at fixed cutoff, and Theorem~\ref{thm:bw} fixes the cap generator and its \(2\pi\) normalization once its theorem-local geometric cap-pair extraction and ordered cut-pair rigidity hypotheses are supplied on that geometric subnet. On the D3 UV side, the theorem-local scaffold is the realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum, together with ordered cut-pair rigidity on the emitted scaling-limit geometric cap pair. Corollary~\ref{cor:n2} then derives the null half-sided modular pair after the null blow-up step, and Lemma~\ref{lem:translation} promotes that pair to the explicit positive null-translation generator on its Stone domain with the affine half-line modular relation. Theorem~\ref{thm:equilibrium} then internalizes the fixed-cap generalized-entropy stationarity step for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family. The canonical theorem-external items are therefore the scaling-limit scope clause, the bounded-interval transport/projective branch where invoked, and the transportable bosonic ordinary/central gauge premises recorded in Assumption~\ref{ass:gaugeprem}. On the central branch, that assumption names the same path-independent transportability whose gluing-side form is the central loop-coherence obstruction, so that obstruction is not listed as a second free theorem-side input. The genuinely noncentral fixed-cutoff branch is tracked instead by Theorems~\ref{thm:hgdatum} and \ref{thm:hgtransport}; it is not itself the ordinary compact-group theorem used by the compact-gauge branch.
...
quantity carried by the forward D10 particle-physics map. That closure writes the outside
detuning as
\[
...
declared quantitative branch. The code surface includes a numerical fixed-point witness
for this map and accepts external inverse-\(\alpha\) values only when they are supplied explicitly
as compare-only metadata; the paper comparison gives
\(P=1.630968209403959\ldots\) from \(\alpha^{-1}=137.035999177\). This is not an
interval-arithmetic proof of uniqueness; the companion local numerical contraction certificate
records the machine-checkable hardening step. A separate hardware note reports an
optical-cavity check of the same fixed-point geometry; this is treated as corroborating
engineering evidence. \(N_{\mathrm{scr}}\) is inferred from the observed
cosmological horizon and enters the cosmological-capacity branch. The D6 normalization uses the
...
neutrino side estimate on that input is legacy bookkeeping rather than the live weighted-cycle
theorem lane.
...
\begin{assumption}[Scaling premise]\label{ass:technical}
This SM/GR derivation paper uses one canonical numbered list for the theorem-external scaling items outside the gauge branch. The fixed-cutoff realized presentation, the local-Gibbs form, quasi-local Lieb--Robinson propagation, endpoint-Lipschitz interval control, the induced finite-dimensional refinement branch, the fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on the realized cap-label-preserving MaxEnt family, and the operational/geometric split of Theorem~\ref{thm:geosubnet} are not listed here because they are established internally from Axioms~\ref{ax:screen}--\ref{ax:maxent}. The external item outside the gauge branch is:
...
\end{itemize}
The fixed-cap generalized-entropy stationarity result is Theorem~\ref{thm:equilibrium}. The bosonic ordinary/central gauge premises are stated separately in Assumption~\ref{ass:gaugeprem}.
\end{assumption}

\begin{assumption}[Bosonic gauge reconstruction premises]\label{ass:gaugeprem}
Whenever compact gauge reconstruction is invoked on the ordinary or central-defect branch, the following additional premises are kept explicit. Together they are the compact-paper form of \textbf{T3}--\textbf{T5}. On the central-defect branch, the matching loop-coherence criterion \textbf{T1} is carried by the same supplied path-independent transportability rather than listed as a second independent premise.
\begin{enumerate}[label=(G\arabic*),leftmargin=2.3em]
\item for every sufficiently fine cutoff \(r\), a fixed-cutoff bosonic collar-sector package \(\mathsf{Sect}_r\): a semisimple rigid \(C^*\)-tensor category of localized transportable edge sectors on the declared overlap collars, with tensor product given by collar concatenation, duals given by orientation reversal / charge conjugation, and bosonic symmetry in the relevant \(3+1\)-dimensional EFT branch;
\item for every refinement \(r\preceq s\), a faithful monoidal \(^*\)-functor \(U_{rs}:\mathsf{Sect}_r\to\mathsf{Sect}_s\) carrying the tensor unit to the tensor unit, preserving localized transportable sector classes together with their overlap-visible support on cofinal tails, and compatible with composition \(U_{su}\!\circ U_{rs}\cong U_{ru}\);
\item faithful stagewise bosonic multiplicity-space functors \(F_r:\mathsf{Sect}_r\to\mathsf{Hilb}_{\mathrm{fd}}\) with objectwise finite-dimensional values, together with monoidal natural isomorphisms \(F_s\!\circ U_{rs}\cong F_r\) compatible with the refinement system.
\end{enumerate}
These premises construct the refinement-limit bosonic sector category used below; they do not prove that the realized transportable branch is nontrivial. The genuinely noncentral fixed-cutoff branch, where the parallel obstruction criterion is \(q_\Sigma=0\), is handled separately by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} and is not itself the ordinary compact-group reconstruction theorem.
\end{assumption}
...
The table below is the dependency map for this SM/GR derivation paper. The ``Immediate OPH ingredients'' column records only parents internal to the OPH program. Imported standard mathematics and declared non-axiom premises or external inputs are stated separately rather than hidden inside the word ``derived.''
...
\textbf{D3} & Theorem~\ref{thm:bw}, Corollary~\ref{cor:lorentz}: geometric modular flow and Lorentz branch & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Bisognano--Wichmann modular geometry;\newline conformal classification on \(S^2\) & T2 together with the theorem-local realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum, and ordered cut-pair rigidity on the emitted scaling-limit geometric cap pair; Theorem~\ref{thm:geosubnet} fixes the target subnet internally; the scaling-limit observer algebra may leave the finite type-I regulator class, so the theorem is automorphism-level there and \(K_C=2\pi B_C\) is only the special type-I limit form & conditional scaling-limit\newline theorem \\
\textbf{D4} & Propositions~\ref{prop:nullec}, \ref{prop:n3}; Corollaries~\ref{cor:n1}, \ref{cor:n2}; Theorem~\ref{lem:density}, Lemma~\ref{lem:translation}; Theorem~\ref{thm:stress}: null modular bridge to \(T_{kk}\), including exact-or-controlled strip additivity, endpoint-Lipschitz renormalized half-line families, the weak tail generator, the derived half-sided modular inclusion, and the explicit positive half-line null-translation generator on its Stone domain with affine half-line modular relations & D2+D3 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & Borchers--Wiesbrock positive-generator theorem for standard half-sided inclusions; Stone's theorem; distributional differentiation on half-lines & the theorem-local null-cut center transfer, the extra inherited left/right strip-split condition needed for the spatial-collar-type tensor decomposition, and the exact-or-controlled Markov hypotheses of Proposition~\ref{prop:nullec} and Corollary~\ref{cor:n1}, plus T2; quasi-local propagation and endpoint-Lipschitz control are supplied internally by Axiom~\ref{ax:maxent} on the local finite-constraint branch, the geometric scaling action on the null half-line blow-up net derives the half-sided modular pair, the same-half-line local modular-Hamiltonian formula is imported only for the generator/charge identification, and bounded-interval formulas require the separate interval-preserving projective branch & conditional bridge theorem \\
\textbf{D5} & Theorem~\ref{thm:equilibrium}, Lemma~\ref{lem:smallball}, Theorem~\ref{thm:einstein}, Corollary~\ref{cor:einstein}: Jacobson-type Einstein branch via a rest-frame relation plus conditional tensor upgrade & D3+D4 and Axioms~\ref{ax:screen}--\ref{ax:entropy} & fixed-volume area-variation identity for small geodesic balls; local quadratic-polarization argument for the tensor upgrade & Theorem~\ref{thm:equilibrium}, which fixes the admissible fixed-cap MaxEnt variation class and derives generalized-entropy stationarity on the realized cap-label-preserving MaxEnt family; the internal small-ball bridge of Lemma~\ref{lem:smallball}, which uses the geometric cap generator together with the D4 half-line generator/charge identification and the separate interval-preserving projective branch; locally Lorentzian \(d=4\) scaling regime; small-ball constancy assumptions; D4 carried remainder negligible at order \(\ell^4\); all local directions/reference states for the tensor upgrade & conditional scaling-limit theorem/corollary \\
...
\textbf{D7} & Theorem~\ref{thm:sector-colimit}, Theorem~\ref{thm:compactgauge}: construction of the refinement-stable bosonic sector category and compact gauge reconstruction & Axioms~\ref{ax:screen}--\ref{ax:entropy} & directed-colimit descent for monoidal \(C^*\)-categories; Doplicher--Roberts / Tannaka reconstruction & Assumption~\ref{ass:gaugeprem} on the ordinary or central-defect bosonic branch; the genuinely noncentral fixed-cutoff branch is handled separately by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} and is not itself the ordinary compact-group reconstruction theorem & conditional structural theorem \\
...
\textbf{D10} & forward gauge-coupling closure and target-free source-only electroweak readout of Section~7 & D9 + pixel ratio \(P\) & printed RG evolution and matching conventions & pixel constraint and the source-only forward transmutation solve \(\mathcal F(\alpha_U;P)=0\); the fixed-cutoff edge heat-kernel / Casimir theorem on the microphysics surface together with the compact-group / Peter--Weyl lift used on the D10 lane; printed beta-function and threshold conventions & quantitative-closure sector \\
\textbf{D12} & Sections~8--10 continuations:\newline charged leptons,\newline proton spin/lifetime,\newline and controlled string/worldsheet effective-description branches & various subsets of D6, D9, and D10 & branch-specific EFT\newline and phenomeno\-logical\newline manipulations & additional ans\"atze such as the uniform\newline \(\mathbb Z_6\) center-label ensemble, texture choices,\newline dark-sector response assumptions,\newline discrete-horizon assumptions, a distinct\newline large-\(N_{\mathrm{edge}}\) regime with fixed\newline \(\tau=tN_{\mathrm{edge}}\) window and uniform\newline genus-remainder control, or extra\newline worldsheet/CFT premises & phenomeno\-logical\newline continuation \\
\end{longtable}
\endgroup

The dependency DAG records the immediate internal parents, imported mathematics, and extra premises for each node.

\begin{theorem}[Summary theorem for the recovered relativity-plus-Standard-Model core]\label{thm:master}
This theorem packages the recovered-core nodes D1--D5 and D7--D9 of the dependency DAG above. Node D6 is not part of this recovered-core theorem because it requires the external screen-capacity input \(N_{\mathrm{scr}}\), but it is the input-dependent global closure of the Einstein branch recovered in item (iii); D10 and D12 are outside this theorem's scope. Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Axiom~\ref{ax:mar}, Assumptions~\ref{ass:technical} and \ref{ass:gaugeprem}, and the hypotheses of Theorems~\ref{thm:confluence}, \ref{thm:bw}, \ref{thm:stress}, \ref{thm:einstein}, \ref{thm:sector-colimit}, \ref{thm:compactgauge}, \ref{thm:sm}, and \ref{thm:hypercharge}, together with Corollaries~\ref{cor:ng}, \ref{cor:nc}, and Proposition~\ref{prop:z6}. Then, in the controlled refinement/scaling regime singled out by Assumption~\ref{ass:technical}:
...
\begin{proof}
Items (i)--(iii) are collected from Theorem~\ref{thm:confluence}, Corollary~\ref{cor:lorentz}, Theorem~\ref{thm:einstein}, and Corollary~\ref{cor:einstein}. On the gauge side, Theorem~\ref{thm:sector-colimit} first assembles the supplied fixed-cutoff bosonic collar-sector packages, their monoidal refinement transport, and their compatible finite multiplicity fibers into the refinement-limit category and fiber functor. The compact gauge reconstruction is then Theorem~\ref{thm:compactgauge}. On the central branch the matching loop-coherence burden is already part of that supplied transportability rather than an extra theorem-side premise. The preceding MaxEnt/local-Gibbs/collar-mixing package is not used there to prove nontriviality of the transportable bosonic ladder. The product gauge structure up to finite quotient is Theorem~\ref{thm:sm}; and the final identification of the exact Standard Model quotient is Theorem~\ref{thm:hypercharge} together with Corollaries~\ref{cor:nc}, \ref{cor:ng}, and Proposition~\ref{prop:z6}.
\end{proof}
...
Einstein dynamics & fundamental field equation & conditional first-variation relation with tensor upgrade from generalized entropy, null modular data, and derived fixed-cap generalized-entropy stationarity \\
Gauge group & model input & conditional construction of the refinement-limit bosonic edge-sector category, then compact group reconstruction from it; exact SM quotient selected on the realized MAR-admissible branch with the realized one-generation/one-Higgs package \\
...
theorem-local burden is repair completeness, together with the Petz support/CPTP clause
...
\]
for each round cap \(C\subset S^2\). Axiom~\ref{ax:maxent} controls the state-side realized branch across refinement through one common finite-dimensional MaxEnt family, and Theorem~\ref{thm:geosubnet} removes the overlap-trivial auxiliary/record layers from the BW target. The theorem-local UV surface requires two explicit objects on that geometric subnet: first a realized transported geometric cap-local system whose carried-collar schedule is derived from the transported fixed-local-collar Markov/faithfulness datum and emits the scaling-limit geometric cap pair from transported marginals, and then ordered cut-pair rigidity on the extracted prime cap pair. The theorem below makes that claim boundary explicit, fixes the \(2\pi\) normalization once those two objects are supplied, and leaves the UV burden exposed rather than hidden.
...
\begin{definition}[Conditional BW-branch observer-relative time reading]\label{def:modulartime}
...
Hence the renormalized half-line null generator is not a separate EFT-side premise: it is the same operator that the effective spacetime description calls the local null-stress charge of the null half-line.
\end{theorem}
...
The only imported continuum input used above is the standard local null modular-Hamiltonian formula for the effective spacetime description of the same half-line family. There is no separate ``null-stress identification premise.'' The downstream boundary is only:
...
\begin{theorem}[Conditional Jacobson-type rest-frame relation from derived fixed-cap generalized-entropy stationarity]\label{thm:einstein}
...
Theorem~\ref{thm:lambda-stack} packages the intended local/global D5\(\to\)D6 handoff. The imported premises are exactly the external screen-capacity input \(N_{\mathrm{scr}}\), the D6 identification \(N_{\mathrm{scr}}=S_{\mathrm{dS}}\), the standard de Sitter entropy relation, and the standard static-patch radius/time formulas; none of these are hidden inside the local null-modular reconstruction. The external item is the screen-capacity identification itself: the local theorem stack does not derive \(N_{\mathrm{scr}}=S_{\mathrm{dS}}\) from the bare local axioms. The vacuum-energy problem is reorganized by separating local null data from that global capacity closure.

\section{Gauge Reconstruction and Standard Model Structure}

\subsection{Compact gauge reconstruction}

At any fixed UV cutoff, edge-center completion equips collars with finitely many sector labels, intertwiners, and fusion rules. These are fixed-cutoff collar data. The local MaxEnt / collar-mixing package established earlier controls only fixed-cutoff recoverability, modular-support localization, and carried error terms on the realized branch. It is logically separate from the question whether transportable edge sectors survive refinement, and it supplies no theorem that the refinement-limit sector category is trivial or nontrivial. A later nontrivial transportable branch is therefore treated here as compatible additional gauge data rather than as something decided by the mixing estimate itself. Whenever compact gauge reconstruction is invoked, the full bosonic refinement ladder of Assumption~\ref{ass:gaugeprem} is the explicit input.

On the ordinary or central-defect branch, the transportability referred to here is path-independent movement of charges between patches in the DHR sense. On the central branch the same issue appears on the gluing side as the central loop-coherence obstruction. The paper therefore does not treat vanishing of that obstruction as a second theorem-side premise beyond the supplied transportable sector system; it is the central-branch obstruction bookkeeping for that same transportability condition. The genuinely noncentral fixed-cutoff branch differs: its obstruction is the crossed-module class \(q_\Sigma\), and that branch is handled by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} rather than by the ordinary compact-group theorem below.

Assume therefore that in the EFT regime there is a directed ladder \((\mathsf{Sect}_r,U_{rs},F_r)\) of transportable bosonic edge-sector packages as in Assumption~\ref{ass:gaugeprem}, and write
\[
\mathsf{Sect}_\infty := \varinjlim_r \mathsf{Sect}_r
\]
for the directed colimit retaining the transportable sectors and intertwiners that persist in that system. This is the category on which Doplicher--Roberts / Tannaka reconstruction is applied. The theorem below is neutral about whether \(\mathsf{Sect}_\infty\) is trivial or nontrivial: if the supplied bosonic refinement ladder of Assumption~\ref{ass:gaugeprem} furnished only the tensor unit, the reconstructed compact group would be the trivial group.

\begin{lemma}[Refinement-stable transportable sector persistence]\label{lem:sector-persistence}
Assume the directed transport system in Assumption~\ref{ass:gaugeprem}, and write \(U_{rs}:\mathsf{Sect}_r\to\mathsf{Sect}_s\) for the refinement functor from regulator scale \(r\) to a finer scale \(s\succeq r\). Let \(\alpha_{r_0}\) be a realized transportable edge-sector class on a sufficiently fine collar whose edge-center support is visible on the declared overlap algebra. Suppose there is a cofinal refinement tail \(T\) above \(r_0\) and representatives \(\alpha_s\in\mathsf{Sect}_s\), \(s\in T\), such that \(U_{st}\alpha_s\simeq \alpha_t\) for \(t\succeq s\) in \(T\), with the same overlap-visible support transported by the corresponding refinement maps. Then the tail determines a unique object \([\alpha]\in\mathsf{Sect}_\infty\). A later refinement stage cannot delete, split, or change the transportability status of that realized class while preserving the stated hypotheses and adding no new overlap-visible support.
\end{lemma}

\begin{proof}
The directed colimit identifies two representatives once their images agree at some common finer regulator stage. The cofinal tail \(T\) therefore supplies a compatible family \((\alpha_s)_{s\in T}\), and its equivalence class is an object \([\alpha]\) of \(\mathsf{Sect}_\infty\). If a second cofinal family \((\beta_u)\) represents the same realized collar class, cofinality gives a common refinement \(v\succeq s,u\). Compatibility of the refinement functors and the declared overlap-visible support gives \(U_{sv}\alpha_s\simeq U_{uv}\beta_u\), so both families define the same colimit object.

Deletion or loss of transportability at a later stage would mean that no compatible representative exists on a cofinal tail, contradicting the persistence hypothesis rather than producing a different colimit object. A split into inequivalent later classes without new overlap-visible support would assign two incompatible refinement images to the same transported edge-center datum, contradicting functorial compatibility on transportable sector classes. If new overlap-visible support appears, it is a new sector datum, not a silent change of the old persistent class. Thus persistence is unique once the directed transport system is supplied; the lemma does not prove that such a nontrivial system exists.
\end{proof}

\begin{theorem}[Construction of the refinement-stable bosonic sector category]\label{thm:sector-colimit}
Assume the bosonic gauge premises of Assumption~\ref{ass:gaugeprem}. Then the directed colimit
\[
\mathsf{Sect}_\infty:=\varinjlim_r \mathsf{Sect}_r
\]
inherits a well-defined semisimple rigid symmetric \(C^*\)-tensor structure from the fixed-cutoff collar-sector packages. Its tensor unit is the persistent vacuum sector, its tensor product is induced from collar concatenation, its duals are induced from the fixed-cutoff orientation-reversal / charge-conjugation duals, and its braiding is the induced bosonic symmetry from the \(3+1\)-dimensional branch. The compatible finite-dimensional multiplicity spaces \(F_r\) descend to a faithful bosonic fiber functor
\[
...
\begin{proof}
Because every \(U_{rs}\) is a faithful monoidal \(^*\)-functor and the coherence maps satisfy \(U_{su}\!\circ U_{rs}\cong U_{ru}\), the directed-colimit category exists with objects given by eventually compatible tails and morphisms given by eventual intertwiner classes. The tensor unit on each \(\mathsf{Sect}_r\) is preserved by refinement, so the vacuum unit determines a unique tensor unit \(\mathbf 1_\infty\) in the colimit.

For persistent objects represented by tails \((X_r)\) and \((Y_r)\), define
\[
[X]\otimes [Y]:=[X_r\otimes_r Y_r]
\]
using any sufficiently fine common stage. This is independent of the chosen stage because the monoidal structure maps of \(U_{rs}\) identify \(U_{rs}(X_r\otimes_r Y_r)\) with \(U_{rs}(X_r)\otimes_s U_{rs}(Y_r)\). The same argument descends associators, unitors, and the bosonic symmetry/braiding maps. Since each \(\mathsf{Sect}_r\) is symmetric, the induced braiding on \(\mathsf{Sect}_\infty\) is symmetric as well.

Rigidity descends stagewise. If \(X_r^\vee\) is a dual of \(X_r\) with evaluation and coevaluation maps
\[
\operatorname{ev}_{X_r}:X_r^\vee\otimes_r X_r\to \mathbf 1_r,
\qquad
\operatorname{coev}_{X_r}:\mathbf 1_r\to X_r\otimes_r X_r^\vee,
\]
then monoidality of the refinement functors carries these maps to the corresponding duality data for \(U_{rs}(X_r)\). Hence dual objects and zig-zag identities pass to the colimit. The \(C^*\)-structure and semisimplicity likewise descend because morphisms are eventual classes of morphisms in semisimple \(C^*\)-categories and the refinement functors preserve the \(^*\)-operation and direct-sum data.

For the fibers, define \(\mathcal F([X])\) to be the stable finite-dimensional Hilbert space represented by any \(F_r(X_r)\). This is well defined because the coherence isomorphisms \(F_s\!\circ U_{rs}\cong F_r\) identify any two sufficiently fine representatives canonically. On morphisms, \(\mathcal F\) is induced by the eventual intertwiner action on those multiplicity spaces. Monoidality and the bosonic unit condition descend from the corresponding properties of the \(F_r\), and faithfulness descends because each \(F_r\) is faithful on its stage. Thus \(\mathcal F\) is a faithful bosonic fiber functor with finite-dimensional values.
\end{proof}
...
\mathcal F:\mathsf{Sect}_\infty\to\mathsf{Hilb}_{\mathrm{fd}}
\]
be the faithful bosonic fiber functor on that refinement-limit sector category in the EFT regime, constructed in Theorem~\ref{thm:sector-colimit}. Then
\[
...
\begin{proof}
The theorem begins only once the bosonic refinement ladder of Assumption~\ref{ass:gaugeprem} has been assembled into the colimit category and fiber functor of Theorem~\ref{thm:sector-colimit}; those data are not consequences of the local-Gibbs or Dobrushin-type collar estimates. On the central branch, no second obstruction premise is appended beyond that supplied transportability hypothesis; the same path-independent transport condition is the gluing-side content of the loop-coherence criterion. Fix a small skeleton of \(\mathsf{Sect}_\infty\), which does not change the tensor category or the fiber functor up to equivalence. For every object \(X\), any monoidal natural automorphism \(\eta\in\mathrm{Aut}_\otimes(\mathcal F)\) has a unitary component \(\eta_X\in U(\mathcal F(X))\) because \(\mathcal F\) is a \(^*\)-functor to finite-dimensional Hilbert spaces. Hence there is an injective map
\[
...
The hypotheses required by Doplicher--Roberts / Tannaka reconstruction are checked explicitly on the constructed pair \((\mathsf{Sect}_\infty,\mathcal F)\): Theorem~\ref{thm:sector-colimit} supplies a rigid symmetric \(C^*\)-tensor category with a faithful bosonic fiber functor of finite-dimensional values. The standard reconstruction theorem therefore applies to that pair and yields a symmetric \(C^*\)-tensor equivalence
\[
\mathsf{Sect}_\infty \simeq \mathrm{Rep}(G),
\]
...
If a fermionic sign object is present, the correct reconstruction statement is super-Tannakian rather than purely Tannakian. This paper does not prove that fermionic/chiral extension or the nontriviality of the realized transportable bosonic ladder; it works only in the bosonic internal-gauge branch once the coherent fixed-cutoff sector packages and their monoidal refinement transport are supplied.
...
\begin{remark}
The same product-group structure removes the usual grand-unification monopole sector. In the present framework, any retained coupling-unification discussion does not proceed by embedding the gauge group into a simple group that is later broken. It proceeds, if at all, only through the separate D10 calibration branch built from shared edge heat-kernel data together with the printed running/matching conventions and extra calibration assumptions. Absence of gauge-mediated proton decay and unification-like running therefore coexist without tension on that branch. Any confinement statement is a separate infrared dynamical issue rather than a corollary of the product-group adjoint argument alone.
\end{remark}
...
constraint above it. The open burden is refinement-stable universality/uniqueness of
...
electromagnetic transport family \(\alpha_{\mathrm{em}}^{-1}(q^2;P)\) and \(\sin^2\theta_W(q^2;P)\), the public
\(W/Z\) pair, and the Thomson endpoint
\[
\alpha_{\mathrm{Th}}^{-1}(P)=\lim_{q^2\to0}\alpha_{\mathrm{em}}^{-1}(q^2;P)
\]
on the Ward-projected \(\U(1)_Q\) lane are forward-emitted outputs of the chosen D10
running/matching package. When compared with observation, they serve as validation of that printed
implementation on the quantitative-closure branch recorded in the premise ledger. The repository
carries a numerical witness for the outer/inner fixed-point closure, while external
fine-structure comparisons enter only as explicit compare metadata, together with a local numerical
contraction certificate for the implemented map. The theorem-external item is an
interval-wide existence/uniqueness or monotonicity proof for the scalar root selected by the printed
quantitative package under arbitrary convention changes.
...
The D5\(\to\)D6 cosmological-capacity stack is explicit: the null-modular reconstruction of Section~\ref{sec:gravity} determines the stress tensor only up to a metric term, so local null data do not fix $\Lambda$. On the D6 branch the extra global premise is the external screen capacity
\[
N_{\mathrm{scr}}=\log\dim\mathcal H_{\mathrm{tot}}
\]
...
In the bosonic EFT branch, compact gauge reconstruction follows only after the explicit coherent refinement ladder of transportable fixed-cutoff collar-sector packages, its bosonic symmetry, and its finite-fiber descent data of Assumption~\ref{ass:gaugeprem} are supplied; on the central branch the corresponding loop-coherence criterion is already carried by that supplied transportability. The genuinely noncentral fixed-cutoff branch is handled separately by crossed-module data rather than by the ordinary compact-group theorem. On the realized MAR-admissible branch with the explicit realized one-generation chiral matter plus one-Higgs package, the Standard Model quotient is then selected by minimal admissibility. This differs sharply from approaches in which the gauge group is specified in advance.
...
The imported premises are exactly the fixed-cutoff edge heat-kernel / Casimir law, the compact
...
does not by itself produce a worldsheet genus expansion.
...
stated large-\(N_{\mathrm{edge}}\) premise supplies that expansion on the fixed-\(\tau\) window
...
\paragraph{Open blockers.}
The criterion theorem does not derive the large-\(N_{\mathrm{edge}}\) regime itself from the compact-gauge
chain, and it does not reprove the imported Gross--Taylor large-\(N\) worldsheet dictionary for
two-dimensional Yang--Mills. The external content is the existence of the large-\(N_{\mathrm{edge}}\)
sequence, the fixed-\(\tau\) window on which the genus expansion holds, and the uniform remainder
control. Any further lift to critical superstring structure additionally requires worldsheet
supersymmetry, critical dimension, modular invariance, anomaly cancellation, GSO projection, and
full massless-spectrum matching beyond the present continuation theorem.
...
N_c=3,
\]
with the exact one-generation hypercharge lattice. Here the count statements are internal to the same realized branch: the D8 minimal coupled-carrier theorem stack fixes the color triplet and hence \(N_c=3\); CKM phase counting and weak-sector asymptotic freedom then bound \(N_g\); MAR fixes the smallest realized value \(N_g=3\); and Witten parity remains only a consistency check on the resulting triplet-doublet package.
\[
Y_Q=\frac16,\qquad
Y_L=-\frac12,\qquad
Y_u=-\frac23,\qquad
...
The relativity claim is conditional and sharp about its scope: if the stated geometric-modular and null-bridge premises are realized in the intended scaling regime, and if the same realized cap-label-preserving MaxEnt family satisfies the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations on that branch, then this paper predicts Lorentz kinematics on the screen and a Jacobson-type Einstein branch. What is being tested here is the existence of that scaling regime and its premises, not a fixed-cutoff matrix-algebra identity.
...
\item baryogenesis estimates based on extra suppression-counting assumptions; a baryogenesis theorem requires a concrete out-of-equilibrium mechanism, derived CP-odd source terms, justified defect/sphaleron counting, washout control, and a freeze-out computation of the final asymmetry;
...
The UV-side burden on the Lorentz branch is therefore the explicit scaling-limit plus geometric-subnet extraction-and-rigidity package under \textbf{T2}, not the mere existence of a finite cell size.
...
Second, one needs ordered cut-pair rigidity on that realized limit. That stage would collapse the residual cap-preserving conformal freedom to the unique BW hyperbolic subgroup, after which the modular KMS condition fixes the \(2\pi\) normalization. Ordered cut-pair rigidity is symbolic until the realized scaling-limit geometric cap pair exists. Until those two objects are internalized, every stronger vacuum or canonical-cap identification is explicitly theorem-external shorthand for the geometric-subnet branch of Theorem~\ref{thm:bw}, not a derived theorem.
...
\item the conditional scaling-limit Einstein branch under the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations and the null modular bridge together with the bounded-interval projective branch;
...
The imported realized-branch premise here is the D9 result that the same realized branch carries the color triplet \(N_c=3\) and the generation count \(N_g=3\).
...
\]
on the frozen authoritative repair surface, but that exact pair is compare-only beneath the target-free source-only theorem. The D11 Higgs/top branch closes as one source-only split theorem on the declared D10/D11 quantitative surface. On the emitted D10 repair tuple \((\eta_{\mathrm{source}},\beta_{EW},\lambda_{EW},\tau_{2,\mathrm{tree}}^{\mathrm{exact}},\delta n_{\mathrm{tree}}^{\mathrm{exact}})\), define
\[
...
direct-top PDG row is open; see \href{https://github.com/FloatingPragma/observer-patch-holography/issues/207}{issue \#207}.
...
\]
Detailed particle-spectrum continuation statements are carried by Ref.~\cite{ophparticles}. That companion paper develops the quark selected-class theorem, the charged-lepton determinant-line boundary, the weighted-cycle neutrino theorem surface, and the hadron execution boundary. They do not belong to the compact-paper claim surface.
...
\]
after which the D10 source basis emits the public electroweak pair and the D11 source-only split theorem with
\[
\rho_{HT}=\log\!\bigl(1+\tau_{2,\mathrm{tree}}^{\mathrm{exact}}\bigr),
\qquad
(\pi_y,\pi_\lambda)\longmapsto (m_t,m_H)
\]
by the declared D11 Jacobian, again with no inverse readback of the internal transmutation data from measured low-energy couplings.
...
Observer-Patch Holography starts from overlap consistency of observer patches and yields, within one framework, several effective structures usually treated separately in modern fundamental physics. This SM/GR derivation paper supports a unique schedule-independent normal form for overlap repair and hence a unique physical UV branch only modulo boundary redundancy, implementation hiding, and inert ancillary stabilization, a conditional Lorentz branch on the extracted prime geometric cap pair, a conditional Jacobson-type Einstein branch under the derived fixed-cap generalized-entropy stationarity theorem for admissible fixed-cap MaxEnt variations and the null modular bridge together with the bounded-interval projective branch, the input-dependent global closure of that same gravity lane once the screen-capacity input is supplied, the Standard Model gauge quotient with exact hypercharges on the realized matter package on the realized MAR one-Higgs branch in the bosonic internal-gauge sector, the realized color triplet \(N_c=3\) and generation count \(N_g=3\) on that branch, and a separate integrated quantitative-closure branch. Phenomenological continuations are discussed separately from the recovered core.

The framework therefore presents general relativity and the Standard Model as effective descriptions arising once the stated scaling-limit, geometric-subnet, and categorical premises are realized. The local quantitative claim surface is equally explicit: the same declared pixel input \(P\) controls the electroweak/Higgs mass trunk and the classical-regime gravity coupling, the invariant causal speed \(c\) is the structural Lorentz output, and the familiar-unit package reads meters, seconds, GeV, and Kelvin from the single local ruler \(\sqrt{a_{\mathrm{cell}}}\) with \(\hbar\) and \(k_B\) used only as display conventions. Open items are sharper quantitative control, proof that the required coherent transportable bosonic refinement ladder is emitted on a realized nontrivial branch rather than merely supplied as gauge data, a fuller fermionic gauge branch, explicit microscopic realizations that emit the prime geometric cap pair and support ordered cut-pair rigidity, and proof of the theorem-side local exact-release objects on the live corpus.
