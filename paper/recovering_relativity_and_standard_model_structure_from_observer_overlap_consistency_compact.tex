...
\]
and then the post-promotion lift whose descended scalar is \(\mu_{\mathrm{phys}}(Y_e)\), with the physical identity-mode equalizer as the exact smaller forcing object beneath that scalar. For quarks, the next exact object is the one-scalar law \ophid{quark_d12_t1_value_law} on the emitted D12 mass ray, with \ophid{intrinsic_scale_law_D12} as the derived wrapper. For neutrinos, the same-label scalar certificate closes beneath the weighted-cycle branch, and the irreducible exact missing object is the reduced absolute-scale correction invariant \(C_\nu\). Thus the shared excitation dictionary is not the missing theorem object. It is the common OPH family base, while old base-6 exponents survive only as compare-only compressions of its excitation action.
...
\]
on the same ordered eigenvalue family, but it remains same-family-only beneath the open theorem chain \(\widehat C_e^{\mathrm{cand}}\to \widehat C_e\) together with the post-promotion lift whose descended scalar is \(\mu_{\mathrm{phys}}(Y_e)\). The quark lane has an exact same-family witness
\[
(u,d,s,c,b,t)=
(0.00216,\ 0.00470,\ 0.0935,\ 1.273,\ 4.183,\ 172.3523553288311)\,\mathrm{GeV},
\]
again same-family-only, because the selected D12 sheet is the wrong CKM branch and the emitted ray waits on the one-scalar law \ophid{quark_d12_t1_value_law}. The neutrino lane has a compare-only exact two-parameter adapter on the explicit positive selector segment with
\[
(m_1,m_2,m_3)=
...
The light-quark lane remains a D12 continuation branch and is not a recovered-core theorem. Its frontier is no longer an open selector search. On the emitted same-label left-handed solver surface the local orbit closes to the singleton \(\sigma_{\mathrm{ref}}\) (\ophid{sigma_ref}). This is a real but negative closure: \(\sigma_{\mathrm{ref}}\) is the wrong D12 CKM sheet, and the corpus emits no sector-attached left-handed same-label branch-changing transport off that selected sheet. Same-sheet overlap scans, chirality-swapped basis diagnostics, and any other non-sector-attached orbit improvement therefore remain compare-only and do not alter the selected-sheet statement.

The mass side is therefore the only next theorem surface on the emitted D12 branch. The same-family branch emits only the one-parameter ray
\[
D12_{ud}^{\mathrm{mass}}
\equiv
\mathcal R_{D12}^{ud}
\]
(paper id \ophid{D12_ud_mass_ray}), not a distinguished modulus on that ray. The sole next theorem object on that emitted ray is the one-scalar law \ophid{quark_d12_t1_value_law}, which identifies the ray modulus as \(t_1\) on \ophid{D12_ud_mass_ray}; \ophid{intrinsic_scale_law_D12} is retained only as the derived wrapper around that scalar law. CKM and CP closure are not a second independent selector task on the emitted surface; they remain downstream of the wrong-sheet no-go unless a genuine sector-attached left-handed same-label branch-changing transport is derived.
