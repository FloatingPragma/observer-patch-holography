...
\path{Lean/Dynamics/CentralBlocks.lean}, and
\path{Lean/Dynamics/CenterSpectral.lean}. On one full finite matrix
...
except Hamiltonian form. The two-block direct sum inherits the converse
blockwise with block fixing derived: the center of the two-block algebra
is exactly the span of its two minimal central projections, every star
automorphism fixes or swaps those projections, the swap branch is
realized by the block-swap equivalence on the square two-block algebra,
and a pointwise-continuous group through the identity keeps each
projection on a continuous path inside a two-point set, hence fixes both
blocks at every parameter. Behind this sits a spectral decomposition:
the center of any star subalgebra of a finite complex matrix algebra is
the span of a minimal orthogonal central resolution of the identity,
unique up to permutation. The parameter is a real number; no physical
clock, source-selected Hamiltonian, or physical time identification is
claimed, and central-block sums beyond two blocks are outside the proved
statements \cite{ophrepo}.
