...
\section*{Downstream physical boundary}
...
contract and may constrain downstream physics. On the unified Echosahedral
...
quotient-visible records. Physical-clock status additionally requires an
...
as a verified domain record with the released consensus code.
\end{theorem}
...
% Verification source and stored receipt:
...
and is fixed by both self-readbacks. This completes the seven-clause
operational contract at one regulator; refinement-natural cross-observer
evidence and higher-overlap coherence remain downstream. Readback idempotence
...
are identities, and genuine coverage semantics for a source-produced net
are open.
...
Two committed regional diamonds carry counted correlation data of
...
stage by the same committed transport pattern as the 86/88 witness, with
the same committed checkpoint family of observer 86 as commutative
...
Both committed carriers therefore carry parallel tower anchorings of the
same declared form (\texttt{bothCarriers\_common\_anchoring}); the earlier
state, in which only the 86/88 diamond carried a tower anchoring, no
longer holds. An exact
shared-observer theorem
(\texttt{towerCarriers\_share\_obs86\_marginal}) relates the two
carriers through their common left observer: the two committed joint
...
to the committed marginal state of observer 86, and the left partial
...
The time-slice direction is approached in
\path{Lean/QFT/PathTimeSliceInterface.lean} by a finite \emph{one-step}
interface. The structure \texttt{TimeIndexedNetEvolution} carries a finite
index and successor, a region family, per-step ambient star-automorphisms
mapping the current regional algebra to the next one, and admitted data in the
current algebra. Its corrected generation clause first evolves those data and
then requires their generated algebra to equal the next regional algebra.
This ordering matters: an earlier version compared the raw, unevolved data
with the next algebra and thereby created a spurious containment and
step-invariance conclusion.

A full finite inhabitant exists over the committed 86/247 net. Its cyclic
\(\operatorname{Fin}(32)\) witness follows the 31 actual adjacent source-row
pairs and adds one declared terminal-to-initial bookkeeping automorphism; all
three corresponding source counts are zero, so that closing automorphism is
not a committed transition. The admitted data are observer 86's lifted source
...
inhabitant. The natural single-projector walk candidate remains a useful
...
This corrected interface and the triple coupling do not supply a time-slice
theorem. They have no multi-step
cocycle or group-action law, Cauchy geometry, clock, physical time, or source
selection.  The common carrier joins the committed state/path/checkpoint data
but supplies no regional-net or tower morphism. The
withdrawn raw-generation argument does not show that a new noninjective net
shape is necessary. Viable routes include an evolution that transports
admitted data between distinct current regional algebras, equal algebras for
appropriate Cauchy neighborhoods, or a fixed algebra with coherent nontrivial
dynamics. The \(\mathbb Z/2\) slot swap remains an internal finite symmetry; the
diagonal-layer join below carries typed embeddings of both carriers, while
physical time, causality, spacetime attachment, and joining at the
regional-net and tower levels stay open.

\paragraph{A typed hinge-fibered join of the two committed carriers.}
The module \path{Lean/QFT/CarrierJoinTransport.lean} joins the two committed
carriers at their diagonal layer, the layer on which every committed counted
...
committed observer-86 occupation law
(\texttt{joinStates\_marginal\_compatible}).  The committed ambient
anchoring equivalences act on the diagonal layer as the committed basis
reindexings, and the join transports along them with commuting squares on
both embeddings; the committed walk-step transports over the 31 actual
...
regional structure of the committed carriers at the layer on which the join
exists.  For each region of the committed diamond, the record layer is the
...
idempotent, and preserves the hinge restriction, and the committed join
...
commutative while the committed left regional algebra is not, so the
commutant distinctions of the operator layer collapse under the diagonal
restriction.  The operator-layer net morphism and the tower morphism stay
open, and physical time, causality, and spacetime attachment stay open
under the registered premises.

\paragraph{The operator-layer morphism boundary.}
The operator-layer residual is made exact on the committed 86/247 objects
in \path{Lean/QFT/OperatorLayerMorphismBoundary.lean}.  In the
embedding direction the record function carrier enters the joint matrix
algebra as an injective unital star algebra homomorphism, the diagonal
embedding; its image lies in the committed regional algebra of a region
...
committed conditional expectations with the record-layer expectations on
every region.  In the projection direction the committed left-slot
conditional expectation is positive, trace preserving, and unital, and
restricts to the record-layer expectation on diagonals; a committed
...
multiplicative on the committed left regional algebra while restricting to
the record-layer morphism on diagonals: the committed noncommuting witness
...
homomorphism carries the committed left regional algebra, or the ambient
joint algebra, into the record carrier at all
(\texttt{twoSlot247\_left\_no\_record\_algHom}), through the committed
...
star homomorphism, while the committed projection is a positive unital
...
different targets, or enriched categorical structures. Net-to-net and tower
morphism structure at the operator layer stays open.
...
those four statements is instantiated on a committed witness net whose top
...
throughout, and the equality it transports along Cauchy embeddings is the
order-theoretic equality of the tower, and the time-indexed evolution
interface and every physical attachment are open premises.
...
locality for the declared-disjoint regions, and the committed
...
For the committed tower interface, the functional need not be added as an
...
Hilbert space and representation above.  The committed one-regulator witness
instantiates this construction, and its GNS unit class is nonzero with norm
one.  This closes the formal algebra--selected-state--Hilbert-space rung.
...
physical time-slice theorem follows.  Thus this is an actual mathematical
Hilbert-space attachment to the committed tower; the finite composition below
adds one Hamiltonian without turning it into a physical QFT.
...
\[
  \left.\frac{d}{dt}\right|_{t=0}\alpha_t(X_L)
    =-i[H_{\mathrm I},X_L]\ne0,
\]
with the committed matrix entry equal to \(i\).  Deleting only the
...
composition gap, but it also exposes the next obstruction exactly.  The
left conditional expectation \(E_L\) is trace preserving but does not preserve
...
represented on its tower-selected GNS Hilbert space, not a state-compatible
physical QFT.  The regulator, factorization, density, Hamiltonian, and flow
parameter are declared rather than source produced or calibrated; the ground
sector remains degenerate; and Lorentzian localization, a spectrum condition,
continuum or infinite-volume control, renormalization flow, field and particle
content, scattering, detector readback, and a physical time-slice theorem are
absent.
...
This is a source-counted finite-history Hamiltonian GNS attachment.  It is
not a physical QFT.  The retained run was hash pinned, but the empirical law
and repair-count postprocessing were not fixed together by a prospective
source contract.  The packet fixes the diagonal density and action values;
the full \(M_8(\mathbb C)\) quantum extension, off-diagonal coherence probe,
one-regulator tower, and real flow parameter are mathematical adapters.  The
nonzero energy of \(\Omega_{\rm hist}\) prevents its promotion to a vacuum or
ground vector.  No regional net is attached to this carrier, and there is no
physical clock or energy calibration, Lorentzian localization, spectrum
condition, continuum or infinite-volume control, renormalization flow,
field/particle interpretation, scattering, detector readback, or physical
time-slice theorem.
(\path{Lean/QFT/SourceHistoryGNSDynamics.lean}).

\paragraph{Region families of the corrected one-step interface.}
The same module settles the region-family freedom of
\texttt{TimeIndexedNetEvolution} over the committed 86/247 diamond in both
...
slice family over the committed walk-step evolution on cyclic
...
automorphism composes the committed walk-step evolution with the internal
slot exchange, the admitted data are the committed source generators of
...
symmetry, not a source transition, and the walk factor carries the declared
index-31 bookkeeping closure; physical time, causality, and the source
content of the registered time-slice premise stay open.
...
\paragraph{An executed locally frozen witness chain.}
The certificate discipline above has one executed finite instance running
from a source run to kernel-checked literals. A bounded run of the
companion simulation engine was locally pre-specified and hash-pinned before
execution, but it has no independent public pre-result timestamp: the contract
and result first enter repository history together. The contract pins a nonconstant protected record, separate
state and transition tables, an exact recognizer, stationarity, and
contraction. The executed run passed those extraction checks; this is not a
prospectively validated or publicly preregistered result. It did not
earn the typed equality that its provisional ``common reference'' label
suggested: the transition stationary law and the empirical state-side
reference are distinct objects. In fact their exact denominator mismatch and
the nonprojector transition eigenmode rule out deterministic pushforward and
nondegenerate intertwining with the idempotent state-side resampling action on
this artifact. A declared extraction script froze
the run's finite payload, and the payload integers are mirrored verbatim
in \path{Lean/QFT/SimEarnedWitness.lean} as literals whose receipts are
kernel-decided, inhabiting a declared payload-attached instance of the observer access-cut
interface of Section~\ref{sec:operational-observer-layer}. Where the
truncated payload cannot support a receipt, the collapse is stated as a
theorem instead of being repaired by synthetic data: on three of the four
mirrored observer supports the public and accessible algebras coincide,
and exactly one support separates a record class by its companion class.
The literals are counts and class labels on that one run under a declared
binning and a declared observer selection rule; no physical claim, no
probability claim beyond realized frequencies, and no canonical status
for the selection are asserted \cite{ophrepo}.
...
The current two-carrier reference fixture has no composable seam triangle, so
...
extra-sector exclusion require separate identifications. Source selection of
the matrix current, matter action, and physical global quotient is work in
progress. Laboratory
current and flux identification, equality with the independently reconstructed
Tannaka current, four-dimensional topological normalization, scalar
multiplicity remain separate. The selected rank-three response and declared
...
transporting the twelve-port Spin packet to that domain. Physical matter-pole
identification, the continuum Spin/locality limit, physical seam selection,
refinement persistence, and laboratory attachment remain separate.
...
orthogonal conditional-expectation projector. A concrete active repair kernel has that status only
...
executable-to-Lean bridge are absent. Direct \(N\) is not evaluable on the
incomplete source antecedent, and the stronger source-class verdict is open.
A positive direct closure requires a complete source antecedent, one physical
zero, the physical-universe carrier attachment, the horizon--record
identification, and the common screen/electroweak load-carrier identification.
These objects are separate from the recovered-core claim set.
...
with universe-level equation \(N=\log M_0(\mathfrak U_N)\), stable whole-fiber target \(\mathfrak F_{r,0}(D_\star)=\{D_\star\}\), and an independent EW/Higgs comparison bridge. Downstream phenomenology requires additional assumptions
