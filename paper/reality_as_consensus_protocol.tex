...
formalized results belong to the stack's sorry-free Lean subset. The paper also explains why
...
\]
Connected conflict components commit atomically through certified union-collar payloads. A
schedule-independent normal form follows when the implementation also supplies protected-support
completeness, conflict completeness, a quotient-level local-diamond receipt, and repair
completeness.
...
\item \textbf{Asynchronous confluence.} For the declared accepted repair law, primitive recovery proposals pass through transactional snapshot/read/write validation, protected-support-complete boundary and sector preservation, and exact descent. The accepted relation induces a total local quotient repair map \(\operatorname{locRep}_\lambda:Q\to Q\) and a total idempotent global repair map \(\operatorname{Rep}_\lambda=\overline{\operatorname{nf}}_\lambda:Q\to Q\). The implementation must certify the local diamond on the physical quotient. Atomic conflict-component commits provide the execution mechanism but do not prove that certificate. Under the local-diamond and repair-completeness hypotheses, every fixed initial quotient state has a unique normal form, independent of update schedule (Proposition~\ref{prop:local-quotient-repair}, Proposition~\ref{prop:local-confluence}, and Theorems~\ref{thm:quotient-repair-normal-form} and \ref{thm:confluence}).
...
The transaction is \textbf{protected-support-complete} when every protected observable that its
write set can change has its full validation support in \(R_\tau\). The conflict graph is
\textbf{protected-conflict-complete} when any two transactions that can jointly change one
protected observable lie in the same connected conflict component. These clauses are stronger
than validation completeness for the descent measure.
...
\begin{proposition}[Quotient local-diamond receipt]\label{prop:local-confluence}
For the accepted repair law of
Definitions~\ref{def:repair}--\ref{def:transactional-repair}, assume protected-support
completeness, protected-conflict completeness, a coherent aggregate union-collar payload from
Proposition~\ref{prop:assocglue}, and the following exported check: every one-step quotient peak
\[
t\longleftarrow s\longrightarrow u
\]
admits a quotient state \(v\) with \(t\to^*v\leftarrow^*u\). Then the transactional repair
relation is locally confluent.
\end{proposition}

\begin{proof}
The exported check is the definition of local confluence. Protected-support and conflict
completeness state which implementation dependencies the check must include. The aggregate
payload receipt makes each conflicting component a well-defined quotient transaction.
\end{proof}
...
to build canonical aggregate payloads. Proposition~\ref{prop:local-confluence} requires an
independent local-diamond check that includes protected-support and conflict completeness.
Stability of either receipt under refinement or branch change is a separate question.
...
and its injectivity modulo hidden-label gauge on the consistent set is machine-checked class-wide
in the released Lean development, together with the resulting cross-source endpoint form for the
...
hypothesis with machine-checked failure witnesses.
\end{remark}
...
record.  Matscheko's OPHProofChain Lean audit proves the sharp two-column
classification:
\[
...
normal-form package: finite exact descent, an independently checked quotient local diamond, and repair
...
Repair converges & finite exact descent; confluence additionally needs an independently checked quotient local diamond and repair completeness \\
