...
control uniqueness, and how cycle holonomy records the exact obstruction to global agreement. The
consensus core is the most heavily verified layer of the program: it sits inside the formalized
subset of 111 sorry-free Lean theorems, and a hostile forty-two-finding adversarial audit
identified no false theorem in the recovered core these results feed. The paper
...
\]
Connected conflict components commit atomically through canonical union-collar payloads, so under
repair completeness every initial quotient state has a unique schedule-independent normal form.
...
\item \textbf{Asynchronous confluence.} For the declared accepted repair law, primitive recovery proposals pass through transactional snapshot/read/write validation, boundary/sector preservation, and exact descent. The accepted relation induces a total local quotient repair map \(\operatorname{locRep}_\lambda:Q\to Q\) and a total idempotent global repair map \(\operatorname{Rep}_\lambda=\overline{\operatorname{nf}}_\lambda:Q\to Q\). Atomic conflict-component commits give the local diamond on the physical quotient; under repair completeness, every fixed initial quotient state has a unique normal form, independent of update schedule (Proposition~\ref{prop:local-quotient-repair}, Theorems~\ref{thm:quotient-repair-normal-form} and \ref{thm:confluence}).
...
other. Therefore each transaction's snapshot remains valid after the other commits, and the
...
In the terminology of the project consistency stack, the quotient
normal-form theorems of this section (Theorem~\ref{thm:quotient-repair-normal-form},
Theorem~\ref{thm:confluence}, and the boundary-conditioned uniqueness statement of
Theorem~\ref{thm:boundary-conditioned-uniqueness} below) are the instantiation of stack row C2
(overlap consensus): the public world is the quotient normal form that survives agreement on
overlaps. The row is falsifiable at this theorem level, and two failure shapes would break it: an
...
Theorem~\ref{thm:boundary-conditioned-uniqueness} holds. Either exhibit defeats C2 on its
declared branch; neither has been produced.
...
distinct consistent quotient endpoints for the same selected data, the correct label is
...
with the robust whole-fiber target \(\mathfrak F_{r,0}(D_\star)=\{D_\star\}\) and an independent EW/Higgs comparison bridge (CL-7). Downstream phenomenology requires additional assumptions
