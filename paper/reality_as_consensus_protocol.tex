...
Lyapunov descent, local-diamond confluence, inverse limits, and recovery channels all enter this
...
core consensus layer concerns convergence: the declared
...
under repair completeness every initial quotient state has a unique schedule-independent normal
form. The stronger claim that all interiors with the same boundary data settle to the same state
requires preservation of that boundary data plus a unique consistent extension in the corresponding
fiber. The second concerns obstructions: pairwise overlap agreement does not ensure a
...
This paper proves a constraint-code firewall and eight core consensus results:
...
starting at \(s_0\) has length at most
\[
...
\begin{proof}
The values of \(\Phi\) strictly decrease along the run, so no value in
...
For the accepted repair law of Definitions~\ref{def:repair}, \ref{def:localfit}, and
\ref{def:assoc}, under Assumption~\ref{ass:complete}, each fixed initial state $s\in\Sigma$ has a unique normal form
\[
...
normal-form package: finite descent, quotient-local confluence, and repair completeness, so every
fixed initial quotient state has a schedule-independent exact normal form in \(\mathcal N\).
...
every active mismatch class required by the exact local-diamond and repair-completeness
...
fiber has a unique consistent extension, holonomy obstructions, gauge-quotient invariance,
separated cofinal refinement-limit consensus classes, controlled coarse-graining compatibility,
the fixed-cutoff operator-record theorem, and conditional noisy fair-block convergence once a
...
therefore decidable by direct iteration of accepted local repairs. This step bound is a termination
statement; the schedule-independent answer depends on the local-diamond and repair-completeness
clauses of Theorem~\ref{thm:confluence}. What this paper does
...
Repair converges & finite Lyapunov descent; confluence additionally needs local diamond and repair completeness \\
