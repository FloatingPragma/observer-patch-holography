...
\author{B.\ M\"uller \and Kai Xue \and Kale Arnav Anirudha \and David Matscheko \and Jonathan Hill}
...
This support upper bound is uniform over fixed exogenous schedule words.  It
does not cover an adaptive scheduler whose next site depends on the current
global record.  It supplies no converse, minimal cone, graph metric,
propagation speed, or claim that influence reaches the boundary of \(B_n(S)\).
...
and functorial refinement maps with compatibility laws. Six Lean modules
carry the layer:
\begin{flushleft}
...
An operational observer over a consensus tower is a structure with seven
clause groups, each a finite statement about declared tower data:
\begin{enumerate}[label=(\roman*),leftmargin=2em]
...
\item readable evidence: a declared readout equal to the trace pairing of
each record element against the observer's state, with every record
element fixed by every declared intervention.
\end{enumerate}
Readback idempotence on the interface and refinement stability of the
readout are derived. The witness data are exact and small: over a
...
non-identity control. Three negative controls violate exactly one clause
each on the same data: an erasing readback breaks the self-readback law,
the identity generator breaks durability, and a resetting prediction
breaks no-regress. The receipt selects no unique observer, attaches no
...
\textbf{Target} & \textbf{Status} & \textbf{Carrier or missing structure} \\
\hline
CPT & not statable: typed exit & no antiunitary conjugation, region
reflection, or distinguished vacuum on the order-theoretic event
regions; the Tomita conjugation is an explicit TODO in the pinned
Mathlib \\
spin--statistics & not statable: typed exit & causal embeddings are
proof-irrelevant, so the event-region category is thin and carries no
symmetry-group action data; no spin structure in the development \\
DHR sectors & finite triviality theorem; full DHR a typed exit & every
bijective localized endomorphism of the finite net is inner, so the
reconstructed gauge group is trivial
(\path{Lean/QFT/SectorInheritance.lean}); no quasi-local algebra or
vacuum representation exists for the full theory \\
field/action reconstruction & gated by design & owned by the separate
finite current/holonomy, matter and Spin action, and period/character
lattice packets, none of which is in the development yet \\
KMS/thermality & finite Gibbs core proved; net-level degenerate;
public-level refuted; horizon a typed exit & the normalised Gibbs state
satisfies the finite algebraic KMS identity on the private block, but
on the constructed example net every functional is KMS for every
grading-preserving flow at every complex time, and the public record
algebra admits no nontrivial continuous star flow at all
(\path{Lean/QFT/StructuralInheritance.lean}) \\
relative-Cauchy stress response & not statable: typed exit & no
Cauchy-embedding class, the time-slice module is a declared unstarted
deliverable, and the pinned Mathlib contains no Lorentzian geometry \\
particle/scattering & not statable: typed exit & no translation action
on event regions and no spectrum condition; no Haag--Ruelle material in
the pinned Mathlib \\
\end{tabular}
\end{center}
...
\paragraph{An executed preregistered witness chain.}
...
companion simulation engine was preregistered: its gate contract was
committed before execution and pins five gates, a nonconstant protected
record, one pinned common reference shared by the state and transition
optimizers, an exact recognizer, stationarity, and contraction. The
executed run passed all five gates. A declared extraction script froze
...
kernel-decided, inhabiting an earned instance of the observer access-cut
