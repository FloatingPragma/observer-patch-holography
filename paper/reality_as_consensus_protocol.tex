...
The paper's own no-go theorem
(Theorem~\ref{thm:bare-consensus-not-einstein-complete}) sets the scope of the
title: the bare consensus reduct does not determine a Lorentzian metric or the
...
\mathsf{Cons}_r
=
(\Sigma_r,\Gamma_r,Q_r,\Phi_r,\to_r,n_r,C_r),
\]
...
\]
The gravity theorem used elsewhere in OPH has a separate five-axiom
recovered-core dependency spine:
\[
\boxed{
\begin{gathered}
...
\]
That branch is carried by the compact SM/GR and microphysics papers, not by the
bare constraint-code theorem above.
