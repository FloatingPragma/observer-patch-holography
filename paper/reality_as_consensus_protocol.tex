...
\author{B.\ M\"uller \and Kai Xue \and Kale Arnav Anirudha \and Jonathan Hill}
...
\[
\boxed{
\mathsf{Cons}_r\Rightarrow\text{quotient normal forms, not geometry.}
}
\]
The gravity theorem used elsewhere in OPH has the separate dependency spine
\[
\boxed{
\begin{gathered}
\mathsf{Cons}_r+\mathsf{GeomRead}
...
\]
is a total map.  Moreover, for every \(x\in Q\),
\[
...
a^{(D)}=E(b)\in C_Q.
\]
Moreover, the consistent boundary fiber is a singleton:
\[
C_Q\cap B^{-1}(b)=\{E(b)\}.
\]
...
record.  The external Lean audit proves the sharp two-column classification:
\[
...
Moreover, near a full-rank exact Markov state \(\sigma\), a perturbation
