...
aggregate union-collar payload from Proposition~\ref{prop:assocglue}.  Then every one-step quotient peak
\[
...
read/write sets, conflict components, aggregate payload hashes, and pre/post protected, semantic-parent,
checkpoint, and descent values needed to check that the concrete engine realizes these premises.
\end{proposition}
...
components, say \(\tau\) and \(\sigma\).  By the conflict definition,
\[
...
and payload remain valid after \(\sigma\), and conversely.  Both second commits are legal.  Since
...
explicit structural assumptions (partial synchrony, one-vote-per-view, certificate semantics,
authentication, and quorum overlap: either the classical exact sizing \(n=3f+1,q=2f+1\) or a
general threshold \(q\) with \(2q\ge n+f+1\)), a \textrm{QBFT}-style interpretation
