...
over past the exact tie surface, with witness
\((\beta,\gamma,\delta)=(8,1,1)\). Conjugating \(\sqrt5\) and swapping
\(\beta\leftrightarrow\gamma\) maps \(d_G\) to \(d_F\) exactly: the
\(F/G\) asymmetry is the Galois image of the sector swap, and the face
orientation selects the branch with the smaller conjugate at every balanced
point. A channel-diagonal invariant metric on the bracket space that is not
