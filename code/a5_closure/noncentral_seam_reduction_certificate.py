...
RECEIPT_SCHEMA = "oph.noncentral_seam_reduction_receipt.v4"
...
def reduced_interface_coefficient_models(
    ...
    return {
        ...
        "status": "reduced_interface_record_superseded_by_complete_classification",
        ...
        "complete_classification_pointer": (
            "the complete-schema resolution is the coefficient-lift "
            "classification: Z7 has no faithful complete realization and Z6 "
            "is the measured central column, so the reduced comparison "
            "carries no axiom-level alternative"
        ),
    }


def equivariant_register_classification() -> dict[str, Any]:
    """Classify the register lane of the complete A1 carrier exactly.

    A coefficient group realized through carrier registers must act on the
    twelve primitive central atoms without splitting them, equivariantly
    under the sixty rotations. The exact computations: the invariant
    partitions of the port set are the discrete partition, the six
    antipodal pairs, and the trivial partition; and the centralizer of the
    rotation group inside the symmetric group on the ports is the
    two-element group generated by the antipode. A faithful register
    translation group therefore has order at most two, so neither Z6 nor
    Z7 admits a faithful register realization, and the audited shortcut
    (tensoring with a central record register) is excluded by the
    primitive-port clause rather than by convention.
    """

    import response_grammar_completeness_certificate as r611

    verts, adjacency, _, _ = r611.port_model()
    rotations = [tuple(row) for row in r611.rotation_permutations(verts, adjacency)]
    require(len(rotations) == 60, "REGISTER_LANE", "sixty rotations required")
    ports = 12

    # Invariant partitions: the minimal block containing each pair, closed
    # under the group action and transitivity.
    def minimal_block(seed_pair: tuple[int, int]) -> frozenset[int]:
        block = set(seed_pair)
        changed = True
        while changed:
            changed = False
            for rotation in rotations:
                image = {rotation[p] for p in block}
                if image & block and not image <= block:
                    block |= image
                    changed = True
                shifted = {p for p in range(ports) if rotation[p] in block}
                if shifted & block and not shifted <= block:
                    block |= shifted
                    changed = True
    ...
    for i in range(ports):
        for j in range(ports):
            if i != j and minimal_block((i, j)) == frozenset({i, j}):
                antipode[i] = j
    antipodal_pairs = sorted(
        tuple(sorted((i, antipode[i]))) for i in range(ports) if antipode[i] is not None
    )
    require(
        block_sizes == [2, 12] and len(set(antipodal_pairs)) == 6,
        "REGISTER_LANE",
        f"the invariant partitions are not discrete/antipodal/trivial: {block_sizes}",
    )

    # Centralizer: a commuting permutation is determined by the image of
    # port zero; enumerate the twelve candidates and keep the consistent
    # ones.
    centralizer: list[tuple[int, ...]] = []
    for image_zero in range(ports):
        candidate: list[int | None] = [None] * ports
        candidate[0] = image_zero
        consistent = True
        changed = True
            ...
            for rotation in rotations:
                for port in range(ports):
                    if candidate[port] is None:
                        continue
                    target = rotation[candidate[port]]
                    source = rotation[port]
                    if candidate[source] is None:
                        candidate[source] = target
                        changed = True
                    elif candidate[source] != target:
                        consistent = False
                        break
                if not consistent:
                    break
        if consistent and None not in candidate and len(set(candidate)) == ports:
            centralizer.append(tuple(candidate))  # type: ignore[arg-type]
    require(
        len(centralizer) == 2,
        "REGISTER_LANE",
        f"the rotation centralizer has order {len(centralizer)}, not two",
    )
    nontrivial = next(c for c in centralizer if c != tuple(range(ports)))
    require(
        all(nontrivial[nontrivial[p]] == p and nontrivial[p] != p for p in range(ports)),
        "REGISTER_LANE",
        "the nontrivial centralizing element is not a fixed-point-free involution",
    )
    ...
    return {
        "invariant_partitions": {
            ...
            "antipodal_pairs": 6,
            "trivial": 1,
            "block_sizes_beyond_singletons": block_sizes,
        },
        "rotation_centralizer": {
            "order": 2,
            "generator": "the antipodal involution",
            "computation": "twelve seed images propagated through all sixty rotations",
        },
        "register_translation_bound": (
            "a faithful equivariant register translation group embeds in the "
            "order-two centralizer"
        ),
        "faithful_register_realizations": {
            "Z2": "the antipodal involution lane, orientation-reversing and typed separately",
            "Z6": "none: 6 > 2",
            "Z7": "none: 7 > 2",
        },
        "hom_counts_into_centralizer": {
            "Z6": hom_count(6, 2),
            "Z7": hom_count(7, 2),
        },
        "primitive_port_clause": (
            "tensoring a local central record algebra with C^n splits the "
            "twelve primitive central port projections; the register lane is "
            "therefore bounded by the centralizer computation above, and the "
            "shortcut is excluded by the A1 primitivity clause"
        ),
    }
...
def complete_coefficient_lift_classification() -> dict[str, Any]:
    ...
    z7_to_z6 = homs_cyclic(7, 6)
    z7_to_z2 = homs_cyclic(7, 2)
    z6_to_z6 = homs_cyclic(6, 6)
    faithful_z6 = [
        image
        for image in z6_to_z6
        if len({(image * k) % 6 for k in range(6)}) == 6
    ]
    require(
        z7_to_z6 == [0] and z7_to_z2 == [0],
        "COEFFICIENT_LIFT",
        "Z7 unexpectedly maps nontrivially into a carrier lane",
    )
    require(
        sorted(faithful_z6) == [1, 5],
        "COEFFICIENT_LIFT",
        f"the faithful Z6 embeddings drifted: {faithful_z6}",
    )
    return {
        "lanes": {
            "register_lane_bound": "order two (the rotation centralizer)",
            "seam_class_lane": "the measured order-six central column of the routed-seam receipt",
            "partition_lane": "no invariant partition has a part count divisible by seven",
        },
        "z7": {
        ...
        "z6": {
            ...
            "a2_a3_surfaces": (
                ...
                "equal-state-weights, and scheduler receipts"
            ),
        },
        "conclusion": (
...
def two_type_sector_classification(nerve: Mapping[str, Any]) -> dict[str, Any]:
    ...
    seam_count, face_count = len(boundary), len(boundary[0])
    require(
        (seam_count, face_count) == (30, 20),
        "TWO_TYPE",
        "the pinned nerve does not have thirty seams and twenty overlaps",
    )
    for edge in range(seam_count):
        signs = sorted(value for value in (boundary[edge][face] for face in range(face_count)) if value)
        require(
    ...
    # Gauge invariance of the total-sum functional, checked exactly: the
    # coboundary of every seam generator has total face sum zero.
    for edge in range(seam_count):
        total = sum(boundary[edge][face] for face in range(face_count))
        require(
            total == 0,
            "TWO_TYPE",
            "a seam coboundary has nonzero total sum; the sector functional is not gauge-invariant",
        )

    def crossed_module_invariants(
        name: str, source: TableGroup, target: TableGroup, delta: Sequence[int]
    ) -> dict[str, Any]:
        kernel = [h for h in range(source.order) if delta[h] == target.identity]
        image = sorted({delta[h] for h in range(source.order)})
        cokernel_order = target.order // len(image)
        return {
            "module": name,
            "pi_2_order": len(kernel),
            "pi_1_order": cokernel_order,
            "contractible": len(kernel) == 1 and cokernel_order == 1,
        }

    s3 = build_symmetric_group_three()
    z2 = build_cyclic_group(2)
    z4 = build_cyclic_group(4)
    trivial = build_cyclic_group(1)

    identity_module = crossed_module_invariants(
        "id: S3 -> S3", s3, s3, list(range(s3.order))
    )
    require(
        identity_module["contractible"],
        "TWO_TYPE",
        "the identity crossed module is not contractible",
    )
    witness_module = crossed_module_invariants(
        "Z2 -> 1", z2, trivial, [0, 0]
    )
    require(
        witness_module["pi_2_order"] == 2 and not witness_module["contractible"],
        "TWO_TYPE",
        "the witness module lost its noncontractible pi_2",
    )
    doubling = [(2 * h) % 4 for h in range(2)]
    injective_module = crossed_module_invariants("Z2 -> Z4 (doubling)", z2, z4, doubling)
    require(
        injective_module["pi_2_order"] == 1 and injective_module["pi_1_order"] == 2,
        "TWO_TYPE",
        "the injective module invariants drifted",
    )

    def sector_census(order: int) -> dict[str, Any]:
        # H^2 of the sphere with cyclic coefficients: the total-sum class.
        witness = [0] * face_count
        witness[0] = 1 % order
        total = sum(witness) % order
        return {
            "coefficient_order": order,
            "sector_group_order": order,
            "classifying_functional": "total face sum modulo the coefficient order",
            "nontrivial_witness_total": total,
            "gauge_invariant": True,
            "presentation_invariant": (
                "the total sum is invariant under every face relabeling"
            ),
        }
    ...
    mechanisms = [
        {
            **witness_module,
            "sector": sector_census(2),
            "matter_effect": (
                "a supplied central character of pi_2 acts on surface-supported "
                "transport exactly as the declared central classes act through "
                "their characters; typed conditional, no identification with "
                "the measured group-extension datum"
            ),
            "classification": "noncontractible_pi2_sector_admitted",
        },
        {
            **crossed_module_invariants("Z3 -> 1", build_cyclic_group(3), trivial, [0, 0, 0]),
            "sector": sector_census(3),
            "matter_effect": "supplied central character of pi_2, typed conditional",
            "classification": "noncontractible_pi2_sector_admitted",
        },
        {
            **crossed_module_invariants("Z6 -> 1", build_cyclic_group(6), trivial, [0] * 6),
            "sector": sector_census(6),
            "matter_effect": "supplied central character of pi_2, typed conditional",
            "classification": "noncontractible_pi2_sector_admitted",
        },
        {
            **identity_module,
            "sector": {"sector_group_order": 1},
            "matter_effect": "none: the 2-type is contractible",
            "classification": "contractible_no_sector",
        },
        {
            **injective_module,
            "sector": {
                "sector_group_order": 1,
                "reason": (
                    "pi_2 is trivial and the flat pi_1 part is gauge-trivial "
                    "on the simply connected nerve"
                ),
            },
            "matter_effect": "none on the sphere",
            "classification": "flat_pi1_gauge_trivial_no_sector",
        },
    ]

    return {
        "nerve_gate": {"seams": seam_count, "overlaps": face_count},
        "gauge_invariance": (
...
def structural_controls(
    ...
    def claim_faithful_z7_register() -> None:
        classification = equivariant_register_classification()
        require(
            classification["hom_counts_into_centralizer"]["Z7"] > 1,
            "COEFFICIENT_LIFT",
            "Z7 has only the trivial map into the register centralizer",
        )

    run(
        "claimed_faithful_z7_register_rejected",
        "COEFFICIENT_LIFT",
        claim_faithful_z7_register,
    )

    def claim_contractible_sector() -> None:
        s3 = groups["s3"]
        kernel = [h for h in range(s3.order) if h == s3.identity]
        require(
            len(kernel) > 1,
            "TWO_TYPE",
            "the identity crossed module has trivial kernel, so no sector "
            "can be promoted from it",
        )

    run(
        "contractible_module_sector_promotion_rejected",
        "TWO_TYPE",
        claim_contractible_sector,
    )
    return results
...
def certificate_payload(
    ...
    register_classification = equivariant_register_classification()
    coefficient_lift = complete_coefficient_lift_classification()
    two_type = two_type_sector_classification(nerve)
    ...
    return {
        ...
        "complete_coefficient_lift": {
            "register_lane": register_classification,
            "classification": coefficient_lift,
        },
        "two_type_sector_classification": two_type,
        ...
        "acceptance_criteria_status": {
            ...
            "effect_reading": (
                ...
                "crossed module Z2 -> 1 carries the total-sum sector with a "
                "character-typed matter effect; contractible and flat-pi_1 "
                "modules supply none"
            ),
        ...
        "verdict": {
            ...
            "independence_boundaries": {
                ...
                "higher_gauge_grammar": (
                    ...
                    "2-type; the noncontractible witness Z2 -> 1 carries the "
