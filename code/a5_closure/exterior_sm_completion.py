...
payload = {
    ...
    "conditional_closure": {
        ...
        "remaining_producers": [
            ...
            "PORT-CURRENT-INNER is closed on the declared response branch (port_current_inner_certificate.py); the response source data (four band scales, one common odd sign) are declared in the source packet, and deriving them from raw consensus dynamics remains a separate open lane",
            "SUPER-TANNAKIAN-MATTER-LIFT is closed on the declared matter-contract branch (super_tannakian_matter_lift_certificate.py): PORT-SPIN-LIFT, the projector-realized fifteen-state module, kernel emission, and MAR nonemptiness; the trace-balanced matter contract is declared in its source packet",
