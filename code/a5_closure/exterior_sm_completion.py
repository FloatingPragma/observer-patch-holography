...
payload = {
    ...
    "conditional_closure": {
        ...
        "remaining_producers": [
            "A1 supplies the unlabeled local icosahedral boundary; the richer Echosahedral counting, response, refinement, and physical carrier lineage remains separately declared or source-gated",
            "PORT-CURRENT-INNER verifies a conditional charged-double-triplet matrix fixture; a source producer must reconstruct its generators and bracket from ordered response histories and realize same-current overlap holonomy",
            "PORT-SPIN-LIFT and the unordered rank-fifteen pair are exact inside the declared current, exterior, scalar, and pinned spin fixtures; source selection of the current and physical matter action, continuum Spin attachment, menu completeness, and laboratory identification remain open",
            "source realization of the conditionally derived BLOCK-DETERMINANT-BALANCE pair",
            "AXIS-CENTER-DESCENT computes an order-six quotient only inside declared matter and axis-relation systems; source selection of the relations, complete characters, a same-source loop-to-kernel theorem, and the physical line spectrum remain open",
