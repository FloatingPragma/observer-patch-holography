...
class AxisCenterDescentTests(unittest.TestCase):
    ...
    def test_only_weight_level_refinement_is_proved(self) -> None:
    ...
    def test_physical_global_form_gate_remains_open(self) -> None:
        self.assertTrue(self.expected["conditional_algebraic_gate"]["passed"])
        self.assertFalse(self.expected["physical_global_form_gate"]["passed"])
        self.assertFalse(
            self.expected["physical_global_form_gate"]["source_derived_deck_loop_class"]
        )
        self.assertFalse(
            self.expected["physical_global_form_gate"]["spacetime_spin_attachment"]
        )
        self.assertFalse(
            self.expected["physical_global_form_gate"]["genuine_line_category_selected"]
        )
        self.assertFalse(
            self.expected["physical_global_form_gate"][
                "instanton_sector_and_action_normalization"
            ]
        )
