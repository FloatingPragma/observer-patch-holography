...
payload = {
    ...
    "theorem_scope": {
        ...
        "physical_gates": [
            ...
            "the PORT-CURRENT-INNER construction is verified by port_current_inner_certificate.py with the charged response representation, four signed coefficients, oriented frame, and physical refinement maps determined by the bound semantic response artifact and recomputed exactly; the laboratory attachment of the source-bound current to measured gauge currents is open (#569)",
            "the super-Tannakian matter lift is verified by super_tannakian_matter_lift_certificate.py over the source-bound current packet: the exact non-split SU(2) double cover (PORT-SPIN-LIFT), the projector-realized fifteen-state chiral module on the auxiliary CAR/Fock space, realized anomaly and Yukawa checks, the emitted action kernel (infinite cyclic on the cover, residual order six modulo the pure deck translations), the nonempty declared candidate class, the derived BLOCK-DETERMINANT-BALANCE charge pair, the derived finite scalar and channel packet, and the artifact-bound physical refinement descent; AXIS-CENTER-DESCENT, menu completeness, and family attachment are the open consumers",
            "AXIS-CENTER-DESCENT identifies the physical group and quotient, consuming the emitted action kernel; BLOCK-DETERMINANT-BALANCE is derived inside the matter-lift packet",
