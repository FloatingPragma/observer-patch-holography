...
| **Conditional axis-centre arithmetic and physical no-go (#567, open)** | Consuming the pinned #314 receipt, exhaustive enumeration over the 36 central candidates gives the diagonal common kernel generated by `(omega_3 I_3, -I_2, e^{i pi/3})`. Quotienting by the full kernel gives the maximal effective image of the declared tensor representation. Its character lattice has basis `(1,0,-2), (0,1,-3), (0,0,6)` and Smith invariants `(1,1,6)`; the exact dual basis is `(1,0,0)`, `(0,1,0)`, `(1/3,1/2,1/6)`. This does **not** select the physical global form or a genuine-line spectrum: the same local tensors descend through quotients by `1`, `Z2`, `Z3`, and `Z6`, with distinct character-residue counts `36,18,12,6`. Also, `h^3=(1,-I_2,-1)` acts trivially and is not fermion parity; no one of the 36 central elements acts as `-1` on all five matter multiplets. A source-derived loop/deck class, spacetime Spin attachment, line category and UV polarization, refinement transport of those structures, and instanton/action data for theta periodicity remain absent. Lean: `Z6Descent.lean`; executable receipt schema v3. |
...
effective quotient. It also proves the relevant obstruction: all four
quotients by subgroups of the diagonal `Z6` accept the same local tensors,
while their global character data differ. Consequently the certificate does
not fix a physical line spectrum or global gauge group. Its physical gate
stays false until a source-derived loop/deck attachment, spacetime Spin
attachment, genuine-line category and UV polarization, refinement-natural
transport, and instanton/action normalization are supplied.
...
- **BLOCK-DETERMINANT-BALANCE** is derived inside the matter packet:
  ...
  `(-1/3, 1/2)`.
- **AXIS-CENTER-DESCENT arithmetic** computes the common kernel and the
  character/dual-cocharacter lattices of its maximal effective quotient.
  The #567 physical gate remains open: the four quotient choices are
  indistinguishable on the declared local tensors, and the packet has no
  source-derived deck loop, spacetime Spin attachment, genuine-line
  polarization, or instanton/theta receipt.
...
- **PHYSICAL-GLOBAL-FORM-ATTACHMENT (#567)**: attach a source-derived
  loop/deck holonomy and spacetime Spin structure, select a genuine-line
  category and mutually-local UV polarization, transport those data
  naturally under refinement, and derive instanton/action normalization
  before stating theta periodicity.
