...
# Files whose job is to talk ABOUT the probes; decl-name matching makes
# self-hits impossible anyway, but exclude them so a future refactor of
# either file cannot manufacture a false red.
TREE_SCAN_EXCLUDE = {GUARD_LEAN.resolve(), MATRIX_LEAN.resolve()}
...
def test_mathlib_provisioned_fail_closed():
    """No pinned Mathlib source, no evaluation — and that is a failure."""
...
def test_mathlib_absence_probes():
    """Rows 1, 6, 7: the cited pinned-Mathlib absences still hold."""
    findings: list[str] = []
...
def test_row1_tomita_todo_still_open():
...
def test_row2_spin_group_still_present():
    ...
    if _require(SPIN_GROUP_FILE, findings, "row-2 cited SpinGroup module"):
        if SPIN_GROUP_DECL not in SPIN_GROUP_FILE.read_text(encoding="utf-8"):
            findings.append(
                f"row 2: '{SPIN_GROUP_DECL}' no longer appears in "
...
def test_tree_absence_probes():
    """Rows 1, 2, 3, 6, 7: the cited tree absences still hold, over the
    ...
    if TIME_SLICE_FILE.exists():
        findings.append(
            f"row 6: {TIME_SLICE_FILE} now exists — the 'declared unstarted "
...
def test_probe_tables_in_sync():
    ...
    for probe in sorted(lean_probes - here):
        findings.append(
            ...
            "probe known to this test — either a new probe was added in Lean "
