# Candidate source text: `OPH+SM-EFT+FJ v1`
...
\[
(SU(3)_c\times SU(2)_L\times U(1)_Y)/\mathbb Z_6
\]

bundle, and the equivalence class of the canonically normalized local action

\[
S_{\rm inv}[G,W,B,H,Q,u,d,L,e;p(Q)]
\]
...
\[
p_F(Q_0)=\{g_s,g,g',m^2,\lambda,Y_u,Y_d,Y_e\}_{\overline{\rm MS},FJ}
\]
...
\[
m^2(Q)=-\lambda(Q)v_F(Q)^2,\qquad v_F(Q)>0.
\]
...
The source emits either:

1. a globally unique deterministic point, licensing \(\mathrm{Law}(z)=\delta(z-z_0)\); or
2. a target-independent joint law/weighted finite ensemble.
...
## Quantization convention (not an OPH axiom)

A simulator may choose any nonsingular BRST gauge-fixing fermion. The reference
validation family is the eight-parameter nonlinear electroweak gauge in Theorem
9 together with a linear QCD \(R_\xi\) gauge. The 45-point sidecar holds the
QCD parameter fixed only because the strict one-loop W/Z lane has no internal
gluon; any mixed QCD correction must add the QCD Nielsen check. Gauge fixing,
ghost coordinates, subtraction scheme and field basis are not observables;
they are accepted only when the extended ST/Nielsen receipts prove pole
invariance.
