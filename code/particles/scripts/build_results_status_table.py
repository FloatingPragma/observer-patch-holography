...
D10_MASS_PAIR_NOTE = (
    ...
    "The electromagnetic row is not taken from the compact hypercharge slice. Its source anchor is `a0 = alpha_em^-1(m_Z^2) = 128.30576920234813` on the same running family, and the Ward-projected `U(1)_Q` transport family is the readout surface. The Thomson endpoint recorded on that surface is reference compare-only until the source transport is emitted on the same branch. The separate `code/P_derivation/fixed_point_witness.py` surface records the outer/inner `P`-closure as a numerical witness and keeps the CODATA/NIST value as compare-only metadata until an interval uniqueness certificate is emitted. "
...
CHARGED_CONTINUATION_NOTE = (
    ...
    "and the downstream algebraic readout from theorem-grade `A_ch(P)`. The open source theorem is the "
    "sector-isolated trace-lift attachment / determinant-normalization identity "
