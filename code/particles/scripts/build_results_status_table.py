...
D10_MASS_PAIR_NOTE = (
    ...
    "The D10 mass-side surface fixes the `W/Z` pair from that source trunk, and the reference-fitted coherent pair serves as compare-only validation on the same family. "
    ...
    "That certificate closes the local `P -> alpha_U -> v/E_star` hierarchy lane. The local/global package "
    "gives bridge-defined `N_CRC^EW=3.5323546226929906511187512962330547600462e122` and zero algebraic bridge residual. "
    "Its separately declared comparison maps give `epsilon_H=0` as a packaging identity; the current source antecedents also admit exact nonzero-defect completions, so no Higgs-naturalness result is promoted. "
    "The conditional screen theorem supplies 12 ports, `X/12`, and a separate 24-slot oriented register; the product adjoint independently "
    "supplies `m_rep=24`. The physical relation `t_tr(P_star)=(P_star/12)*log(N_CRC^EW/pi)` requires "
    "`HIERARCHY-SCREEN-READOUT`. Source production of that readout and the screen selector is work in progress. The public Thomson endpoint transport and full no-G "
...
D11_NOTE = (
    ...
    "threshold surface explicit, emits the unpromoted D10 repair tuple `(eta_source, beta_EW, lambda_EW, "
    "tau2_tree_exact, delta_n_tree_exact)`, and then emits a conditional Higgs/top split candidate. "
    "The old one-scalar fixed ray remains a lower companion branch. The conditional split artifact uses the shared scalar "
    "`rho_HT = log(1 + tau2_tree_exact)` together with declared residual selectors whose source uniqueness and deformation rigidity remain open: "
    ...
    "Promotion of the Higgs mass row is blocked by the D10 repair gate, source provenance for the D11 core, Jacobian, and residual selectors, and the scale, running, matching, and physical-pole gates. "
    ...
    "corpus gap certificate by `code/particles/runs/calibration/direct_top_bridge_contract.json` (no bridge is derived in the corpus; a future bridge stays open). "
    "The old one-scalar seed `sigma_D11_HT = alpha_u * cos(2*theta_W0) / sqrt(pi)` remains on disk as the fixed-ray companion branch beneath this conditional artifact. "
...
CHARGED_CONTINUATION_NOTE = (
    ...
    "That attachment is the named live route, and the certificate does not deny a future universal `P`-driven charged-mass law. "
    "The no-go content of this lane is the common-shift theorem: every readout of the centered charged data is shift-invariant, "
    "so centered data alone do not emit the absolute scale."
)
...
def _d11_exact_pair_allowed(payload: Dict[str, Any]) -> bool:
    ...
    return (
        bool(payload.get("source_surface_promotable", False))
        and bool(payload.get("public_surface_candidate_allowed", False))
...
def prediction_surface_for_row(row_spec: Dict[str, Any], surface_state: Dict[str, Any], *, with_hadrons: bool) -> str:
    ...
    if particle_id == "higgs" and D11_EXACT_SPLIT_PAIR.exists():
        exact_pair = json.loads(D11_EXACT_SPLIT_PAIR.read_text(encoding="utf-8"))
        return (
            "local_d11_source_split_exact_pair_theorem"
            if _d11_exact_pair_allowed(exact_pair)
            else "conditional_d11_declared_surface_split_candidate"
        )
    if particle_id == "higgs" and D11_EXACT_HIGGS_PROMOTION.exists():
        exact_higgs = json.loads(D11_EXACT_HIGGS_PROMOTION.read_text(encoding="utf-8"))
        return (
            "local_d11_exact_higgs_promotion"
            if bool(exact_higgs.get("public_surface_candidate_allowed", False))
            else "compare_only_d11_target_anchored_higgs_exactifier"
        )
