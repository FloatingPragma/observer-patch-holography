...
This document defines a fixed-cutoff experimental kernel for GitHub issue
`#509`. It is a declared, falsifiable hardware model. It is not a new OPH
theorem and it does not claim that repair dynamics select a physical vacuum.
...
The diagnostic is secondary. It does not turn a sampler into a source-law
derivation.
...
The canonical JSON bytes of the reveal file receive a salted SHA-256
commitment. Only opaque identifiers, circuit hashes, resource envelopes, and
candidate anonymous likelihood tables are available to the analyst. The blind
analysis hash is frozen before the held-out backend is run. Reveal occurs only
after every submitted job, failed submission, layout, and exclusion has been
recorded.
...
The primary endpoint is the pooled held-out `S3` joint process likelihood for
all heated-state, decision, final-state, and leakage outcomes. The comparison
uses the independently calibrated observation channel and equal prior odds.

The preregistration target is:

- Bayes factor greater than `10` against every frozen null on each independent
  backend/layout pair;
- pooled Bayes factor greater than `100`;
...
leakage remain explicit outcome categories. Readout mitigation is fit only on
the separate calibration circuits and is frozen before held-out analysis.

## Failure Rule

The reduced repair-kernel claim fails if either of these occurs:

- both independent held-out systems favor a frozen null over the record-gated
  kernel by at least `100:1`; or
...
models. It cannot produce a Bayes factor for OPH against unrestricted quantum
mechanics.
...
No IBM job is submitted by any of these files.
