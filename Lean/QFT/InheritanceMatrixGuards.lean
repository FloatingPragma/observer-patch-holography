...
in pinned Mathlib.  Such a citation is true today and goes false
silently when the pin is bumped to a Mathlib that has grown the missing
structure, or when a module lands in the tree that supplies it.  Row 4
already went stale once, on the day it merged, and was corrected by
hand.  This module makes the staleness loud: it re-checks the cited
absences on every build and **fails to elaborate** the moment one of
them stops being true, so a stale exit turns the build red instead of
...
| 1 | `Mathlib.Analysis.InnerProductSpace.StandardSubspace` still exists (cited TODO home) | presence |
| 2 | `Mathlib.LinearAlgebra.CliffordAlgebra.SpinGroup` and `spinGroup` still exist (cited as present-but-unconnected) | presence |
...
* Row 5's horizon exit cites no external structure (it is the typed
  exit of `QFT/StructuralInheritance.lean` itself, already pinned by
  ...
  [{ row := 1
   ...
   { row := 6
     ...
     probes := ["lorentz", "globallyhyperbolic", "globalhyperbolic",
       "cauchysurface", "cauchyembedding"] },
   { row := 7
     claim := "zero Haag/scattering material in pinned Mathlib"
     probes := ["haag", "ruelle", "scattering"] }]
...
def presenceCitations : List PresenceCitation :=
  [{ row := 1
     claim := "the cited Tomita-TODO module still exists"
     module? := some `Mathlib.Analysis.InnerProductSpace.StandardSubspace },
   { row := 2
     claim := "SpinGroup.lean still exists (cited as unconnected)"
...
/-- Every absence-probed row is one of the exit-or-gated rows of the
matrix — probes never attach to the two theorem-shaped rows except
row 3, whose probes watch its full-DHR exit half. -/
theorem absence_probe_rows :
    ((mathlibAbsenceProbes ++ projectAbsenceProbes).map (·.row)).all
      (fun i => [1, 2, 3, 6, 7].contains i) = true := rfl

/-- The three prose-only exit rows Bernhard named — 1, 6, 7 — each
carry at least one pinned-Mathlib absence probe. -/
