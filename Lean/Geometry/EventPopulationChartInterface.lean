...
`EventPopulationChartInterface`, and proves only their finite chart-level
consequences for each supplied inhabitant.
...
The composed theorem `finiteEventChartConsequences` derives, from the bundle alone
...
four-dimensional declared module.  Stable causality, open chart images,
observer-to-physical-event attachment, and causal faithfulness to physical
signal reachability remain supplied scientific premises.  This interface
supports at most a formal finite precursor: source-atlas production,
physical-event population, and clock attachment are not represented by any
field here.
-/
...
/-! ## Source-derived finite order attachment

`EventPopulationChartInterface` predates the semantic-provenance pass and
therefore carries an arbitrary reflexive precedence field.  The wrapper
below removes that freedom at the finite informational-order boundary: its
precedence must be exactly the reflexive closure generated by one
authenticated semantic event log.  The atlas, separation, population map,
Lorentz module, cone equivalence, and observer/physical interpretation are
still explicit fields of the wrapped geometry interface.  Consequently the
bridge feeds source-derived finite order into the supplied finite chart package but
does not derive a manifold, Lorentzian realization, physical causal
faithfulness, or continuum limit from order alone. -/

/-- A finite event-population/chart interface whose precedence is fixed by
authenticated source-derived semantic provenance rather than independently
declared. -/
structure SourceDerivedOrderEventPopulationChartInterface
    (Register Value Event Chart : Type*) [DecidableEq Register]
    [Fintype Event] where
  /-- The authenticated semantic event log on the finite event carrier. -/
  semanticLog : SemanticEventLog Register Value Event
  /-- The remaining finite population, chart, separation, and cone data. -/
  geometry : EventPopulationChartInterface Event Chart
  /-- The old precedence field is not free: it is exactly the reflexive
  closure of the source-generated strict order. -/
  precedence_eq_generated : ∀ e f : Event,
    geometry.prec e f ↔ semanticLog.GeneratedBeforeEq e f

namespace SourceDerivedOrderEventPopulationChartInterface

variable {Register Value Event Chart : Type*} [DecidableEq Register]
variable [Fintype Event]
variable (S : SourceDerivedOrderEventPopulationChartInterface
  Register Value Event Chart)

/-- The source-generated finite informational order is realized as the
declared future-cone relation in every supplied chart.  The result is
conditional on the wrapped atlas and cone attachment; it does not derive
those geometric data from the order. -/
theorem generatedBeforeEq_iff_displacement_futureCausal
    (i : Chart) (e f : Event) :
    S.semanticLog.GeneratedBeforeEq e f ↔
      IsFutureCausal (S.geometry.atlas.displacement i e f) :=
  (S.precedence_eq_generated e f).symm.trans
    (S.geometry.prec_iff_displacement_futureCausal i e f)

/-- The committed rich-region order lands in the source-generated event
order through the supplied population map.  Population and the rich-region
identification remain explicit inputs. -/
theorem richRegion_le_iff_generatedBeforeEq (U V : RichRegion) :
    RichRegion.le U V ↔
      S.semanticLog.GeneratedBeforeEq
        (S.geometry.populate U) (S.geometry.populate V) :=
  (S.geometry.populate_le_iff U V).trans
    (S.precedence_eq_generated _ _)

/-- An authenticated direct parent is future-causal in every supplied
chart, via the explicit finite geometry attachment. -/
...
end SourceDerivedOrderEventPopulationChartInterface

/-! ## The composed theorem -/

/-- **The composed OL-A4 interface theorem.**  Every inhabitant of the
typed bundle carries the following finite chart-level consequences: the
coordinate module has real dimension four; the chart atlas is
...
theorem finiteEventChartConsequences {Event Chart : Type*} [Fintype Event]
...
bundle carries the finite chart-level consequences; the bundle is
...
theorem eventPopulationChartInterface_receipt :
  ...
  ⟨fun _ _ _ S => finiteEventChartConsequences S,
...
#print axioms OPH.EventPopulation.SourceDerivedOrderEventPopulationChartInterface.generatedBeforeEq_iff_displacement_futureCausal
#print axioms OPH.EventPopulation.SourceDerivedOrderEventPopulationChartInterface.richRegion_le_iff_generatedBeforeEq
#print axioms OPH.EventPopulation.SourceDerivedOrderEventPopulationChartInterface.parentEdge_displacement_futureCausal
#print axioms OPH.EventPopulation.finiteEventChartConsequences
