...
/- A dependency-free model of the two T6 path transformations.
   This file intentionally uses only Lean core so that the logical argument is
   checked independently of the frozen repository's build cache. -/
...
`target_iff` only identifies the already-canonical Boolean target with the
