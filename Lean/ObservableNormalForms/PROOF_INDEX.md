...
| Fixed computation-federation progress | `OPH.RepairUniversality.FixedFederation.canonicalAcceptedStep_wellFounded`; `attemptRun_eventually_constant`; `nodePathwiseWeakFair_eventually_consensus`; `fixed_federation_fair_universality` | ✅ | One generated-node federation reads all inputs from preserved initial ports. Genuine repairs strictly descend in dependency order; attempts stabilize, and continuously-enabled node fairness supplies consensus. The weak historical step is excluded. |
| Fixed computation output endpoint | `OPH.RepairUniversality.FixedFederation.fixedObserverEndpointUniqueOutput`; `OPH.Tower.FixedFederationEndpoint.fairAttempt_reaches_unique_output` | ✅ | Input observation is preserved and complete normal forms are consensus states, so any same-input normal endpoints agree on the output register. Unused raw registers and finite `PublicWorld` construction are outside scope. |
