...
| Cumulative attempt-capacity classification, `thm:oph-cumulative-attempt-capacity` | `OPH.Execution.boundedWaste_eventually_normal`; `CumulativeCapacityExamples.delayed_normalizing_attempt_no_go`; `firstBroken_attempt_threshold_iff`; `twoCell_attempt_threshold_iff` | ✅ | Every scheduler invocation costs one unit, including stutter. Arbitrary normalizing schedulers have no mismatch-only finite attempt horizon; bounded waste gives the `(q + 1)`-scaled horizon; two nonisomorphic canonical finite sources make the work-conserving threshold sharp. No physical time, rate, or resource interpretation follows. |
