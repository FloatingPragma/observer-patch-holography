...
Physics normally begins by supplying spacetime, quantum fields, a gauge group,
and a list of measured constants. OPH asks a
...
Learning about that world is one operation inside the mechanism. A candidate
universe must support observers that can read it, and their readback
...
branch condition remain visible. Its numbers must be returned by the same
self-reading consistency loop that returns its laws.

## The Quantitative Core: P
...
would cease to describe the same observer system. The closure equation makes
...
## The Global Extension: N

$N$ proposes a self-read condition for the correctable public-record capacity
of the whole observer system:
