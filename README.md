...
sense, meaning a fixed-point computation whose settled observer-readable output
is experienced from inside as history. That does not mean an external machine
rendering particle positions tick by tick. The fundamental description has no
global simulator timeline; time is part of the internal readout. Yes,
...
process. OPH computes the fixed point whose internal readout is a world; it
does not produce a frame sequence for an outside viewer. This is the
mathematical and empirical proof surface for OPH as the correct theory of
