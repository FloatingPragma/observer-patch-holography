...
- **Relativity and gravity from observer geometry.** Exact finite results
  establish Lorentz-frame kinematics, mass-shell structure, and an
  authenticated informational order generated by which committed records
  actually read which written versions. This finite order is causal-set-like,
  but does not by itself constitute a physical causal set or a spacetime
  continuum. Rank-three unit directions are algebraically equivalent to
  S2/null-ray labels, but a four-dimensional reading requires the physically
  faithful images of authenticated local links to cover those directions
  densely and isotropically. Faithful
  embedding, manifoldlikeness, physical cone attachment, count-to-volume
  calibration, and refinement that glues the rank-three neighborhoods with
  cubic carrier-ball growth while preserving order, directions, and density
  are the bridge to the conditional Einstein reconstruction. A
  machine-checked composition theorem fixes the finite order, rank, direction,
  and local-metric relations for every supplied atlas/frame packet; it does
  not construct that packet or the continuum. A second exact theorem works
  inside a supplied 3+1 tensor algebra and uses nine fixed algebraic inverse
  images of its coordinate tomography frame to derive all-null balance and an
  Einstein-form tensor relation on the same finite event carrier; it does not
  derive 3+1 dimensions from the event order. That distinguished nine-set is
  coordinate-fixed, not selected by the records and not invariant as a set
  under every spatial or Lorentz transformation. The
  informational order shares that carrier but does not select those
  directions or tensors. Interpreting the tensor as smooth physical curvature
  requires the stated
  attachment, refinement, volume, and continuum inputs on one compatible
  tower.
