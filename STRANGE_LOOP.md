...
- **Gödel and Hofstadter.** Sufficiently rich systems contain their own description; the
  ...
  is what makes a universe possible, and that the closure has exactly two coordinates
  and zero dials.

Each leg is standard. The SLH claim is the conjunction: seven threads, one architecture, and the
architecture has a computable fixed point.
...
converts the program's structure into its argument: the measured constants are
